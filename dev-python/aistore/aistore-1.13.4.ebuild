# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Client-side APIs to access and utilize clusters, buckets, and objects on AIStore."

HOMEPAGE="https://aistore.nvidia.com"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="botocore etl pytorch"
DEPENDENCIES="dev-python/braceexpand[${PYTHON_USEDEP}]
	>=dev-python/cloudpickle-3.0.0[${PYTHON_USEDEP}]
	>=dev-python/humanize-4.9.0[${PYTHON_USEDEP}]
	dev-python/msgspec[${PYTHON_USEDEP}]
	>=dev-python/overrides-7.4.0[${PYTHON_USEDEP}]
	>=dev-python/packaging-23.0[${PYTHON_USEDEP}]
	>=dev-python/pydantic-2.5.0[${PYTHON_USEDEP}]
	>=dev-python/python-dateutil-2.9.0[${PYTHON_USEDEP}]
	>=dev-python/pyyaml-6.0.1[${PYTHON_USEDEP}]
	>=dev-python/requests-2.28.2[${PYTHON_USEDEP}]
	>=dev-python/tenacity-9.0.0[${PYTHON_USEDEP}]
	>=dev-python/urllib3-1.26.18[${PYTHON_USEDEP}]
	>=dev-python/xxhash-3.3.0[${PYTHON_USEDEP}]
	botocore? ( dev-python/wrapt[${PYTHON_USEDEP}] )
	etl? ( >=dev-python/aiofiles-23.2.1[${PYTHON_USEDEP}] )
	etl? ( dev-python/fastapi[${PYTHON_USEDEP}] )
	etl? ( >=dev-python/flask-2.3.0[${PYTHON_USEDEP}] )
	etl? ( dev-python/gunicorn[${PYTHON_USEDEP}] )
	etl? ( >=dev-python/httpx-0.28.0[${PYTHON_USEDEP}] )
	etl? ( >=dev-python/uvicorn-0.32.0[${PYTHON_USEDEP}] )
	pytorch? ( dev-python/alive-progress[${PYTHON_USEDEP}] )
	pytorch? ( dev-python/torch[${PYTHON_USEDEP}] )
	pytorch? ( dev-python/torchdata[${PYTHON_USEDEP}] )
	pytorch? ( dev-python/webdataset[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
