# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="0.15.0a5.dev0"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="For documenting hardware projects with minimal effort, so you can stop writing and GitBuilding. [badver 0.15.0a5.dev0]"

HOMEPAGE="https://gitbuilding.io/"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev gui"
DEPENDENCIES="dev-python/argparse[${PYTHON_USEDEP}]
	dev-python/colorama[${PYTHON_USEDEP}]
	>=dev-python/defusedxml-0.7.1[${PYTHON_USEDEP}]
	dev-python/exsource-tools[${PYTHON_USEDEP}]
	dev-python/flask[${PYTHON_USEDEP}]
	dev-python/flask-cors[${PYTHON_USEDEP}]
	dev-python/jinja2[${PYTHON_USEDEP}]
	dev-python/jsonschema[${PYTHON_USEDEP}]
	dev-python/latex2mathml[${PYTHON_USEDEP}]
	>=dev-python/markdown-3.2[${PYTHON_USEDEP}]
	>=dev-python/marshmallow-3.22[${PYTHON_USEDEP}]
	>=dev-python/pathspec-0.11[${PYTHON_USEDEP}]
	>=dev-python/pygments-2.14[${PYTHON_USEDEP}]
	dev-python/python-frontmatter[${PYTHON_USEDEP}]
	>=dev-python/pyyaml-5.1[${PYTHON_USEDEP}]
	>=dev-python/regex-2022.1.18[${PYTHON_USEDEP}]
	dev-python/requests[${PYTHON_USEDEP}]
	dev-python/waitress[${PYTHON_USEDEP}]
	>=dev-python/watchdog-3.0[${PYTHON_USEDEP}]
	>=dev-python/weasyprint-60.1[${PYTHON_USEDEP}]
	dev? ( dev-python/beautifulsoup4[${PYTHON_USEDEP}] )
	dev? ( dev-python/build[${PYTHON_USEDEP}] )
	dev? ( dev-python/coverage[${PYTHON_USEDEP}] )
	dev? ( dev-python/curlylint[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/hatchling-1.22.2[${PYTHON_USEDEP}] )
	dev? ( dev-python/html5lib[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pkginfo-1.10.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/pydeps[${PYTHON_USEDEP}] )
	dev? ( dev-python/pylint[${PYTHON_USEDEP}] )
	dev? ( dev-python/twine[${PYTHON_USEDEP}] )
	gui? ( dev-python/pyqt5[${PYTHON_USEDEP}] )
	gui? ( dev-python/pyqtwebengine[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
