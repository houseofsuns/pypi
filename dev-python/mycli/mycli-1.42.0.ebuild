# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="CLI for MySQL Database. With auto-completion and syntax highlighting."

HOMEPAGE="http://mycli.net"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all dev llm ssh"
DEPENDENCIES=">=dev-python/click-8.3.1[${PYTHON_USEDEP}]
	>=dev-python/cryptography-1.0.0[${PYTHON_USEDEP}]
	>=dev-python/pygments-1.6[${PYTHON_USEDEP}]
	<dev-python/prompt-toolkit-4.0.0[${PYTHON_USEDEP}]
	>=dev-python/pymysql-0.9.2[${PYTHON_USEDEP}]
	<dev-python/sqlparse-0.6.0[${PYTHON_USEDEP}]
	~dev-python/sqlglot-27.0[${PYTHON_USEDEP}]
	>=dev-python/configobj-5.0.5[${PYTHON_USEDEP}]
	>=dev-python/cli-helpers-2.7.0[${PYTHON_USEDEP}]
	>=dev-python/pyperclip-1.8.1[${PYTHON_USEDEP}]
	dev-python/pycryptodomex[${PYTHON_USEDEP}]
	>=dev-python/pyfzf-0.3.1[${PYTHON_USEDEP}]
	ssh? ( dev-python/paramiko[${PYTHON_USEDEP}] )
	ssh? ( dev-python/sshtunnel[${PYTHON_USEDEP}] )
	llm? ( dev-python/llm[${PYTHON_USEDEP}] )
	llm? ( dev-python/setuptools[${PYTHON_USEDEP}] )
	llm? ( dev-python/pip[${PYTHON_USEDEP}] )
	all? ( dev-python/mycli[${PYTHON_USEDEP}] )
	all? ( dev-python/mycli[${PYTHON_USEDEP}] )
	dev? ( dev-python/behave[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/coverage-7.2.7[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/mypy-1.18.1[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pexpect-4.9.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-7.4.4[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-cov-4.1.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/tox-4.8.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/pdbpp[${PYTHON_USEDEP}] )
	dev? ( dev-python/paramiko[${PYTHON_USEDEP}] )
	dev? ( dev-python/sshtunnel[${PYTHON_USEDEP}] )
	dev? ( dev-python/llm[${PYTHON_USEDEP}] )
	dev? ( dev-python/setuptools[${PYTHON_USEDEP}] )
	dev? ( dev-python/pip[${PYTHON_USEDEP}] )
	dev? ( dev-python/ruff[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
