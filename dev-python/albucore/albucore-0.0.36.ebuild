# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="High-performance image processing functions for deep learning and computer vision."

HOMEPAGE=""
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="contrib contrib-headless gui headless"
DEPENDENCIES=">=dev-python/numpy-1.24.4[${PYTHON_USEDEP}]
	>=dev-python/simsimd-5.9.2[${PYTHON_USEDEP}]
	dev-python/stringzilla[${PYTHON_USEDEP}]
	contrib? ( dev-python/opencv-contrib-python[${PYTHON_USEDEP}] )
	contrib-headless? ( dev-python/opencv-contrib-python-headless[${PYTHON_USEDEP}] )
	gui? ( dev-python/opencv-python[${PYTHON_USEDEP}] )
	headless? ( dev-python/opencv-python-headless[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
