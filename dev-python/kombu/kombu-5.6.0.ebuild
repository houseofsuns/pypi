...
IUSE="azureservicebus azurestoragequeues confluentkafka consul gcpubsub mongodb msgpack pyro redis slmq sqlalchemy sqs yaml zookeeper"
DEPENDENCIES="<dev-python/amqp-6.0.0[${PYTHON_USEDEP}]
	...
	msgpack? ( ~dev-python/msgpack-1.1.2[${PYTHON_USEDEP}] )
	yaml? ( >=dev-python/pyyaml-3.10[${PYTHON_USEDEP}] )
	redis? ( <dev-python/redis-6.5[${PYTHON_USEDEP}] )
	mongodb? ( ~dev-python/pymongo-4.15.3[${PYTHON_USEDEP}] )
	...
	gcpubsub? ( ~dev-python/grpcio-1.75.1[${PYTHON_USEDEP}] )
	gcpubsub? ( ~dev-python/protobuf-6.32.1[${PYTHON_USEDEP}] )
