# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Python REST and Realtime client library SDK for Ably realtime messaging service"

HOMEPAGE="https://ably.com"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="crypto oldcrypto vcdiff"
DEPENDENCIES="<dev-python/h2-5.0.0[${PYTHON_USEDEP}]
	<dev-python/httpx-1.0[${PYTHON_USEDEP}]
	dev-python/methoddispatch[${PYTHON_USEDEP}]
	<dev-python/msgpack-2.0.0[${PYTHON_USEDEP}]
	oldcrypto? ( dev-python/pycrypto[${PYTHON_USEDEP}] )
	crypto? ( dev-python/pycryptodome[${PYTHON_USEDEP}] )
	dev-python/pyee[${PYTHON_USEDEP}]
	vcdiff? ( dev-python/vcdiff-decoder[${PYTHON_USEDEP}] )
	<dev-python/websockets-16.0[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
