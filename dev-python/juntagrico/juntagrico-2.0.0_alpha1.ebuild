# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="2.0.0a1"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="juntagrico is a management platform for community gardens and vegetable cooperatives."

HOMEPAGE="https://www.juntagrico.org"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE=""
DEPENDENCIES="~dev-python/django-5.2.3[${PYTHON_USEDEP}]
	dev-python/django-admin-sortable2[${PYTHON_USEDEP}]
	~dev-python/django-crispy-forms-2.5.0[${PYTHON_USEDEP}]
	~dev-python/crispy-bootstrap4-2025.6.0[${PYTHON_USEDEP}]
	dev-python/django-impersonate[${PYTHON_USEDEP}]
	~dev-python/django-polymorphic-4.10.3[${PYTHON_USEDEP}]
	dev-python/django-richtextfield[${PYTHON_USEDEP}]
	dev-python/django-select2[${PYTHON_USEDEP}]
	dev-python/django-import-export[${PYTHON_USEDEP}]
	~dev-python/icalendar-6.3.0[${PYTHON_USEDEP}]
	dev-python/schwifty[${PYTHON_USEDEP}]
	dev-python/xhtml2pdf[${PYTHON_USEDEP}]
	<dev-python/svglib-1.6.0[${PYTHON_USEDEP}]
	~dev-python/xlsxwriter-3.2.3[${PYTHON_USEDEP}]
	~dev-python/python-dateutil-2.9.0[${PYTHON_USEDEP}]
	dev-python/fontawesomefree[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
