# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A collection of self-contained fsspec-based filesystems"

HOMEPAGE="https://github.com/iterative/morefs"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all asynclocal asynclocalfs dev memfs tests"
DEPENDENCIES="dev-python/fsspec[${PYTHON_USEDEP}]
	all? ( dev-python/pygtrie[${PYTHON_USEDEP}] )
	all? ( >=dev-python/fsspec-2022.10.0[${PYTHON_USEDEP}] )
	all? ( dev-python/aiofile[${PYTHON_USEDEP}] )
	asynclocal? ( >=dev-python/fsspec-2022.10.0[${PYTHON_USEDEP}] )
	asynclocal? ( dev-python/aiofile[${PYTHON_USEDEP}] )
	asynclocalfs? ( >=dev-python/fsspec-2022.10.0[${PYTHON_USEDEP}] )
	asynclocalfs? ( dev-python/aiofile[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pytest-7.2.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pytest-sugar-0.9.5[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pytest-cov-3.0.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pytest-mock-3.8.2[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pytest-asyncio-0.21.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pylint-2.15.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/mypy-0.971[${PYTHON_USEDEP}] )
	dev? ( dev-python/pygtrie[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/fsspec-2022.10.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/aiofile[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-aiofiles[${PYTHON_USEDEP}] )
	memfs? ( dev-python/pygtrie[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pytest-7.2.0[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pytest-sugar-0.9.5[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pytest-cov-3.0.0[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pytest-mock-3.8.2[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pytest-asyncio-0.21.0[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pylint-2.15.0[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/mypy-0.971[${PYTHON_USEDEP}] )
	tests? ( dev-python/pygtrie[${PYTHON_USEDEP}] )
	tests? ( >=dev-python/fsspec-2022.10.0[${PYTHON_USEDEP}] )
	tests? ( dev-python/aiofile[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
