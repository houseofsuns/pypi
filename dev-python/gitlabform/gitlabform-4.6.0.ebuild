# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="🏗 Specialized configuration as a code tool for GitLab projects, groups and more using hierarchical configuration written in YAML"

HOMEPAGE="https://gitlabform.github.io/gitlabform/"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="docs test"
DEPENDENCIES="~dev-python/certifi-2025.8.3[${PYTHON_USEDEP}]
	~dev-python/cli-ui-0.19.0[${PYTHON_USEDEP}]
	dev-python/ez-yaml[${PYTHON_USEDEP}]
	~dev-python/jinja2-3.1.6[${PYTHON_USEDEP}]
	dev-python/luddite[${PYTHON_USEDEP}]
	~dev-python/markupsafe-3.0.3[${PYTHON_USEDEP}]
	~dev-python/mergedeep-1.3.4[${PYTHON_USEDEP}]
	~dev-python/packaging-25.0[${PYTHON_USEDEP}]
	dev-python/python-gitlab[${PYTHON_USEDEP}]
	dev-python/python-gitlab[${PYTHON_USEDEP}]
	~dev-python/requests-2.32.5[${PYTHON_USEDEP}]
	~dev-python/ruamel-yaml-0.17.21[${PYTHON_USEDEP}]
	dev-python/types-requests[${PYTHON_USEDEP}]
	~dev-python/types-setuptools-80.9.0.20250822[${PYTHON_USEDEP}]
	dev-python/yamlpath[${PYTHON_USEDEP}]
	test? ( ~dev-python/coverage-7.10.7[${PYTHON_USEDEP}] )
	test? ( ~dev-python/cryptography-46.0.2[${PYTHON_USEDEP}] )
	test? ( ~dev-python/deepdiff-8.6.1[${PYTHON_USEDEP}] )
	test? ( ~dev-python/mypy-1.18.2[${PYTHON_USEDEP}] )
	test? ( ~dev-python/mypy-extensions-1.1.0[${PYTHON_USEDEP}] )
	test? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-8.4.2[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-cov-7.0.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-rerunfailures-16.0.1[${PYTHON_USEDEP}] )
	test? ( dev-python/xkcdpass[${PYTHON_USEDEP}] )
	docs? ( dev-python/mkdocs[${PYTHON_USEDEP}] )
	docs? ( dev-python/mkdocs-material[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
