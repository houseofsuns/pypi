# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Synapse Intelligence Analysis Framework"

HOMEPAGE="https://vertex.link"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev docs"
DEPENDENCIES="<dev-python/pyopenssl-24.3.0[${PYTHON_USEDEP}]
	<dev-python/cryptography-44.0.0[${PYTHON_USEDEP}]
	<dev-python/msgpack-1.2.0[${PYTHON_USEDEP}]
	<dev-python/xxhash-3.6.0[${PYTHON_USEDEP}]
	<dev-python/lmdb-1.8.0[${PYTHON_USEDEP}]
	<dev-python/tornado-7.0.0[${PYTHON_USEDEP}]
	>=dev-python/regex-2022.9.11[${PYTHON_USEDEP}]
	<dev-python/pyyaml-6.1.0[${PYTHON_USEDEP}]
	<dev-python/aiohttp-4.0[${PYTHON_USEDEP}]
	<dev-python/aiohttp-socks-0.11.0[${PYTHON_USEDEP}]
	dev-python/aiosmtplib[${PYTHON_USEDEP}]
	<dev-python/prompt-toolkit-3.1.0[${PYTHON_USEDEP}]
	~dev-python/lark-1.2.2[${PYTHON_USEDEP}]
	<dev-python/pygments-2.20.0[${PYTHON_USEDEP}]
	<dev-python/packaging-25.0[${PYTHON_USEDEP}]
	<dev-python/fastjsonschema-2.22.0[${PYTHON_USEDEP}]
	dev-python/stix2-validator[${PYTHON_USEDEP}]
	<dev-python/vcrpy-5.2.0[${PYTHON_USEDEP}]
	<dev-python/base58-2.2.0[${PYTHON_USEDEP}]
	dev-python/python-bitcoinlib[${PYTHON_USEDEP}]
	<dev-python/pycryptodome-3.24.0[${PYTHON_USEDEP}]
	<dev-python/typing-extensions-5.0.0[${PYTHON_USEDEP}]
	dev-python/scalecodec[${PYTHON_USEDEP}]
	<dev-python/cbor2-5.7.0[${PYTHON_USEDEP}]
	dev-python/bech32[${PYTHON_USEDEP}]
	<dev-python/oauthlib-4.0.0[${PYTHON_USEDEP}]
	<dev-python/idna-3.11[${PYTHON_USEDEP}]
	<dev-python/python-dateutil-3.0[${PYTHON_USEDEP}]
	<dev-python/pytz-2025.3[${PYTHON_USEDEP}]
	<dev-python/beautifulsoup4-5.0[${PYTHON_USEDEP}]
	dev-python/vtx-yyjson[${PYTHON_USEDEP}]
	dev? ( <dev-python/pytest-8.0.0[${PYTHON_USEDEP}] )
	dev? ( <dev-python/autopep8-3.0.0[${PYTHON_USEDEP}] )
	dev? ( <dev-python/pytest-cov-5.0.0[${PYTHON_USEDEP}] )
	dev? ( <dev-python/pycodestyle-3.0.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/bump2version[${PYTHON_USEDEP}] )
	dev? ( <dev-python/pytest-xdist-4.0.0[${PYTHON_USEDEP}] )
	dev? ( <dev-python/coverage-8.0.0[${PYTHON_USEDEP}] )
	docs? ( <dev-python/sphinx-9.0.0[${PYTHON_USEDEP}] )
	docs? ( <dev-python/sphinx-rtd-theme-4.0.0[${PYTHON_USEDEP}] )
	docs? ( <dev-python/sphinx-notfound-page-2.0.0[${PYTHON_USEDEP}] )
	docs? ( <dev-python/jinja2-4.0.0[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
