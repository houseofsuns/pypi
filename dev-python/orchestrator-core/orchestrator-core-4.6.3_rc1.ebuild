# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="4.6.3rc1"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="This is the orchestrator workflow engine."

HOMEPAGE="https://workfloworchestrator.org/orchestrator-core"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="agent celery search"
DEPENDENCIES="~dev-python/alembic-1.16.1[${PYTHON_USEDEP}]
	>=dev-python/anyio-3.7.0[${PYTHON_USEDEP}]
	>=dev-python/apscheduler-3.11.0[${PYTHON_USEDEP}]
	~dev-python/click-8.0[${PYTHON_USEDEP}]
	~dev-python/deepmerge-2.0[${PYTHON_USEDEP}]
	>=dev-python/deprecated-1.2.18[${PYTHON_USEDEP}]
	dev-python/fastapi[${PYTHON_USEDEP}]
	dev-python/fastapi-etag[${PYTHON_USEDEP}]
	>=dev-python/itsdangerous-2.2.0[${PYTHON_USEDEP}]
	~dev-python/jinja2-3.1.6[${PYTHON_USEDEP}]
	~dev-python/more-itertools-10.7.0[${PYTHON_USEDEP}]
	dev-python/nwa-stdlib[${PYTHON_USEDEP}]
	dev-python/oauth2-lib[${PYTHON_USEDEP}]
	~dev-python/orjson-3.10.18[${PYTHON_USEDEP}]
	dev-python/pgvector[${PYTHON_USEDEP}]
	~dev-python/prometheus-client-0.22.1[${PYTHON_USEDEP}]
	dev-python/psycopg2-binary[${PYTHON_USEDEP}]
	dev-python/pydantic-forms[${PYTHON_USEDEP}]
	dev-python/pydantic-settings[${PYTHON_USEDEP}]
	~dev-python/pydantic-2.11.7[${PYTHON_USEDEP}]
	~dev-python/python-dateutil-2.8.2[${PYTHON_USEDEP}]
	dev-python/python-rapidjson[${PYTHON_USEDEP}]
	~dev-python/pytz-2025.2[${PYTHON_USEDEP}]
	~dev-python/redis-5.1.1[${PYTHON_USEDEP}]
	~dev-python/semver-3.0.4[${PYTHON_USEDEP}]
	dev-python/sentry-sdk[${PYTHON_USEDEP}]
	~dev-python/sqlalchemy-2.0.44[${PYTHON_USEDEP}]
	dev-python/SQLAlchemy-Utils[${PYTHON_USEDEP}]
	dev-python/strawberry-graphql[${PYTHON_USEDEP}]
	>=dev-python/structlog-25.4.0[${PYTHON_USEDEP}]
	~dev-python/tabulate-0.9.0[${PYTHON_USEDEP}]
	~dev-python/typer-0.15.4[${PYTHON_USEDEP}]
	~dev-python/uvicorn-0.34.0[${PYTHON_USEDEP}]
	agent? ( dev-python/pydantic-ai-slim[${PYTHON_USEDEP}] )
	agent? ( dev-python/ag-ui-protocol[${PYTHON_USEDEP}] )
	agent? ( dev-python/litellm[${PYTHON_USEDEP}] )
	celery? ( dev-python/celery[${PYTHON_USEDEP}] )
	search? ( dev-python/litellm[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
