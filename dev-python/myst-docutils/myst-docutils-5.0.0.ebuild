# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="An extended [CommonMark](https://spec.commonmark.org/) compliant parser,"

HOMEPAGE="https://github.com/executablebooks/MyST-Parser"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="code-style linkify rtd sphinx testing testing-docutils"
DEPENDENCIES="dev-python/jinja2[${PYTHON_USEDEP}]
	~dev-python/markdown-it-py-4.0[${PYTHON_USEDEP}]
	~dev-python/mdit-py-plugins-0.5[${PYTHON_USEDEP}]
	dev-python/pyyaml[${PYTHON_USEDEP}]
	code-style? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	linkify? ( ~dev-python/linkify-it-py-2.0[${PYTHON_USEDEP}] )
	rtd? ( >=dev-python/sphinx-8.0[${PYTHON_USEDEP}] )
	rtd? ( dev-python/ipython[${PYTHON_USEDEP}] )
	rtd? ( dev-python/sphinx-book-theme[${PYTHON_USEDEP}] )
	rtd? ( dev-python/sphinx_design[${PYTHON_USEDEP}] )
	rtd? ( dev-python/sphinx-copybutton[${PYTHON_USEDEP}] )
	rtd? ( dev-python/sphinxext-rediraffe[${PYTHON_USEDEP}] )
	rtd? ( dev-python/sphinxext-opengraph[${PYTHON_USEDEP}] )
	rtd? ( dev-python/sphinx_pyscript[${PYTHON_USEDEP}] )
	rtd? ( dev-python/sphinx_tippy[${PYTHON_USEDEP}] )
	rtd? ( dev-python/sphinx-autodoc2[${PYTHON_USEDEP}] )
	rtd? ( dev-python/sphinx-togglebutton[${PYTHON_USEDEP}] )
	sphinx? ( <dev-python/docutils-0.23[${PYTHON_USEDEP}] )
	sphinx? ( <dev-python/sphinx-10.0[${PYTHON_USEDEP}] )
	testing? ( dev-python/beautifulsoup4[${PYTHON_USEDEP}] )
	testing? ( dev-python/coverage[${PYTHON_USEDEP}] )
	testing? ( dev-python/defusedxml[${PYTHON_USEDEP}] )
	testing? ( <dev-python/pytest-10.0[${PYTHON_USEDEP}] )
	testing? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	testing? ( dev-python/pytest-regressions[${PYTHON_USEDEP}] )
	testing? ( ~dev-python/pytest-param-files-0.6.0[${PYTHON_USEDEP}] )
	testing? ( ~dev-python/sphinx-pytest-0.3.0[${PYTHON_USEDEP}] )
	testing? ( <dev-python/pygments-2.20[${PYTHON_USEDEP}] )
	testing-docutils? ( dev-python/pygments[${PYTHON_USEDEP}] )
	testing-docutils? ( <dev-python/pytest-10.0[${PYTHON_USEDEP}] )
	testing-docutils? ( ~dev-python/pytest-param-files-0.6.0[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
