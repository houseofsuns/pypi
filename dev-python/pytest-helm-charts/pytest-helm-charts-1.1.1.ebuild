# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="pytest-helm-charts"
REALVERSION="1.1.1"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_9,3_10,3_11} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A plugin to provide different types and configs of Kubernetes clusters that can be used for testing."

HOMEPAGE="https://github.com/giantswarm/pytest-helm-charts"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/ca/52/8cf0019ccf42574e9fef8925002d06f1f7de9e41d2fe20a1ba7af1cd14b5/pytest_helm_charts-${REALVERSION}.tar.gz"
SOURCEFILE="pytest_helm_charts-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="docs"
DEPENDENCIES="dev-python/pytest[${PYTHON_USEDEP}]
	dev-python/pykube-ng[${PYTHON_USEDEP}]
	docs? ( dev-python/mkdocs[${PYTHON_USEDEP}] )
	docs? ( dev-python/mkapi[${PYTHON_USEDEP}] )
	dev-python/Deprecated[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
