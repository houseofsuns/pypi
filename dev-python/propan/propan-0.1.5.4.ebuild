# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Propan framework: the simplest way to work with a messaging queues"

HOMEPAGE="https://lancetnik.github.io/Propan/"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/p/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="async-kafka async-nats async-rabbit async-redis async-sqs dev dev-doc doc test testsuite"
DEPENDENCIES="dev-python/fast-depends[${PYTHON_USEDEP}]
	dev-python/typer[${PYTHON_USEDEP}]
	dev-python/uvloop[${PYTHON_USEDEP}]
	dev-python/watchfiles[${PYTHON_USEDEP}]
	async-kafka? ( dev-python/aiokafka[${PYTHON_USEDEP}] )
	async-nats? ( dev-python/nats-py[${PYTHON_USEDEP}] )
	async-rabbit? ( dev-python/aio-pika[${PYTHON_USEDEP}] )
	async-redis? ( dev-python/redis[${PYTHON_USEDEP}] )
	async-sqs? ( dev-python/aiobotocore[${PYTHON_USEDEP}] )
	dev? ( dev-python/black[${PYTHON_USEDEP}] )
	dev? ( dev-python/isort[${PYTHON_USEDEP}] )
	dev? ( dev-python/mypy[${PYTHON_USEDEP}] )
	dev? ( dev-python/propan[${PYTHON_USEDEP}] )
	dev? ( dev-python/propan[${PYTHON_USEDEP}] )
	dev? ( dev-python/ruff[${PYTHON_USEDEP}] )
	dev? ( dev-python/typer[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-pyyaml[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-redis[${PYTHON_USEDEP}] )
	dev-doc? ( dev-python/cairosvg[${PYTHON_USEDEP}] )
	dev-doc? ( dev-python/mdx-include[${PYTHON_USEDEP}] )
	dev-doc? ( dev-python/mkdocs-glightbox[${PYTHON_USEDEP}] )
	dev-doc? ( dev-python/mkdocs-macros-plugin[${PYTHON_USEDEP}] )
	dev-doc? ( dev-python/mkdocs-material[${PYTHON_USEDEP}] )
	dev-doc? ( dev-python/mkdocs-static-i18n[${PYTHON_USEDEP}] )
	dev-doc? ( dev-python/pillow[${PYTHON_USEDEP}] )
	dev-doc? ( dev-python/typer[${PYTHON_USEDEP}] )
	doc? ( dev-python/email-validator[${PYTHON_USEDEP}] )
	doc? ( dev-python/fastapi[${PYTHON_USEDEP}] )
	doc? ( dev-python/jsonref[${PYTHON_USEDEP}] )
	doc? ( dev-python/polyfactory[${PYTHON_USEDEP}] )
	doc? ( dev-python/pyyaml[${PYTHON_USEDEP}] )
	doc? ( dev-python/uvicorn[${PYTHON_USEDEP}] )
	test? ( dev-python/fastapi[${PYTHON_USEDEP}] )
	test? ( dev-python/httpx[${PYTHON_USEDEP}] )
	test? ( dev-python/propan[${PYTHON_USEDEP}] )
	test? ( dev-python/propan[${PYTHON_USEDEP}] )
	test? ( dev-python/propan[${PYTHON_USEDEP}] )
	test? ( dev-python/propan[${PYTHON_USEDEP}] )
	test? ( dev-python/propan[${PYTHON_USEDEP}] )
	test? ( dev-python/propan[${PYTHON_USEDEP}] )
	test? ( dev-python/propan[${PYTHON_USEDEP}] )
	testsuite? ( dev-python/coverage[${PYTHON_USEDEP}] )
	testsuite? ( dev-python/pydantic-settings[${PYTHON_USEDEP}] )
	testsuite? ( dev-python/pytest-asyncio[${PYTHON_USEDEP}] )
	testsuite? ( dev-python/pytest[${PYTHON_USEDEP}] )
	testsuite? ( dev-python/python-dotenv[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
