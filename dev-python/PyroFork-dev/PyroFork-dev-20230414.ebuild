# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="2.1.5a1.dev202304141818"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Elegant, modern and asynchronous Telegram MTProto API framework in Python for users and bots [badver 2.1.5a1.dev202304141818]"

HOMEPAGE="https://github.com/Mayuri-Chan/pyrofork"
LICENSE="LGPL-3"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE=""
DEPENDENCIES="~dev-python/pyaes-1.6.1[${PYTHON_USEDEP}]
	~dev-python/pymediainfo-6.0.1[${PYTHON_USEDEP}]
	dev-python/PySocks[${PYTHON_USEDEP}]
	<dev-python/aiosqlite-0.19.0[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
