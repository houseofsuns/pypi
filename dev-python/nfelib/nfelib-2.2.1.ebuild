# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="nfelib: electronic invoicing library for Brazil"

HOMEPAGE="https://github.com/akretion/nfelib"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="pdf sign soap test"
DEPENDENCIES="dev-python/lxml[${PYTHON_USEDEP}]
	dev-python/xsdata[${PYTHON_USEDEP}]
	sign? ( dev-python/erpbrasil-assinatura[${PYTHON_USEDEP}] )
	pdf? ( dev-python/BrazilFiscalReport[${PYTHON_USEDEP}] )
	soap? ( dev-python/xsdata[${PYTHON_USEDEP}] )
	soap? ( dev-python/erpbrasil-assinatura[${PYTHON_USEDEP}] )
	soap? ( dev-python/brazil-fiscal-client[${PYTHON_USEDEP}] )
	test? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-benchmark[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	test? ( dev-python/xmldiff[${PYTHON_USEDEP}] )
	test? ( dev-python/requests[${PYTHON_USEDEP}] )
	test? ( dev-python/requests-pkcs12[${PYTHON_USEDEP}] )
	test? ( dev-python/decorator[${PYTHON_USEDEP}] )
	test? ( dev-python/beautifulsoup4[${PYTHON_USEDEP}] )
	test? ( dev-python/erpbrasil-assinatura[${PYTHON_USEDEP}] )
	test? ( dev-python/BrazilFiscalReport[${PYTHON_USEDEP}] )
	test? ( dev-python/beautifulsoup4[${PYTHON_USEDEP}] )
	test? ( dev-python/pandas[${PYTHON_USEDEP}] )
	test? ( dev-python/brazil-fiscal-client[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
