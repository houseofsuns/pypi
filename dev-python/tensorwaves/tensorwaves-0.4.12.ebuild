# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Python fitter package for multiple computational back-ends"

HOMEPAGE=""
LICENSE="GPLv3 or later"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all dev doc jax jupyter numba phasespace phsp pwa scipy sty tensorflow test test-types tf types viz"
DEPENDENCIES=">=dev-python/pyyaml-5.1[${PYTHON_USEDEP}]
	>=dev-python/attrs-20.1.0[${PYTHON_USEDEP}]
	>=dev-python/iminuit-2.0[${PYTHON_USEDEP}]
	dev-python/numpy[${PYTHON_USEDEP}]
	>=dev-python/sympy-1.9[${PYTHON_USEDEP}]
	>=dev-python/tqdm-4.24.0[${PYTHON_USEDEP}]
	all? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	all? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	all? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	all? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	all? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	all? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	dev? ( dev-python/sphinx-autobuild[${PYTHON_USEDEP}] )
	dev? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	dev? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	dev? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	dev? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	dev? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/tox-1.9[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/sphinx-3.0[${PYTHON_USEDEP}] )
	doc? ( dev-python/ipympl[${PYTHON_USEDEP}] )
	doc? ( dev-python/jupyter[${PYTHON_USEDEP}] )
	doc? ( dev-python/matplotlib[${PYTHON_USEDEP}] )
	doc? ( dev-python/myst-nb[${PYTHON_USEDEP}] )
	doc? ( dev-python/pandas[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-api-relink[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-book-theme[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-codeautolink[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-comments[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-copybutton[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx_design[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-thebe[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-togglebutton[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphobjinv[${PYTHON_USEDEP}] )
	doc? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	jax? ( dev-python/jax[${PYTHON_USEDEP}] )
	jax? ( dev-python/jaxlib[${PYTHON_USEDEP}] )
	jupyter? ( dev-python/black[${PYTHON_USEDEP}] )
	jupyter? ( dev-python/isort[${PYTHON_USEDEP}] )
	jupyter? ( dev-python/jupyterlab[${PYTHON_USEDEP}] )
	jupyter? ( dev-python/jupyterlab-code-formatter[${PYTHON_USEDEP}] )
	jupyter? ( dev-python/jupyterlab-git[${PYTHON_USEDEP}] )
	jupyter? ( dev-python/jupyterlab-lsp[${PYTHON_USEDEP}] )
	jupyter? ( dev-python/jupyterlab-myst[${PYTHON_USEDEP}] )
	jupyter? ( dev-python/python-lsp-ruff[${PYTHON_USEDEP}] )
	jupyter? ( dev-python/python-lsp-server[${PYTHON_USEDEP}] )
	jupyter? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	numba? ( dev-python/numba[${PYTHON_USEDEP}] )
	phasespace? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	phsp? ( dev-python/phasespace[${PYTHON_USEDEP}] )
	phsp? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	pwa? ( dev-python/ampform[${PYTHON_USEDEP}] )
	pwa? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	scipy? ( >=dev-python/scipy-1.0[${PYTHON_USEDEP}] )
	sty? ( >=dev-python/mypy-0.570[${PYTHON_USEDEP}] )
	sty? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	sty? ( dev-python/ruff[${PYTHON_USEDEP}] )
	sty? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	tensorflow? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	test? ( dev-python/ampform[${PYTHON_USEDEP}] )
	test? ( dev-python/nbmake[${PYTHON_USEDEP}] )
	test? ( dev-python/pyarrow[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-benchmark[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-xdist[${PYTHON_USEDEP}] )
	test? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	test-types? ( dev-python/ipython[${PYTHON_USEDEP}] )
	test-types? ( dev-python/pytest[${PYTHON_USEDEP}] )
	test-types? ( >=dev-python/pytest-mock-3.3.0[${PYTHON_USEDEP}] )
	tf? ( dev-python/tensorflow[${PYTHON_USEDEP}] )
	tf? ( dev-python/tensorflow[${PYTHON_USEDEP}] )
	types? ( dev-python/sphinx-api-relink[${PYTHON_USEDEP}] )
	types? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	types? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	types? ( dev-python/tensorwaves[${PYTHON_USEDEP}] )
	types? ( dev-python/types-PyYAML[${PYTHON_USEDEP}] )
	types? ( dev-python/types-requests[${PYTHON_USEDEP}] )
	types? ( dev-python/types-tensorflow[${PYTHON_USEDEP}] )
	types? ( dev-python/types-tqdm[${PYTHON_USEDEP}] )
	viz? ( dev-python/graphviz[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
