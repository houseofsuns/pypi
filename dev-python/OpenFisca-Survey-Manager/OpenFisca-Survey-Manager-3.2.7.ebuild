# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A tool for managing survey/administrative data and import them in OpenFisca"

HOMEPAGE="https://github.com/openfisca/openfisca-survey-manager"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/73/ee/9aa20a88d5f6e9ca3377e311db64e3bb3e66b3b59653344e445f28857f62/openfisca_survey_manager-${REALVERSION}.tar.gz"
SOURCEFILE="openfisca_survey_manager-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="casd dev matching sas"
DEPENDENCIES="<dev-python/chardet-6.0[${PYTHON_USEDEP}]
	dev-python/configparser[${PYTHON_USEDEP}]
	<dev-python/humanize-5.0[${PYTHON_USEDEP}]
	<dev-python/numpy-2.0[${PYTHON_USEDEP}]
	dev-python/OpenFisca-Core[${PYTHON_USEDEP}]
	<dev-python/pandas-3.0[${PYTHON_USEDEP}]
	<dev-python/pyarrow-19.0.0[${PYTHON_USEDEP}]
	<dev-python/pyxdg-0.29[${PYTHON_USEDEP}]
	<dev-python/pyyaml-7.0[${PYTHON_USEDEP}]
	<dev-python/tables-4.0[${PYTHON_USEDEP}]
	<dev-python/tabulate-0.10.0[${PYTHON_USEDEP}]
	dev-python/weightedcalcs[${PYTHON_USEDEP}]
	dev-python/wquantiles[${PYTHON_USEDEP}]
	matching? ( <dev-python/rpy2-4.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/coveralls[${PYTHON_USEDEP}] )
	dev? ( dev-python/openfisca-country-template[${PYTHON_USEDEP}] )
	dev? ( dev-python/pyreadstat[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	dev? ( <dev-python/scipy-2.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-order[${PYTHON_USEDEP}] )
	dev? ( dev-python/ruff[${PYTHON_USEDEP}] )
	dev? ( dev-python/isort[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	dev? ( dev-python/ipdb[${PYTHON_USEDEP}] )
	dev? ( dev-python/pdbpp[${PYTHON_USEDEP}] )
	casd? ( dev-python/pytest[${PYTHON_USEDEP}] )
	casd? ( <dev-python/scipy-2.0[${PYTHON_USEDEP}] )
	casd? ( dev-python/ruff[${PYTHON_USEDEP}] )
	casd? ( dev-python/isort[${PYTHON_USEDEP}] )
	sas? ( dev-python/pyreadstat[${PYTHON_USEDEP}] )
	sas? ( dev-python/sas7bdat[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
