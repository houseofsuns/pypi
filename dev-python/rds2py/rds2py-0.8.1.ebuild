# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Parse and construct Python representations for datasets stored in RDS files"

HOMEPAGE="https://github.com/biocpy/rds2py"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="optional testing"
DEPENDENCIES="dev-python/numpy[${PYTHON_USEDEP}]
	dev-python/biocutils[${PYTHON_USEDEP}]
	optional? ( dev-python/pandas[${PYTHON_USEDEP}] )
	optional? ( dev-python/hdf5array[${PYTHON_USEDEP}] )
	optional? ( dev-python/scipy[${PYTHON_USEDEP}] )
	optional? ( dev-python/BiocFrame[${PYTHON_USEDEP}] )
	optional? ( dev-python/GenomicRanges[${PYTHON_USEDEP}] )
	optional? ( dev-python/SummarizedExperiment[${PYTHON_USEDEP}] )
	optional? ( dev-python/SingleCellExperiment[${PYTHON_USEDEP}] )
	optional? ( dev-python/MultiAssayExperiment[${PYTHON_USEDEP}] )
	optional? ( dev-python/compressed-lists[${PYTHON_USEDEP}] )
	optional? ( dev-python/biocutils[${PYTHON_USEDEP}] )
	testing? ( dev-python/setuptools[${PYTHON_USEDEP}] )
	testing? ( dev-python/pytest[${PYTHON_USEDEP}] )
	testing? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	testing? ( dev-python/pandas[${PYTHON_USEDEP}] )
	testing? ( dev-python/hdf5array[${PYTHON_USEDEP}] )
	testing? ( dev-python/scipy[${PYTHON_USEDEP}] )
	testing? ( dev-python/BiocFrame[${PYTHON_USEDEP}] )
	testing? ( dev-python/GenomicRanges[${PYTHON_USEDEP}] )
	testing? ( dev-python/SummarizedExperiment[${PYTHON_USEDEP}] )
	testing? ( dev-python/SingleCellExperiment[${PYTHON_USEDEP}] )
	testing? ( dev-python/MultiAssayExperiment[${PYTHON_USEDEP}] )
	testing? ( dev-python/compressed-lists[${PYTHON_USEDEP}] )
	testing? ( dev-python/biocutils[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
