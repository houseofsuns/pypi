# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Command line client for interaction with DANDI instances"

HOMEPAGE="http://dandiarchive.org"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all extensions extras style test tools"
DEPENDENCIES="dev-python/bidsschematools[${PYTHON_USEDEP}]
	dev-python/bids-validator-deno[${PYTHON_USEDEP}]
	<dev-python/click-8.2.0[${PYTHON_USEDEP}]
	dev-python/click-didyoumean[${PYTHON_USEDEP}]
	dev-python/dandischema[${PYTHON_USEDEP}]
	dev-python/etelemetry[${PYTHON_USEDEP}]
	dev-python/fasteners[${PYTHON_USEDEP}]
	dev-python/fscacher[${PYTHON_USEDEP}]
	dev-python/hdmf[${PYTHON_USEDEP}]
	dev-python/humanize[${PYTHON_USEDEP}]
	dev-python/interleave[${PYTHON_USEDEP}]
	dev-python/joblib[${PYTHON_USEDEP}]
	>dev-python/keyring-23.9.0[${PYTHON_USEDEP}]
	dev-python/keyrings-alt[${PYTHON_USEDEP}]
	dev-python/packaging[${PYTHON_USEDEP}]
	dev-python/platformdirs[${PYTHON_USEDEP}]
	dev-python/pycryptodomex[${PYTHON_USEDEP}]
	~dev-python/pydantic-2.0[${PYTHON_USEDEP}]
	dev-python/pynwb[${PYTHON_USEDEP}]
	dev-python/numcodecs[${PYTHON_USEDEP}]
	dev-python/nwbinspector[${PYTHON_USEDEP}]
	dev-python/pyout[${PYTHON_USEDEP}]
	dev-python/python-dateutil[${PYTHON_USEDEP}]
	~dev-python/requests-2.20[${PYTHON_USEDEP}]
	<dev-python/ruamel-yaml-1.0[${PYTHON_USEDEP}]
	dev-python/semantic-version[${PYTHON_USEDEP}]
	dev-python/tenacity[${PYTHON_USEDEP}]
	dev-python/tensorstore[${PYTHON_USEDEP}]
	>=dev-python/urllib3-2.0.0[${PYTHON_USEDEP}]
	~dev-python/yarl-1.9[${PYTHON_USEDEP}]
	dev-python/zarr[${PYTHON_USEDEP}]
	dev-python/zarr-checksum[${PYTHON_USEDEP}]
	extensions? ( dev-python/allensdk[${PYTHON_USEDEP}] )
	extensions? ( dev-python/ndx-hed[${PYTHON_USEDEP}] )
	extensions? ( dev-python/ndx-events[${PYTHON_USEDEP}] )
	extras? ( dev-python/duecredit[${PYTHON_USEDEP}] )
	extras? ( dev-python/fsspec[${PYTHON_USEDEP}] )
	style? ( dev-python/flake8[${PYTHON_USEDEP}] )
	style? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	test? ( dev-python/anys[${PYTHON_USEDEP}] )
	test? ( dev-python/coverage[${PYTHON_USEDEP}] )
	test? ( dev-python/opencv-python[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-mock[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-rerunfailures[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-timeout[${PYTHON_USEDEP}] )
	test? ( >dev-python/responses-0.24.0[${PYTHON_USEDEP}] )
	test? ( dev-python/vcrpy[${PYTHON_USEDEP}] )
	tools? ( dev-python/boto3[${PYTHON_USEDEP}] )
	all? ( dev-python/allensdk[${PYTHON_USEDEP}] )
	all? ( dev-python/ndx-hed[${PYTHON_USEDEP}] )
	all? ( dev-python/ndx-events[${PYTHON_USEDEP}] )
	all? ( dev-python/duecredit[${PYTHON_USEDEP}] )
	all? ( dev-python/fsspec[${PYTHON_USEDEP}] )
	all? ( dev-python/flake8[${PYTHON_USEDEP}] )
	all? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	all? ( dev-python/anys[${PYTHON_USEDEP}] )
	all? ( dev-python/coverage[${PYTHON_USEDEP}] )
	all? ( dev-python/opencv-python[${PYTHON_USEDEP}] )
	all? ( dev-python/pytest[${PYTHON_USEDEP}] )
	all? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	all? ( dev-python/pytest-mock[${PYTHON_USEDEP}] )
	all? ( dev-python/pytest-rerunfailures[${PYTHON_USEDEP}] )
	all? ( dev-python/pytest-timeout[${PYTHON_USEDEP}] )
	all? ( >dev-python/responses-0.24.0[${PYTHON_USEDEP}] )
	all? ( dev-python/vcrpy[${PYTHON_USEDEP}] )
	all? ( dev-python/boto3[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
