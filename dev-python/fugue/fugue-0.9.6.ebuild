# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="An abstraction layer for distributed computation"

HOMEPAGE="http://github.com/fugue-project/fugue"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all cpp-sql-parser dask duckdb ibis notebook polars ray spark sql"
DEPENDENCIES="dev-python/triad[${PYTHON_USEDEP}]
	dev-python/adagio[${PYTHON_USEDEP}]
	<dev-python/pandas-3.0[${PYTHON_USEDEP}]
	sql? ( dev-python/qpd[${PYTHON_USEDEP}] )
	sql? ( dev-python/fugue-sql-antlr[${PYTHON_USEDEP}] )
	sql? ( <dev-python/sqlglot-28.0[${PYTHON_USEDEP}] )
	sql? ( dev-python/jinja2[${PYTHON_USEDEP}] )
	cpp-sql-parser? ( dev-python/fugue-sql-antlr[${PYTHON_USEDEP}] )
	spark? ( dev-python/pyspark[${PYTHON_USEDEP}] )
	spark? ( >=dev-python/zstandard-0.25.0[${PYTHON_USEDEP}] )
	dask? ( dev-python/dask[${PYTHON_USEDEP}] )
	dask? ( >=dev-python/pyarrow-7.0.0[${PYTHON_USEDEP}] )
	dask? ( >=dev-python/pandas-2.0.2[${PYTHON_USEDEP}] )
	ray? ( dev-python/ray[${PYTHON_USEDEP}] )
	ray? ( dev-python/duckdb[${PYTHON_USEDEP}] )
	ray? ( >=dev-python/pyarrow-7.0.0[${PYTHON_USEDEP}] )
	ray? ( dev-python/pandas[${PYTHON_USEDEP}] )
	duckdb? ( dev-python/qpd[${PYTHON_USEDEP}] )
	duckdb? ( dev-python/fugue-sql-antlr[${PYTHON_USEDEP}] )
	duckdb? ( <dev-python/sqlglot-28.0[${PYTHON_USEDEP}] )
	duckdb? ( dev-python/jinja2[${PYTHON_USEDEP}] )
	duckdb? ( dev-python/duckdb[${PYTHON_USEDEP}] )
	duckdb? ( dev-python/numpy[${PYTHON_USEDEP}] )
	polars? ( dev-python/polars[${PYTHON_USEDEP}] )
	ibis? ( dev-python/qpd[${PYTHON_USEDEP}] )
	ibis? ( dev-python/fugue-sql-antlr[${PYTHON_USEDEP}] )
	ibis? ( <dev-python/sqlglot-28.0[${PYTHON_USEDEP}] )
	ibis? ( dev-python/jinja2[${PYTHON_USEDEP}] )
	ibis? ( dev-python/ibis-framework[${PYTHON_USEDEP}] )
	notebook? ( dev-python/notebook[${PYTHON_USEDEP}] )
	notebook? ( dev-python/jupyterlab[${PYTHON_USEDEP}] )
	notebook? ( >=dev-python/ipython-7.10.0[${PYTHON_USEDEP}] )
	all? ( dev-python/qpd[${PYTHON_USEDEP}] )
	all? ( dev-python/fugue-sql-antlr[${PYTHON_USEDEP}] )
	all? ( <dev-python/sqlglot-28.0[${PYTHON_USEDEP}] )
	all? ( dev-python/jinja2[${PYTHON_USEDEP}] )
	all? ( dev-python/pyspark[${PYTHON_USEDEP}] )
	all? ( dev-python/dask[${PYTHON_USEDEP}] )
	all? ( dev-python/dask-sql[${PYTHON_USEDEP}] )
	all? ( dev-python/ray[${PYTHON_USEDEP}] )
	all? ( dev-python/notebook[${PYTHON_USEDEP}] )
	all? ( dev-python/jupyterlab[${PYTHON_USEDEP}] )
	all? ( >=dev-python/ipython-7.10.0[${PYTHON_USEDEP}] )
	all? ( dev-python/duckdb[${PYTHON_USEDEP}] )
	all? ( >=dev-python/pyarrow-6.0.1[${PYTHON_USEDEP}] )
	all? ( >=dev-python/pandas-2.0.2[${PYTHON_USEDEP}] )
	all? ( dev-python/ibis-framework[${PYTHON_USEDEP}] )
	all? ( dev-python/polars[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
