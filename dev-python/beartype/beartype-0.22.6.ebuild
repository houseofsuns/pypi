# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Unbearably fast near-real-time pure-Python runtime-static type-checker."

HOMEPAGE=""
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev doc-ghp doc-rtd test test-tox test-tox-coverage"
DEPENDENCIES="dev? ( dev-python/autoapi[${PYTHON_USEDEP}] )
	dev? ( dev-python/celery[${PYTHON_USEDEP}] )
	dev? ( dev-python/click[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/coverage-5.5[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/docutils-0.22.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/equinox[${PYTHON_USEDEP}] )
	dev? ( dev-python/fastmcp[${PYTHON_USEDEP}] )
	dev? ( dev-python/jax[${PYTHON_USEDEP}] )
	dev? ( dev-python/jaxtyping[${PYTHON_USEDEP}] )
	dev? ( dev-python/langchain[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/mypy-0.800[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/nuitka-1.2.6[${PYTHON_USEDEP}] )
	dev? ( dev-python/numba[${PYTHON_USEDEP}] )
	dev? ( dev-python/numpy[${PYTHON_USEDEP}] )
	dev? ( dev-python/pandera[${PYTHON_USEDEP}] )
	dev? ( dev-python/poetry[${PYTHON_USEDEP}] )
	dev? ( dev-python/polars[${PYTHON_USEDEP}] )
	dev? ( <=dev-python/pydata-sphinx-theme-0.7.2[${PYTHON_USEDEP}] )
	dev? ( dev-python/pygments[${PYTHON_USEDEP}] )
	dev? ( dev-python/pyright[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-6.2.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/redis[${PYTHON_USEDEP}] )
	dev? ( dev-python/rich-click[${PYTHON_USEDEP}] )
	dev? ( dev-python/setuptools[${PYTHON_USEDEP}] )
	dev? ( dev-python/sphinx[${PYTHON_USEDEP}] )
	dev? ( <dev-python/sphinx-6.0.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/sphinxext-opengraph[${PYTHON_USEDEP}] )
	dev? ( dev-python/sqlalchemy[${PYTHON_USEDEP}] )
	dev? ( dev-python/torch[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/tox-3.20.1[${PYTHON_USEDEP}] )
	dev? ( dev-python/typer[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/typing-extensions-3.10.0.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/xarray[${PYTHON_USEDEP}] )
	doc-ghp? ( >=dev-python/mkdocs-material-9.6.0[${PYTHON_USEDEP}] )
	doc-ghp? ( dev-python/mkdocstrings-python-xref[${PYTHON_USEDEP}] )
	doc-ghp? ( >=dev-python/mkdocstrings-python-1.16.0[${PYTHON_USEDEP}] )
	doc-rtd? ( dev-python/autoapi[${PYTHON_USEDEP}] )
	doc-rtd? ( <=dev-python/pydata-sphinx-theme-0.7.2[${PYTHON_USEDEP}] )
	doc-rtd? ( dev-python/setuptools[${PYTHON_USEDEP}] )
	doc-rtd? ( <dev-python/sphinx-6.0.0[${PYTHON_USEDEP}] )
	doc-rtd? ( dev-python/sphinxext-opengraph[${PYTHON_USEDEP}] )
	test? ( dev-python/celery[${PYTHON_USEDEP}] )
	test? ( dev-python/click[${PYTHON_USEDEP}] )
	test? ( >=dev-python/coverage-5.5[${PYTHON_USEDEP}] )
	test? ( >=dev-python/docutils-0.22.0[${PYTHON_USEDEP}] )
	test? ( dev-python/equinox[${PYTHON_USEDEP}] )
	test? ( dev-python/fastmcp[${PYTHON_USEDEP}] )
	test? ( dev-python/jax[${PYTHON_USEDEP}] )
	test? ( dev-python/jaxtyping[${PYTHON_USEDEP}] )
	test? ( dev-python/langchain[${PYTHON_USEDEP}] )
	test? ( >=dev-python/mypy-0.800[${PYTHON_USEDEP}] )
	test? ( >=dev-python/nuitka-1.2.6[${PYTHON_USEDEP}] )
	test? ( dev-python/numba[${PYTHON_USEDEP}] )
	test? ( dev-python/numpy[${PYTHON_USEDEP}] )
	test? ( dev-python/pandera[${PYTHON_USEDEP}] )
	test? ( dev-python/poetry[${PYTHON_USEDEP}] )
	test? ( dev-python/polars[${PYTHON_USEDEP}] )
	test? ( dev-python/pygments[${PYTHON_USEDEP}] )
	test? ( dev-python/pyright[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-6.2.0[${PYTHON_USEDEP}] )
	test? ( dev-python/redis[${PYTHON_USEDEP}] )
	test? ( dev-python/rich-click[${PYTHON_USEDEP}] )
	test? ( dev-python/sphinx[${PYTHON_USEDEP}] )
	test? ( dev-python/sqlalchemy[${PYTHON_USEDEP}] )
	test? ( dev-python/torch[${PYTHON_USEDEP}] )
	test? ( >=dev-python/tox-3.20.1[${PYTHON_USEDEP}] )
	test? ( dev-python/typer[${PYTHON_USEDEP}] )
	test? ( >=dev-python/typing-extensions-3.10.0.0[${PYTHON_USEDEP}] )
	test? ( dev-python/xarray[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/celery[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/click[${PYTHON_USEDEP}] )
	test-tox? ( >=dev-python/docutils-0.22.0[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/equinox[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/fastmcp[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/jax[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/jaxtyping[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/langchain[${PYTHON_USEDEP}] )
	test-tox? ( >=dev-python/mypy-0.800[${PYTHON_USEDEP}] )
	test-tox? ( >=dev-python/nuitka-1.2.6[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/numba[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/numpy[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/pandera[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/poetry[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/polars[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/pygments[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/pyright[${PYTHON_USEDEP}] )
	test-tox? ( >=dev-python/pytest-6.2.0[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/redis[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/rich-click[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/sphinx[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/sqlalchemy[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/torch[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/typer[${PYTHON_USEDEP}] )
	test-tox? ( >=dev-python/typing-extensions-3.10.0.0[${PYTHON_USEDEP}] )
	test-tox? ( dev-python/xarray[${PYTHON_USEDEP}] )
	test-tox-coverage? ( >=dev-python/coverage-5.5[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
