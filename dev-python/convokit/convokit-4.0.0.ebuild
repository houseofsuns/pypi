# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="ConvoKit"

HOMEPAGE="https://github.com/CornellNLP/ConvoKit"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="craft forecaster genai llm"
DEPENDENCIES=">=dev-python/matplotlib-3.0.0[${PYTHON_USEDEP}]
	>dev-python/scipy-1.14[${PYTHON_USEDEP}]
	>=dev-python/pandas-1.5.0[${PYTHON_USEDEP}]
	>=dev-python/numpy-2.0.0[${PYTHON_USEDEP}]
	dev-python/msgpack-numpy[${PYTHON_USEDEP}]
	dev-python/spacy[${PYTHON_USEDEP}]
	>=dev-python/scikit-learn-1.0[${PYTHON_USEDEP}]
	dev-python/nltk[${PYTHON_USEDEP}]
	>=dev-python/dill-0.2.9[${PYTHON_USEDEP}]
	>=dev-python/joblib-0.13.2[${PYTHON_USEDEP}]
	dev-python/clean-text[${PYTHON_USEDEP}]
	>=dev-python/unidecode-1.1.1[${PYTHON_USEDEP}]
	>=dev-python/tqdm-4.64.0[${PYTHON_USEDEP}]
	>=dev-python/pymongo-4.0[${PYTHON_USEDEP}]
	>=dev-python/pyyaml-5.4.1[${PYTHON_USEDEP}]
	>=dev-python/dnspython-1.16.0[${PYTHON_USEDEP}]
	dev-python/thinc[${PYTHON_USEDEP}]
	~dev-python/h5py-3.12.1[${PYTHON_USEDEP}]
	>=dev-python/numexpr-2.8.0[${PYTHON_USEDEP}]
	dev-python/ruff[${PYTHON_USEDEP}]
	dev-python/bottleneck[${PYTHON_USEDEP}]
	dev-python/accelerate[${PYTHON_USEDEP}]
	dev-python/peft[${PYTHON_USEDEP}]
	dev-python/bitsandbytes[${PYTHON_USEDEP}]
	dev-python/transformers[${PYTHON_USEDEP}]
	dev-python/unsloth[${PYTHON_USEDEP}]
	dev-python/trl[${PYTHON_USEDEP}]
	dev-python/tensorflow[${PYTHON_USEDEP}]
	dev-python/tf-keras[${PYTHON_USEDEP}]
	dev-python/evaluate[${PYTHON_USEDEP}]
	dev-python/sentence-transformers[${PYTHON_USEDEP}]
	dev-python/datasets[${PYTHON_USEDEP}]
	craft? ( dev-python/torch[${PYTHON_USEDEP}] )
	forecaster? ( dev-python/torch[${PYTHON_USEDEP}] )
	forecaster? ( dev-python/datasets[${PYTHON_USEDEP}] )
	genai? ( dev-python/openai[${PYTHON_USEDEP}] )
	genai? ( dev-python/google-genai[${PYTHON_USEDEP}] )
	llm? ( dev-python/torch[${PYTHON_USEDEP}] )
	llm? ( dev-python/accelerate[${PYTHON_USEDEP}] )
	llm? ( dev-python/peft[${PYTHON_USEDEP}] )
	llm? ( dev-python/bitsandbytes[${PYTHON_USEDEP}] )
	llm? ( dev-python/transformers[${PYTHON_USEDEP}] )
	llm? ( dev-python/unsloth[${PYTHON_USEDEP}] )
	llm? ( dev-python/trl[${PYTHON_USEDEP}] )
	llm? ( dev-python/tensorflow[${PYTHON_USEDEP}] )
	llm? ( dev-python/tf-keras[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
