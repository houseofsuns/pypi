# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Nigthly build of Snowflake Connector for Python"

HOMEPAGE="https://www.snowflake.com/"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="aio aioboto boto development pandas secure-local-storage"
DEPENDENCIES="<dev-python/asn1crypto-2.0.0[${PYTHON_USEDEP}]
	>=dev-python/cryptography-44.0.1[${PYTHON_USEDEP}]
	<dev-python/pyopenssl-26.0.0[${PYTHON_USEDEP}]
	<dev-python/pyjwt-3.0.0[${PYTHON_USEDEP}]
	dev-python/pytz[${PYTHON_USEDEP}]
	<dev-python/requests-3.0.0[${PYTHON_USEDEP}]
	dev-python/packaging[${PYTHON_USEDEP}]
	<dev-python/charset-normalizer-4.0[${PYTHON_USEDEP}]
	<dev-python/idna-4.0[${PYTHON_USEDEP}]
	>=dev-python/certifi-2024.7.4[${PYTHON_USEDEP}]
	<dev-python/typing-extensions-5.0[${PYTHON_USEDEP}]
	<dev-python/filelock-4.0[${PYTHON_USEDEP}]
	>=dev-python/sortedcontainers-2.4.0[${PYTHON_USEDEP}]
	<dev-python/platformdirs-5.0.0[${PYTHON_USEDEP}]
	dev-python/tomlkit[${PYTHON_USEDEP}]
	>=dev-python/boto3-1.24[${PYTHON_USEDEP}]
	>=dev-python/botocore-1.24[${PYTHON_USEDEP}]
	boto? ( >=dev-python/boto3-1.24[${PYTHON_USEDEP}] )
	boto? ( >=dev-python/botocore-1.24[${PYTHON_USEDEP}] )
	development? ( dev-python/cython[${PYTHON_USEDEP}] )
	development? ( dev-python/coverage[${PYTHON_USEDEP}] )
	development? ( dev-python/mitmproxy[${PYTHON_USEDEP}] )
	development? ( dev-python/more-itertools[${PYTHON_USEDEP}] )
	development? ( <=dev-python/numpy-2.2.4[${PYTHON_USEDEP}] )
	development? ( dev-python/pendulum[${PYTHON_USEDEP}] )
	development? ( dev-python/pexpect[${PYTHON_USEDEP}] )
	development? ( <dev-python/pytest-7.5.0[${PYTHON_USEDEP}] )
	development? ( dev-python/pytest-asyncio[${PYTHON_USEDEP}] )
	development? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	development? ( <dev-python/pytest-rerunfailures-16.0[${PYTHON_USEDEP}] )
	development? ( dev-python/pytest-timeout[${PYTHON_USEDEP}] )
	development? ( dev-python/pytest-xdist[${PYTHON_USEDEP}] )
	development? ( dev-python/pytzdata[${PYTHON_USEDEP}] )
	development? ( dev-python/responses[${PYTHON_USEDEP}] )
	pandas? ( <dev-python/pandas-3.0.0[${PYTHON_USEDEP}] )
	pandas? ( <dev-python/pandas-3.0.0[${PYTHON_USEDEP}] )
	pandas? ( >=dev-python/pyarrow-14.0.1[${PYTHON_USEDEP}] )
	secure-local-storage? ( <dev-python/keyring-26.0.0[${PYTHON_USEDEP}] )
	aio? ( >=dev-python/aiohttp-3.12.14[${PYTHON_USEDEP}] )
	aio? ( dev-python/aioboto3[${PYTHON_USEDEP}] )
	aioboto? ( dev-python/aioboto3[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
