# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="0.2.0.post1"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=wheel

inherit python-r1 gs-pypi

DESCRIPTION="Smash your AI models [wheel]"

HOMEPAGE=""
LICENSE="All Rights Reserved"
SRC_URI="https://files.pythonhosted.org/packages/py3/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}-py3-none-any.whl"
SOURCEFILE="${REALNAME}-${REALVERSION}-py3-none-any.whl"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="autoawq dev full onediff stable-fast stable-fast-cu11 tests"
DEPENDENCIES="dev-python/ConfigSpace[${PYTHON_USEDEP}]
	dev-python/deepcache[${PYTHON_USEDEP}]
	dev-python/auto-gptq[${PYTHON_USEDEP}]
	autoawq? ( dev-python/autoawq[${PYTHON_USEDEP}] )
	full? ( dev-python/autoawq[${PYTHON_USEDEP}] )
	full? ( dev-python/autoawq[${PYTHON_USEDEP}] )
	dev-python/bitsandbytes[${PYTHON_USEDEP}]
	dev? ( dev-python/black[${PYTHON_USEDEP}] )
	tests? ( dev-python/black[${PYTHON_USEDEP}] )
	dev-python/blobfile[${PYTHON_USEDEP}]
	dev? ( dev-python/build[${PYTHON_USEDEP}] )
	dev-python/codecarbon[${PYTHON_USEDEP}]
	dev-python/colorama[${PYTHON_USEDEP}]
	tests? ( dev-python/coverage[${PYTHON_USEDEP}] )
	dev-python/ctranslate2[${PYTHON_USEDEP}]
	dev-python/cython[${PYTHON_USEDEP}]
	dev-python/diffusers[${PYTHON_USEDEP}]
	tests? ( dev-python/docutils[${PYTHON_USEDEP}] )
	dev? ( dev-python/flake8[${PYTHON_USEDEP}] )
	tests? ( dev-python/flake8[${PYTHON_USEDEP}] )
	dev? ( dev-python/flake8-annotations[${PYTHON_USEDEP}] )
	tests? ( dev-python/flake8-annotations[${PYTHON_USEDEP}] )
	dev? ( dev-python/flake8-docstrings[${PYTHON_USEDEP}] )
	tests? ( dev-python/flake8-docstrings[${PYTHON_USEDEP}] )
	dev-python/hqq[${PYTHON_USEDEP}]
	dev-python/huggingface-hub[${PYTHON_USEDEP}]
	>=dev-python/ipywidgets-8.1.5[${PYTHON_USEDEP}]
	dev? ( dev-python/isort[${PYTHON_USEDEP}] )
	tests? ( dev-python/isort[${PYTHON_USEDEP}] )
	dev-python/jsonschema[${PYTHON_USEDEP}]
	dev? ( dev-python/jupyterlab[${PYTHON_USEDEP}] )
	tests? ( dev-python/jupyterlab[${PYTHON_USEDEP}] )
	dev-python/lago-python-client[${PYTHON_USEDEP}]
	dev-python/librosa[${PYTHON_USEDEP}]
	tests? ( dev-python/mypy[${PYTHON_USEDEP}] )
	onediff? ( dev-python/nexfort[${PYTHON_USEDEP}] )
	full? ( dev-python/nexfort[${PYTHON_USEDEP}] )
	dev? ( dev-python/notebook[${PYTHON_USEDEP}] )
	>=dev-python/numpy-1.24.4[${PYTHON_USEDEP}]
	dev? ( dev-python/numpydoc-validation[${PYTHON_USEDEP}] )
	tests? ( dev-python/numpydoc-validation[${PYTHON_USEDEP}] )
	onediff? ( dev-python/onediff[${PYTHON_USEDEP}] )
	full? ( dev-python/onediff[${PYTHON_USEDEP}] )
	>=dev-python/opentelemetry-api-1.30.0[${PYTHON_USEDEP}]
	dev-python/opentelemetry-exporter-otlp[${PYTHON_USEDEP}]
	>=dev-python/opentelemetry-sdk-1.30.0[${PYTHON_USEDEP}]
	dev-python/openvino[${PYTHON_USEDEP}]
	dev-python/optimum[${PYTHON_USEDEP}]
	dev-python/optimum-quanto[${PYTHON_USEDEP}]
	dev-python/optuna[${PYTHON_USEDEP}]
	dev-python/packaging[${PYTHON_USEDEP}]
	dev-python/peft[${PYTHON_USEDEP}]
	>=dev-python/pillow-9.5.0[${PYTHON_USEDEP}]
	dev-python/platformdirs[${PYTHON_USEDEP}]
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	dev-python/protobuf[${PYTHON_USEDEP}]
	dev? ( dev-python/pyc-wheel[${PYTHON_USEDEP}] )
	dev-python/pynvml[${PYTHON_USEDEP}]
	tests? ( ~dev-python/pytest-7.4.4[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	dev-python/python-dotenv[${PYTHON_USEDEP}]
	dev-python/pytorch-lightning[${PYTHON_USEDEP}]
	>=dev-python/requests-2.31.0[${PYTHON_USEDEP}]
	dev-python/sentencepiece[${PYTHON_USEDEP}]
	dev-python/setuptools[${PYTHON_USEDEP}]
	dev-python/soundfile[${PYTHON_USEDEP}]
	stable-fast? ( dev-python/stable-fast-pruna[${PYTHON_USEDEP}] )
	full? ( dev-python/stable-fast-pruna[${PYTHON_USEDEP}] )
	dev-python/thop[${PYTHON_USEDEP}]
	dev-python/timm[${PYTHON_USEDEP}]
	dev-python/torch[${PYTHON_USEDEP}]
	dev-python/torch-pruning[${PYTHON_USEDEP}]
	dev-python/torchao[${PYTHON_USEDEP}]
	dev-python/torchmetrics[${PYTHON_USEDEP}]
	dev-python/torchvision[${PYTHON_USEDEP}]
	dev-python/transformers[${PYTHON_USEDEP}]
	dev-python/trl[${PYTHON_USEDEP}]
	dev? ( dev-python/twine[${PYTHON_USEDEP}] )
	tests? ( dev-python/types-PyYAML[${PYTHON_USEDEP}] )
	dev-python/wget[${PYTHON_USEDEP}]
	dev-python/whisper-s2t[${PYTHON_USEDEP}]
	stable-fast? ( dev-python/xformers[${PYTHON_USEDEP}] )
	stable-fast-cu11? ( dev-python/xformers[${PYTHON_USEDEP}] )
	full? ( dev-python/xformers[${PYTHON_USEDEP}] )
	virtual/allow-pypi-wheels"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
