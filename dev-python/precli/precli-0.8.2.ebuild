# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Precaution security static analysis command line"

HOMEPAGE="https://github.com/securesauce/precli"
LICENSE="Other/Proprietary License"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE=""
DEPENDENCIES="~dev-python/tomli-w-1.2.0[${PYTHON_USEDEP}]
	~dev-python/rich-14.1.0[${PYTHON_USEDEP}]
	~dev-python/tree-sitter-0.25.1[${PYTHON_USEDEP}]
	dev-python/ignorelib[${PYTHON_USEDEP}]
	~dev-python/requests-2.32.5[${PYTHON_USEDEP}]
	~dev-python/sarif-om-1.0.4[${PYTHON_USEDEP}]
	~dev-python/jschema-to-python-1.2.3[${PYTHON_USEDEP}]
	dev-python/tree-sitter-go[${PYTHON_USEDEP}]
	dev-python/tree-sitter-java[${PYTHON_USEDEP}]
	dev-python/tree-sitter-python[${PYTHON_USEDEP}]
	~dev-python/pyyaml-6.0.2[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
