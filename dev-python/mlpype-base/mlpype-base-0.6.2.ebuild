# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION=""

HOMEPAGE=""
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev strict test"
DEPENDENCIES="dev-python/docstring-parser[${PYTHON_USEDEP}]
	>=dev-python/pydantic-1.10.7[${PYTHON_USEDEP}]
	>=dev-python/joblib-1.1.1[${PYTHON_USEDEP}]
	~dev-python/pyyaml-6.0.1[${PYTHON_USEDEP}]
	dev-python/jinja[${PYTHON_USEDEP}]
	dev? ( dev-python/docstring-parser[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pydantic-1.10.7[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/joblib-1.1.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pyyaml-6.0.1[${PYTHON_USEDEP}] )
	dev? ( dev-python/jinja[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/build-0.8.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pypiserver-1.5.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/twine-4.0.1[${PYTHON_USEDEP}] )
	dev? ( <dev-python/importlib-metadata-8.0.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/sphinx-autoapi-3.1.2[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/sphinx-rtd-theme-2.0.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/myst-parser-3.0.1[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	strict? ( dev-python/docstring-parser[${PYTHON_USEDEP}] )
	strict? ( ~dev-python/pydantic-1.10.7[${PYTHON_USEDEP}] )
	strict? ( ~dev-python/joblib-1.1.1[${PYTHON_USEDEP}] )
	strict? ( ~dev-python/pyyaml-6.0.1[${PYTHON_USEDEP}] )
	strict? ( dev-python/jinja[${PYTHON_USEDEP}] )
	test? ( dev-python/docstring-parser[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pydantic-1.10.7[${PYTHON_USEDEP}] )
	test? ( ~dev-python/joblib-1.1.1[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pyyaml-6.0.1[${PYTHON_USEDEP}] )
	test? ( dev-python/jinja[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
