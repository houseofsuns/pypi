# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A domain-specific language for modeling convex optimization problems in Python."

HOMEPAGE="https://github.com/cvxpy/cvxpy"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="cbc cuopt cvxopt daqp diffcp doc ecos ecos-bb glop glpk glpk-mi gurobi highs mosek pdlp piqp proxqp qoco scip scipy testing xpress"
DEPENDENCIES=">=dev-python/numpy-1.22.4[${PYTHON_USEDEP}]
	>=dev-python/scipy-1.13.0[${PYTHON_USEDEP}]
	cbc? ( dev-python/cylp[${PYTHON_USEDEP}] )
	cuopt? ( dev-python/cuopt-cu12[${PYTHON_USEDEP}] )
	cuopt? ( dev-python/cuopt-sh-client[${PYTHON_USEDEP}] )
	cuopt? ( dev-python/nvidia-cuda-runtime-cu12[${PYTHON_USEDEP}] )
	cvxopt? ( dev-python/cvxopt[${PYTHON_USEDEP}] )
	diffcp? ( dev-python/diffcp[${PYTHON_USEDEP}] )
	ecos? ( dev-python/ecos[${PYTHON_USEDEP}] )
	ecos-bb? ( dev-python/ecos[${PYTHON_USEDEP}] )
	glop? ( dev-python/ortools[${PYTHON_USEDEP}] )
	glpk? ( dev-python/cvxopt[${PYTHON_USEDEP}] )
	glpk-mi? ( dev-python/cvxopt[${PYTHON_USEDEP}] )
	gurobi? ( dev-python/gurobipy[${PYTHON_USEDEP}] )
	highs? ( dev-python/highspy[${PYTHON_USEDEP}] )
	mosek? ( dev-python/Mosek[${PYTHON_USEDEP}] )
	pdlp? ( dev-python/ortools[${PYTHON_USEDEP}] )
	piqp? ( dev-python/piqp[${PYTHON_USEDEP}] )
	proxqp? ( dev-python/proxsuite[${PYTHON_USEDEP}] )
	qoco? ( dev-python/qoco[${PYTHON_USEDEP}] )
	scip? ( dev-python/PySCIPOpt[${PYTHON_USEDEP}] )
	scipy? ( dev-python/scipy[${PYTHON_USEDEP}] )
	xpress? ( dev-python/xpress[${PYTHON_USEDEP}] )
	daqp? ( dev-python/daqp[${PYTHON_USEDEP}] )
	testing? ( dev-python/pytest[${PYTHON_USEDEP}] )
	testing? ( dev-python/hypothesis[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinxcontrib-jquery[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-inline-tabs[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx_design[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-immaterial[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
