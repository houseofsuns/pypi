# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Python package for interacting with IP Fabric"

HOMEPAGE="https://gitlab.com/ip-fabric/integrations/python-ipfabric"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all cli cve dynamic-attributes examples matrix pd webhook"
DEPENDENCIES="dev-python/niquests[${PYTHON_USEDEP}]
	<dev-python/packaging-26.0[${PYTHON_USEDEP}]
	<dev-python/pydantic-3.0.0[${PYTHON_USEDEP}]
	dev-python/pydantic-settings[${PYTHON_USEDEP}]
	dev-python/pydantic-extra-types[${PYTHON_USEDEP}]
	<dev-python/pyjwt-3.0.0[${PYTHON_USEDEP}]
	<dev-python/python-dateutil-3.0.0[${PYTHON_USEDEP}]
	<dev-python/pytz-2026.0[${PYTHON_USEDEP}]
	<dev-python/python-dotenv-2.0[${PYTHON_USEDEP}]
	dev-python/macaddress[${PYTHON_USEDEP}]
	<dev-python/deepdiff-9.0.0[${PYTHON_USEDEP}]
	dev-python/case-insensitive-dictionary[${PYTHON_USEDEP}]
	cve? ( <dev-python/pandas-3.0.0[${PYTHON_USEDEP}] )
	matrix? ( <dev-python/pandas-3.0.0[${PYTHON_USEDEP}] )
	cli? ( <dev-python/pandas-3.0.0[${PYTHON_USEDEP}] )
	examples? ( <dev-python/pandas-3.0.0[${PYTHON_USEDEP}] )
	pd? ( <dev-python/pandas-3.0.0[${PYTHON_USEDEP}] )
	all? ( <dev-python/pandas-3.0.0[${PYTHON_USEDEP}] )
	cve? ( <dev-python/openpyxl-4.0.0[${PYTHON_USEDEP}] )
	matrix? ( <dev-python/openpyxl-4.0.0[${PYTHON_USEDEP}] )
	cli? ( <dev-python/openpyxl-4.0.0[${PYTHON_USEDEP}] )
	examples? ( <dev-python/openpyxl-4.0.0[${PYTHON_USEDEP}] )
	dynamic-attributes? ( <dev-python/openpyxl-4.0.0[${PYTHON_USEDEP}] )
	all? ( <dev-python/openpyxl-4.0.0[${PYTHON_USEDEP}] )
	examples? ( <dev-python/python-json-logger-3.0.0[${PYTHON_USEDEP}] )
	all? ( <dev-python/python-json-logger-3.0.0[${PYTHON_USEDEP}] )
	examples? ( <dev-python/pyyaml-7.0.0[${PYTHON_USEDEP}] )
	dynamic-attributes? ( <dev-python/pyyaml-7.0.0[${PYTHON_USEDEP}] )
	all? ( <dev-python/pyyaml-7.0.0[${PYTHON_USEDEP}] )
	cli? ( <dev-python/rich-15.0[${PYTHON_USEDEP}] )
	examples? ( <dev-python/rich-15.0[${PYTHON_USEDEP}] )
	all? ( <dev-python/rich-15.0[${PYTHON_USEDEP}] )
	matrix? ( <dev-python/jinja2-4.0.0[${PYTHON_USEDEP}] )
	all? ( <dev-python/jinja2-4.0.0[${PYTHON_USEDEP}] )
	webhook? ( dev-python/fastapi[${PYTHON_USEDEP}] )
	all? ( dev-python/fastapi[${PYTHON_USEDEP}] )
	webhook? ( <dev-python/uvicorn-0.39.0[${PYTHON_USEDEP}] )
	all? ( <dev-python/uvicorn-0.39.0[${PYTHON_USEDEP}] )
	webhook? ( dev-python/sqlmodel[${PYTHON_USEDEP}] )
	all? ( dev-python/sqlmodel[${PYTHON_USEDEP}] )
	dynamic-attributes? ( dev-python/pyyaml-include[${PYTHON_USEDEP}] )
	all? ( dev-python/pyyaml-include[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
