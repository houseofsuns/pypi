...
IUSE="accelerate aleph-alpha all allenai amazon anthropic bhasa cleva cohere decodingtrust dev google heim human-evaluation image2struct images metrics mistral models mongo openai openvino plots proxy-server reka scenarios slurm summarization together unitxt vlm yandex"
DEPENDENCIES="~dev-python/cattrs-22.2[${PYTHON_USEDEP}]
	...
	bhasa? ( dev-python/pythainlp[${PYTHON_USEDEP}] )
	bhasa? ( dev-python/pyonmttok[${PYTHON_USEDEP}] )
	bhasa? ( dev-python/sacrebleu[${PYTHON_USEDEP}] )
	...
	heim? ( ~dev-python/scikit-image-0.21.0[${PYTHON_USEDEP}] )
	heim? ( dev-python/crfm-helm[${PYTHON_USEDEP}] )
