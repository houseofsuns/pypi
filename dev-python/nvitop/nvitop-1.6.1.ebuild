# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="An interactive NVIDIA-GPU process viewer and beyond, the one-stop solution for GPU process management."

HOMEPAGE="https://github.com/XuehaiPan/nvitop"
LICENSE="Apache-2.0 AND GPL-3.0-only"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="cuda10 cuda11 cuda12 cuda13 exporter lint prometheus"
DEPENDENCIES="<dev-python/nvidia-ml-py-13.581.0_alpha0[${PYTHON_USEDEP}]
	>=dev-python/psutil-5.6.6[${PYTHON_USEDEP}]
	exporter? ( dev-python/nvitop-exporter[${PYTHON_USEDEP}] )
	prometheus? ( dev-python/nvitop-exporter[${PYTHON_USEDEP}] )
	lint? ( dev-python/ruff[${PYTHON_USEDEP}] )
	lint? ( dev-python/pylint[${PYTHON_USEDEP}] )
	lint? ( dev-python/xdoctest[${PYTHON_USEDEP}] )
	lint? ( dev-python/mypy[${PYTHON_USEDEP}] )
	lint? ( dev-python/typing-extensions[${PYTHON_USEDEP}] )
	lint? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	cuda10? ( ~dev-python/nvidia-ml-py-11.450.51[${PYTHON_USEDEP}] )
	cuda11? ( <=dev-python/nvidia-ml-py-11.525.150[${PYTHON_USEDEP}] )
	cuda12? ( <=dev-python/nvidia-ml-py-12.575.51[${PYTHON_USEDEP}] )
	cuda13? ( <=dev-python/nvidia-ml-py-13.580.82[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
