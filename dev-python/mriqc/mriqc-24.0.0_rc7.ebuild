# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="24.0.0rc7"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Automated Quality Control and visual reports for Quality Assessment of structural (T1w, T2w) and functional MRI of the brain."

HOMEPAGE="https://github.com/nipreps/mriqc"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all container dev doc docs notebook other others rodents test tests"
DEPENDENCIES="dev-python/dipy[${PYTHON_USEDEP}]
	~dev-python/markupsafe-2.0.1[${PYTHON_USEDEP}]
	dev-python/matplotlib[${PYTHON_USEDEP}]
	dev-python/migas[${PYTHON_USEDEP}]
	dev-python/mriqc-learn[${PYTHON_USEDEP}]
	dev-python/nibabel[${PYTHON_USEDEP}]
	dev-python/nilearn[${PYTHON_USEDEP}]
	dev-python/nipype[${PYTHON_USEDEP}]
	dev-python/nireports[${PYTHON_USEDEP}]
	dev-python/nitransforms[${PYTHON_USEDEP}]
	dev-python/niworkflows[${PYTHON_USEDEP}]
	~dev-python/numpy-1.20[${PYTHON_USEDEP}]
	~dev-python/pandas-1.0[${PYTHON_USEDEP}]
	dev-python/pybids[${PYTHON_USEDEP}]
	dev-python/pyyaml[${PYTHON_USEDEP}]
	dev-python/scikit-learn[${PYTHON_USEDEP}]
	~dev-python/scipy-1.8[${PYTHON_USEDEP}]
	dev-python/statsmodels[${PYTHON_USEDEP}]
	dev-python/templateflow[${PYTHON_USEDEP}]
	dev-python/toml[${PYTHON_USEDEP}]
	dev-python/torch[${PYTHON_USEDEP}]
	all? ( dev-python/coverage[${PYTHON_USEDEP}] )
	all? ( dev-python/datalad[${PYTHON_USEDEP}] )
	all? ( dev-python/datalad-osf[${PYTHON_USEDEP}] )
	all? ( dev-python/ipython[${PYTHON_USEDEP}] )
	all? ( dev-python/jupyter[${PYTHON_USEDEP}] )
	all? ( dev-python/mock[${PYTHON_USEDEP}] )
	all? ( dev-python/nipy[${PYTHON_USEDEP}] )
	all? ( dev-python/nipype[${PYTHON_USEDEP}] )
	all? ( dev-python/nirodents[${PYTHON_USEDEP}] )
	all? ( dev-python/nitime[${PYTHON_USEDEP}] )
	all? ( dev-python/packaging[${PYTHON_USEDEP}] )
	all? ( >=dev-python/pydot-1.2.3[${PYTHON_USEDEP}] )
	all? ( dev-python/pydotplus[${PYTHON_USEDEP}] )
	all? ( dev-python/pytest[${PYTHON_USEDEP}] )
	all? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	all? ( dev-python/pytest-env[${PYTHON_USEDEP}] )
	all? ( dev-python/pytest-xdist[${PYTHON_USEDEP}] )
	all? ( dev-python/scikit-image[${PYTHON_USEDEP}] )
	all? ( dev-python/seaborn[${PYTHON_USEDEP}] )
	all? ( dev-python/sphinx-argparse[${PYTHON_USEDEP}] )
	all? ( dev-python/sphinx-rtd-theme[${PYTHON_USEDEP}] )
	all? ( ~dev-python/sphinx-5.0[${PYTHON_USEDEP}] )
	all? ( dev-python/xvfbwrapper[${PYTHON_USEDEP}] )
	container? ( dev-python/datalad[${PYTHON_USEDEP}] )
	container? ( dev-python/datalad-osf[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/black-22.3.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/Flake8-pyproject[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/isort-5.10.1[${PYTHON_USEDEP}] )
	doc? ( dev-python/nipype[${PYTHON_USEDEP}] )
	doc? ( dev-python/packaging[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/pydot-1.2.3[${PYTHON_USEDEP}] )
	doc? ( dev-python/pydotplus[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-argparse[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-rtd-theme[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinx-5.0[${PYTHON_USEDEP}] )
	docs? ( dev-python/nipype[${PYTHON_USEDEP}] )
	docs? ( dev-python/packaging[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/pydot-1.2.3[${PYTHON_USEDEP}] )
	docs? ( dev-python/pydotplus[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-argparse[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-rtd-theme[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/sphinx-5.0[${PYTHON_USEDEP}] )
	notebook? ( dev-python/ipython[${PYTHON_USEDEP}] )
	notebook? ( dev-python/jupyter[${PYTHON_USEDEP}] )
	notebook? ( dev-python/nipy[${PYTHON_USEDEP}] )
	other? ( dev-python/nitime[${PYTHON_USEDEP}] )
	other? ( dev-python/scikit-image[${PYTHON_USEDEP}] )
	other? ( dev-python/seaborn[${PYTHON_USEDEP}] )
	other? ( dev-python/xvfbwrapper[${PYTHON_USEDEP}] )
	others? ( dev-python/nitime[${PYTHON_USEDEP}] )
	others? ( dev-python/scikit-image[${PYTHON_USEDEP}] )
	others? ( dev-python/seaborn[${PYTHON_USEDEP}] )
	others? ( dev-python/xvfbwrapper[${PYTHON_USEDEP}] )
	rodents? ( dev-python/nirodents[${PYTHON_USEDEP}] )
	test? ( dev-python/coverage[${PYTHON_USEDEP}] )
	test? ( dev-python/mock[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-env[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-xdist[${PYTHON_USEDEP}] )
	tests? ( dev-python/coverage[${PYTHON_USEDEP}] )
	tests? ( dev-python/mock[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-env[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-xdist[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
