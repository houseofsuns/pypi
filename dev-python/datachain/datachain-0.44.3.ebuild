# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Wrangle unstructured AI data at scale"

HOMEPAGE=""
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="audio dev docs examples hf postgres remote tests torch vector video"
DEPENDENCIES="dev-python/pyyaml[${PYTHON_USEDEP}]
	dev-python/tomlkit[${PYTHON_USEDEP}]
	dev-python/tqdm[${PYTHON_USEDEP}]
	<dev-python/numpy-3.0[${PYTHON_USEDEP}]
	>=dev-python/pandas-2.0.0[${PYTHON_USEDEP}]
	>=dev-python/ujson-5.10.0[${PYTHON_USEDEP}]
	dev-python/packaging[${PYTHON_USEDEP}]
	dev-python/pyarrow[${PYTHON_USEDEP}]
	dev-python/typing-extensions[${PYTHON_USEDEP}]
	>=dev-python/python-dateutil-2.0[${PYTHON_USEDEP}]
	dev-python/dateparser[${PYTHON_USEDEP}]
	>=dev-python/attrs-21.3.0[${PYTHON_USEDEP}]
	>=dev-python/fsspec-2024.2.0[${PYTHON_USEDEP}]
	dev-python/s3fs[${PYTHON_USEDEP}]
	dev-python/gcsfs[${PYTHON_USEDEP}]
	dev-python/adlfs[${PYTHON_USEDEP}]
	dev-python/dvc-data[${PYTHON_USEDEP}]
	dev-python/dvc-objects[${PYTHON_USEDEP}]
	<dev-python/shtab-2.0[${PYTHON_USEDEP}]
	>=dev-python/sqlalchemy-2.0[${PYTHON_USEDEP}]
	~dev-python/multiprocess-0.70.16[${PYTHON_USEDEP}]
	dev-python/cloudpickle[${PYTHON_USEDEP}]
	dev-python/pydantic[${PYTHON_USEDEP}]
	>=dev-python/jmespath-1.0[${PYTHON_USEDEP}]
	dev-python/datamodel-code-generator[${PYTHON_USEDEP}]
	<dev-python/pillow-13.0[${PYTHON_USEDEP}]
	<dev-python/msgpack-2.0[${PYTHON_USEDEP}]
	dev-python/psutil[${PYTHON_USEDEP}]
	dev-python/huggingface-hub[${PYTHON_USEDEP}]
	dev-python/iterative-telemetry[${PYTHON_USEDEP}]
	dev-python/platformdirs[${PYTHON_USEDEP}]
	dev-python/dvc-studio-client[${PYTHON_USEDEP}]
	dev-python/tabulate[${PYTHON_USEDEP}]
	dev-python/websockets[${PYTHON_USEDEP}]
	docs? ( >=dev-python/mkdocs-1.5.2[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/mkdocs-gen-files-0.5.0[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/mkdocs-material-9.5.22[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/mkdocstrings-python-1.6.3[${PYTHON_USEDEP}] )
	docs? ( dev-python/mkdocs-literate-nav[${PYTHON_USEDEP}] )
	docs? ( dev-python/mkdocs-section-index[${PYTHON_USEDEP}] )
	docs? ( dev-python/eval-type-backport[${PYTHON_USEDEP}] )
	torch? ( dev-python/torch[${PYTHON_USEDEP}] )
	torch? ( dev-python/torchvision[${PYTHON_USEDEP}] )
	torch? ( dev-python/transformers[${PYTHON_USEDEP}] )
	audio? ( dev-python/soundfile[${PYTHON_USEDEP}] )
	remote? ( dev-python/lz4[${PYTHON_USEDEP}] )
	remote? ( >=dev-python/requests-2.22.0[${PYTHON_USEDEP}] )
	vector? ( dev-python/usearch[${PYTHON_USEDEP}] )
	hf? ( dev-python/numba[${PYTHON_USEDEP}] )
	hf? ( dev-python/datasets[${PYTHON_USEDEP}] )
	hf? ( dev-python/datasets[${PYTHON_USEDEP}] )
	hf? ( >=dev-python/fsspec-2024.12.0[${PYTHON_USEDEP}] )
	video? ( dev-python/ffmpeg-python[${PYTHON_USEDEP}] )
	video? ( >=dev-python/imageio-2.37.0[${PYTHON_USEDEP}] )
	video? ( dev-python/opencv-python[${PYTHON_USEDEP}] )
	postgres? ( dev-python/psycopg2-binary[${PYTHON_USEDEP}] )
	tests? ( dev-python/datachain[${PYTHON_USEDEP}] )
	tests? ( <dev-python/pytest-10.0[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-asyncio[${PYTHON_USEDEP}] )
	tests? ( >=dev-python/pytest-sugar-0.9.6[${PYTHON_USEDEP}] )
	tests? ( >=dev-python/pytest-cov-4.1.0[${PYTHON_USEDEP}] )
	tests? ( >=dev-python/pytest-mock-3.12.0[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-servers[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-benchmark[${PYTHON_USEDEP}] )
	tests? ( >=dev-python/pytest-xdist-3.3.1[${PYTHON_USEDEP}] )
	tests? ( dev-python/azure-storage-blob[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-dotenv[${PYTHON_USEDEP}] )
	tests? ( dev-python/virtualenv[${PYTHON_USEDEP}] )
	tests? ( dev-python/dulwich[${PYTHON_USEDEP}] )
	tests? ( dev-python/hypothesis[${PYTHON_USEDEP}] )
	tests? ( dev-python/aiotools[${PYTHON_USEDEP}] )
	tests? ( dev-python/requests-mock[${PYTHON_USEDEP}] )
	tests? ( dev-python/scipy[${PYTHON_USEDEP}] )
	tests? ( dev-python/ultralytics[${PYTHON_USEDEP}] )
	dev? ( dev-python/datachain[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/mypy-1.19.1[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-python-dateutil[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-dateparser[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-pytz[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-PyYAML[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-requests[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-tabulate[${PYTHON_USEDEP}] )
	examples? ( dev-python/datachain[${PYTHON_USEDEP}] )
	examples? ( dev-python/defusedxml[${PYTHON_USEDEP}] )
	examples? ( dev-python/accelerate[${PYTHON_USEDEP}] )
	examples? ( dev-python/huggingface-hub[${PYTHON_USEDEP}] )
	examples? ( dev-python/ultralytics[${PYTHON_USEDEP}] )
	examples? ( dev-python/open-clip-torch[${PYTHON_USEDEP}] )
	examples? ( dev-python/openai[${PYTHON_USEDEP}] )
	examples? ( dev-python/torchaudio[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
