# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A hierarchical data modeling framework for modern science data standards"

HOMEPAGE="https://github.com/hdmf-dev/hdmf"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all docs min-reqs sparse termset test tqdm zarr"
DEPENDENCIES=">=dev-python/h5py-3.1.0[${PYTHON_USEDEP}]
	>=dev-python/jsonschema-3.2.0[${PYTHON_USEDEP}]
	>=dev-python/numpy-1.19.3[${PYTHON_USEDEP}]
	>=dev-python/pandas-1.2.0[${PYTHON_USEDEP}]
	>=dev-python/ruamel-yaml-0.16[${PYTHON_USEDEP}]
	all? ( dev-python/codespell[${PYTHON_USEDEP}] )
	all? ( dev-python/hdmf-zarr[${PYTHON_USEDEP}] )
	all? ( dev-python/linkml-runtime[${PYTHON_USEDEP}] )
	all? ( dev-python/matplotlib[${PYTHON_USEDEP}] )
	all? ( dev-python/numcodecs[${PYTHON_USEDEP}] )
	all? ( dev-python/oaklib[${PYTHON_USEDEP}] )
	all? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	all? ( dev-python/pytest[${PYTHON_USEDEP}] )
	all? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	all? ( dev-python/python-dateutil[${PYTHON_USEDEP}] )
	all? ( >=dev-python/pyyaml-6.0.1[${PYTHON_USEDEP}] )
	all? ( dev-python/ruff[${PYTHON_USEDEP}] )
	all? ( dev-python/schemasheets[${PYTHON_USEDEP}] )
	all? ( >=dev-python/scipy-1.7[${PYTHON_USEDEP}] )
	all? ( dev-python/sphinx-copybutton[${PYTHON_USEDEP}] )
	all? ( dev-python/sphinx-gallery[${PYTHON_USEDEP}] )
	all? ( >=dev-python/sphinx-rtd-theme-1.0[${PYTHON_USEDEP}] )
	all? ( >=dev-python/sphinx-4.0[${PYTHON_USEDEP}] )
	all? ( dev-python/tox[${PYTHON_USEDEP}] )
	all? ( >=dev-python/tqdm-4.41.0[${PYTHON_USEDEP}] )
	all? ( dev-python/zarr[${PYTHON_USEDEP}] )
	docs? ( dev-python/matplotlib[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-copybutton[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-gallery[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-rtd-theme-1.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-4.0[${PYTHON_USEDEP}] )
	min-reqs? ( ~dev-python/h5py-3.1.0[${PYTHON_USEDEP}] )
	min-reqs? ( ~dev-python/jsonschema-3.2.0[${PYTHON_USEDEP}] )
	min-reqs? ( ~dev-python/numpy-1.19.3[${PYTHON_USEDEP}] )
	min-reqs? ( ~dev-python/pandas-1.2.0[${PYTHON_USEDEP}] )
	min-reqs? ( ~dev-python/ruamel-yaml-0.16.0[${PYTHON_USEDEP}] )
	min-reqs? ( ~dev-python/scipy-1.7.0[${PYTHON_USEDEP}] )
	min-reqs? ( ~dev-python/tqdm-4.41.0[${PYTHON_USEDEP}] )
	min-reqs? ( dev-python/zarr[${PYTHON_USEDEP}] )
	sparse? ( >=dev-python/scipy-1.7[${PYTHON_USEDEP}] )
	termset? ( dev-python/linkml-runtime[${PYTHON_USEDEP}] )
	termset? ( dev-python/oaklib[${PYTHON_USEDEP}] )
	termset? ( >=dev-python/pyyaml-6.0.1[${PYTHON_USEDEP}] )
	termset? ( dev-python/schemasheets[${PYTHON_USEDEP}] )
	test? ( dev-python/codespell[${PYTHON_USEDEP}] )
	test? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	test? ( dev-python/python-dateutil[${PYTHON_USEDEP}] )
	test? ( dev-python/ruff[${PYTHON_USEDEP}] )
	test? ( dev-python/tox[${PYTHON_USEDEP}] )
	tqdm? ( >=dev-python/tqdm-4.41.0[${PYTHON_USEDEP}] )
	zarr? ( dev-python/hdmf-zarr[${PYTHON_USEDEP}] )
	zarr? ( dev-python/numcodecs[${PYTHON_USEDEP}] )
	zarr? ( dev-python/zarr[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
