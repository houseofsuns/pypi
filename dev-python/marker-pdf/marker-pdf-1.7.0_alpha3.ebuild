# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="1.7.0a3"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Convert documents to markdown with high speed and accuracy."

HOMEPAGE="https://github.com/VikParuchuri/marker"
LICENSE="GPL-3+"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="full"
DEPENDENCIES="<dev-python/pillow-11.0.0[${PYTHON_USEDEP}]
	<dev-python/pydantic-3.0.0[${PYTHON_USEDEP}]
	dev-python/pydantic-settings[${PYTHON_USEDEP}]
	dev-python/transformers[${PYTHON_USEDEP}]
	<dev-python/python-dotenv-2.0.0[${PYTHON_USEDEP}]
	dev-python/torch[${PYTHON_USEDEP}]
	<dev-python/tqdm-5.0.0[${PYTHON_USEDEP}]
	<dev-python/ftfy-7.0.0[${PYTHON_USEDEP}]
	<dev-python/rapidfuzz-4.0.0[${PYTHON_USEDEP}]
	dev-python/surya-ocr[${PYTHON_USEDEP}]
	<dev-python/regex-2025.0.0[${PYTHON_USEDEP}]
	dev-python/pdftext[${PYTHON_USEDEP}]
	dev-python/markdownify[${PYTHON_USEDEP}]
	<dev-python/click-9.0.0[${PYTHON_USEDEP}]
	<dev-python/markdown2-3.0.0[${PYTHON_USEDEP}]
	<dev-python/filetype-2.0.0[${PYTHON_USEDEP}]
	<dev-python/scikit-learn-2.0.0[${PYTHON_USEDEP}]
	dev-python/google-genai[${PYTHON_USEDEP}]
	dev-python/anthropic[${PYTHON_USEDEP}]
	dev-python/pre-commit[${PYTHON_USEDEP}]
	full? ( dev-python/mammoth[${PYTHON_USEDEP}] )
	full? ( <dev-python/openpyxl-4.0.0[${PYTHON_USEDEP}] )
	full? ( dev-python/python-pptx[${PYTHON_USEDEP}] )
	full? ( dev-python/EbookLib[${PYTHON_USEDEP}] )
	full? ( <dev-python/weasyprint-64.0[${PYTHON_USEDEP}] )
	dev-python/openai[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
