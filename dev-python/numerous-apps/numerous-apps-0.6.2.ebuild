# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Application package for numerous"

HOMEPAGE="https://github.com/numerous-com/numerous-app"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="auth-db dev"
DEPENDENCIES="dev-python/fastapi[${PYTHON_USEDEP}]
	~dev-python/uvicorn-0.32.1[${PYTHON_USEDEP}]
	~dev-python/jinja2-3.1.4[${PYTHON_USEDEP}]
	dev-python/anywidget[${PYTHON_USEDEP}]
	>=dev-python/numpy-2.0.0[${PYTHON_USEDEP}]
	~dev-python/pydantic-2.10.3[${PYTHON_USEDEP}]
	>=dev-python/pyjwt-2.8.0[${PYTHON_USEDEP}]
	auth-db? ( >=dev-python/sqlalchemy-2.0.0[${PYTHON_USEDEP}] )
	auth-db? ( dev-python/asyncpg[${PYTHON_USEDEP}] )
	auth-db? ( >=dev-python/aiosqlite-0.19.0[${PYTHON_USEDEP}] )
	auth-db? ( >=dev-python/bcrypt-4.1.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/anywidget[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/python-dotenv-1.0.1[${PYTHON_USEDEP}] )
	dev? ( dev-python/numerous-widgets[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/numpy-2.0.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pytest-8.3.4[${PYTHON_USEDEP}] )
	dev? ( dev-python/ruff[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/mypy-1.11.2[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/coverage-7.6.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/twine-5.1.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/mkdocs-1.6.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/mkdocstrings-0.26.2[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/mkdocstrings-python-1.12.2[${PYTHON_USEDEP}] )
	dev? ( dev-python/mkdocs-snippet-plugin[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/black-24.10.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/mkdocs-material-9.5.46[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/mkdocs-gen-files-0.5.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/mkdocs-section-index[${PYTHON_USEDEP}] )
	dev? ( dev-python/mkdocs-literate-nav[${PYTHON_USEDEP}] )
	dev? ( dev-python/marimo[${PYTHON_USEDEP}] )
	dev? ( dev-python/panel[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pytest-cov-4.1.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/httpx-0.28.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pytest-asyncio-0.25.2[${PYTHON_USEDEP}] )
	dev? ( dev-python/playwright[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/psutil-6.1.1[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
