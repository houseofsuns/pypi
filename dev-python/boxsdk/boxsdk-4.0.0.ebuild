# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Official Box Python SDK"

HOMEPAGE="https://github.com/box/box-python-sdk"
LICENSE="Apache Software License, Version 2.0, http://www.apache.org/licenses/LICENSE-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="coveralls dev gh jwt redis test"
DEPENDENCIES=">=dev-python/attrs-17.3.0[${PYTHON_USEDEP}]
	dev-python/urllib3[${PYTHON_USEDEP}]
	dev-python/dataclasses[${PYTHON_USEDEP}]
	<dev-python/requests-3.0[${PYTHON_USEDEP}]
	<dev-python/requests-toolbelt-2.0[${PYTHON_USEDEP}]
	dev-python/python-dateutil[${PYTHON_USEDEP}]
	jwt? ( <dev-python/pyjwt-2.0[${PYTHON_USEDEP}] )
	jwt? ( >=dev-python/cryptography-3.0[${PYTHON_USEDEP}] )
	redis? ( >=dev-python/redis-2.10.3[${PYTHON_USEDEP}] )
	coveralls? ( dev-python/coveralls[${PYTHON_USEDEP}] )
	coveralls? ( <dev-python/tox-5.0[${PYTHON_USEDEP}] )
	coveralls? ( dev-python/setuptools[${PYTHON_USEDEP}] )
	dev? ( <dev-python/tox-5.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/setuptools[${PYTHON_USEDEP}] )
	gh? ( dev-python/tox-gh-actions[${PYTHON_USEDEP}] )
	gh? ( <dev-python/tox-5.0[${PYTHON_USEDEP}] )
	gh? ( dev-python/setuptools[${PYTHON_USEDEP}] )
	test? ( dev-python/bottle[${PYTHON_USEDEP}] )
	test? ( >dev-python/jsonpatch-1.14[${PYTHON_USEDEP}] )
	test? ( <dev-python/sqlalchemy-1.4.0[${PYTHON_USEDEP}] )
	test? ( <dev-python/pytest-9.0[${PYTHON_USEDEP}] )
	test? ( <dev-python/pytest-timeout-3.0.0[${PYTHON_USEDEP}] )
	test? ( <dev-python/pytest-cov-6.0[${PYTHON_USEDEP}] )
	test? ( <dev-python/pytest-lazy-fixtures-2.0[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-rerunfailures[${PYTHON_USEDEP}] )
	test? ( dev-python/pytz[${PYTHON_USEDEP}] )
	test? ( <dev-python/urllib3-2.0[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
