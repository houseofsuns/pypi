# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=wheel

inherit python-r1 gs-pypi

DESCRIPTION="Reusable Python utilities to avoid reinventing the wheel. [wheel]"

HOMEPAGE="https://codeberg.org/ipamo/zut"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/py3/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}-py3-none-any.whl"
SOURCEFILE="${REALNAME}-${REALVERSION}-py3-none-any.whl"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all excel mariadb mssql mysql perf pg postgresql sqlite sqlserver tabulate"
DEPENDENCIES="pg? ( dev-python/psycopg[${PYTHON_USEDEP}] )
	pg? ( dev-python/sqlparse[${PYTHON_USEDEP}] )
	postgresql? ( dev-python/psycopg[${PYTHON_USEDEP}] )
	postgresql? ( dev-python/sqlparse[${PYTHON_USEDEP}] )
	mysql? ( dev-python/mysqlclient[${PYTHON_USEDEP}] )
	mysql? ( dev-python/sqlparse[${PYTHON_USEDEP}] )
	mariadb? ( dev-python/mysqlclient[${PYTHON_USEDEP}] )
	mariadb? ( dev-python/sqlparse[${PYTHON_USEDEP}] )
	sqlserver? ( dev-python/pyodbc[${PYTHON_USEDEP}] )
	sqlserver? ( dev-python/sqlparams[${PYTHON_USEDEP}] )
	sqlserver? ( dev-python/sqlparse[${PYTHON_USEDEP}] )
	mssql? ( dev-python/pyodbc[${PYTHON_USEDEP}] )
	mssql? ( dev-python/sqlparams[${PYTHON_USEDEP}] )
	mssql? ( dev-python/sqlparse[${PYTHON_USEDEP}] )
	sqlite? ( dev-python/sqlparse[${PYTHON_USEDEP}] )
	tabulate? ( dev-python/tabulate[${PYTHON_USEDEP}] )
	excel? ( dev-python/openpyxl[${PYTHON_USEDEP}] )
	excel? ( dev-python/defusedxml[${PYTHON_USEDEP}] )
	perf? ( dev-python/psutil[${PYTHON_USEDEP}] )
	all? ( dev-python/psycopg[${PYTHON_USEDEP}] )
	all? ( dev-python/mysqlclient[${PYTHON_USEDEP}] )
	all? ( dev-python/pyodbc[${PYTHON_USEDEP}] )
	all? ( dev-python/sqlparams[${PYTHON_USEDEP}] )
	all? ( dev-python/sqlparse[${PYTHON_USEDEP}] )
	all? ( dev-python/tabulate[${PYTHON_USEDEP}] )
	all? ( dev-python/openpyxl[${PYTHON_USEDEP}] )
	all? ( dev-python/defusedxml[${PYTHON_USEDEP}] )
	all? ( dev-python/psutil[${PYTHON_USEDEP}] )
	virtual/allow-pypi-wheels"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
