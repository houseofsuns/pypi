# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_9,3_10,3_11} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A package providing template applications for data services, and a python SDK to the Octue API."

HOMEPAGE="https://www.github.com/octue/octue-sdk-python"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/o/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dataflow hdf5"
DEPENDENCIES="dev-python/click[${PYTHON_USEDEP}]
	dev-python/coolname[${PYTHON_USEDEP}]
	dev-python/Flask[${PYTHON_USEDEP}]
	dev-python/google-auth[${PYTHON_USEDEP}]
	dev-python/google-cloud-pubsub[${PYTHON_USEDEP}]
	dev-python/google-cloud-secret-manager[${PYTHON_USEDEP}]
	dev-python/google-cloud-storage[${PYTHON_USEDEP}]
	dev-python/google-crc32c[${PYTHON_USEDEP}]
	dev-python/gunicorn[${PYTHON_USEDEP}]
	dev-python/python-dateutil[${PYTHON_USEDEP}]
	dev-python/pyyaml[${PYTHON_USEDEP}]
	hdf5? ( dev-python/h5py[${PYTHON_USEDEP}] )
	dataflow? ( dev-python/apache-beam[${PYTHON_USEDEP}] )
	dev-python/twined[${PYTHON_USEDEP}]
	dev-python/packaging[${PYTHON_USEDEP}]
	dataflow? ( dev-python/cachetools[${PYTHON_USEDEP}] )
	dataflow? ( dev-python/google-apitools[${PYTHON_USEDEP}] )
	dataflow? ( dev-python/google-auth-httplib2[${PYTHON_USEDEP}] )
	dataflow? ( dev-python/google-cloud-datastore[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
