# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="open-autonomy"
REALVERSION="0.10.1"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_9,3_10,3_11} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A framework for the creation of autonomous agent services."

HOMEPAGE="https://github.com/valory-xyz/open-autonomy.git"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/o/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all cli"
DEPENDENCIES="dev-python/Flask[${PYTHON_USEDEP}]
	dev-python/open-aea[${PYTHON_USEDEP}]
	dev-python/pandas[${PYTHON_USEDEP}]
	dev-python/watchdog[${PYTHON_USEDEP}]
	dev-python/pytest[${PYTHON_USEDEP}]
	dev-python/open-aea-ledger-ethereum[${PYTHON_USEDEP}]
	dev-python/docker-compose[${PYTHON_USEDEP}]
	dev-python/werkzeug[${PYTHON_USEDEP}]
	dev-python/docker[${PYTHON_USEDEP}]
	dev-python/click[${PYTHON_USEDEP}]
	dev-python/open-aea-cli-ipfs[${PYTHON_USEDEP}]
	dev-python/open-aea-ledger-ethereum-hwi[${PYTHON_USEDEP}]
	all? ( dev-python/open-aea-ledger-ethereum-hwi[${PYTHON_USEDEP}] )
	all? ( dev-python/open-aea-cli-ipfs[${PYTHON_USEDEP}] )
	all? ( dev-python/click[${PYTHON_USEDEP}] )
	cli? ( dev-python/click[${PYTHON_USEDEP}] )
	cli? ( dev-python/open-aea-cli-ipfs[${PYTHON_USEDEP}] )
	cli? ( dev-python/open-aea-ledger-ethereum-hwi[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
