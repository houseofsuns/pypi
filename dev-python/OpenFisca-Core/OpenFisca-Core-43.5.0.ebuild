# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A versatile microsimulation free software"

HOMEPAGE="https://github.com/openfisca/openfisca-core"
LICENSE="https://www.fsf.org/licensing/licenses/agpl-3.0.html"
SRC_URI="https://files.pythonhosted.org/packages/69/6e/e5aab6806467e676e87efd7257e03b13b70a1ae3a2e3eb363b500bd660f6/openfisca_core-${REALVERSION}.tar.gz"
SOURCEFILE="openfisca_core-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="ci dev excel tracker web-api"
DEPENDENCIES="<dev-python/pyyaml-7.0[${PYTHON_USEDEP}]
	dev-python/StrEnum[${PYTHON_USEDEP}]
	dev-python/dpath[${PYTHON_USEDEP}]
	<dev-python/numexpr-3.0[${PYTHON_USEDEP}]
	<dev-python/numpy-2.0[${PYTHON_USEDEP}]
	dev-python/pendulum[${PYTHON_USEDEP}]
	<dev-python/psutil-6.0[${PYTHON_USEDEP}]
	<dev-python/pytest-9.0[${PYTHON_USEDEP}]
	<dev-python/sortedcontainers-3.0[${PYTHON_USEDEP}]
	<dev-python/typing-extensions-5.0[${PYTHON_USEDEP}]
	web-api? ( <dev-python/flask-4.0[${PYTHON_USEDEP}] )
	web-api? ( <dev-python/flask-cors-4.0[${PYTHON_USEDEP}] )
	web-api? ( dev-python/gunicorn[${PYTHON_USEDEP}] )
	web-api? ( <dev-python/werkzeug-4.0[${PYTHON_USEDEP}] )
	dev? ( <dev-python/black-25.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/codespell[${PYTHON_USEDEP}] )
	dev? ( <dev-python/colorama-0.5[${PYTHON_USEDEP}] )
	dev? ( dev-python/darglint[${PYTHON_USEDEP}] )
	dev? ( <dev-python/flake8-8.0.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/flake8-bugbear[${PYTHON_USEDEP}] )
	dev? ( dev-python/flake8-docstrings[${PYTHON_USEDEP}] )
	dev? ( dev-python/flake8-print[${PYTHON_USEDEP}] )
	dev? ( dev-python/flake8-rst-docstrings[${PYTHON_USEDEP}] )
	dev? ( <dev-python/idna-4.0[${PYTHON_USEDEP}] )
	dev? ( <dev-python/isort-6.0[${PYTHON_USEDEP}] )
	dev? ( <dev-python/mypy-2.0[${PYTHON_USEDEP}] )
	dev? ( <dev-python/openapi-spec-validator-0.8.0[${PYTHON_USEDEP}] )
	dev? ( <dev-python/pylint-4.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/pylint-per-file-ignores[${PYTHON_USEDEP}] )
	dev? ( dev-python/pyright[${PYTHON_USEDEP}] )
	dev? ( dev-python/ruff[${PYTHON_USEDEP}] )
	dev? ( dev-python/ruff-lsp[${PYTHON_USEDEP}] )
	dev? ( <dev-python/flask-4.0[${PYTHON_USEDEP}] )
	dev? ( <dev-python/flask-cors-4.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/gunicorn[${PYTHON_USEDEP}] )
	dev? ( <dev-python/werkzeug-4.0[${PYTHON_USEDEP}] )
	dev? ( <dev-python/openpyxl-4.0[${PYTHON_USEDEP}] )
	ci? ( <dev-python/build-0.11.0[${PYTHON_USEDEP}] )
	ci? ( <dev-python/twine-7.0[${PYTHON_USEDEP}] )
	ci? ( <dev-python/wheel-0.41.0[${PYTHON_USEDEP}] )
	tracker? ( dev-python/openfisca-tracker[${PYTHON_USEDEP}] )
	excel? ( <dev-python/openpyxl-4.0[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
