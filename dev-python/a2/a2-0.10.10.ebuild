# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Package for predicting information about the weather from social media data as application 2 for maelstrom project"

HOMEPAGE=""
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="benchmarks coordinates deberta deberta-tf fancy-plotting llama-chatbot notebooks tracking tweets xarray-extra"
DEPENDENCIES="notebooks? ( <dev-python/ipykernel-7.0.0[${PYTHON_USEDEP}] )
	notebooks? ( <dev-python/ipywidgets-8.0.0[${PYTHON_USEDEP}] )
	<dev-python/pandas-2.0.0[${PYTHON_USEDEP}]
	xarray-extra? ( <dev-python/xarray-2024.0.0[${PYTHON_USEDEP}] )
	coordinates? ( <dev-python/pyproj-4.0.0[${PYTHON_USEDEP}] )
	xarray-extra? ( dev-python/ecmwflibs[${PYTHON_USEDEP}] )
	<dev-python/matplotlib-4.0.0[${PYTHON_USEDEP}]
	fancy-plotting? ( <dev-python/plotly-6.0.0[${PYTHON_USEDEP}] )
	fancy-plotting? ( <dev-python/seaborn-0.13.0[${PYTHON_USEDEP}] )
	<dev-python/scikit-learn-2.0.0[${PYTHON_USEDEP}]
	dev-python/datasets[${PYTHON_USEDEP}]
	tracking? ( dev-python/mantik[${PYTHON_USEDEP}] )
	benchmarks? ( dev-python/pynvml[${PYTHON_USEDEP}] )
	llama-chatbot? ( dev-python/sentencepiece[${PYTHON_USEDEP}] )
	deberta? ( dev-python/sentencepiece[${PYTHON_USEDEP}] )
	deberta-tf? ( dev-python/sentencepiece[${PYTHON_USEDEP}] )
	llama-chatbot? ( dev-python/transformers[${PYTHON_USEDEP}] )
	deberta? ( dev-python/transformers[${PYTHON_USEDEP}] )
	deberta-tf? ( dev-python/transformers[${PYTHON_USEDEP}] )
	tweets? ( <dev-python/requests-3.0.0[${PYTHON_USEDEP}] )
	tweets? ( <dev-python/emoji-1.0.0[${PYTHON_USEDEP}] )
	tweets? ( dev-python/spacymoji[${PYTHON_USEDEP}] )
	tweets? ( dev-python/jsonlines[${PYTHON_USEDEP}] )
	tweets? ( dev-python/tweepy[${PYTHON_USEDEP}] )
	deberta? ( <dev-python/bottleneck-2.0.0[${PYTHON_USEDEP}] )
	deberta-tf? ( <dev-python/bottleneck-2.0.0[${PYTHON_USEDEP}] )
	deberta? ( dev-python/sentence-transformers[${PYTHON_USEDEP}] )
	deberta-tf? ( dev-python/sentence-transformers[${PYTHON_USEDEP}] )
	deberta? ( dev-python/nltk[${PYTHON_USEDEP}] )
	deberta-tf? ( dev-python/nltk[${PYTHON_USEDEP}] )
	dev-python/logging_tree[${PYTHON_USEDEP}]
	xarray-extra? ( <dev-python/netcdf4-2.0.0[${PYTHON_USEDEP}] )
	deberta-tf? ( dev-python/tf-keras[${PYTHON_USEDEP}] )
	dev-python/accelerate[${PYTHON_USEDEP}]
	dev-python/deep500[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
