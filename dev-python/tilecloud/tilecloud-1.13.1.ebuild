# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Tools for managing tiles"

HOMEPAGE=""
LICENSE="BSD-2"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all aws azure prometheus redis wsgi"
DEPENDENCIES="~dev-python/pillow-11.2.1[${PYTHON_USEDEP}]
	azure? ( dev-python/azure-identity[${PYTHON_USEDEP}] )
	all? ( dev-python/azure-identity[${PYTHON_USEDEP}] )
	azure? ( dev-python/azure-storage-blob[${PYTHON_USEDEP}] )
	all? ( dev-python/azure-storage-blob[${PYTHON_USEDEP}] )
	aws? ( ~dev-python/boto3-1.37.25[${PYTHON_USEDEP}] )
	all? ( ~dev-python/boto3-1.37.25[${PYTHON_USEDEP}] )
	~dev-python/bottle-0.13.3[${PYTHON_USEDEP}]
	~dev-python/cryptography-44.0.2[${PYTHON_USEDEP}]
	prometheus? ( ~dev-python/prometheus-client-0.21.1[${PYTHON_USEDEP}] )
	all? ( ~dev-python/prometheus-client-0.21.1[${PYTHON_USEDEP}] )
	~dev-python/pyproj-3.7.1[${PYTHON_USEDEP}]
	wsgi? ( dev-python/pyramid[${PYTHON_USEDEP}] )
	all? ( dev-python/pyramid[${PYTHON_USEDEP}] )
	redis? ( ~dev-python/redis-5.2.1[${PYTHON_USEDEP}] )
	all? ( ~dev-python/redis-5.2.1[${PYTHON_USEDEP}] )
	~dev-python/requests-2.32.3[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
