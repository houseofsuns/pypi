# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="This client library is designed to support the DataRobot API."

HOMEPAGE="https://datarobot.com"
LICENSE="DataRobot Tool and Utility Agreement"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="auth auth-authlib core databricks dev docs examples fs images lint release test"
DEPENDENCIES=">=dev-python/pandas-0.15[${PYTHON_USEDEP}]
	dev-python/numpy[${PYTHON_USEDEP}]
	>=dev-python/pyyaml-3.11[${PYTHON_USEDEP}]
	>=dev-python/requests-2.28.1[${PYTHON_USEDEP}]
	>=dev-python/requests-toolbelt-0.6[${PYTHON_USEDEP}]
	dev-python/trafaret[${PYTHON_USEDEP}]
	>=dev-python/urllib3-1.23[${PYTHON_USEDEP}]
	<dev-python/typing-extensions-5.0[${PYTHON_USEDEP}]
	dev-python/StrEnum[${PYTHON_USEDEP}]
	dev? ( <dev-python/pytest-8.4.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/responses-0.21[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pytest-asyncio-0.21.1[${PYTHON_USEDEP}] )
	dev? ( dev-python/pyarrow[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pillow-10.4.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/databricks-connect[${PYTHON_USEDEP}] )
	dev? ( dev-python/Authlib[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pydantic-2.11.3[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/httpx-0.28.1[${PYTHON_USEDEP}] )
	dev? ( dev-python/respx[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/fsspec-2025.5.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/ruff[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/mypy-1.16.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-PyYAML[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-python-dateutil[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-pytz[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-requests[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-urllib3[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-decorator[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pillow-10.4.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/databricks-connect[${PYTHON_USEDEP}] )
	dev? ( dev-python/Authlib[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pydantic-2.11.3[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/httpx-0.28.1[${PYTHON_USEDEP}] )
	dev? ( dev-python/respx[${PYTHON_USEDEP}] )
	dev? ( dev-python/pydantic-settings[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pydantic-2.2.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/psutil[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/fsspec-2025.5.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pillow-10.4.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/sphinx-8.1.3[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/sphinx-rtd-theme-3.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/sphinx_external_toc[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/nbsphinx-0.9.5[${PYTHON_USEDEP}] )
	dev? ( dev-python/jupyter-contrib-nbextensions[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/sphinx-autodoc-typehints-2.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/sphinxcontrib-spelling-8.0.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pyenchant-3.2.2[${PYTHON_USEDEP}] )
	dev? ( dev-python/sphinx-copybutton[${PYTHON_USEDEP}] )
	dev? ( dev-python/sphinx-markdown-builder[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/myst-parser-4.0.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/pydantic-settings[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pydantic-2.2.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/psutil[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/fsspec-2025.5.0[${PYTHON_USEDEP}] )
	examples? ( <=dev-python/jupyter-5.0[${PYTHON_USEDEP}] )
	examples? ( dev-python/fredapi[${PYTHON_USEDEP}] )
	examples? ( >=dev-python/matplotlib-2.1.0[${PYTHON_USEDEP}] )
	examples? ( <=dev-python/seaborn-0.8[${PYTHON_USEDEP}] )
	examples? ( <=dev-python/scikit-learn-0.18.2[${PYTHON_USEDEP}] )
	examples? ( dev-python/wordcloud[${PYTHON_USEDEP}] )
	examples? ( <=dev-python/colour-0.1.4[${PYTHON_USEDEP}] )
	release? ( dev-python/zest-releaser[${PYTHON_USEDEP}] )
	lint? ( dev-python/ruff[${PYTHON_USEDEP}] )
	lint? ( ~dev-python/mypy-1.16.0[${PYTHON_USEDEP}] )
	lint? ( dev-python/types-PyYAML[${PYTHON_USEDEP}] )
	lint? ( dev-python/types-python-dateutil[${PYTHON_USEDEP}] )
	lint? ( dev-python/types-pytz[${PYTHON_USEDEP}] )
	lint? ( dev-python/types-requests[${PYTHON_USEDEP}] )
	lint? ( dev-python/types-urllib3[${PYTHON_USEDEP}] )
	lint? ( dev-python/types-decorator[${PYTHON_USEDEP}] )
	lint? ( ~dev-python/pillow-10.4.0[${PYTHON_USEDEP}] )
	lint? ( dev-python/databricks-connect[${PYTHON_USEDEP}] )
	lint? ( dev-python/Authlib[${PYTHON_USEDEP}] )
	lint? ( >=dev-python/pydantic-2.11.3[${PYTHON_USEDEP}] )
	lint? ( >=dev-python/httpx-0.28.1[${PYTHON_USEDEP}] )
	lint? ( dev-python/respx[${PYTHON_USEDEP}] )
	lint? ( dev-python/pydantic-settings[${PYTHON_USEDEP}] )
	lint? ( >=dev-python/pydantic-2.2.0[${PYTHON_USEDEP}] )
	lint? ( dev-python/psutil[${PYTHON_USEDEP}] )
	lint? ( >=dev-python/fsspec-2025.5.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-8.1.3[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-rtd-theme-3.0[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx_external_toc[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/nbsphinx-0.9.5[${PYTHON_USEDEP}] )
	docs? ( dev-python/jupyter-contrib-nbextensions[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-autodoc-typehints-2.0[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/sphinxcontrib-spelling-8.0.0[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/pyenchant-3.2.2[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-copybutton[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-markdown-builder[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/myst-parser-4.0.0[${PYTHON_USEDEP}] )
	images? ( ~dev-python/pillow-10.4.0[${PYTHON_USEDEP}] )
	test? ( <dev-python/pytest-8.4.0[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	test? ( ~dev-python/responses-0.21[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-asyncio-0.21.1[${PYTHON_USEDEP}] )
	test? ( dev-python/pyarrow[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pillow-10.4.0[${PYTHON_USEDEP}] )
	test? ( dev-python/databricks-connect[${PYTHON_USEDEP}] )
	test? ( dev-python/Authlib[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pydantic-2.11.3[${PYTHON_USEDEP}] )
	test? ( >=dev-python/httpx-0.28.1[${PYTHON_USEDEP}] )
	test? ( dev-python/respx[${PYTHON_USEDEP}] )
	test? ( >=dev-python/fsspec-2025.5.0[${PYTHON_USEDEP}] )
	databricks? ( dev-python/databricks-connect[${PYTHON_USEDEP}] )
	auth? ( >=dev-python/pydantic-2.11.3[${PYTHON_USEDEP}] )
	auth? ( >=dev-python/httpx-0.28.1[${PYTHON_USEDEP}] )
	auth-authlib? ( dev-python/Authlib[${PYTHON_USEDEP}] )
	auth-authlib? ( >=dev-python/pydantic-2.11.3[${PYTHON_USEDEP}] )
	auth-authlib? ( >=dev-python/httpx-0.28.1[${PYTHON_USEDEP}] )
	core? ( dev-python/pydantic-settings[${PYTHON_USEDEP}] )
	core? ( >=dev-python/pydantic-2.2.0[${PYTHON_USEDEP}] )
	core? ( dev-python/psutil[${PYTHON_USEDEP}] )
	fs? ( >=dev-python/fsspec-2025.5.0[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
