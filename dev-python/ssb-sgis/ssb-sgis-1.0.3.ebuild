# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="GIS functions used at Statistics Norway."

HOMEPAGE="https://github.com/statisticsnorway/ssb-sgis"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all bucket test torch xarray"
DEPENDENCIES="dev-python/affine[${PYTHON_USEDEP}]
	dev-python/branca[${PYTHON_USEDEP}]
	all? ( dev-python/dapla-toolbelt[${PYTHON_USEDEP}] )
	bucket? ( dev-python/dapla-toolbelt[${PYTHON_USEDEP}] )
	>=dev-python/dask-2024.1.1[${PYTHON_USEDEP}]
	dev-python/dask-geopandas[${PYTHON_USEDEP}]
	dev-python/folium[${PYTHON_USEDEP}]
	all? ( dev-python/gcsfs[${PYTHON_USEDEP}] )
	bucket? ( dev-python/gcsfs[${PYTHON_USEDEP}] )
	dev-python/geocoder[${PYTHON_USEDEP}]
	dev-python/geopandas[${PYTHON_USEDEP}]
	dev-python/igraph[${PYTHON_USEDEP}]
	>=dev-python/ipython-8.13.2[${PYTHON_USEDEP}]
	dev-python/jenkspy[${PYTHON_USEDEP}]
	>=dev-python/jinja-3.1.3[${PYTHON_USEDEP}]
	>=dev-python/joblib-1.4.0[${PYTHON_USEDEP}]
	dev-python/mapclassify[${PYTHON_USEDEP}]
	>=dev-python/matplotlib-3.7.0[${PYTHON_USEDEP}]
	>=dev-python/networkx-3.0[${PYTHON_USEDEP}]
	>=dev-python/numpy-1.24.2[${PYTHON_USEDEP}]
	>=dev-python/pandas-2.2.1[${PYTHON_USEDEP}]
	>=dev-python/pyarrow-11.0.0[${PYTHON_USEDEP}]
	>=dev-python/pyproj-3.6.1[${PYTHON_USEDEP}]
	dev-python/rasterio[${PYTHON_USEDEP}]
	>=dev-python/requests-2.28.2[${PYTHON_USEDEP}]
	all? ( dev-python/rioxarray[${PYTHON_USEDEP}] )
	xarray? ( dev-python/rioxarray[${PYTHON_USEDEP}] )
	test? ( dev-python/rioxarray[${PYTHON_USEDEP}] )
	dev-python/Rtree[${PYTHON_USEDEP}]
	>=dev-python/scikit-learn-1.2.1[${PYTHON_USEDEP}]
	>=dev-python/shapely-2.0.1[${PYTHON_USEDEP}]
	all? ( dev-python/torch[${PYTHON_USEDEP}] )
	torch? ( dev-python/torch[${PYTHON_USEDEP}] )
	test? ( dev-python/torch[${PYTHON_USEDEP}] )
	all? ( dev-python/torchgeo[${PYTHON_USEDEP}] )
	torch? ( dev-python/torchgeo[${PYTHON_USEDEP}] )
	test? ( dev-python/torchgeo[${PYTHON_USEDEP}] )
	>=dev-python/typing-extensions-4.11.0[${PYTHON_USEDEP}]
	all? ( >=dev-python/xarray-2024.3.0[${PYTHON_USEDEP}] )
	xarray? ( >=dev-python/xarray-2024.3.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/xarray-2024.3.0[${PYTHON_USEDEP}] )
	dev-python/xyzservices[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
