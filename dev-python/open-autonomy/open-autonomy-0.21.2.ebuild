# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A framework for the creation of autonomous agent services."

HOMEPAGE="https://github.com/valory-xyz/open-autonomy.git"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all cli"
DEPENDENCIES="<dev-python/flask-3.0.0[${PYTHON_USEDEP}]
	dev-python/open-aea[${PYTHON_USEDEP}]
	>=dev-python/watchdog-2.1.6[${PYTHON_USEDEP}]
	~dev-python/pytest-7.4.4[${PYTHON_USEDEP}]
	dev-python/valory-docker-compose[${PYTHON_USEDEP}]
	~dev-python/werkzeug-2.0.3[${PYTHON_USEDEP}]
	~dev-python/docker-7.1.0[${PYTHON_USEDEP}]
	dev-python/hexbytes[${PYTHON_USEDEP}]
	<dev-python/jsonschema-4.4.0[${PYTHON_USEDEP}]
	<dev-python/protobuf-4.25.0[${PYTHON_USEDEP}]
	dev-python/gql[${PYTHON_USEDEP}]
	<dev-python/requests-2.32.5[${PYTHON_USEDEP}]
	~dev-python/requests-toolbelt-1.0.0[${PYTHON_USEDEP}]
	<dev-python/aiohttp-4.0.0[${PYTHON_USEDEP}]
	<=dev-python/typing-extensions-4.13.2[${PYTHON_USEDEP}]
	<dev-python/click-9.0[${PYTHON_USEDEP}]
	dev-python/open-aea-cli-ipfs[${PYTHON_USEDEP}]
	~dev-python/texttable-1.6.7[${PYTHON_USEDEP}]
	<dev-python/python-dotenv-0.22.0[${PYTHON_USEDEP}]
	<dev-python/pytest-7.5.0[${PYTHON_USEDEP}]
	<dev-python/coverage-8.0.0[${PYTHON_USEDEP}]
	cli? ( <dev-python/click-9.0[${PYTHON_USEDEP}] )
	cli? ( dev-python/open-aea-cli-ipfs[${PYTHON_USEDEP}] )
	cli? ( ~dev-python/texttable-1.6.7[${PYTHON_USEDEP}] )
	cli? ( <dev-python/python-dotenv-0.22.0[${PYTHON_USEDEP}] )
	cli? ( <dev-python/pytest-7.5.0[${PYTHON_USEDEP}] )
	cli? ( <dev-python/coverage-8.0.0[${PYTHON_USEDEP}] )
	all? ( dev-python/open-aea-cli-ipfs[${PYTHON_USEDEP}] )
	all? ( <dev-python/click-9.0[${PYTHON_USEDEP}] )
	all? ( <dev-python/pytest-7.5.0[${PYTHON_USEDEP}] )
	all? ( <dev-python/coverage-8.0.0[${PYTHON_USEDEP}] )
	all? ( <dev-python/python-dotenv-0.22.0[${PYTHON_USEDEP}] )
	all? ( ~dev-python/texttable-1.6.7[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
