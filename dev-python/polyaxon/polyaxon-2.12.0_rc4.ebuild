# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="2.12.0rc4"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Command Line Interface (CLI) and client to interact with Polyaxon API."

HOMEPAGE="https://github.com/polyaxon/polyaxon"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="azure dev docker fs fsspec gcs git init k8s-async numpy s3 sandbox sidecar"
DEPENDENCIES="<dev-python/click-9.0.0[${PYTHON_USEDEP}]
	dev-python/clipped[${PYTHON_USEDEP}]
	dev-python/vents[${PYTHON_USEDEP}]
	<dev-python/jinja2-3.1.5[${PYTHON_USEDEP}]
	dev-python/kubernetes[${PYTHON_USEDEP}]
	>=dev-python/python-dateutil-2.7.3[${PYTHON_USEDEP}]
	>=dev-python/pytz-2019.2[${PYTHON_USEDEP}]
	>=dev-python/pyyaml-5.1[${PYTHON_USEDEP}]
	>=dev-python/orjson-3.7[${PYTHON_USEDEP}]
	>=dev-python/psutil-5.4.7[${PYTHON_USEDEP}]
	>=dev-python/requests-2.20.1[${PYTHON_USEDEP}]
	>=dev-python/requests-toolbelt-0.8.0[${PYTHON_USEDEP}]
	>=dev-python/rich-12.0.0[${PYTHON_USEDEP}]
	dev-python/sentry-sdk[${PYTHON_USEDEP}]
	>=dev-python/urllib3-1.25.6[${PYTHON_USEDEP}]
	>=dev-python/certifi-2022.12.7[${PYTHON_USEDEP}]
	>=dev-python/pydantic-1.10.2[${PYTHON_USEDEP}]
	dev-python/traceml[${PYTHON_USEDEP}]
	dev-python/hypertune[${PYTHON_USEDEP}]
	dev? ( ~dev-python/moto-5.0.13[${PYTHON_USEDEP}] )
	gcs? ( dev-python/gcsfs[${PYTHON_USEDEP}] )
	s3? ( dev-python/s3fs[${PYTHON_USEDEP}] )
	azure? ( dev-python/adlfs[${PYTHON_USEDEP}] )
	fsspec? ( dev-python/fsspec[${PYTHON_USEDEP}] )
	docker? ( dev-python/docker[${PYTHON_USEDEP}] )
	git? ( dev-python/gitpython[${PYTHON_USEDEP}] )
	numpy? ( dev-python/numpy[${PYTHON_USEDEP}] )
	k8s-async? ( dev-python/kubernetes-asyncio[${PYTHON_USEDEP}] )
	k8s-async? ( >=dev-python/aiohttp-3.0.0[${PYTHON_USEDEP}] )
	k8s-async? ( dev-python/anyio[${PYTHON_USEDEP}] )
	fs? ( dev-python/adlfs[${PYTHON_USEDEP}] )
	fs? ( ~dev-python/fsspec-2025.10.0[${PYTHON_USEDEP}] )
	fs? ( dev-python/gcsfs[${PYTHON_USEDEP}] )
	fs? ( dev-python/s3fs[${PYTHON_USEDEP}] )
	init? ( dev-python/docker[${PYTHON_USEDEP}] )
	init? ( <dev-python/gitpython-3.2.0[${PYTHON_USEDEP}] )
	init? ( <=dev-python/aiofiles-25.1.0[${PYTHON_USEDEP}] )
	sidecar? ( <=dev-python/aiofiles-25.1.0[${PYTHON_USEDEP}] )
	sidecar? ( dev-python/pandas[${PYTHON_USEDEP}] )
	sidecar? ( dev-python/anyio[${PYTHON_USEDEP}] )
	sandbox? ( dev-python/haupt[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
