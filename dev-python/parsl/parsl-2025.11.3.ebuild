# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Simple data dependent workflows in Python"

HOMEPAGE="https://github.com/Parsl/parsl"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all aws azure docs flux globus-compute globus-transfer google-cloud gssapi kubernetes monitoring proxystore radical-pilot visualization workqueue"
DEPENDENCIES=">=dev-python/pyzmq-17.1.2[${PYTHON_USEDEP}]
	<dev-python/typeguard-5.0[${PYTHON_USEDEP}]
	<dev-python/typing-extensions-5.0[${PYTHON_USEDEP}]
	dev-python/dill[${PYTHON_USEDEP}]
	dev-python/tblib[${PYTHON_USEDEP}]
	dev-python/requests[${PYTHON_USEDEP}]
	dev-python/sortedcontainers[${PYTHON_USEDEP}]
	>=dev-python/psutil-5.5.1[${PYTHON_USEDEP}]
	dev-python/setproctitle[${PYTHON_USEDEP}]
	<dev-python/filelock-4.0[${PYTHON_USEDEP}]
	monitoring? ( <dev-python/sqlalchemy-2.1[${PYTHON_USEDEP}] )
	visualization? ( >=dev-python/pydot-1.4.2[${PYTHON_USEDEP}] )
	visualization? ( <dev-python/networkx-3.3[${PYTHON_USEDEP}] )
	visualization? ( >=dev-python/flask-1.0.2[${PYTHON_USEDEP}] )
	visualization? ( dev-python/flask-sqlalchemy[${PYTHON_USEDEP}] )
	visualization? ( <dev-python/pandas-3.0[${PYTHON_USEDEP}] )
	visualization? ( dev-python/plotly[${PYTHON_USEDEP}] )
	visualization? ( dev-python/python-daemon[${PYTHON_USEDEP}] )
	aws? ( dev-python/boto3[${PYTHON_USEDEP}] )
	kubernetes? ( dev-python/kubernetes[${PYTHON_USEDEP}] )
	docs? ( <=dev-python/ipython-8.6.0[${PYTHON_USEDEP}] )
	docs? ( dev-python/nbsphinx[${PYTHON_USEDEP}] )
	docs? ( <dev-python/sphinx-8.0[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-rtd-theme[${PYTHON_USEDEP}] )
	google-cloud? ( dev-python/google-auth[${PYTHON_USEDEP}] )
	google-cloud? ( dev-python/google-api-python-client[${PYTHON_USEDEP}] )
	gssapi? ( dev-python/python-gssapi[${PYTHON_USEDEP}] )
	azure? ( dev-python/azure[${PYTHON_USEDEP}] )
	azure? ( dev-python/msrestazure[${PYTHON_USEDEP}] )
	workqueue? ( dev-python/work-queue[${PYTHON_USEDEP}] )
	flux? ( dev-python/pyyaml[${PYTHON_USEDEP}] )
	flux? ( dev-python/cffi[${PYTHON_USEDEP}] )
	flux? ( dev-python/jsonschema[${PYTHON_USEDEP}] )
	proxystore? ( dev-python/proxystore[${PYTHON_USEDEP}] )
	radical-pilot? ( dev-python/radical-pilot[${PYTHON_USEDEP}] )
	radical-pilot? ( dev-python/radical-utils[${PYTHON_USEDEP}] )
	globus-compute? ( dev-python/globus-compute-sdk[${PYTHON_USEDEP}] )
	globus-transfer? ( dev-python/globus-sdk[${PYTHON_USEDEP}] )
	all? ( <dev-python/sqlalchemy-2.1[${PYTHON_USEDEP}] )
	all? ( >=dev-python/pydot-1.4.2[${PYTHON_USEDEP}] )
	all? ( <dev-python/networkx-3.3[${PYTHON_USEDEP}] )
	all? ( >=dev-python/flask-1.0.2[${PYTHON_USEDEP}] )
	all? ( dev-python/flask-sqlalchemy[${PYTHON_USEDEP}] )
	all? ( <dev-python/pandas-3.0[${PYTHON_USEDEP}] )
	all? ( dev-python/plotly[${PYTHON_USEDEP}] )
	all? ( dev-python/python-daemon[${PYTHON_USEDEP}] )
	all? ( dev-python/boto3[${PYTHON_USEDEP}] )
	all? ( dev-python/kubernetes[${PYTHON_USEDEP}] )
	all? ( <=dev-python/ipython-8.6.0[${PYTHON_USEDEP}] )
	all? ( dev-python/nbsphinx[${PYTHON_USEDEP}] )
	all? ( <dev-python/sphinx-8.0[${PYTHON_USEDEP}] )
	all? ( dev-python/sphinx-rtd-theme[${PYTHON_USEDEP}] )
	all? ( dev-python/google-auth[${PYTHON_USEDEP}] )
	all? ( dev-python/google-api-python-client[${PYTHON_USEDEP}] )
	all? ( dev-python/python-gssapi[${PYTHON_USEDEP}] )
	all? ( dev-python/azure[${PYTHON_USEDEP}] )
	all? ( dev-python/msrestazure[${PYTHON_USEDEP}] )
	all? ( dev-python/work-queue[${PYTHON_USEDEP}] )
	all? ( dev-python/pyyaml[${PYTHON_USEDEP}] )
	all? ( dev-python/cffi[${PYTHON_USEDEP}] )
	all? ( dev-python/jsonschema[${PYTHON_USEDEP}] )
	all? ( dev-python/proxystore[${PYTHON_USEDEP}] )
	all? ( dev-python/radical-pilot[${PYTHON_USEDEP}] )
	all? ( dev-python/radical-utils[${PYTHON_USEDEP}] )
	all? ( dev-python/globus-compute-sdk[${PYTHON_USEDEP}] )
	all? ( dev-python/globus-sdk[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
