# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A server for ATAK, WinTAK, and iTAK"

HOMEPAGE="https://github.com/brian7704/OpenTAKServer"
LICENSE="GPL-3+"
SRC_URI="https://files.pythonhosted.org/packages/f6/8d/6d692e187a5ea777a3a56be0b82ab6223f0ffe9df87a4de51c8ee1d885f4/opentakserver-${REALVERSION}.tar.gz"
SOURCEFILE="opentakserver-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE=""
DEPENDENCIES="dev-python/adsbxcot[${PYTHON_USEDEP}]
	dev-python/androguard[${PYTHON_USEDEP}]
	dev-python/aiscot[${PYTHON_USEDEP}]
	~dev-python/beautifulsoup4-4.14.2[${PYTHON_USEDEP}]
	~dev-python/colorlog-6.10.1[${PYTHON_USEDEP}]
	dev-python/DateTime[${PYTHON_USEDEP}]
	~dev-python/dulwich-0.24.1[${PYTHON_USEDEP}]
	dev-python/eventlet[${PYTHON_USEDEP}]
	~dev-python/flask-3.1.2[${PYTHON_USEDEP}]
	dev-python/Flask-APScheduler[${PYTHON_USEDEP}]
	~dev-python/flask-babel-4.0.0[${PYTHON_USEDEP}]
	~dev-python/flask-cors-6.0.1[${PYTHON_USEDEP}]
	dev-python/flask-ldap3-login[${PYTHON_USEDEP}]
	dev-python/Flask-Mailman[${PYTHON_USEDEP}]
	~dev-python/flask-migrate-4.1.0[${PYTHON_USEDEP}]
	dev-python/Flask-Security-Too[${PYTHON_USEDEP}]
	dev-python/Flask-SocketIO[${PYTHON_USEDEP}]
	~dev-python/flask-sqlalchemy-3.1.1[${PYTHON_USEDEP}]
	dev-python/gevent[${PYTHON_USEDEP}]
	~dev-python/httpx-0.28.1[${PYTHON_USEDEP}]
	~dev-python/kombu-5.6.0[${PYTHON_USEDEP}]
	dev-python/lastversion[${PYTHON_USEDEP}]
	~dev-python/lxml-6.0.2[${PYTHON_USEDEP}]
	dev-python/meshtastic[${PYTHON_USEDEP}]
	~dev-python/pika-1.3.2[${PYTHON_USEDEP}]
	~dev-python/poetry-2.2.1[${PYTHON_USEDEP}]
	dev-python/poetry-dynamic-versioning[${PYTHON_USEDEP}]
	~dev-python/protobuf-6.33.1[${PYTHON_USEDEP}]
	~dev-python/psutil-7.1.3[${PYTHON_USEDEP}]
	~dev-python/psycopg-3.2.12[${PYTHON_USEDEP}]
	dev-python/pyfiglet[${PYTHON_USEDEP}]
	dev-python/pygc[${PYTHON_USEDEP}]
	~dev-python/pyjwt-2.10.0[${PYTHON_USEDEP}]
	dev-python/pytak[${PYTHON_USEDEP}]
	~dev-python/pytest-8.3.3[${PYTHON_USEDEP}]
	~dev-python/pytest-cov-6.0.0[${PYTHON_USEDEP}]
	dev-python/python-ffmpeg[${PYTHON_USEDEP}]
	dev-python/python-socketio[${PYTHON_USEDEP}]
	~dev-python/pyopenssl-25.3.0[${PYTHON_USEDEP}]
	~dev-python/pyotp-2.9.0[${PYTHON_USEDEP}]
	~dev-python/pyyaml-6.0.3[${PYTHON_USEDEP}]
	~dev-python/semver-3.0.4[${PYTHON_USEDEP}]
	~dev-python/sqlalchemy-2.0.44[${PYTHON_USEDEP}]
	~dev-python/simplekml-1.3.6[${PYTHON_USEDEP}]
	dev-python/SQLAlchemy-Utils[${PYTHON_USEDEP}]
	~dev-python/tldextract-5.3.0[${PYTHON_USEDEP}]
	dev-python/unishox2-py3[${PYTHON_USEDEP}]
	dev-python/yt-dlp[${PYTHON_USEDEP}]
	~dev-python/zope-event-5.1.1[${PYTHON_USEDEP}]
	~dev-python/zope-interface-8.1.1[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
