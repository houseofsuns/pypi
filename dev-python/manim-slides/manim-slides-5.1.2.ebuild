# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Tool for live presentations using manim"

HOMEPAGE="https://github.com/jeertmans/manim-slides"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="docs full magic manim manimgl pyqt6 pyqt6-full pyside6 pyside6-full sphinx-directive"
DEPENDENCIES="dev-python/av[${PYTHON_USEDEP}]
	>=dev-python/click-8.1.3[${PYTHON_USEDEP}]
	>=dev-python/click-default-group-1.2.2[${PYTHON_USEDEP}]
	>=dev-python/jinja-3.1.2[${PYTHON_USEDEP}]
	>=dev-python/lxml-4.9.2[${PYTHON_USEDEP}]
	>=dev-python/numpy-1.19[${PYTHON_USEDEP}]
	dev-python/opencv-python[${PYTHON_USEDEP}]
	>=dev-python/pillow-9.5.0[${PYTHON_USEDEP}]
	>=dev-python/pydantic-2.0.1[${PYTHON_USEDEP}]
	dev-python/pydantic-extra-types[${PYTHON_USEDEP}]
	dev-python/python-pptx[${PYTHON_USEDEP}]
	>=dev-python/QtPy-2.4.1[${PYTHON_USEDEP}]
	>=dev-python/requests-2.28.1[${PYTHON_USEDEP}]
	>=dev-python/rich-13.3.2[${PYTHON_USEDEP}]
	dev-python/rtoml[${PYTHON_USEDEP}]
	>=dev-python/tqdm-4.64.1[${PYTHON_USEDEP}]
	docs? ( dev-python/manim-slides[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/furo-2023.5.20[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/ipykernel-6.25.1[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/myst-parser-2.0.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/nbsphinx-0.9.2[${PYTHON_USEDEP}] )
	docs? ( dev-python/pandoc[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-7.0.1[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-click[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-copybutton-0.5.1[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinxext-opengraph[${PYTHON_USEDEP}] )
	full? ( dev-python/manim-slides[${PYTHON_USEDEP}] )
	magic? ( dev-python/manim-slides[${PYTHON_USEDEP}] )
	magic? ( >=dev-python/ipython-8.12.2[${PYTHON_USEDEP}] )
	manim? ( dev-python/manim[${PYTHON_USEDEP}] )
	manimgl? ( dev-python/manimgl[${PYTHON_USEDEP}] )
	pyqt6? ( >=dev-python/PyQt6-6.6.1[${PYTHON_USEDEP}] )
	pyqt6-full? ( dev-python/manim-slides[${PYTHON_USEDEP}] )
	pyside6? ( <dev-python/pyside6-6.5.3[${PYTHON_USEDEP}] )
	pyside6-full? ( dev-python/manim-slides[${PYTHON_USEDEP}] )
	sphinx-directive? ( >=dev-python/docutils-0.20.1[${PYTHON_USEDEP}] )
	sphinx-directive? ( dev-python/manim-slides[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
