# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python3_12 )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Nasy's Deep Learning Toolkit"

HOMEPAGE=""
LICENSE="GPL-3"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="doc mac"
DEPENDENCIES="dev-python/jax[${PYTHON_USEDEP}]
	>=dev-python/numpy-1.26.4[${PYTHON_USEDEP}]
	>=dev-python/rich-13.7.1[${PYTHON_USEDEP}]
	dev-python/jaxtyping[${PYTHON_USEDEP}]
	dev-python/optax[${PYTHON_USEDEP}]
	dev-python/equinox[${PYTHON_USEDEP}]
	>=dev-python/beartype-0.18.5[${PYTHON_USEDEP}]
	>=dev-python/scikit-image-0.23.2[${PYTHON_USEDEP}]
	dev-python/orbax-checkpoint[${PYTHON_USEDEP}]
	>=dev-python/scikit-learn-1.5.0[${PYTHON_USEDEP}]
	dev-python/einops[${PYTHON_USEDEP}]
	doc? ( >=dev-python/mkdocs-material-9.5.27[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/mkdocs-1.6.0[${PYTHON_USEDEP}] )
	doc? ( dev-python/mike[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/mkdocstrings-0.25.1[${PYTHON_USEDEP}] )
	mac? ( dev-python/torch[${PYTHON_USEDEP}] )
	mac? ( dev-python/tensorflow[${PYTHON_USEDEP}] )
	mac? ( dev-python/tensordict[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
