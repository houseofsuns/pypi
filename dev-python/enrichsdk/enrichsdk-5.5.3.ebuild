# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Enrich Developer Kit"

HOMEPAGE="http://github.com/pingali/scribble-enrichsdk"
LICENSE="All rights reserved"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE=""
DEPENDENCIES="dev-python/ruptures[${PYTHON_USEDEP}]
	dev-python/wheel[${PYTHON_USEDEP}]
	>=dev-python/click-7.1.2[${PYTHON_USEDEP}]
	~dev-python/aioitertools-0.8.0[${PYTHON_USEDEP}]
	~dev-python/typing-extensions-4.8.0[${PYTHON_USEDEP}]
	dev-python/glob2[${PYTHON_USEDEP}]
	dev-python/httpx[${PYTHON_USEDEP}]
	dev-python/h2[${PYTHON_USEDEP}]
	~dev-python/chardet-4.0.0[${PYTHON_USEDEP}]
	>=dev-python/requests-2.32.0[${PYTHON_USEDEP}]
	~dev-python/requests-oauthlib-0.8.0[${PYTHON_USEDEP}]
	>=dev-python/pytest-5.5.3[${PYTHON_USEDEP}]
	<dev-python/pandas-1.5[${PYTHON_USEDEP}]
	dev-python/distributed[${PYTHON_USEDEP}]
	~dev-python/idna-3.7[${PYTHON_USEDEP}]
	>=dev-python/coverage-7.4.3[${PYTHON_USEDEP}]
	dev-python/flake8[${PYTHON_USEDEP}]
	dev-python/raven[${PYTHON_USEDEP}]
	~dev-python/python-json-logger-2.0.4[${PYTHON_USEDEP}]
	>=dev-python/python-dateutil-2.8.1[${PYTHON_USEDEP}]
	<=dev-python/pydantic-1.10.10[${PYTHON_USEDEP}]
	<=dev-python/numpy-1.23.10[${PYTHON_USEDEP}]
	dev-python/s3fs[${PYTHON_USEDEP}]
	dev-python/boto3[${PYTHON_USEDEP}]
	>=dev-python/botocore-1.34.0[${PYTHON_USEDEP}]
	dev-python/aiobotocore[${PYTHON_USEDEP}]
	dev-python/gcsfs[${PYTHON_USEDEP}]
	dev-python/colored[${PYTHON_USEDEP}]
	~dev-python/humanize-0.5.1[${PYTHON_USEDEP}]
	>=dev-python/pytz-2020.1[${PYTHON_USEDEP}]
	>=dev-python/jinja-3.1.3[${PYTHON_USEDEP}]
	dev-python/pytest-cov[${PYTHON_USEDEP}]
	>=dev-python/markdown-2.9.10[${PYTHON_USEDEP}]
	<dev-python/prompt-toolkit-3.1.0[${PYTHON_USEDEP}]
	>=dev-python/pyarrow-0.9.0[${PYTHON_USEDEP}]
	dev-python/cytoolz[${PYTHON_USEDEP}]
	>=dev-python/jsonschema-3.2.0[${PYTHON_USEDEP}]
	<=dev-python/scipy-1.13.1[${PYTHON_USEDEP}]
	dev-python/seaborn[${PYTHON_USEDEP}]
	>=dev-python/packaging-20.0[${PYTHON_USEDEP}]
	dev-python/prefect[${PYTHON_USEDEP}]
	~dev-python/griffe-0.25.1[${PYTHON_USEDEP}]
	>=dev-python/distro-1.4.0[${PYTHON_USEDEP}]
	>=dev-python/jupyter-core-5.7.0[${PYTHON_USEDEP}]
	>=dev-python/nbformat-5.5.3[${PYTHON_USEDEP}]
	>=dev-python/tzlocal-2.0.0[${PYTHON_USEDEP}]
	dev-python/texttable[${PYTHON_USEDEP}]
	dev-python/pykafka[${PYTHON_USEDEP}]
	dev-python/redis[${PYTHON_USEDEP}]
	dev-python/GitPython[${PYTHON_USEDEP}]
	dev-python/logstash_formatter[${PYTHON_USEDEP}]
	dev-python/PyHive[${PYTHON_USEDEP}]
	dev-python/pyfiglet[${PYTHON_USEDEP}]
	<dev-python/sqlalchemy-2.0[${PYTHON_USEDEP}]
	dev-python/kafka-python[${PYTHON_USEDEP}]
	dev-python/pykafka[${PYTHON_USEDEP}]
	dev-python/papermill[${PYTHON_USEDEP}]
	<=dev-python/tenacity-8.3.0[${PYTHON_USEDEP}]
	dev-python/sqllineage[${PYTHON_USEDEP}]
	dev-python/google-cloud-logging[${PYTHON_USEDEP}]
	dev-python/unidecode[${PYTHON_USEDEP}]
	dev-python/Faker[${PYTHON_USEDEP}]
	dev-python/xlsxwriter[${PYTHON_USEDEP}]
	dev-python/cryptography[${PYTHON_USEDEP}]
	dev-python/pymongo[${PYTHON_USEDEP}]
	dev-python/great-expectations[${PYTHON_USEDEP}]
	dev-python/pandas-stubs[${PYTHON_USEDEP}]
	dev-python/openai[${PYTHON_USEDEP}]
	dev-python/pretty-html-table[${PYTHON_USEDEP}]
	>=dev-python/plotly-4.0.0[${PYTHON_USEDEP}]
	dev-python/prophet[${PYTHON_USEDEP}]
	<=dev-python/scikit-learn-1.4.0[${PYTHON_USEDEP}]
	dev-python/imblearn[${PYTHON_USEDEP}]
	dev-python/msgraph-sdk[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
