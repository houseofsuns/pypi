# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Free Open Source Electronic Document Management System"

HOMEPAGE="https://www.mayan-edms.com"
LICENSE="GPL-2"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE=""
DEPENDENCIES="~dev-python/django-4.2.27[${PYTHON_USEDEP}]
	dev-python/CairoSVG[${PYTHON_USEDEP}]
	~dev-python/markdown-3.10[${PYTHON_USEDEP}]
	~dev-python/pillow-12.0.0[${PYTHON_USEDEP}]
	~dev-python/pyyaml-6.0.3[${PYTHON_USEDEP}]
	~dev-python/whoosh-2.7.4[${PYTHON_USEDEP}]
	~dev-python/boto3-1.40.24[${PYTHON_USEDEP}]
	dev-python/celery[${PYTHON_USEDEP}]
	dev-python/dateparser[${PYTHON_USEDEP}]
	dev-python/django-activity-stream[${PYTHON_USEDEP}]
	~dev-python/django-auth-ldap-5.2.0[${PYTHON_USEDEP}]
	dev-python/django-celery-beat[${PYTHON_USEDEP}]
	~dev-python/django-cors-headers-4.9.0[${PYTHON_USEDEP}]
	dev-python/django-formtools[${PYTHON_USEDEP}]
	dev-python/django-model-utils[${PYTHON_USEDEP}]
	dev-python/django-mptt[${PYTHON_USEDEP}]
	dev-python/django-qsstats-magic[${PYTHON_USEDEP}]
	dev-python/django-solo[${PYTHON_USEDEP}]
	dev-python/django-storages[${PYTHON_USEDEP}]
	dev-python/django-stronghold[${PYTHON_USEDEP}]
	dev-python/django-widget-tweaks[${PYTHON_USEDEP}]
	~dev-python/djangorestframework-3.16.1[${PYTHON_USEDEP}]
	dev-python/djangorestframework-recursive[${PYTHON_USEDEP}]
	dev-python/drf-yasg[${PYTHON_USEDEP}]
	~dev-python/elasticsearch-9.1.0[${PYTHON_USEDEP}]
	dev-python/extract-msg[${PYTHON_USEDEP}]
	dev-python/flex[${PYTHON_USEDEP}]
	~dev-python/furl-2.1.4[${PYTHON_USEDEP}]
	dev-python/fusepy[${PYTHON_USEDEP}]
	dev-python/gevent[${PYTHON_USEDEP}]
	dev-python/google-cloud-storage[${PYTHON_USEDEP}]
	~dev-python/graphviz-0.21[${PYTHON_USEDEP}]
	~dev-python/greenlet-3.2.4[${PYTHON_USEDEP}]
	dev-python/gunicorn[${PYTHON_USEDEP}]
	~dev-python/jsonschema-4.25.1[${PYTHON_USEDEP}]
	dev-python/mozilla-django-oidc[${PYTHON_USEDEP}]
	~dev-python/nh3-0.3.2[${PYTHON_USEDEP}]
	~dev-python/node-semver-0.9.0[${PYTHON_USEDEP}]
	dev-python/ollama[${PYTHON_USEDEP}]
	dev-python/openai[${PYTHON_USEDEP}]
	~dev-python/pycountry-24.6.1[${PYTHON_USEDEP}]
	~dev-python/pycryptodome-3.23.0[${PYTHON_USEDEP}]
	~dev-python/pyotp-2.9.0[${PYTHON_USEDEP}]
	~dev-python/pypdf-6.4.0[${PYTHON_USEDEP}]
	~dev-python/python-dateutil-2.9.0_p0[${PYTHON_USEDEP}]
	~dev-python/python-gnupg-0.5.5[${PYTHON_USEDEP}]
	~dev-python/pytz-2025.2[${PYTHON_USEDEP}]
	~dev-python/qrcode-8.2[${PYTHON_USEDEP}]
	~dev-python/requests-2.32.5[${PYTHON_USEDEP}]
	dev-python/sentry-sdk[${PYTHON_USEDEP}]
	~dev-python/sh-2.2.2[${PYTHON_USEDEP}]
	~dev-python/swagger-spec-validator-3.0.4[${PYTHON_USEDEP}]
	dev-python/whitenoise[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
