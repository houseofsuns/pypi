# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python3_14 )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Collection of python tools to re-use common code across scrapers"

HOMEPAGE="https://github.com/openzim/python-scraperlib"
LICENSE="GPL-3+"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="check dev docs lint scripts test"
DEPENDENCIES="<dev-python/babel-3.0[${PYTHON_USEDEP}]
	<dev-python/beartype-0.23[${PYTHON_USEDEP}]
	<dev-python/beautifulsoup4-5.0[${PYTHON_USEDEP}]
	dev-python/CairoSVG[${PYTHON_USEDEP}]
	dev-python/colorthief[${PYTHON_USEDEP}]
	<dev-python/idna-4.0[${PYTHON_USEDEP}]
	dev-python/iso639-lang[${PYTHON_USEDEP}]
	dev-python/libzim[${PYTHON_USEDEP}]
	<dev-python/lxml-7.0[${PYTHON_USEDEP}]
	dev-python/optimize-images[${PYTHON_USEDEP}]
	~dev-python/piexif-1.1.3[${PYTHON_USEDEP}]
	<dev-python/pillow-13.0[${PYTHON_USEDEP}]
	dev-python/PyMuPDF[${PYTHON_USEDEP}]
	<dev-python/python-magic-0.5[${PYTHON_USEDEP}]
	dev-python/python-resize-image[${PYTHON_USEDEP}]
	>=dev-python/regex-2020.7.14[${PYTHON_USEDEP}]
	<dev-python/requests-3.0[${PYTHON_USEDEP}]
	dev-python/types-xxhash[${PYTHON_USEDEP}]
	<dev-python/urllib3-2.6.0[${PYTHON_USEDEP}]
	<dev-python/xxhash-4.0[${PYTHON_USEDEP}]
	dev-python/yt-dlp[${PYTHON_USEDEP}]
	check? ( dev-python/pyright[${PYTHON_USEDEP}] )
	check? ( ~dev-python/pytest-9.0.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/black-25.11.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/coverage-7.11.3[${PYTHON_USEDEP}] )
	dev? ( dev-python/invoke[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/ipython-9.7.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/jinja2-3.1.6[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/mkdocs-gen-files-0.5.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/mkdocs-include-markdown-plugin[${PYTHON_USEDEP}] )
	dev? ( dev-python/mkdocs-literate-nav[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/mkdocs-material-9.7.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/mkdocs-1.6.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/mkdocstrings-0.30.1[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pymdown-extensions-10.17.1[${PYTHON_USEDEP}] )
	dev? ( dev-python/pyright[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pytest-mock-3.15.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pytest-9.0.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pyyaml-6.0.3[${PYTHON_USEDEP}] )
	dev? ( dev-python/ruff[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/mkdocs-gen-files-0.5.0[${PYTHON_USEDEP}] )
	docs? ( dev-python/mkdocs-include-markdown-plugin[${PYTHON_USEDEP}] )
	docs? ( dev-python/mkdocs-literate-nav[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/mkdocs-material-9.7.0[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/mkdocs-1.6.1[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/mkdocstrings-0.30.1[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/pymdown-extensions-10.17.1[${PYTHON_USEDEP}] )
	lint? ( ~dev-python/black-25.11.0[${PYTHON_USEDEP}] )
	lint? ( dev-python/ruff[${PYTHON_USEDEP}] )
	scripts? ( dev-python/invoke[${PYTHON_USEDEP}] )
	scripts? ( ~dev-python/jinja2-3.1.6[${PYTHON_USEDEP}] )
	scripts? ( ~dev-python/pyyaml-6.0.3[${PYTHON_USEDEP}] )
	test? ( ~dev-python/coverage-7.11.3[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-mock-3.15.1[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-9.0.1[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
