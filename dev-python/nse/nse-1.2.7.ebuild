# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Unofficial Python Api for NSE India stock exchange"

HOMEPAGE="https://github.com/BennyThadikaran/NseIndiaApi"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev local server"
DEPENDENCIES="dev-python/mthrottle[${PYTHON_USEDEP}]
	dev? ( ~dev-python/furo-2023.9.10[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/sphinx-7.4.7[${PYTHON_USEDEP}] )
	local? ( >=dev-python/requests-2.31[${PYTHON_USEDEP}] )
	server? ( ~dev-python/httpx-0.28.1[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
