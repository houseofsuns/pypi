# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Spidermon is a framework to build monitors for Scrapy spiders."

HOMEPAGE="https://github.com/scrapinghub/spidermon"
LICENSE="BSD"
SRC_URI="https://files.pythonhosted.org/packages/source/s/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="docs monitoring tests validation"
DEPENDENCIES="dev-python/jsonschema[${PYTHON_USEDEP}]
	dev-python/python-slugify[${PYTHON_USEDEP}]
	docs? ( dev-python/sphinx[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-rtd-theme[${PYTHON_USEDEP}] )
	docs? ( dev-python/s3cmd[${PYTHON_USEDEP}] )
	monitoring? ( dev-python/scrapy[${PYTHON_USEDEP}] )
	monitoring? ( dev-python/Jinja2[${PYTHON_USEDEP}] )
	monitoring? ( dev-python/scrapinghub[${PYTHON_USEDEP}] )
	monitoring? ( dev-python/slack-sdk[${PYTHON_USEDEP}] )
	monitoring? ( dev-python/boto[${PYTHON_USEDEP}] )
	monitoring? ( dev-python/boto3[${PYTHON_USEDEP}] )
	monitoring? ( dev-python/premailer[${PYTHON_USEDEP}] )
	monitoring? ( dev-python/sentry-sdk[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-mock[${PYTHON_USEDEP}] )
	tests? ( dev-python/jinja2[${PYTHON_USEDEP}] )
	tests? ( dev-python/boto3[${PYTHON_USEDEP}] )
	tests? ( dev-python/lxml[${PYTHON_USEDEP}] )
	tests? ( dev-python/premailer[${PYTHON_USEDEP}] )
	tests? ( dev-python/scrapinghub[${PYTHON_USEDEP}] )
	tests? ( dev-python/scrapinghub-entrypoint-scrapy[${PYTHON_USEDEP}] )
	tests? ( dev-python/scrapy[${PYTHON_USEDEP}] )
	tests? ( dev-python/slack-sdk[${PYTHON_USEDEP}] )
	tests? ( dev-python/twisted[${PYTHON_USEDEP}] )
	tests? ( dev-python/itemadapter[${PYTHON_USEDEP}] )
	validation? ( dev-python/schematics[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
