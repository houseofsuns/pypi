# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="discord-ext-music"
REALVERSION="0.3.0"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_9,3_10,3_11} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="An easy-to-use music extension for discord.py"

HOMEPAGE="https://github.com/mansuf/discord-ext-music"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/d/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all av docs equalizer miniaudio"
DEPENDENCIES="dev-python/discord-py[${PYTHON_USEDEP}]
	all? ( dev-python/pydub[${PYTHON_USEDEP}] )
	all? ( dev-python/scipy[${PYTHON_USEDEP}] )
	all? ( dev-python/miniaudio[${PYTHON_USEDEP}] )
	all? ( dev-python/av[${PYTHON_USEDEP}] )
	av? ( dev-python/av[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx[${PYTHON_USEDEP}] )
	docs? ( dev-python/furo[${PYTHON_USEDEP}] )
	equalizer? ( dev-python/pydub[${PYTHON_USEDEP}] )
	equalizer? ( dev-python/scipy[${PYTHON_USEDEP}] )
	miniaudio? ( dev-python/miniaudio[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
