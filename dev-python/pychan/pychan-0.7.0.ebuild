# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A Python library for interacting with 4chan in a programmatically-friendly way."

HOMEPAGE="https://github.com/cooperwalbrun/pychan"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="development github_actions testing"
DEPENDENCIES="<dev-python/beautifulsoup4-5.0[${PYTHON_USEDEP}]
	dev-python/importlib-metadata[${PYTHON_USEDEP}]
	<dev-python/pyrate-limiter-3.0[${PYTHON_USEDEP}]
	<dev-python/requests-3.0[${PYTHON_USEDEP}]
	dev-python/types-beautifulsoup4[${PYTHON_USEDEP}]
	dev-python/types-requests[${PYTHON_USEDEP}]
	development? ( dev-python/mypy[${PYTHON_USEDEP}] )
	development? ( dev-python/pytest[${PYTHON_USEDEP}] )
	development? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	development? ( dev-python/pytest-mock[${PYTHON_USEDEP}] )
	development? ( dev-python/responses[${PYTHON_USEDEP}] )
	development? ( dev-python/tox[${PYTHON_USEDEP}] )
	development? ( dev-python/yapf[${PYTHON_USEDEP}] )
	github_actions? ( dev-python/mypy[${PYTHON_USEDEP}] )
	github_actions? ( dev-python/pytest[${PYTHON_USEDEP}] )
	github_actions? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	github_actions? ( dev-python/pytest-mock[${PYTHON_USEDEP}] )
	github_actions? ( dev-python/responses[${PYTHON_USEDEP}] )
	github_actions? ( dev-python/tox[${PYTHON_USEDEP}] )
	github_actions? ( dev-python/tox-gh-actions[${PYTHON_USEDEP}] )
	testing? ( dev-python/mypy[${PYTHON_USEDEP}] )
	testing? ( dev-python/pytest[${PYTHON_USEDEP}] )
	testing? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	testing? ( dev-python/pytest-mock[${PYTHON_USEDEP}] )
	testing? ( dev-python/responses[${PYTHON_USEDEP}] )
	testing? ( dev-python/tox[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
