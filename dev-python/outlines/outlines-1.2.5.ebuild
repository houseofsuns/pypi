# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Probabilistic Generative Model Programming"

HOMEPAGE="https://github.com/dottxt-ai/outlines"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="airports anthropic countries dottxt gemini llamacpp llguidance mlxlm ollama openai sglang test test-gpu tgi transformers vllm xgrammar"
DEPENDENCIES="dev-python/jinja2[${PYTHON_USEDEP}]
	dev-python/cloudpickle[${PYTHON_USEDEP}]
	dev-python/diskcache[${PYTHON_USEDEP}]
	>=dev-python/pydantic-2.0[${PYTHON_USEDEP}]
	dev-python/jsonschema[${PYTHON_USEDEP}]
	dev-python/pillow[${PYTHON_USEDEP}]
	dev-python/typing-extensions[${PYTHON_USEDEP}]
	dev-python/outlines-core[${PYTHON_USEDEP}]
	dev-python/genson[${PYTHON_USEDEP}]
	dev-python/jsonpath-ng[${PYTHON_USEDEP}]
	anthropic? ( dev-python/anthropic[${PYTHON_USEDEP}] )
	dottxt? ( dev-python/dottxt[${PYTHON_USEDEP}] )
	gemini? ( dev-python/google-genai[${PYTHON_USEDEP}] )
	llamacpp? ( dev-python/huggingface-hub[${PYTHON_USEDEP}] )
	llamacpp? ( dev-python/llama-cpp-python[${PYTHON_USEDEP}] )
	llamacpp? ( dev-python/numba[${PYTHON_USEDEP}] )
	mlxlm? ( dev-python/datasets[${PYTHON_USEDEP}] )
	mlxlm? ( dev-python/mlx[${PYTHON_USEDEP}] )
	mlxlm? ( dev-python/mlx-lm[${PYTHON_USEDEP}] )
	ollama? ( dev-python/ollama[${PYTHON_USEDEP}] )
	openai? ( dev-python/openai[${PYTHON_USEDEP}] )
	sglang? ( dev-python/openai[${PYTHON_USEDEP}] )
	tgi? ( dev-python/huggingface-hub[${PYTHON_USEDEP}] )
	transformers? ( dev-python/accelerate[${PYTHON_USEDEP}] )
	transformers? ( dev-python/datasets[${PYTHON_USEDEP}] )
	transformers? ( dev-python/transformers[${PYTHON_USEDEP}] )
	transformers? ( dev-python/setuptools[${PYTHON_USEDEP}] )
	vllm? ( dev-python/openai[${PYTHON_USEDEP}] )
	xgrammar? ( dev-python/xgrammar[${PYTHON_USEDEP}] )
	llguidance? ( dev-python/llguidance[${PYTHON_USEDEP}] )
	airports? ( dev-python/airportsdata[${PYTHON_USEDEP}] )
	countries? ( dev-python/iso3166[${PYTHON_USEDEP}] )
	test? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-benchmark[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-mock[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-asyncio[${PYTHON_USEDEP}] )
	test? ( >=dev-python/coverage-5.1[${PYTHON_USEDEP}] )
	test? ( dev-python/diff-cover[${PYTHON_USEDEP}] )
	test? ( dev-python/accelerate[${PYTHON_USEDEP}] )
	test? ( <dev-python/beartype-0.16.0[${PYTHON_USEDEP}] )
	test? ( dev-python/responses[${PYTHON_USEDEP}] )
	test? ( dev-python/llama-cpp-python[${PYTHON_USEDEP}] )
	test? ( dev-python/mlx-lm[${PYTHON_USEDEP}] )
	test? ( dev-python/huggingface-hub[${PYTHON_USEDEP}] )
	test? ( dev-python/openai[${PYTHON_USEDEP}] )
	test? ( dev-python/datasets[${PYTHON_USEDEP}] )
	test? ( dev-python/anthropic[${PYTHON_USEDEP}] )
	test? ( dev-python/google-genai[${PYTHON_USEDEP}] )
	test? ( dev-python/transformers[${PYTHON_USEDEP}] )
	test? ( dev-python/pillow[${PYTHON_USEDEP}] )
	test? ( dev-python/jax[${PYTHON_USEDEP}] )
	test? ( dev-python/flax[${PYTHON_USEDEP}] )
	test? ( <dev-python/numpy-2.2.0[${PYTHON_USEDEP}] )
	test? ( dev-python/numba[${PYTHON_USEDEP}] )
	test? ( dev-python/torch[${PYTHON_USEDEP}] )
	test? ( dev-python/tensorflow[${PYTHON_USEDEP}] )
	test? ( dev-python/tf-keras[${PYTHON_USEDEP}] )
	test? ( dev-python/ollama[${PYTHON_USEDEP}] )
	test? ( dev-python/dottxt[${PYTHON_USEDEP}] )
	test? ( dev-python/sentencepiece[${PYTHON_USEDEP}] )
	test? ( dev-python/mkdocs-gen-files[${PYTHON_USEDEP}] )
	test? ( dev-python/llguidance[${PYTHON_USEDEP}] )
	test? ( dev-python/xgrammar[${PYTHON_USEDEP}] )
	test? ( dev-python/airportsdata[${PYTHON_USEDEP}] )
	test? ( dev-python/iso3166[${PYTHON_USEDEP}] )
	test? ( dev-python/requests[${PYTHON_USEDEP}] )
	test-gpu? ( dev-python/outlines[${PYTHON_USEDEP}] )
	test-gpu? ( dev-python/vllm[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
