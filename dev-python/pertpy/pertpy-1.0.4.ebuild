# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_12,3_13} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Perturbation Analysis in the scverse ecosystem."

HOMEPAGE=""
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all de dev doc milo-edger scgen tcoda test"
DEPENDENCIES="dev-python/adjustText[${PYTHON_USEDEP}]
	dev-python/arviz[${PYTHON_USEDEP}]
	dev-python/blitzgsea[${PYTHON_USEDEP}]
	dev-python/fast-array-utils[${PYTHON_USEDEP}]
	dev-python/filelock[${PYTHON_USEDEP}]
	dev-python/flax[${PYTHON_USEDEP}]
	dev-python/funsor[${PYTHON_USEDEP}]
	dev-python/mudata[${PYTHON_USEDEP}]
	dev-python/numpyro[${PYTHON_USEDEP}]
	dev-python/openpyxl[${PYTHON_USEDEP}]
	dev-python/ott-jax[${PYTHON_USEDEP}]
	dev-python/PubChemPy[${PYTHON_USEDEP}]
	dev-python/pyarrow[${PYTHON_USEDEP}]
	dev-python/requests[${PYTHON_USEDEP}]
	dev-python/rich[${PYTHON_USEDEP}]
	dev-python/scanpy[${PYTHON_USEDEP}]
	>=dev-python/scikit-learn-1.4[${PYTHON_USEDEP}]
	dev-python/scikit-misc[${PYTHON_USEDEP}]
	dev-python/sparsecca[${PYTHON_USEDEP}]
	all? ( dev-python/ete4[${PYTHON_USEDEP}] )
	all? ( dev-python/formulaic[${PYTHON_USEDEP}] )
	all? ( dev-python/formulaic-contrasts[${PYTHON_USEDEP}] )
	all? ( dev-python/pydeseq2[${PYTHON_USEDEP}] )
	all? ( dev-python/pyqt6[${PYTHON_USEDEP}] )
	all? ( dev-python/scvi-tools[${PYTHON_USEDEP}] )
	all? ( dev-python/toytree[${PYTHON_USEDEP}] )
	de? ( dev-python/formulaic[${PYTHON_USEDEP}] )
	de? ( dev-python/formulaic-contrasts[${PYTHON_USEDEP}] )
	de? ( dev-python/pydeseq2[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/docutils-0.8[${PYTHON_USEDEP}] )
	doc? ( dev-python/ipykernel[${PYTHON_USEDEP}] )
	doc? ( dev-python/ipython[${PYTHON_USEDEP}] )
	doc? ( dev-python/myst-nb[${PYTHON_USEDEP}] )
	doc? ( dev-python/nbsphinx[${PYTHON_USEDEP}] )
	doc? ( dev-python/nbsphinx-link[${PYTHON_USEDEP}] )
	doc? ( dev-python/pygments[${PYTHON_USEDEP}] )
	doc? ( dev-python/scanpydoc[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-autodoc-typehints[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-automodapi[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-book-theme[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-copybutton[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx_design[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-gallery[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-issues[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-last-updated-by-git[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-remove-toctrees[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-tabs[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/sphinx-8.1[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/sphinxcontrib-bibtex-1.0.0[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinxext-opengraph[${PYTHON_USEDEP}] )
	milo-edger? ( dev-python/rpy2[${PYTHON_USEDEP}] )
	scgen? ( dev-python/scvi-tools[${PYTHON_USEDEP}] )
	tcoda? ( dev-python/ete4[${PYTHON_USEDEP}] )
	tcoda? ( dev-python/pyqt6[${PYTHON_USEDEP}] )
	tcoda? ( dev-python/toytree[${PYTHON_USEDEP}] )
	test? ( dev-python/coverage[${PYTHON_USEDEP}] )
	test? ( dev-python/leidenalg[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
