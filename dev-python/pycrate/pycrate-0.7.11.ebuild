# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A software suite to handle various data and protocol formats"

HOMEPAGE="https://github.com/pycrate-org/pycrate/"
LICENSE="LGPL v2.1+"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="corenet diameter-dict nas5g naslte sedebugmux"
DEPENDENCIES="naslte? ( dev-python/cryptomobile[${PYTHON_USEDEP}] )
	nas5g? ( dev-python/cryptomobile[${PYTHON_USEDEP}] )
	corenet? ( dev-python/pysctp[${PYTHON_USEDEP}] )
	corenet? ( dev-python/cryptomobile[${PYTHON_USEDEP}] )
	diameter-dict? ( dev-python/lxml[${PYTHON_USEDEP}] )
	sedebugmux? ( dev-python/crcmod[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
