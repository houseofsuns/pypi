# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="2.0.0rc3"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Django integration with Google App Engine"

HOMEPAGE="https://gitlab.com/potato-oss/djangae/djangae"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="test"
DEPENDENCIES="<=dev-python/django-3.3[${PYTHON_USEDEP}]
	dev-python/django-gcloud-connectors[${PYTHON_USEDEP}]
	>=dev-python/google-api-python-client-2.27.0[${PYTHON_USEDEP}]
	dev-python/google-cloud-tasks[${PYTHON_USEDEP}]
	dev-python/google-cloud-logging[${PYTHON_USEDEP}]
	>=dev-python/psutil-5.7.3[${PYTHON_USEDEP}]
	>=dev-python/requests-2.22.0[${PYTHON_USEDEP}]
	~dev-python/cryptography-3.4.6[${PYTHON_USEDEP}]
	~dev-python/python-jose-3.2.0[${PYTHON_USEDEP}]
	dev-python/google-cloud-storage[${PYTHON_USEDEP}]
	>=dev-python/google-auth-2.3.2[${PYTHON_USEDEP}]
	test? ( dev-python/beautifulsoup4[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
