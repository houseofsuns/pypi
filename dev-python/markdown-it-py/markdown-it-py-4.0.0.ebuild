# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Python port of markdown-it. Markdown parsing, done right!"

HOMEPAGE="https://github.com/executablebooks/markdown-it-py"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="benchmarking compare linkify plugins profiling rtd testing"
DEPENDENCIES="~dev-python/mdurl-0.1[${PYTHON_USEDEP}]
	benchmarking? ( dev-python/psutil[${PYTHON_USEDEP}] )
	benchmarking? ( dev-python/pytest[${PYTHON_USEDEP}] )
	benchmarking? ( dev-python/pytest-benchmark[${PYTHON_USEDEP}] )
	compare? ( ~dev-python/commonmark-0.9[${PYTHON_USEDEP}] )
	compare? ( ~dev-python/markdown-3.4[${PYTHON_USEDEP}] )
	compare? ( ~dev-python/mistletoe-1.0[${PYTHON_USEDEP}] )
	compare? ( ~dev-python/mistune-3.0[${PYTHON_USEDEP}] )
	compare? ( dev-python/panflute[${PYTHON_USEDEP}] )
	compare? ( dev-python/markdown-it-pyrs[${PYTHON_USEDEP}] )
	linkify? ( <dev-python/linkify-it-py-3.0[${PYTHON_USEDEP}] )
	plugins? ( >=dev-python/mdit-py-plugins-0.5.0[${PYTHON_USEDEP}] )
	profiling? ( dev-python/gprof2dot[${PYTHON_USEDEP}] )
	rtd? ( >=dev-python/mdit-py-plugins-0.5.0[${PYTHON_USEDEP}] )
	rtd? ( dev-python/myst-parser[${PYTHON_USEDEP}] )
	rtd? ( dev-python/pyyaml[${PYTHON_USEDEP}] )
	rtd? ( dev-python/sphinx[${PYTHON_USEDEP}] )
	rtd? ( dev-python/sphinx-copybutton[${PYTHON_USEDEP}] )
	rtd? ( dev-python/sphinx_design[${PYTHON_USEDEP}] )
	rtd? ( dev-python/sphinx-book-theme[${PYTHON_USEDEP}] )
	rtd? ( dev-python/jupyter-sphinx[${PYTHON_USEDEP}] )
	rtd? ( dev-python/ipykernel[${PYTHON_USEDEP}] )
	testing? ( dev-python/coverage[${PYTHON_USEDEP}] )
	testing? ( dev-python/pytest[${PYTHON_USEDEP}] )
	testing? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	testing? ( dev-python/pytest-regressions[${PYTHON_USEDEP}] )
	testing? ( dev-python/requests[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
