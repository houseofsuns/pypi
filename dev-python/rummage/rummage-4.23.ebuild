# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A GUI search and replace app."

HOMEPAGE="https://github.com/facelessuser/Rummage"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="extras"
DEPENDENCIES=">=dev-python/backrefs-6.1[${PYTHON_USEDEP}]
	>=dev-python/bracex-2.6[${PYTHON_USEDEP}]
	>=dev-python/chardet-5.2.0[${PYTHON_USEDEP}]
	dev-python/coloraide[${PYTHON_USEDEP}]
	>=dev-python/filelock-3.16[${PYTHON_USEDEP}]
	>=dev-python/markdown-3.10[${PYTHON_USEDEP}]
	>=dev-python/pygments-2.19.2[${PYTHON_USEDEP}]
	>=dev-python/pymdown-extensions-10.20[${PYTHON_USEDEP}]
	>=dev-python/send2trash-1.8.3[${PYTHON_USEDEP}]
	<dev-python/wcmatch-11.0[${PYTHON_USEDEP}]
	>=dev-python/wxpython-4.1.0[${PYTHON_USEDEP}]
	extras? ( >=dev-python/faust-cchardet-2.1.19[${PYTHON_USEDEP}] )
	extras? ( >=dev-python/regex-2025.7.34[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
