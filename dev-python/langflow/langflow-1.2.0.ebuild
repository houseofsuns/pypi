# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=wheel

inherit python-r1 gs-pypi

DESCRIPTION="A Python package with a built-in web application [wheel]"

HOMEPAGE=""
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/py3/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}-py3-none-any.whl"
SOURCEFILE="${REALNAME}-${REALVERSION}-py3-none-any.whl"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="cassio clickhouse-connect couchbase deploy local nv-ingest"
DEPENDENCIES="dev-python/ag2[${PYTHON_USEDEP}]
	dev-python/aiofile[${PYTHON_USEDEP}]
	dev-python/apify-client[${PYTHON_USEDEP}]
	dev-python/arize-phoenix-otel[${PYTHON_USEDEP}]
	dev-python/assemblyai[${PYTHON_USEDEP}]
	dev-python/astra-assistants[${PYTHON_USEDEP}]
	dev-python/atlassian-python-api[${PYTHON_USEDEP}]
	~dev-python/beautifulsoup4-4.12.3[${PYTHON_USEDEP}]
	~dev-python/boto3-1.34.162[${PYTHON_USEDEP}]
	<dev-python/certifi-2025.0.0[${PYTHON_USEDEP}]
	~dev-python/certifi-2024.8.30[${PYTHON_USEDEP}]
	dev-python/chromadb[${PYTHON_USEDEP}]
	dev-python/composio-core[${PYTHON_USEDEP}]
	dev-python/composio-langchain[${PYTHON_USEDEP}]
	dev-python/crewai[${PYTHON_USEDEP}]
	dev-python/dspy-ai[${PYTHON_USEDEP}]
	dev-python/duckduckgo-search[${PYTHON_USEDEP}]
	~dev-python/elasticsearch-8.16.0[${PYTHON_USEDEP}]
	dev-python/faiss-cpu[${PYTHON_USEDEP}]
	dev-python/fake-useragent[${PYTHON_USEDEP}]
	dev-python/fastavro[${PYTHON_USEDEP}]
	dev-python/GitPython[${PYTHON_USEDEP}]
	~dev-python/google-api-python-client-2.154.0[${PYTHON_USEDEP}]
	dev-python/google-search-results[${PYTHON_USEDEP}]
	dev-python/huggingface-hub[${PYTHON_USEDEP}]
	~dev-python/jq-1.8.0[${PYTHON_USEDEP}]
	dev-python/json-repair[${PYTHON_USEDEP}]
	dev-python/kubernetes[${PYTHON_USEDEP}]
	dev-python/langchain-anthropic[${PYTHON_USEDEP}]
	dev-python/langchain-astradb[${PYTHON_USEDEP}]
	dev-python/langchain-aws[${PYTHON_USEDEP}]
	dev-python/langchain-chroma[${PYTHON_USEDEP}]
	dev-python/langchain-cohere[${PYTHON_USEDEP}]
	dev-python/langchain-community[${PYTHON_USEDEP}]
	dev-python/langchain-elasticsearch[${PYTHON_USEDEP}]
	dev-python/langchain-google-calendar-tools[${PYTHON_USEDEP}]
	dev-python/langchain-google-community[${PYTHON_USEDEP}]
	dev-python/langchain-google-genai[${PYTHON_USEDEP}]
	dev-python/langchain-google-vertexai[${PYTHON_USEDEP}]
	dev-python/langchain-groq[${PYTHON_USEDEP}]
	dev-python/langchain-milvus[${PYTHON_USEDEP}]
	dev-python/langchain-mistralai[${PYTHON_USEDEP}]
	dev-python/langchain-mongodb[${PYTHON_USEDEP}]
	dev-python/langchain-nvidia-ai-endpoints[${PYTHON_USEDEP}]
	dev-python/langchain-ollama[${PYTHON_USEDEP}]
	dev-python/langchain-openai[${PYTHON_USEDEP}]
	dev-python/langchain-pinecone[${PYTHON_USEDEP}]
	dev-python/langchain-sambanova[${PYTHON_USEDEP}]
	dev-python/langchain-unstructured[${PYTHON_USEDEP}]
	dev-python/langchain[${PYTHON_USEDEP}]
	dev-python/langflow-base[${PYTHON_USEDEP}]
	dev-python/langfuse[${PYTHON_USEDEP}]
	dev-python/langsmith[${PYTHON_USEDEP}]
	dev-python/langwatch[${PYTHON_USEDEP}]
	~dev-python/lark-1.2.2[${PYTHON_USEDEP}]
	dev-python/litellm[${PYTHON_USEDEP}]
	~dev-python/markdown-3.7[${PYTHON_USEDEP}]
	~dev-python/markupsafe-3.0.2[${PYTHON_USEDEP}]
	dev-python/mcp[${PYTHON_USEDEP}]
	dev-python/mem0ai[${PYTHON_USEDEP}]
	dev-python/metal-sdk[${PYTHON_USEDEP}]
	dev-python/metaphor-python[${PYTHON_USEDEP}]
	dev-python/needle-python[${PYTHON_USEDEP}]
	~dev-python/networkx-3.4.2[${PYTHON_USEDEP}]
	dev-python/nltk[${PYTHON_USEDEP}]
	~dev-python/numexpr-2.10.2[${PYTHON_USEDEP}]
	dev-python/openinference-instrumentation-langchain[${PYTHON_USEDEP}]
	~dev-python/opensearch-py-2.8.0[${PYTHON_USEDEP}]
	dev-python/pgvector[${PYTHON_USEDEP}]
	~dev-python/pyarrow-19.0.0[${PYTHON_USEDEP}]
	dev-python/pydantic-ai[${PYTHON_USEDEP}]
	dev-python/pydantic-settings[${PYTHON_USEDEP}]
	~dev-python/pymongo-4.10.1[${PYTHON_USEDEP}]
	dev-python/pytube[${PYTHON_USEDEP}]
	dev-python/qdrant-client[${PYTHON_USEDEP}]
	dev-python/qianfan[${PYTHON_USEDEP}]
	dev-python/ragstack-ai-knowledge-store[${PYTHON_USEDEP}]
	~dev-python/redis-5.2.1[${PYTHON_USEDEP}]
	dev-python/scrapegraph-py[${PYTHON_USEDEP}]
	dev-python/smolagents[${PYTHON_USEDEP}]
	dev-python/spider-client[${PYTHON_USEDEP}]
	<dev-python/sqlalchemy-3.0.0[${PYTHON_USEDEP}]
	dev-python/sseclient-py[${PYTHON_USEDEP}]
	dev-python/supabase[${PYTHON_USEDEP}]
	dev-python/types-cachetools[${PYTHON_USEDEP}]
	dev-python/upstash-vector[${PYTHON_USEDEP}]
	>=dev-python/uv-0.5.7[${PYTHON_USEDEP}]
	dev-python/weaviate-client[${PYTHON_USEDEP}]
	dev-python/wikipedia[${PYTHON_USEDEP}]
	dev-python/wolframalpha[${PYTHON_USEDEP}]
	dev-python/yfinance[${PYTHON_USEDEP}]
	dev-python/youtube-transcript-api[${PYTHON_USEDEP}]
	dev-python/zep-python[${PYTHON_USEDEP}]
	cassio? ( dev-python/cassio[${PYTHON_USEDEP}] )
	clickhouse-connect? ( dev-python/clickhouse-connect[${PYTHON_USEDEP}] )
	couchbase? ( dev-python/couchbase[${PYTHON_USEDEP}] )
	deploy? ( dev-python/celery[${PYTHON_USEDEP}] )
	deploy? ( dev-python/flower[${PYTHON_USEDEP}] )
	local? ( dev-python/ctransformers[${PYTHON_USEDEP}] )
	local? ( dev-python/llama-cpp-python[${PYTHON_USEDEP}] )
	local? ( dev-python/sentence-transformers[${PYTHON_USEDEP}] )
	nv-ingest? ( dev-python/llama-index-embeddings-nvidia[${PYTHON_USEDEP}] )
	nv-ingest? ( dev-python/nv-ingest-client[${PYTHON_USEDEP}] )
	nv-ingest? ( dev-python/pymilvus[${PYTHON_USEDEP}] )
	nv-ingest? ( dev-python/python-pptx[${PYTHON_USEDEP}] )
	virtual/allow-pypi-wheels"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
