# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Python API for MISP."

HOMEPAGE=""
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="brotli docs email fileobjects openioc pdfexport url virustotal"
DEPENDENCIES="openioc? ( <dev-python/beautifulsoup4-4.14[${PYTHON_USEDEP}] )
	>=dev-python/deprecated-1.3.1[${PYTHON_USEDEP}]
	docs? ( <dev-python/docutils-0.22[${PYTHON_USEDEP}] )
	email? ( dev-python/extract-msg[${PYTHON_USEDEP}] )
	fileobjects? ( dev-python/lief[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/myst-parser-4.0.1[${PYTHON_USEDEP}] )
	email? ( dev-python/oletools[${PYTHON_USEDEP}] )
	fileobjects? ( dev-python/pure-magic-rs[${PYTHON_USEDEP}] )
	fileobjects? ( dev-python/pydeep2[${PYTHON_USEDEP}] )
	url? ( dev-python/pyfaup-rs[${PYTHON_USEDEP}] )
	>=dev-python/python-dateutil-2.9.0_p0[${PYTHON_USEDEP}]
	pdfexport? ( >=dev-python/reportlab-4.4.7[${PYTHON_USEDEP}] )
	>=dev-python/requests-2.32.5[${PYTHON_USEDEP}]
	docs? ( <dev-python/sphinx-9.0[${PYTHON_USEDEP}] )
	docs? ( <dev-python/sphinx-autodoc-typehints-3.6[${PYTHON_USEDEP}] )
	brotli? ( >=dev-python/urllib3-2.6.3[${PYTHON_USEDEP}] )
	virustotal? ( >=dev-python/validators-0.35.0[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
