# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="4.0.0a2"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Nautobot Secrets Providers App"

HOMEPAGE="https://github.com/nautobot/nautobot-app-secrets-providers"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all aws azure hashicorp onepassword thycotic"
DEPENDENCIES="all? ( dev-python/azure-identity[${PYTHON_USEDEP}] )
	azure? ( dev-python/azure-identity[${PYTHON_USEDEP}] )
	all? ( dev-python/azure-keyvault-secrets[${PYTHON_USEDEP}] )
	azure? ( dev-python/azure-keyvault-secrets[${PYTHON_USEDEP}] )
	all? ( <dev-python/boto3-2.0.0[${PYTHON_USEDEP}] )
	aws? ( <dev-python/boto3-2.0.0[${PYTHON_USEDEP}] )
	all? ( <dev-python/hvac-1.1.0[${PYTHON_USEDEP}] )
	hashicorp? ( <dev-python/hvac-1.1.0[${PYTHON_USEDEP}] )
	dev-python/nautobot[${PYTHON_USEDEP}]
	all? ( dev-python/onepassword-sdk[${PYTHON_USEDEP}] )
	onepassword? ( dev-python/onepassword-sdk[${PYTHON_USEDEP}] )
	all? ( dev-python/python-tss-sdk[${PYTHON_USEDEP}] )
	thycotic? ( dev-python/python-tss-sdk[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
