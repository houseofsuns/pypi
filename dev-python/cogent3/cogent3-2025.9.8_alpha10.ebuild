# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="2025.9.8a10"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="COmparative GENomics Toolkit 3: genomic sequence analysis within notebooks or on compute systems with 1000s of CPUs."

HOMEPAGE=""
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev doc extra test typing"
DEPENDENCIES="dev-python/charset-normalizer[${PYTHON_USEDEP}]
	>dev-python/loky-3.5.0[${PYTHON_USEDEP}]
	dev-python/numpy[${PYTHON_USEDEP}]
	dev-python/numba[${PYTHON_USEDEP}]
	dev-python/numba[${PYTHON_USEDEP}]
	dev-python/numba[${PYTHON_USEDEP}]
	dev-python/scipy[${PYTHON_USEDEP}]
	dev-python/scitrack[${PYTHON_USEDEP}]
	dev-python/stevedore[${PYTHON_USEDEP}]
	dev-python/tqdm[${PYTHON_USEDEP}]
	dev-python/typing-extensions[${PYTHON_USEDEP}]
	dev? ( dev-python/flit[${PYTHON_USEDEP}] )
	dev? ( dev-python/nox[${PYTHON_USEDEP}] )
	dev? ( dev-python/numpydoc[${PYTHON_USEDEP}] )
	dev? ( dev-python/scriv[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-tqdm[${PYTHON_USEDEP}] )
	dev? ( dev-python/cogent3[${PYTHON_USEDEP}] )
	dev? ( dev-python/cogent3[${PYTHON_USEDEP}] )
	dev? ( dev-python/cogent3[${PYTHON_USEDEP}] )
	doc? ( dev-python/click[${PYTHON_USEDEP}] )
	doc? ( dev-python/cogent3[${PYTHON_USEDEP}] )
	doc? ( dev-python/cogent3-h5seqs[${PYTHON_USEDEP}] )
	doc? ( dev-python/jupyter-sphinx[${PYTHON_USEDEP}] )
	doc? ( dev-python/jupyter-client[${PYTHON_USEDEP}] )
	doc? ( dev-python/jupytext[${PYTHON_USEDEP}] )
	doc? ( >dev-python/nbconvert-5.4[${PYTHON_USEDEP}] )
	doc? ( dev-python/nbformat[${PYTHON_USEDEP}] )
	doc? ( dev-python/nbsphinx[${PYTHON_USEDEP}] )
	doc? ( dev-python/numpydoc[${PYTHON_USEDEP}] )
	doc? ( dev-python/piqtree[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-autobuild[${PYTHON_USEDEP}] )
	doc? ( dev-python/pydata-sphinx-theme[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx_design[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinxcontrib-bibtex[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinxcontrib-video[${PYTHON_USEDEP}] )
	doc? ( dev-python/iplotx[${PYTHON_USEDEP}] )
	extra? ( dev-python/ipykernel[${PYTHON_USEDEP}] )
	extra? ( dev-python/ipython[${PYTHON_USEDEP}] )
	extra? ( dev-python/ipywidgets[${PYTHON_USEDEP}] )
	extra? ( dev-python/jupyterlab[${PYTHON_USEDEP}] )
	extra? ( dev-python/kaleido[${PYTHON_USEDEP}] )
	extra? ( dev-python/notebook[${PYTHON_USEDEP}] )
	extra? ( dev-python/pandas[${PYTHON_USEDEP}] )
	extra? ( dev-python/pillow[${PYTHON_USEDEP}] )
	extra? ( dev-python/plotly[${PYTHON_USEDEP}] )
	extra? ( dev-python/plotly-stubs[${PYTHON_USEDEP}] )
	extra? ( dev-python/psutil[${PYTHON_USEDEP}] )
	test? ( dev-python/cogent3-h5seqs[${PYTHON_USEDEP}] )
	test? ( dev-python/piqtree[${PYTHON_USEDEP}] )
	test? ( dev-python/ruff[${PYTHON_USEDEP}] )
	test? ( dev-python/kaleido[${PYTHON_USEDEP}] )
	test? ( dev-python/pandas[${PYTHON_USEDEP}] )
	test? ( dev-python/plotly[${PYTHON_USEDEP}] )
	test? ( dev-python/plotly-stubs[${PYTHON_USEDEP}] )
	test? ( dev-python/psutil[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-xdist[${PYTHON_USEDEP}] )
	test? ( dev-python/tinydb[${PYTHON_USEDEP}] )
	test? ( dev-python/nox[${PYTHON_USEDEP}] )
	typing? ( ~dev-python/mypy-1.18.2[${PYTHON_USEDEP}] )
	typing? ( dev-python/plotly-stubs[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
