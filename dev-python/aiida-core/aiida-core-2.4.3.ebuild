...
DESCRIPTION="AiiDA is a workflow manager for computational science with a strong focus on provenance, performance and extensibility. [topver 2.5.0]"
...
DEPENDENCIES="~dev-python/alembic-1.2[${PYTHON_USEDEP}]
	...
	docs? ( ~dev-python/sphinx-notfound-page-0.5[${PYTHON_USEDEP}] )
	...
	pre-commit? ( ~dev-python/packaging-20.9[${PYTHON_USEDEP}] )
	...
	pre-commit? ( ~dev-python/sqlalchemy-1.4.29[${PYTHON_USEDEP}] )
	...
	tests? ( ~dev-python/pytest-timeout-1.3[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pytest-cov-2.7[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pytest-rerunfailures-9.1[${PYTHON_USEDEP}] )
	...
	tests? ( ~dev-python/docutils-0.16[${PYTHON_USEDEP}] )"
