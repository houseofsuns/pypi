# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="2.10.0rc3"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A fast FDTD solver"

HOMEPAGE="https://github.com/flexcompute/tidy3d"
LICENSE="LGPL-2+"
SRC_URI="https://files.pythonhosted.org/packages/66/24/d04d66fd77632418ce507a9b262a5e8eb3fa0fa676ff865cf12e11d0a028/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="design dev docs extras gdstk heatcharge pytorch ruff scikit-rf tests trimesh vtk"
DEPENDENCIES="dev-python/pyyaml[${PYTHON_USEDEP}]
	dev-python/autograd[${PYTHON_USEDEP}]
	dev? ( dev-python/bayesian-optimization[${PYTHON_USEDEP}] )
	design? ( dev-python/bayesian-optimization[${PYTHON_USEDEP}] )
	<dev-python/boto3-2.0.0[${PYTHON_USEDEP}]
	<dev-python/click-9.0.0[${PYTHON_USEDEP}]
	dev? ( dev-python/cma[${PYTHON_USEDEP}] )
	docs? ( dev-python/cma[${PYTHON_USEDEP}] )
	dev? ( dev-python/coverage[${PYTHON_USEDEP}] )
	dev-python/dask[${PYTHON_USEDEP}]
	dev? ( dev-python/devsim[${PYTHON_USEDEP}] )
	docs? ( dev-python/devsim[${PYTHON_USEDEP}] )
	heatcharge? ( dev-python/devsim[${PYTHON_USEDEP}] )
	dev? ( dev-python/diff-cover[${PYTHON_USEDEP}] )
	dev? ( dev-python/dill[${PYTHON_USEDEP}] )
	dev? ( dev-python/gdstk[${PYTHON_USEDEP}] )
	docs? ( dev-python/gdstk[${PYTHON_USEDEP}] )
	gdstk? ( dev-python/gdstk[${PYTHON_USEDEP}] )
	dev? ( dev-python/grcwa[${PYTHON_USEDEP}] )
	docs? ( dev-python/grcwa[${PYTHON_USEDEP}] )
	dev-python/h5netcdf[${PYTHON_USEDEP}]
	<dev-python/h5py-3.15[${PYTHON_USEDEP}]
	>=dev-python/importlib-metadata-6.0.0[${PYTHON_USEDEP}]
	dev? ( dev-python/ipython[${PYTHON_USEDEP}] )
	dev? ( dev-python/ipython[${PYTHON_USEDEP}] )
	docs? ( dev-python/ipython[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/jinja2-3.1.2[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/jinja2-3.1.2[${PYTHON_USEDEP}] )
	dev-python/joblib[${PYTHON_USEDEP}]
	dev? ( dev-python/jupyter[${PYTHON_USEDEP}] )
	docs? ( dev-python/jupyter[${PYTHON_USEDEP}] )
	dev-python/matplotlib[${PYTHON_USEDEP}]
	dev? ( dev-python/memory-profiler[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/mypy-1.13.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/myst-parser[${PYTHON_USEDEP}] )
	docs? ( dev-python/myst-parser[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/nbconvert-7.11.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/nbconvert-7.11.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/nbdime[${PYTHON_USEDEP}] )
	docs? ( dev-python/nbdime[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/nbsphinx-0.8.7[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/nbsphinx-0.8.7[${PYTHON_USEDEP}] )
	dev? ( <dev-python/networkx-3.0.0[${PYTHON_USEDEP}] )
	trimesh? ( <dev-python/networkx-3.0.0[${PYTHON_USEDEP}] )
	dev-python/numpy[${PYTHON_USEDEP}]
	dev? ( dev-python/openpyxl[${PYTHON_USEDEP}] )
	docs? ( dev-python/openpyxl[${PYTHON_USEDEP}] )
	dev? ( dev-python/optax[${PYTHON_USEDEP}] )
	docs? ( dev-python/optax[${PYTHON_USEDEP}] )
	dev-python/pandas[${PYTHON_USEDEP}]
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	tests? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	dev? ( dev-python/psutil[${PYTHON_USEDEP}] )
	tests? ( dev-python/psutil[${PYTHON_USEDEP}] )
	<dev-python/pydantic-3.0[${PYTHON_USEDEP}]
	dev? ( >=dev-python/pydata-sphinx-theme-0.13.3[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/pydata-sphinx-theme-0.13.3[${PYTHON_USEDEP}] )
	dev? ( dev-python/pygad[${PYTHON_USEDEP}] )
	design? ( dev-python/pygad[${PYTHON_USEDEP}] )
	dev-python/pyjwt[${PYTHON_USEDEP}]
	dev? ( dev-python/pylint[${PYTHON_USEDEP}] )
	tests? ( dev-python/pylint[${PYTHON_USEDEP}] )
	docs? ( dev-python/pylint[${PYTHON_USEDEP}] )
	dev-python/pyroots[${PYTHON_USEDEP}]
	dev? ( dev-python/pyswarms[${PYTHON_USEDEP}] )
	design? ( dev-python/pyswarms[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-8.1[${PYTHON_USEDEP}] )
	tests? ( >=dev-python/pytest-8.1[${PYTHON_USEDEP}] )
	dev? ( <dev-python/pytest-cov-7.0.0[${PYTHON_USEDEP}] )
	tests? ( <dev-python/pytest-cov-7.0.0[${PYTHON_USEDEP}] )
	dev? ( <dev-python/pytest-env-2.0.0[${PYTHON_USEDEP}] )
	tests? ( <dev-python/pytest-env-2.0.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-timeout[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-timeout[${PYTHON_USEDEP}] )
	dev? ( <dev-python/pytest-xdist-4.0.0[${PYTHON_USEDEP}] )
	tests? ( <dev-python/pytest-xdist-4.0.0[${PYTHON_USEDEP}] )
	<dev-python/requests-3.0.0[${PYTHON_USEDEP}]
	dev-python/responses[${PYTHON_USEDEP}]
	<dev-python/rich-14.0[${PYTHON_USEDEP}]
	dev? ( ~dev-python/rtree-1.2.0[${PYTHON_USEDEP}] )
	trimesh? ( ~dev-python/rtree-1.2.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/ruff[${PYTHON_USEDEP}] )
	ruff? ( dev-python/ruff[${PYTHON_USEDEP}] )
	dev? ( dev-python/sax[${PYTHON_USEDEP}] )
	docs? ( dev-python/sax[${PYTHON_USEDEP}] )
	dev? ( dev-python/scikit-rf[${PYTHON_USEDEP}] )
	scikit-rf? ( dev-python/scikit-rf[${PYTHON_USEDEP}] )
	dev-python/scipy[${PYTHON_USEDEP}]
	<dev-python/shapely-3.0[${PYTHON_USEDEP}]
	dev? ( dev-python/signac[${PYTHON_USEDEP}] )
	docs? ( dev-python/signac[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/sphinx-6.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-6.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/sphinx-book-theme[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-book-theme[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/sphinx-copybutton-0.5.2[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-copybutton-0.5.2[${PYTHON_USEDEP}] )
	dev? ( dev-python/sphinx_design[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx_design[${PYTHON_USEDEP}] )
	dev? ( dev-python/sphinx-favicon[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-favicon[${PYTHON_USEDEP}] )
	dev? ( dev-python/sphinx-notfound-page[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-notfound-page[${PYTHON_USEDEP}] )
	dev? ( dev-python/sphinx-sitemap[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-sitemap[${PYTHON_USEDEP}] )
	dev? ( dev-python/sphinx-tabs[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-tabs[${PYTHON_USEDEP}] )
	dev? ( dev-python/sphinxemoji[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinxemoji[${PYTHON_USEDEP}] )
	extras? ( dev-python/tidy3d-extras[${PYTHON_USEDEP}] )
	dev? ( dev-python/tmm[${PYTHON_USEDEP}] )
	docs? ( dev-python/tmm[${PYTHON_USEDEP}] )
	dev-python/toml[${PYTHON_USEDEP}]
	<dev-python/tomlkit-0.14.0[${PYTHON_USEDEP}]
	dev? ( dev-python/torch[${PYTHON_USEDEP}] )
	pytorch? ( dev-python/torch[${PYTHON_USEDEP}] )
	dev? ( dev-python/torch[${PYTHON_USEDEP}] )
	pytorch? ( dev-python/torch[${PYTHON_USEDEP}] )
	dev? ( dev-python/tox[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/trimesh-4.6[${PYTHON_USEDEP}] )
	tests? ( >=dev-python/trimesh-4.6[${PYTHON_USEDEP}] )
	trimesh? ( >=dev-python/trimesh-4.6[${PYTHON_USEDEP}] )
	heatcharge? ( >=dev-python/trimesh-4.6[${PYTHON_USEDEP}] )
	dev-python/typing-extensions[${PYTHON_USEDEP}]
	dev? ( dev-python/vtk[${PYTHON_USEDEP}] )
	vtk? ( dev-python/vtk[${PYTHON_USEDEP}] )
	heatcharge? ( dev-python/vtk[${PYTHON_USEDEP}] )
	>=dev-python/xarray-2023.8[${PYTHON_USEDEP}]
	dev? ( dev-python/zizmor[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
