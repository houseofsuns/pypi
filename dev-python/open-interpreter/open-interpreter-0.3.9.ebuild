# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Let language models run code"

HOMEPAGE=""
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="local os safe server"
DEPENDENCIES="os? ( dev-python/opencv-python[${PYTHON_USEDEP}] )
	local? ( dev-python/opencv-python[${PYTHON_USEDEP}] )
	os? ( dev-python/PyAutoGUI[${PYTHON_USEDEP}] )
	os? ( dev-python/plyer[${PYTHON_USEDEP}] )
	os? ( dev-python/PyWinCtl[${PYTHON_USEDEP}] )
	os? ( <dev-python/pytesseract-0.4.0[${PYTHON_USEDEP}] )
	local? ( <dev-python/pytesseract-0.4.0[${PYTHON_USEDEP}] )
	os? ( dev-python/sentence-transformers[${PYTHON_USEDEP}] )
	dev-python/nltk[${PYTHON_USEDEP}]
	os? ( <dev-python/ipywidgets-9.0.0[${PYTHON_USEDEP}] )
	os? ( dev-python/torch[${PYTHON_USEDEP}] )
	local? ( dev-python/torch[${PYTHON_USEDEP}] )
	os? ( dev-python/timm[${PYTHON_USEDEP}] )
	os? ( dev-python/screeninfo[${PYTHON_USEDEP}] )
	safe? ( dev-python/semgrep[${PYTHON_USEDEP}] )
	local? ( dev-python/transformers[${PYTHON_USEDEP}] )
	local? ( dev-python/einops[${PYTHON_USEDEP}] )
	local? ( dev-python/torchvision[${PYTHON_USEDEP}] )
	local? ( dev-python/easyocr[${PYTHON_USEDEP}] )
	server? ( dev-python/fastapi[${PYTHON_USEDEP}] )
	server? ( <dev-python/uvicorn-0.31.0[${PYTHON_USEDEP}] )
	server? ( dev-python/janus[${PYTHON_USEDEP}] )
	dev-python/setuptools[${PYTHON_USEDEP}]
	<dev-python/astor-0.9.0[${PYTHON_USEDEP}]
	dev-python/git-python[${PYTHON_USEDEP}]
	dev-python/inquirer[${PYTHON_USEDEP}]
	<dev-python/pyyaml-7.0.0[${PYTHON_USEDEP}]
	<dev-python/rich-14.0.0[${PYTHON_USEDEP}]
	<dev-python/six-2.0.0[${PYTHON_USEDEP}]
	dev-python/tokentrim[${PYTHON_USEDEP}]
	dev-python/wget[${PYTHON_USEDEP}]
	<dev-python/psutil-6.0.0[${PYTHON_USEDEP}]
	dev-python/html2image[${PYTHON_USEDEP}]
	<dev-python/send2trash-2.0.0[${PYTHON_USEDEP}]
	<dev-python/ipykernel-7.0.0[${PYTHON_USEDEP}]
	<dev-python/jupyter-client-9.0.0[${PYTHON_USEDEP}]
	<dev-python/matplotlib-4.0.0[${PYTHON_USEDEP}]
	dev-python/toml[${PYTHON_USEDEP}]
	dev-python/tiktoken[${PYTHON_USEDEP}]
	<dev-python/platformdirs-5.0.0[${PYTHON_USEDEP}]
	<dev-python/pydantic-3.0.0[${PYTHON_USEDEP}]
	dev-python/google-generativeai[${PYTHON_USEDEP}]
	<dev-python/pyperclip-2.0.0[${PYTHON_USEDEP}]
	dev-python/yaspin[${PYTHON_USEDEP}]
	dev-python/shortuuid[${PYTHON_USEDEP}]
	dev-python/litellm[${PYTHON_USEDEP}]
	<dev-python/starlette-0.38.0[${PYTHON_USEDEP}]
	dev-python/typer[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
