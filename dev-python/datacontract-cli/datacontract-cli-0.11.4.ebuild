...
IUSE="all api athena avro bigquery csv databricks dbml dbt dev duckdb excel iceberg kafka oracle parquet postgres protobuf rdf s3 snowflake sqlserver trino"
DEPENDENCIES="<dev-python/typer-0.22[${PYTHON_USEDEP}]
	...
	kafka? ( <dev-python/numpy-2.0.0[${PYTHON_USEDEP}] )
	...
	duckdb? ( dev-python/soda-core-duckdb[${PYTHON_USEDEP}] )
	parquet? ( >=dev-python/pyarrow-18.1.0[${PYTHON_USEDEP}] )
	rdf? ( ~dev-python/rdflib-7.5.0[${PYTHON_USEDEP}] )
	api? ( dev-python/fastapi[${PYTHON_USEDEP}] )
	api? ( ~dev-python/uvicorn-0.40.0[${PYTHON_USEDEP}] )
