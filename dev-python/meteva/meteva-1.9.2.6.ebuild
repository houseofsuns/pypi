# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A collections of functions for meteorological verification."

HOMEPAGE=""
LICENSE="GPL-3"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE=""
DEPENDENCIES=">=dev-python/shapely-1.8.0[${PYTHON_USEDEP}]
	>=dev-python/numpy-1.12.1[${PYTHON_USEDEP}]
	>=dev-python/pandas-1.0.4[${PYTHON_USEDEP}]
	>=dev-python/netcdf4-1.4.1[${PYTHON_USEDEP}]
	>=dev-python/scipy-0.19.0[${PYTHON_USEDEP}]
	>=dev-python/xarray-0.10.0[${PYTHON_USEDEP}]
	>=dev-python/scikit-learn-0.21.2[${PYTHON_USEDEP}]
	>=dev-python/matplotlib-3.2.2[${PYTHON_USEDEP}]
	>=dev-python/httplib2-0.12.0[${PYTHON_USEDEP}]
	>=dev-python/protobuf-1.0.0[${PYTHON_USEDEP}]
	dev-python/pyshp[${PYTHON_USEDEP}]
	>=dev-python/tables-3.4.4[${PYTHON_USEDEP}]
	>=dev-python/urllib3-1.21.1[${PYTHON_USEDEP}]
	dev-python/pynverse[${PYTHON_USEDEP}]
	>=dev-python/importlib-resources-5.2.1[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
