# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="aws-account"
REALVERSION="0.2.4"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Print out AWS account and identity information to verify which account/organization is currently in use."

HOMEPAGE="https://github.com/rollwagen/aws-account"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/35/fe/cc289be6aec210989bc6ea92072db8ee5ffb311b24b69932215cc8be5800/aws_account-${REALVERSION}.tar.gz"
SOURCEFILE="aws_account-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE=""
DEPENDENCIES="dev-python/click[${PYTHON_USEDEP}]
	dev-python/boto3[${PYTHON_USEDEP}]
	dev-python/colorama[${PYTHON_USEDEP}]
	dev-python/mypy-boto3-sts[${PYTHON_USEDEP}]
	dev-python/mypy-boto3-sso[${PYTHON_USEDEP}]
	dev-python/mypy-boto3-iam[${PYTHON_USEDEP}]
	dev-python/mypy-boto3-organizations[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
