# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A Python lib for solving & executing graphs of functions, with \`pandas\` in mind"

HOMEPAGE="http://github.com/pygraphkit/graphtik"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all dev dill matplot plot sphinx test"
DEPENDENCIES="dev-python/boltons[${PYTHON_USEDEP}]
	dev-python/networkx[${PYTHON_USEDEP}]
	all? ( dev-python/pydot[${PYTHON_USEDEP}] )
	all? ( dev-python/html5lib[${PYTHON_USEDEP}] )
	all? ( dev-python/pandas[${PYTHON_USEDEP}] )
	all? ( dev-python/pytest-sugar[${PYTHON_USEDEP}] )
	all? ( dev-python/sphinxcontrib-spelling[${PYTHON_USEDEP}] )
	all? ( dev-python/matplotlib[${PYTHON_USEDEP}] )
	all? ( >=dev-python/sphinx-2.0[${PYTHON_USEDEP}] )
	all? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	all? ( dev-python/markupsafe[${PYTHON_USEDEP}] )
	all? ( dev-python/pytest[${PYTHON_USEDEP}] )
	all? ( dev-python/pytest-sphinx[${PYTHON_USEDEP}] )
	all? ( >=dev-python/jinja-3.0[${PYTHON_USEDEP}] )
	all? ( dev-python/readme-renderer[${PYTHON_USEDEP}] )
	all? ( dev-python/dill[${PYTHON_USEDEP}] )
	all? ( dev-python/wheel[${PYTHON_USEDEP}] )
	all? ( dev-python/black[${PYTHON_USEDEP}] )
	all? ( dev-python/pylint[${PYTHON_USEDEP}] )
	all? ( dev-python/mypy[${PYTHON_USEDEP}] )
	dev? ( dev-python/pydot[${PYTHON_USEDEP}] )
	dev? ( dev-python/html5lib[${PYTHON_USEDEP}] )
	dev? ( dev-python/pandas[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-sugar[${PYTHON_USEDEP}] )
	dev? ( dev-python/sphinxcontrib-spelling[${PYTHON_USEDEP}] )
	dev? ( dev-python/matplotlib[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/sphinx-2.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	dev? ( dev-python/markupsafe[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-sphinx[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/jinja-3.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/readme-renderer[${PYTHON_USEDEP}] )
	dev? ( dev-python/dill[${PYTHON_USEDEP}] )
	dev? ( dev-python/wheel[${PYTHON_USEDEP}] )
	dev? ( dev-python/black[${PYTHON_USEDEP}] )
	dev? ( dev-python/pylint[${PYTHON_USEDEP}] )
	dev? ( dev-python/mypy[${PYTHON_USEDEP}] )
	dill? ( dev-python/dill[${PYTHON_USEDEP}] )
	matplot? ( dev-python/pydot[${PYTHON_USEDEP}] )
	matplot? ( >=dev-python/jinja-3.0[${PYTHON_USEDEP}] )
	matplot? ( dev-python/markupsafe[${PYTHON_USEDEP}] )
	matplot? ( dev-python/matplotlib[${PYTHON_USEDEP}] )
	plot? ( dev-python/pydot[${PYTHON_USEDEP}] )
	plot? ( >=dev-python/jinja-3.0[${PYTHON_USEDEP}] )
	plot? ( dev-python/markupsafe[${PYTHON_USEDEP}] )
	sphinx? ( dev-python/pydot[${PYTHON_USEDEP}] )
	sphinx? ( >=dev-python/jinja-3.0[${PYTHON_USEDEP}] )
	sphinx? ( dev-python/markupsafe[${PYTHON_USEDEP}] )
	sphinx? ( >=dev-python/sphinx-2.0[${PYTHON_USEDEP}] )
	test? ( dev-python/pydot[${PYTHON_USEDEP}] )
	test? ( dev-python/html5lib[${PYTHON_USEDEP}] )
	test? ( dev-python/pandas[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-sugar[${PYTHON_USEDEP}] )
	test? ( dev-python/sphinxcontrib-spelling[${PYTHON_USEDEP}] )
	test? ( dev-python/matplotlib[${PYTHON_USEDEP}] )
	test? ( >=dev-python/sphinx-2.0[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	test? ( dev-python/markupsafe[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-sphinx[${PYTHON_USEDEP}] )
	test? ( >=dev-python/jinja-3.0[${PYTHON_USEDEP}] )
	test? ( dev-python/readme-renderer[${PYTHON_USEDEP}] )
	test? ( dev-python/dill[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
