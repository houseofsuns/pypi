# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="TerraTorch - The geospatial foundation model fine-tuning toolkit"

HOMEPAGE=""
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev geobenchv2 mmseg peft surya test visualize vllm vllm-test wxc"
DEPENDENCIES="dev-python/torch[${PYTHON_USEDEP}]
	>=dev-python/numpy-2.2[${PYTHON_USEDEP}]
	dev-python/torchvision[${PYTHON_USEDEP}]
	dev-python/rioxarray[${PYTHON_USEDEP}]
	dev-python/albumentations[${PYTHON_USEDEP}]
	dev-python/albucore[${PYTHON_USEDEP}]
	dev-python/rasterio[${PYTHON_USEDEP}]
	dev-python/torchmetrics[${PYTHON_USEDEP}]
	dev-python/geopandas[${PYTHON_USEDEP}]
	dev-python/lightly[${PYTHON_USEDEP}]
	dev-python/h5py[${PYTHON_USEDEP}]
	dev-python/lightning[${PYTHON_USEDEP}]
	dev-python/segmentation-models-pytorch[${PYTHON_USEDEP}]
	dev-python/jsonargparse[${PYTHON_USEDEP}]
	dev-python/torchgeo[${PYTHON_USEDEP}]
	dev-python/einops[${PYTHON_USEDEP}]
	dev-python/timm[${PYTHON_USEDEP}]
	dev-python/pycocotools[${PYTHON_USEDEP}]
	dev-python/huggingface-hub[${PYTHON_USEDEP}]
	dev-python/tifffile[${PYTHON_USEDEP}]
	dev-python/python-box[${PYTHON_USEDEP}]
	dev-python/tqdm[${PYTHON_USEDEP}]
	dev-python/wandb[${PYTHON_USEDEP}]
	dev-python/tensorboard[${PYTHON_USEDEP}]
	dev-python/diffusers[${PYTHON_USEDEP}]
	>=dev-python/scikit-learn-1.3.2[${PYTHON_USEDEP}]
	dev-python/scikit-image[${PYTHON_USEDEP}]
	dev? ( dev-python/black[${PYTHON_USEDEP}] )
	dev? ( dev-python/mkdocs-material[${PYTHON_USEDEP}] )
	dev? ( dev-python/mkdocstrings[${PYTHON_USEDEP}] )
	dev? ( dev-python/mike[${PYTHON_USEDEP}] )
	dev? ( dev-python/mkdocs-git-revision-date-localized-plugin[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-coverage[${PYTHON_USEDEP}] )
	test? ( dev-python/coverage-badge[${PYTHON_USEDEP}] )
	test? ( dev-python/peft[${PYTHON_USEDEP}] )
	test? ( dev-python/diffusers[${PYTHON_USEDEP}] )
	test? ( dev-python/tokenizers[${PYTHON_USEDEP}] )
	test? ( dev-python/dask[${PYTHON_USEDEP}] )
	test? ( dev-python/wandb[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-sugar[${PYTHON_USEDEP}] )
	test? ( dev-python/numba[${PYTHON_USEDEP}] )
	test? ( dev-python/hdf5plugin[${PYTHON_USEDEP}] )
	test? ( dev-python/h5netcdf[${PYTHON_USEDEP}] )
	test? ( dev-python/geobenchv2[${PYTHON_USEDEP}] )
	surya? ( dev-python/numba[${PYTHON_USEDEP}] )
	surya? ( dev-python/hdf5plugin[${PYTHON_USEDEP}] )
	surya? ( dev-python/h5netcdf[${PYTHON_USEDEP}] )
	surya? ( dev-python/terratorch-surya[${PYTHON_USEDEP}] )
	mmseg? ( dev-python/mmcv[${PYTHON_USEDEP}] )
	mmseg? ( dev-python/ftfy[${PYTHON_USEDEP}] )
	mmseg? ( dev-python/regex[${PYTHON_USEDEP}] )
	mmseg? ( dev-python/openmim[${PYTHON_USEDEP}] )
	wxc? ( dev-python/prithviwxc[${PYTHON_USEDEP}] )
	wxc? ( dev-python/granitewxc[${PYTHON_USEDEP}] )
	visualize? ( dev-python/torchview[${PYTHON_USEDEP}] )
	visualize? ( dev-python/graphviz[${PYTHON_USEDEP}] )
	peft? ( dev-python/peft[${PYTHON_USEDEP}] )
	geobenchv2? ( dev-python/geobenchv2[${PYTHON_USEDEP}] )
	geobenchv2? ( dev-python/mlflow[${PYTHON_USEDEP}] )
	vllm? ( dev-python/geobenchv2[${PYTHON_USEDEP}] )
	vllm? ( dev-python/vllm[${PYTHON_USEDEP}] )
	vllm-test? ( dev-python/pillow[${PYTHON_USEDEP}] )
	vllm-test? ( dev-python/ImageHash[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
