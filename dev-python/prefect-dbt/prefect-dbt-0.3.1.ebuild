# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="prefect-dbt"
REALVERSION="0.3.1"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_9,3_10,3_11} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Prefect integration for working with dbt"

HOMEPAGE="https://github.com/PrefectHQ/prefect-dbt"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/p/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all_extras bigquery dev postgres snowflake"
DEPENDENCIES="dev-python/prefect[${PYTHON_USEDEP}]
	dev-python/dbt-core[${PYTHON_USEDEP}]
	dev-python/prefect-shell[${PYTHON_USEDEP}]
	dev-python/sgqlc[${PYTHON_USEDEP}]
	all_extras? ( dev-python/dbt-bigquery[${PYTHON_USEDEP}] )
	all_extras? ( dev-python/dbt-postgres[${PYTHON_USEDEP}] )
	all_extras? ( dev-python/dbt-snowflake[${PYTHON_USEDEP}] )
	all_extras? ( dev-python/prefect-gcp[${PYTHON_USEDEP}] )
	all_extras? ( dev-python/prefect-snowflake[${PYTHON_USEDEP}] )
	all_extras? ( dev-python/prefect-sqlalchemy[${PYTHON_USEDEP}] )
	bigquery? ( dev-python/prefect-gcp[${PYTHON_USEDEP}] )
	bigquery? ( dev-python/dbt-bigquery[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest[${PYTHON_USEDEP}] )
	dev? ( dev-python/black[${PYTHON_USEDEP}] )
	dev? ( dev-python/flake8[${PYTHON_USEDEP}] )
	dev? ( dev-python/mypy[${PYTHON_USEDEP}] )
	dev? ( dev-python/mkdocs[${PYTHON_USEDEP}] )
	dev? ( dev-python/mkdocs-material[${PYTHON_USEDEP}] )
	dev? ( dev-python/mkdocstrings[${PYTHON_USEDEP}] )
	dev? ( dev-python/isort[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-asyncio[${PYTHON_USEDEP}] )
	dev? ( dev-python/mkdocs-gen-files[${PYTHON_USEDEP}] )
	dev? ( dev-python/interrogate[${PYTHON_USEDEP}] )
	dev? ( dev-python/coverage[${PYTHON_USEDEP}] )
	dev? ( dev-python/respx[${PYTHON_USEDEP}] )
	dev? ( dev-python/pillow[${PYTHON_USEDEP}] )
	dev? ( dev-python/dbt-bigquery[${PYTHON_USEDEP}] )
	dev? ( dev-python/dbt-postgres[${PYTHON_USEDEP}] )
	dev? ( dev-python/dbt-snowflake[${PYTHON_USEDEP}] )
	dev? ( dev-python/prefect-gcp[${PYTHON_USEDEP}] )
	dev? ( dev-python/prefect-snowflake[${PYTHON_USEDEP}] )
	dev? ( dev-python/prefect-sqlalchemy[${PYTHON_USEDEP}] )
	postgres? ( dev-python/prefect-sqlalchemy[${PYTHON_USEDEP}] )
	postgres? ( dev-python/dbt-postgres[${PYTHON_USEDEP}] )
	snowflake? ( dev-python/prefect-snowflake[${PYTHON_USEDEP}] )
	snowflake? ( dev-python/dbt-snowflake[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
