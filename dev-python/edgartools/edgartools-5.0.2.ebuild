# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Navigate Edgar filings with ease"

HOMEPAGE=""
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="ai ai-dev all-cloud azure cloud data gcs s3 test-harness"
DEPENDENCIES=">=dev-python/beautifulsoup4-4.10.0[${PYTHON_USEDEP}]
	~dev-python/hishel-0.1.3[${PYTHON_USEDEP}]
	>=dev-python/httpx-0.25.0[${PYTHON_USEDEP}]
	dev-python/httpxthrottlecache[${PYTHON_USEDEP}]
	>=dev-python/humanize-4.0.0[${PYTHON_USEDEP}]
	>=dev-python/jinja2-3.1.0[${PYTHON_USEDEP}]
	>=dev-python/lxml-4.4[${PYTHON_USEDEP}]
	>=dev-python/nest-asyncio-1.5.1[${PYTHON_USEDEP}]
	>=dev-python/orjson-3.6.0[${PYTHON_USEDEP}]
	>=dev-python/pandas-2.0.0[${PYTHON_USEDEP}]
	>=dev-python/pyarrow-17.0.0[${PYTHON_USEDEP}]
	>=dev-python/pydantic-2.0.0[${PYTHON_USEDEP}]
	dev-python/rank-bm25[${PYTHON_USEDEP}]
	>=dev-python/rapidfuzz-3.5.0[${PYTHON_USEDEP}]
	>=dev-python/rich-13.8.0[${PYTHON_USEDEP}]
	dev-python/stamina[${PYTHON_USEDEP}]
	>=dev-python/tabulate-0.9.0[${PYTHON_USEDEP}]
	>=dev-python/textdistance-4.5.0[${PYTHON_USEDEP}]
	>=dev-python/tqdm-4.62.0[${PYTHON_USEDEP}]
	>=dev-python/unidecode-1.2.0[${PYTHON_USEDEP}]
	ai? ( dev-python/mcp[${PYTHON_USEDEP}] )
	ai? ( dev-python/tiktoken[${PYTHON_USEDEP}] )
	ai-dev? ( >=dev-python/pytest-mock-3.12.0[${PYTHON_USEDEP}] )
	ai-dev? ( >=dev-python/responses-0.24.0[${PYTHON_USEDEP}] )
	all-cloud? ( dev-python/adlfs[${PYTHON_USEDEP}] )
	all-cloud? ( >=dev-python/fsspec-2023.1.0[${PYTHON_USEDEP}] )
	all-cloud? ( dev-python/gcsfs[${PYTHON_USEDEP}] )
	all-cloud? ( dev-python/s3fs[${PYTHON_USEDEP}] )
	azure? ( dev-python/adlfs[${PYTHON_USEDEP}] )
	azure? ( >=dev-python/fsspec-2023.1.0[${PYTHON_USEDEP}] )
	cloud? ( >=dev-python/fsspec-2023.1.0[${PYTHON_USEDEP}] )
	data? ( dev-python/duckdb[${PYTHON_USEDEP}] )
	gcs? ( >=dev-python/fsspec-2023.1.0[${PYTHON_USEDEP}] )
	gcs? ( dev-python/gcsfs[${PYTHON_USEDEP}] )
	s3? ( >=dev-python/fsspec-2023.1.0[${PYTHON_USEDEP}] )
	s3? ( dev-python/s3fs[${PYTHON_USEDEP}] )
	test-harness? ( >=dev-python/click-8.0.0[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
