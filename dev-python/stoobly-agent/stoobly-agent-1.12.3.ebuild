# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Record, mock, and test HTTP(s) requests. CLI agent for Stoobly"

HOMEPAGE="https://github.com/Stoobly/stoobly-agent"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE=""
DEPENDENCIES="<dev-python/click-9.0.0[${PYTHON_USEDEP}]
	<dev-python/docker-8.0[${PYTHON_USEDEP}]
	<dev-python/diff-match-patch-20241022.0[${PYTHON_USEDEP}]
	<dev-python/distro-1.10[${PYTHON_USEDEP}]
	<dev-python/dnspython-2.8[${PYTHON_USEDEP}]
	>=dev-python/httptools-0.4.0[${PYTHON_USEDEP}]
	>=dev-python/jmespath-1.0.0[${PYTHON_USEDEP}]
	<dev-python/mergedeep-1.4[${PYTHON_USEDEP}]
	dev-python/mitmproxy[${PYTHON_USEDEP}]
	<dev-python/openapi-core-0.20.0[${PYTHON_USEDEP}]
	<dev-python/multipart-1.3[${PYTHON_USEDEP}]
	<dev-python/python-dotenv-2.0.0[${PYTHON_USEDEP}]
	>=dev-python/pyyaml-6.0.1[${PYTHON_USEDEP}]
	>=dev-python/requests-2.31.0[${PYTHON_USEDEP}]
	dev-python/stoobly-orator[${PYTHON_USEDEP}]
	>=dev-python/tabulate-0.8.8[${PYTHON_USEDEP}]
	<=dev-python/watchdog-6.1.0[${PYTHON_USEDEP}]
	dev-python/yamale[${PYTHON_USEDEP}]
	<dev-python/filelock-4.0.0[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
