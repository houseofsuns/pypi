# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Python CoAP library"

HOMEPAGE="https://christian.amsuess.com/tools/aiocoap/"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all docs oscore prettyprint tinydtls ws"
DEPENDENCIES="all? ( dev-python/cbor2[${PYTHON_USEDEP}] )
	all? ( >=dev-python/cryptography-2.0[${PYTHON_USEDEP}] )
	all? ( dev-python/filelock[${PYTHON_USEDEP}] )
	all? ( dev-python/ge25519[${PYTHON_USEDEP}] )
	all? ( dev-python/DTLSSocket[${PYTHON_USEDEP}] )
	all? ( dev-python/websockets[${PYTHON_USEDEP}] )
	all? ( dev-python/termcolor[${PYTHON_USEDEP}] )
	all? ( dev-python/pygments[${PYTHON_USEDEP}] )
	all? ( dev-python/cbor-diag[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-argparse[${PYTHON_USEDEP}] )
	docs? ( dev-python/cbor2[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/cryptography-2.0[${PYTHON_USEDEP}] )
	docs? ( dev-python/filelock[${PYTHON_USEDEP}] )
	docs? ( dev-python/ge25519[${PYTHON_USEDEP}] )
	docs? ( dev-python/websockets[${PYTHON_USEDEP}] )
	docs? ( dev-python/termcolor[${PYTHON_USEDEP}] )
	docs? ( dev-python/pygments[${PYTHON_USEDEP}] )
	docs? ( dev-python/cbor-diag[${PYTHON_USEDEP}] )
	oscore? ( dev-python/cbor2[${PYTHON_USEDEP}] )
	oscore? ( >=dev-python/cryptography-2.0[${PYTHON_USEDEP}] )
	oscore? ( dev-python/filelock[${PYTHON_USEDEP}] )
	oscore? ( dev-python/ge25519[${PYTHON_USEDEP}] )
	prettyprint? ( dev-python/termcolor[${PYTHON_USEDEP}] )
	prettyprint? ( dev-python/cbor2[${PYTHON_USEDEP}] )
	prettyprint? ( dev-python/pygments[${PYTHON_USEDEP}] )
	prettyprint? ( dev-python/cbor-diag[${PYTHON_USEDEP}] )
	tinydtls? ( dev-python/DTLSSocket[${PYTHON_USEDEP}] )
	ws? ( dev-python/websockets[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
