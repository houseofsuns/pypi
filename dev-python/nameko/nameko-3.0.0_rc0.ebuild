# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="3.0.0rc0"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A microservices framework for Python that lets service developers concentrate on application logic and encourages testability. [newver 3.0.0rc11]"

HOMEPAGE="http://github.com/nameko/nameko"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev docs examples"
DEPENDENCIES="dev-python/eventlet[${PYTHON_USEDEP}]
	<dev-python/kombu-5.0[${PYTHON_USEDEP}]
	>=dev-python/mock-1.2[${PYTHON_USEDEP}]
	dev-python/path-py[${PYTHON_USEDEP}]
	>=dev-python/pyyaml-3.10[${PYTHON_USEDEP}]
	>=dev-python/requests-1.2.0[${PYTHON_USEDEP}]
	>=dev-python/six-1.9.0[${PYTHON_USEDEP}]
	>=dev-python/werkzeug-0.9[${PYTHON_USEDEP}]
	>=dev-python/wrapt-1.0.0[${PYTHON_USEDEP}]
	dev? ( ~dev-python/coverage-4.5.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/flake8-3.3.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/isort-4.2.15[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/mccabe-0.6.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pycodestyle-2.3.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pyflakes-1.5.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pylint-1.7.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pytest-3.6.3[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pytest-cov-2.5.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pytest-timeout-1.3.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/requests-2.19.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/urllib3-1.23[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/websocket-client-0.48.0[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/pyenchant-1.6.6[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/sphinx-1.3[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/sphinxcontrib-spelling-2.1.1[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-nameko-theme[${PYTHON_USEDEP}] )
	examples? ( dev-python/nameko-sqlalchemy[${PYTHON_USEDEP}] )
	examples? ( ~dev-python/pyjwt-1.5.2[${PYTHON_USEDEP}] )
	examples? ( ~dev-python/moto-1.3.6[${PYTHON_USEDEP}] )
	examples? ( ~dev-python/bcrypt-3.1.3[${PYTHON_USEDEP}] )
	examples? ( ~dev-python/regex-2018.2.21[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
