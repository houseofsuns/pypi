# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Pulp Django Application and Related Modules"

HOMEPAGE="https://pulpproject.org"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="azure diagnostics google kafka prometheus s3 sftp"
DEPENDENCIES="<=dev-python/aiodns-3.3.0[${PYTHON_USEDEP}]
	<dev-python/aiofiles-24.2.0[${PYTHON_USEDEP}]
	<dev-python/aiohttp-3.12[${PYTHON_USEDEP}]
	dev-python/asyncio-throttle[${PYTHON_USEDEP}]
	<dev-python/async-timeout-4.0.4[${PYTHON_USEDEP}]
	<dev-python/backoff-2.2.2[${PYTHON_USEDEP}]
	<=dev-python/click-8.1.8[${PYTHON_USEDEP}]
	<dev-python/cryptography-44.0.4[${PYTHON_USEDEP}]
	~dev-python/django-4.2.0[${PYTHON_USEDEP}]
	<=dev-python/django-filter-25.1[${PYTHON_USEDEP}]
	dev-python/django-guid[${PYTHON_USEDEP}]
	dev-python/django-import-export[${PYTHON_USEDEP}]
	dev-python/django-lifecycle[${PYTHON_USEDEP}]
	<=dev-python/djangorestframework-3.16.0[${PYTHON_USEDEP}]
	dev-python/djangorestframework-queryfields[${PYTHON_USEDEP}]
	dev-python/drf-access-policy[${PYTHON_USEDEP}]
	dev-python/drf-nested-routers[${PYTHON_USEDEP}]
	dev-python/drf-spectacular[${PYTHON_USEDEP}]
	dev-python/dynaconf[${PYTHON_USEDEP}]
	dev-python/gunicorn[${PYTHON_USEDEP}]
	<=dev-python/importlib-metadata-6.0.1[${PYTHON_USEDEP}]
	dev-python/jinja[${PYTHON_USEDEP}]
	dev-python/json-stream[${PYTHON_USEDEP}]
	<dev-python/jq-1.9.0[${PYTHON_USEDEP}]
	<dev-python/pyopenssl-26.0[${PYTHON_USEDEP}]
	<dev-python/opentelemetry-api-1.33[${PYTHON_USEDEP}]
	<dev-python/opentelemetry-sdk-1.33[${PYTHON_USEDEP}]
	dev-python/opentelemetry-exporter-otlp-proto-http[${PYTHON_USEDEP}]
	<dev-python/protobuf-6.0[${PYTHON_USEDEP}]
	dev-python/pulp-glue[${PYTHON_USEDEP}]
	dev-python/pygtrie[${PYTHON_USEDEP}]
	<=dev-python/psycopg-3.2.7[${PYTHON_USEDEP}]
	<=dev-python/pyparsing-3.2.3[${PYTHON_USEDEP}]
	<=dev-python/python-gnupg-0.5.4[${PYTHON_USEDEP}]
	<=dev-python/pyyaml-6.0.2[${PYTHON_USEDEP}]
	<dev-python/redis-5.2.2[${PYTHON_USEDEP}]
	dev-python/tablib[${PYTHON_USEDEP}]
	<=dev-python/url-normalize-2.2.1[${PYTHON_USEDEP}]
	dev-python/uuid6[${PYTHON_USEDEP}]
	dev-python/whitenoise[${PYTHON_USEDEP}]
	<dev-python/yarl-1.20.1[${PYTHON_USEDEP}]
	sftp? ( dev-python/django-storages[${PYTHON_USEDEP}] )
	s3? ( dev-python/django-storages[${PYTHON_USEDEP}] )
	google? ( dev-python/django-storages[${PYTHON_USEDEP}] )
	azure? ( dev-python/django-storages[${PYTHON_USEDEP}] )
	prometheus? ( dev-python/django-prometheus[${PYTHON_USEDEP}] )
	kafka? ( dev-python/cloudevents[${PYTHON_USEDEP}] )
	kafka? ( dev-python/confluent-kafka[${PYTHON_USEDEP}] )
	diagnostics? ( dev-python/pyinstrument[${PYTHON_USEDEP}] )
	diagnostics? ( dev-python/memray[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
