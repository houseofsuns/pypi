# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Implement minimal boilerplate CLIs derived from type hints and parse from command line, config files and environment variables."

HOMEPAGE=""
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all argcomplete coverage dev doc fsspec jsonnet jsonschema maintainer omegaconf reconplogger ruamel ruyaml shtab signatures test test-no-urls toml urls"
DEPENDENCIES=">=dev-python/pyyaml-3.13[${PYTHON_USEDEP}]
	all? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	all? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	all? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	all? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	all? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	all? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	all? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	all? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	all? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	all? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	signatures? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	signatures? ( dev-python/docstring_parser[${PYTHON_USEDEP}] )
	signatures? ( dev-python/typeshed-client[${PYTHON_USEDEP}] )
	jsonschema? ( >=dev-python/jsonschema-3.2.0[${PYTHON_USEDEP}] )
	jsonnet? ( dev-python/jsonnet[${PYTHON_USEDEP}] )
	toml? ( dev-python/toml[${PYTHON_USEDEP}] )
	urls? ( >=dev-python/requests-2.18.4[${PYTHON_USEDEP}] )
	fsspec? ( >=dev-python/fsspec-0.8.4[${PYTHON_USEDEP}] )
	shtab? ( >=dev-python/shtab-1.7.1[${PYTHON_USEDEP}] )
	argcomplete? ( >=dev-python/argcomplete-3.5.1[${PYTHON_USEDEP}] )
	ruamel? ( >=dev-python/ruamel-yaml-0.18.15[${PYTHON_USEDEP}] )
	ruyaml? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	omegaconf? ( dev-python/omegaconf[${PYTHON_USEDEP}] )
	reconplogger? ( dev-python/reconplogger[${PYTHON_USEDEP}] )
	test? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	test? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	test? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	test? ( dev-python/types-PyYAML[${PYTHON_USEDEP}] )
	test? ( dev-python/types-requests[${PYTHON_USEDEP}] )
	test? ( >=dev-python/responses-0.12.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pydantic-2.3.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/attrs-22.2.0[${PYTHON_USEDEP}] )
	test-no-urls? ( >=dev-python/pytest-6.2.5[${PYTHON_USEDEP}] )
	test-no-urls? ( >=dev-python/pytest-subtests-0.8.0[${PYTHON_USEDEP}] )
	coverage? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	coverage? ( >=dev-python/pytest-cov-4.0.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	dev? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	dev? ( dev-python/jsonargparse[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/tox-3.25.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/build-0.10.0[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/sphinx-1.7.9[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/sphinx-rtd-theme-1.2.2[${PYTHON_USEDEP}] )
	doc? ( dev-python/autodocsumm[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/sphinx-autodoc-typehints-1.19.5[${PYTHON_USEDEP}] )
	maintainer? ( dev-python/bump2version[${PYTHON_USEDEP}] )
	maintainer? ( >=dev-python/twine-4.0.2[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
