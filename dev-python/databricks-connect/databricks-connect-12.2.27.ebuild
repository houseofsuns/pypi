# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Databricks Connect Client [top-max 14.3.2]"

HOMEPAGE=""
LICENSE="Databricks Proprietary License"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="connect ml mllib pandas-on-spark sql"
DEPENDENCIES="dev-python/py4j[${PYTHON_USEDEP}]
	dev-python/six[${PYTHON_USEDEP}]
	ml? ( >=dev-python/numpy-1.15[${PYTHON_USEDEP}] )
	mllib? ( >=dev-python/numpy-1.15[${PYTHON_USEDEP}] )
	sql? ( >=dev-python/pandas-1.0.5[${PYTHON_USEDEP}] )
	sql? ( >=dev-python/pyarrow-1.0.0[${PYTHON_USEDEP}] )
	sql? ( >=dev-python/numpy-1.15[${PYTHON_USEDEP}] )
	pandas-on-spark? ( >=dev-python/pandas-1.0.5[${PYTHON_USEDEP}] )
	pandas-on-spark? ( >=dev-python/pyarrow-1.0.0[${PYTHON_USEDEP}] )
	pandas-on-spark? ( >=dev-python/numpy-1.15[${PYTHON_USEDEP}] )
	connect? ( >=dev-python/pandas-1.0.5[${PYTHON_USEDEP}] )
	connect? ( >=dev-python/pyarrow-1.0.0[${PYTHON_USEDEP}] )
	connect? ( >=dev-python/grpcio-1.48.1[${PYTHON_USEDEP}] )
	connect? ( dev-python/grpcio-status[${PYTHON_USEDEP}] )
	connect? ( >=dev-python/googleapis-common-protos-1.56.4[${PYTHON_USEDEP}] )
	connect? ( >=dev-python/numpy-1.15[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
