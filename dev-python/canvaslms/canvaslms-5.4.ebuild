# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Command-line interface to Canvas LMS"

HOMEPAGE="https://github.com/dbosk/canvaslms"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="diff llm"
DEPENDENCIES=">=dev-python/appdirs-1.4.4[${PYTHON_USEDEP}]
	<dev-python/argcomplete-4.0[${PYTHON_USEDEP}]
	>=dev-python/cachetools-5.3.1[${PYTHON_USEDEP}]
	dev-python/canvasapi[${PYTHON_USEDEP}]
	<dev-python/keyring-26.0[${PYTHON_USEDEP}]
	dev-python/pypandoc[${PYTHON_USEDEP}]
	>=dev-python/arrow-1.2.3[${PYTHON_USEDEP}]
	<dev-python/rich-15.0[${PYTHON_USEDEP}]
	>=dev-python/pyyaml-5.1[${PYTHON_USEDEP}]
	llm? ( dev-python/llm[${PYTHON_USEDEP}] )
	llm? ( dev-python/llm-openai-plugin[${PYTHON_USEDEP}] )
	llm? ( dev-python/llm-anthropic[${PYTHON_USEDEP}] )
	llm? ( dev-python/llm-gemini[${PYTHON_USEDEP}] )
	llm? ( dev-python/llm-python[${PYTHON_USEDEP}] )
	llm? ( dev-python/llm-azure[${PYTHON_USEDEP}] )
	diff? ( <dev-python/patiencediff-0.3.0[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
