# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=wheel

inherit python-r1 gs-pypi

DESCRIPTION="A package for light scattering computation. [wheel]"

HOMEPAGE="https://github.com/MartinPdeS/PyMieSim"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/cp310/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}-cp310-cp310-macosx_10_13_universal2.whl"
SOURCEFILE="${REALNAME}-${REALVERSION}-cp310-cp310-macosx_10_13_universal2.whl"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="documentation testing"
DEPENDENCIES=">=dev-python/numpy-1.26.0[${PYTHON_USEDEP}]
	~dev-python/pydantic-2.6.3[${PYTHON_USEDEP}]
	~dev-python/scipy-1.11.3[${PYTHON_USEDEP}]
	dev-python/DataVisual[${PYTHON_USEDEP}]
	dev-python/PyOptik[${PYTHON_USEDEP}]
	~dev-python/tabulate-0.9.0[${PYTHON_USEDEP}]
	documentation? ( ~dev-python/numpydoc-1.6.0[${PYTHON_USEDEP}] )
	documentation? ( >=dev-python/sphinx-5.1.1[${PYTHON_USEDEP}] )
	documentation? ( ~dev-python/sphinx-gallery-0.15.0[${PYTHON_USEDEP}] )
	documentation? ( ~dev-python/sphinx-rtd-theme-2.0.0[${PYTHON_USEDEP}] )
	documentation? ( ~dev-python/pydata-sphinx-theme-0.14.1[${PYTHON_USEDEP}] )
	documentation? ( dev-python/PyMieScatt[${PYTHON_USEDEP}] )
	testing? ( >=dev-python/pytest-0.6[${PYTHON_USEDEP}] )
	testing? ( >=dev-python/pytest-cov-2.0[${PYTHON_USEDEP}] )
	testing? ( dev-python/pytest-json-report[${PYTHON_USEDEP}] )
	testing? ( >=dev-python/mypy-0.910[${PYTHON_USEDEP}] )
	testing? ( >=dev-python/flake8-3.9[${PYTHON_USEDEP}] )
	testing? ( >=dev-python/tox-3.24[${PYTHON_USEDEP}] )
	testing? ( ~dev-python/coverage-6.5.0[${PYTHON_USEDEP}] )
	testing? ( dev-python/PyMieScatt[${PYTHON_USEDEP}] )
	virtual/allow-pypi-wheels"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
