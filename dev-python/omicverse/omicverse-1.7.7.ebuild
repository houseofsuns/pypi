# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="OmicVerse: A single pipeline for exploring the entire transcriptome universe"

HOMEPAGE=""
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="full"
DEPENDENCIES="dev-python/tomli[${PYTHON_USEDEP}]
	>=dev-python/numpy-1.23[${PYTHON_USEDEP}]
	dev-python/scanpy[${PYTHON_USEDEP}]
	>=dev-python/pandas-1.5[${PYTHON_USEDEP}]
	>dev-python/matplotlib-3.6[${PYTHON_USEDEP}]
	>=dev-python/scikit-learn-1.2[${PYTHON_USEDEP}]
	<dev-python/scipy-1.12[${PYTHON_USEDEP}]
	>=dev-python/networkx-2.8[${PYTHON_USEDEP}]
	>=dev-python/multiprocess-0.70[${PYTHON_USEDEP}]
	>=dev-python/seaborn-0.11[${PYTHON_USEDEP}]
	dev-python/DateTime[${PYTHON_USEDEP}]
	>=dev-python/statsmodels-0.13[${PYTHON_USEDEP}]
	>=dev-python/ipywidgets-8.0[${PYTHON_USEDEP}]
	dev-python/lifelines[${PYTHON_USEDEP}]
	dev-python/ktplotspy[${PYTHON_USEDEP}]
	dev-python/python-dotplot[${PYTHON_USEDEP}]
	>=dev-python/boltons-23.0[${PYTHON_USEDEP}]
	dev-python/ctxcore[${PYTHON_USEDEP}]
	>=dev-python/termcolor-2.1[${PYTHON_USEDEP}]
	dev-python/pygam[${PYTHON_USEDEP}]
	>=dev-python/pillow-9.0[${PYTHON_USEDEP}]
	dev-python/gdown[${PYTHON_USEDEP}]
	dev-python/igraph[${PYTHON_USEDEP}]
	dev-python/leidenalg[${PYTHON_USEDEP}]
	dev-python/graphtools[${PYTHON_USEDEP}]
	>=dev-python/tqdm-4.64[${PYTHON_USEDEP}]
	dev-python/pydeseq2[${PYTHON_USEDEP}]
	dev-python/mofax[${PYTHON_USEDEP}]
	dev-python/adjustText[${PYTHON_USEDEP}]
	dev-python/scikit-misc[${PYTHON_USEDEP}]
	dev-python/metatime[${PYTHON_USEDEP}]
	dev-python/einops[${PYTHON_USEDEP}]
	dev-python/tensorboard[${PYTHON_USEDEP}]
	dev-python/pynvml[${PYTHON_USEDEP}]
	dev-python/plotly[${PYTHON_USEDEP}]
	dev-python/numba[${PYTHON_USEDEP}]
	dev-python/tqdm[${PYTHON_USEDEP}]
	>=dev-python/requests-2.0[${PYTHON_USEDEP}]
	dev-python/transformers[${PYTHON_USEDEP}]
	dev-python/marsilea[${PYTHON_USEDEP}]
	dev-python/wandb[${PYTHON_USEDEP}]
	full? ( dev-python/dynamo-release[${PYTHON_USEDEP}] )
	full? ( dev-python/squidpy[${PYTHON_USEDEP}] )
	full? ( dev-python/tangram-sc[${PYTHON_USEDEP}] )
	full? ( dev-python/pertpy[${PYTHON_USEDEP}] )
	full? ( dev-python/toytree[${PYTHON_USEDEP}] )
	full? ( dev-python/arviz[${PYTHON_USEDEP}] )
	full? ( dev-python/ete3[${PYTHON_USEDEP}] )
	full? ( dev-python/scvi-tools[${PYTHON_USEDEP}] )
	full? ( dev-python/pymde[${PYTHON_USEDEP}] )
	full? ( dev-python/torchdr[${PYTHON_USEDEP}] )
	full? ( dev-python/memento-de[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
