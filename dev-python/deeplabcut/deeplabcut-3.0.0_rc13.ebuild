# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="3.0.0rc13"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Markerless pose-estimation of user-defined features with deep learning"

HOMEPAGE="https://github.com/DeepLabCut/DeepLabCut"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="apple-mchips docs gui modelzoo openvino tf wandb"
DEPENDENCIES="dev-python/dlclibrary[${PYTHON_USEDEP}]
	dev-python/albumentations[${PYTHON_USEDEP}]
	dev-python/einops[${PYTHON_USEDEP}]
	dev-python/filterpy[${PYTHON_USEDEP}]
	>=dev-python/ruamel-yaml-0.15.0[${PYTHON_USEDEP}]
	dev-python/imgaug[${PYTHON_USEDEP}]
	dev-python/imageio-ffmpeg[${PYTHON_USEDEP}]
	dev-python/numba[${PYTHON_USEDEP}]
	<dev-python/matplotlib-3.9[${PYTHON_USEDEP}]
	>=dev-python/networkx-2.6[${PYTHON_USEDEP}]
	<dev-python/numpy-2.0.0[${PYTHON_USEDEP}]
	>=dev-python/pandas-1.0.1[${PYTHON_USEDEP}]
	>=dev-python/scikit-image-0.17[${PYTHON_USEDEP}]
	>=dev-python/scikit-learn-1.0[${PYTHON_USEDEP}]
	>=dev-python/scipy-1.9[${PYTHON_USEDEP}]
	>=dev-python/statsmodels-0.11[${PYTHON_USEDEP}]
	dev-python/timm[${PYTHON_USEDEP}]
	dev-python/torch[${PYTHON_USEDEP}]
	dev-python/torchvision[${PYTHON_USEDEP}]
	dev-python/tqdm[${PYTHON_USEDEP}]
	dev-python/pycocotools[${PYTHON_USEDEP}]
	dev-python/pyyaml[${PYTHON_USEDEP}]
	>=dev-python/pillow-7.1[${PYTHON_USEDEP}]
	dev-python/tables[${PYTHON_USEDEP}]
	apple-mchips? ( dev-python/tensorflow-macos[${PYTHON_USEDEP}] )
	apple-mchips? ( dev-python/tf-keras[${PYTHON_USEDEP}] )
	apple-mchips? ( dev-python/tensorflow-metal[${PYTHON_USEDEP}] )
	apple-mchips? ( dev-python/tensorpack[${PYTHON_USEDEP}] )
	apple-mchips? ( dev-python/tf-slim[${PYTHON_USEDEP}] )
	docs? ( dev-python/numpydoc[${PYTHON_USEDEP}] )
	gui? ( dev-python/pyside6[${PYTHON_USEDEP}] )
	gui? ( ~dev-python/qdarkstyle-3.1[${PYTHON_USEDEP}] )
	gui? ( dev-python/napari-deeplabcut[${PYTHON_USEDEP}] )
	modelzoo? ( dev-python/huggingface-hub[${PYTHON_USEDEP}] )
	openvino? ( dev-python/openvino-dev[${PYTHON_USEDEP}] )
	tf? ( dev-python/tensorpack[${PYTHON_USEDEP}] )
	tf? ( dev-python/tf-slim[${PYTHON_USEDEP}] )
	tf? ( dev-python/tensorflow[${PYTHON_USEDEP}] )
	tf? ( dev-python/tf-keras[${PYTHON_USEDEP}] )
	wandb? ( dev-python/wandb[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
