# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="OpenInference LangChain Instrumentation"

HOMEPAGE="https://github.com/Arize-ai/openinference/tree/main/python/instrumentation/openinference-instrumentation-langchain"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="instruments test type-check"
DEPENDENCIES="dev-python/openinference-instrumentation[${PYTHON_USEDEP}]
	dev-python/openinference-semantic-conventions[${PYTHON_USEDEP}]
	dev-python/opentelemetry-api[${PYTHON_USEDEP}]
	dev-python/opentelemetry-instrumentation[${PYTHON_USEDEP}]
	dev-python/opentelemetry-semantic-conventions[${PYTHON_USEDEP}]
	dev-python/wrapt[${PYTHON_USEDEP}]
	instruments? ( dev-python/langchain-core[${PYTHON_USEDEP}] )
	test? ( dev-python/langchain-community[${PYTHON_USEDEP}] )
	test? ( dev-python/langchain-core[${PYTHON_USEDEP}] )
	test? ( dev-python/langchain-google-vertexai[${PYTHON_USEDEP}] )
	test? ( dev-python/langchain-openai[${PYTHON_USEDEP}] )
	test? ( dev-python/langchain[${PYTHON_USEDEP}] )
	test? ( dev-python/langsmith[${PYTHON_USEDEP}] )
	test? ( dev-python/opentelemetry-sdk[${PYTHON_USEDEP}] )
	test? ( dev-python/portpicker[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-recording[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-rerunfailures[${PYTHON_USEDEP}] )
	test? ( dev-python/respx[${PYTHON_USEDEP}] )
	test? ( dev-python/starlette[${PYTHON_USEDEP}] )
	test? ( dev-python/uvicorn[${PYTHON_USEDEP}] )
	test? ( >=dev-python/vcrpy-6.0.1[${PYTHON_USEDEP}] )
	type-check? ( dev-python/langchain-core[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
