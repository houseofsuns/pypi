# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Django CMS Quick & Easy provides all important modules to run new page withouta lot of coding. Aims to do it very easily and securely."

HOMEPAGE="https://websites.pages.nic.cz/django-cms-qe"
LICENSE="BSD"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="build cms4 dev mysql newsblog psql test"
DEPENDENCIES="dev-python/django-cms[${PYTHON_USEDEP}]
	dev-python/django-filer[${PYTHON_USEDEP}]
	dev-python/djangocms-admin-style[${PYTHON_USEDEP}]
	dev-python/djangocms-alias[${PYTHON_USEDEP}]
	dev-python/djangocms-frontend[${PYTHON_USEDEP}]
	dev-python/djangocms-text[${PYTHON_USEDEP}]
	dev-python/djangocms-versioning[${PYTHON_USEDEP}]
	~dev-python/easy-thumbnails-2.10[${PYTHON_USEDEP}]
	~dev-python/argon2-cffi-25.1[${PYTHON_USEDEP}]
	dev-python/django-axes[${PYTHON_USEDEP}]
	dev-python/django-constance[${PYTHON_USEDEP}]
	dev-python/django-csp[${PYTHON_USEDEP}]
	dev-python/django-import-export[${PYTHON_USEDEP}]
	dev-python/django-tablib[${PYTHON_USEDEP}]
	dev-python/djangocms-file[${PYTHON_USEDEP}]
	dev-python/djangocms-deleted-pages[${PYTHON_USEDEP}]
	dev-python/djangocms-googlemap[${PYTHON_USEDEP}]
	dev-python/djangocms-icon[${PYTHON_USEDEP}]
	dev-python/djangocms-link[${PYTHON_USEDEP}]
	dev-python/djangocms-picture[${PYTHON_USEDEP}]
	dev-python/python-environ[${PYTHON_USEDEP}]
	dev-python/django-haystack[${PYTHON_USEDEP}]
	dev-python/djangocms-aldryn-forms[${PYTHON_USEDEP}]
	dev-python/djangocms-aldryn-search[${PYTHON_USEDEP}]
	dev-python/mailchimp3[${PYTHON_USEDEP}]
	~dev-python/whoosh-2.7[${PYTHON_USEDEP}]
	~dev-python/markdown-3.9[${PYTHON_USEDEP}]
	~dev-python/django-filter-25.2[${PYTHON_USEDEP}]
	dev-python/django-rest-knox[${PYTHON_USEDEP}]
	~dev-python/djangorestframework-3.16[${PYTHON_USEDEP}]
	dev-python/drf-spectacular[${PYTHON_USEDEP}]
	dev? ( dev-python/django-simple-captcha[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/django-debug-toolbar-6.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/django-extensions[${PYTHON_USEDEP}] )
	test? ( dev-python/flake8[${PYTHON_USEDEP}] )
	test? ( dev-python/isort[${PYTHON_USEDEP}] )
	test? ( dev-python/mypy[${PYTHON_USEDEP}] )
	test? ( dev-python/pylint[${PYTHON_USEDEP}] )
	test? ( dev-python/pylint-django[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-8.4[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-cov-7.0[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-data[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-django-4.11[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-env-1.2[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-pythonpath[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-sugar-1.1[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-watch[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pyvirtualdisplay-3.0[${PYTHON_USEDEP}] )
	test? ( dev-python/webdriverwrapper[${PYTHON_USEDEP}] )
	test? ( dev-python/django-simple-captcha[${PYTHON_USEDEP}] )
	test? ( dev-python/testfixtures[${PYTHON_USEDEP}] )
	test? ( dev-python/tzdata[${PYTHON_USEDEP}] )
	build? ( <dev-python/jinja2-3.1.0[${PYTHON_USEDEP}] )
	build? ( ~dev-python/sphinx-1.8.5[${PYTHON_USEDEP}] )
	psql? ( dev-python/psycopg2[${PYTHON_USEDEP}] )
	mysql? ( ~dev-python/mysqlclient-2.2[${PYTHON_USEDEP}] )
	newsblog? ( dev-python/djangocms-aldryn-newsblog[${PYTHON_USEDEP}] )
	cms4? ( dev-python/django-cms[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
