# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Import, export, process, analyze and view triangular meshes."

HOMEPAGE="https://github.com/mikedh/trimesh"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all easy recommends test"
DEPENDENCIES="dev-python/numpy[${PYTHON_USEDEP}]
	all? ( dev-python/python-fcl[${PYTHON_USEDEP}] )
	all? ( dev-python/chardet[${PYTHON_USEDEP}] )
	all? ( dev-python/scikit-image[${PYTHON_USEDEP}] )
	all? ( dev-python/jsonschema[${PYTHON_USEDEP}] )
	all? ( dev-python/pycollada[${PYTHON_USEDEP}] )
	all? ( dev-python/setuptools[${PYTHON_USEDEP}] )
	all? ( dev-python/svg-path[${PYTHON_USEDEP}] )
	all? ( dev-python/networkx[${PYTHON_USEDEP}] )
	all? ( dev-python/shapely[${PYTHON_USEDEP}] )
	all? ( dev-python/mapbox-earcut[${PYTHON_USEDEP}] )
	all? ( dev-python/pillow[${PYTHON_USEDEP}] )
	all? ( dev-python/embreex[${PYTHON_USEDEP}] )
	all? ( dev-python/scipy[${PYTHON_USEDEP}] )
	all? ( dev-python/requests[${PYTHON_USEDEP}] )
	all? ( dev-python/xxhash[${PYTHON_USEDEP}] )
	all? ( dev-python/pyglet[${PYTHON_USEDEP}] )
	all? ( dev-python/lxml[${PYTHON_USEDEP}] )
	all? ( dev-python/psutil[${PYTHON_USEDEP}] )
	all? ( dev-python/xatlas[${PYTHON_USEDEP}] )
	all? ( dev-python/rtree[${PYTHON_USEDEP}] )
	all? ( dev-python/colorlog[${PYTHON_USEDEP}] )
	easy? ( dev-python/mapbox-earcut[${PYTHON_USEDEP}] )
	easy? ( dev-python/chardet[${PYTHON_USEDEP}] )
	easy? ( dev-python/pillow[${PYTHON_USEDEP}] )
	easy? ( dev-python/embreex[${PYTHON_USEDEP}] )
	easy? ( dev-python/scipy[${PYTHON_USEDEP}] )
	easy? ( dev-python/requests[${PYTHON_USEDEP}] )
	easy? ( dev-python/xxhash[${PYTHON_USEDEP}] )
	easy? ( dev-python/setuptools[${PYTHON_USEDEP}] )
	easy? ( dev-python/svg-path[${PYTHON_USEDEP}] )
	easy? ( dev-python/jsonschema[${PYTHON_USEDEP}] )
	easy? ( dev-python/pycollada[${PYTHON_USEDEP}] )
	easy? ( dev-python/networkx[${PYTHON_USEDEP}] )
	easy? ( dev-python/lxml[${PYTHON_USEDEP}] )
	easy? ( dev-python/rtree[${PYTHON_USEDEP}] )
	easy? ( dev-python/shapely[${PYTHON_USEDEP}] )
	easy? ( dev-python/colorlog[${PYTHON_USEDEP}] )
	recommends? ( dev-python/sympy[${PYTHON_USEDEP}] )
	recommends? ( dev-python/glooey[${PYTHON_USEDEP}] )
	recommends? ( dev-python/meshio[${PYTHON_USEDEP}] )
	test? ( dev-python/pyinstrument[${PYTHON_USEDEP}] )
	test? ( dev-python/ezdxf[${PYTHON_USEDEP}] )
	test? ( dev-python/pymeshlab[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest[${PYTHON_USEDEP}] )
	test? ( dev-python/ruff[${PYTHON_USEDEP}] )
	test? ( dev-python/coveralls[${PYTHON_USEDEP}] )
	test? ( dev-python/autopep8[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
