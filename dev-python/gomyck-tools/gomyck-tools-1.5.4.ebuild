# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=wheel

inherit python-r1 gs-pypi

DESCRIPTION="A tools collection for python development by hao474798383 [wheel]"

HOMEPAGE=""
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/py3/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}-py3-none-any.whl"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}-py3-none-any.whl"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="cut kwc ml"
DEPENDENCIES="~dev-python/jsonpickle-4.1.1[${PYTHON_USEDEP}]
	~dev-python/sqlalchemy-2.0.44[${PYTHON_USEDEP}]
	~dev-python/chardet-5.2.0[${PYTHON_USEDEP}]
	dev-python/psycopg2-binary[${PYTHON_USEDEP}]
	~dev-python/croniter-5.0.1[${PYTHON_USEDEP}]
	dev-python/gmssl[${PYTHON_USEDEP}]
	~dev-python/psutil-6.1.0[${PYTHON_USEDEP}]
	~dev-python/jsonpath-ng-1.7.0[${PYTHON_USEDEP}]
	~dev-python/bottle-0.13.4[${PYTHON_USEDEP}]
	~dev-python/requests-2.32.3[${PYTHON_USEDEP}]
	~dev-python/urllib3-1.26.20[${PYTHON_USEDEP}]
	dev-python/kafka-python[${PYTHON_USEDEP}]
	dev-python/bs4[${PYTHON_USEDEP}]
	~dev-python/paho-mqtt-2.1.0[${PYTHON_USEDEP}]
	~dev-python/fuzzywuzzy-0.18.0[${PYTHON_USEDEP}]
	~dev-python/pymysql-1.1.1[${PYTHON_USEDEP}]
	dev-python/pyzipper[${PYTHON_USEDEP}]
	~dev-python/prometheus-client-0.23.1[${PYTHON_USEDEP}]
	~dev-python/paramiko-3.5.0[${PYTHON_USEDEP}]
	~dev-python/pyjwt-2.10.1[${PYTHON_USEDEP}]
	~dev-python/cryptography-46.0.3[${PYTHON_USEDEP}]
	~dev-python/redis-7.1.0[${PYTHON_USEDEP}]
	>=dev-python/uvloop-0.22.1[${PYTHON_USEDEP}]
	>=dev-python/pyyaml-6.0.3[${PYTHON_USEDEP}]
	dev-python/uiautomation[${PYTHON_USEDEP}]
	kwc? ( >=dev-python/imageio-2.37.0[${PYTHON_USEDEP}] )
	kwc? ( dev-python/wordcloud[${PYTHON_USEDEP}] )
	kwc? ( dev-python/jieba[${PYTHON_USEDEP}] )
	kwc? ( dev-python/paddlepaddle[${PYTHON_USEDEP}] )
	cut? ( dev-python/jieba[${PYTHON_USEDEP}] )
	cut? ( dev-python/paddlepaddle[${PYTHON_USEDEP}] )
	ml? ( >=dev-python/scikit-learn-1.7.1[${PYTHON_USEDEP}] )
	ml? ( >=dev-python/pandas-2.3.2[${PYTHON_USEDEP}] )
	ml? ( >=dev-python/lxml-6.0.1[${PYTHON_USEDEP}] )
	ml? ( >=dev-python/xlrd-2.0.2[${PYTHON_USEDEP}] )
	virtual/allow-pypi-wheels"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
