# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A complete web automation framework for end-to-end testing."

HOMEPAGE="https://github.com/seleniumbase/SeleniumBase"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="allure coverage flake8 ipdb mss pdfminer pillow playwright proxy psutil pyautogui selenium-stealth selenium-wire"
DEPENDENCIES="~dev-python/python-xlib-0.33[${PYTHON_USEDEP}]
	dev-python/PyAutoGUI[${PYTHON_USEDEP}]
	~dev-python/markdown-it-py-4.0.0[${PYTHON_USEDEP}]
	~dev-python/mdurl-0.1.2[${PYTHON_USEDEP}]
	<dev-python/rich-15.0[${PYTHON_USEDEP}]
	allure? ( dev-python/allure-pytest[${PYTHON_USEDEP}] )
	allure? ( dev-python/allure-python-commons[${PYTHON_USEDEP}] )
	allure? ( dev-python/allure-behave[${PYTHON_USEDEP}] )
	coverage? ( >=dev-python/coverage-7.13.1[${PYTHON_USEDEP}] )
	coverage? ( >=dev-python/pytest-cov-7.0.0[${PYTHON_USEDEP}] )
	flake8? ( ~dev-python/flake8-7.3.0[${PYTHON_USEDEP}] )
	flake8? ( ~dev-python/mccabe-0.7.0[${PYTHON_USEDEP}] )
	flake8? ( ~dev-python/pyflakes-3.4.0[${PYTHON_USEDEP}] )
	flake8? ( ~dev-python/pycodestyle-2.14.0[${PYTHON_USEDEP}] )
	ipdb? ( ~dev-python/ipdb-0.13.13[${PYTHON_USEDEP}] )
	ipdb? ( ~dev-python/ipython-7.34.0[${PYTHON_USEDEP}] )
	mss? ( ~dev-python/mss-10.1.0[${PYTHON_USEDEP}] )
	pdfminer? ( dev-python/pdfminer-six[${PYTHON_USEDEP}] )
	pdfminer? ( ~dev-python/cryptography-46.0.3[${PYTHON_USEDEP}] )
	pdfminer? ( ~dev-python/cffi-2.0.0[${PYTHON_USEDEP}] )
	>=dev-python/pip-25.3[${PYTHON_USEDEP}]
	>=dev-python/packaging-25.0[${PYTHON_USEDEP}]
	>=dev-python/setuptools-80.9.0[${PYTHON_USEDEP}]
	>=dev-python/wheel-0.45.1[${PYTHON_USEDEP}]
	>=dev-python/attrs-25.4.0[${PYTHON_USEDEP}]
	>=dev-python/certifi-2026.1.4[${PYTHON_USEDEP}]
	>=dev-python/exceptiongroup-1.3.1[${PYTHON_USEDEP}]
	>=dev-python/websockets-16.0[${PYTHON_USEDEP}]
	>=dev-python/filelock-3.20.3[${PYTHON_USEDEP}]
	>=dev-python/fasteners-0.20[${PYTHON_USEDEP}]
	dev-python/mycdp[${PYTHON_USEDEP}]
	dev-python/pynose[${PYTHON_USEDEP}]
	>=dev-python/platformdirs-4.5.1[${PYTHON_USEDEP}]
	>=dev-python/typing-extensions-4.15.0[${PYTHON_USEDEP}]
	dev-python/sbvirtualdisplay[${PYTHON_USEDEP}]
	>=dev-python/markupsafe-3.0.3[${PYTHON_USEDEP}]
	>=dev-python/jinja2-3.1.6[${PYTHON_USEDEP}]
	>=dev-python/six-1.17.0[${PYTHON_USEDEP}]
	>=dev-python/parse-1.20.2[${PYTHON_USEDEP}]
	>=dev-python/parse-type-0.6.6[${PYTHON_USEDEP}]
	>=dev-python/colorama-0.4.6[${PYTHON_USEDEP}]
	>=dev-python/pyyaml-6.0.3[${PYTHON_USEDEP}]
	>=dev-python/pygments-2.19.2[${PYTHON_USEDEP}]
	dev-python/tabcompleter[${PYTHON_USEDEP}]
	dev-python/pdbp[${PYTHON_USEDEP}]
	>=dev-python/idna-3.11[${PYTHON_USEDEP}]
	~dev-python/chardet-5.2.0[${PYTHON_USEDEP}]
	<dev-python/charset-normalizer-4.0[${PYTHON_USEDEP}]
	<dev-python/urllib3-3.0[${PYTHON_USEDEP}]
	~dev-python/requests-2.32.5[${PYTHON_USEDEP}]
	~dev-python/sniffio-1.3.1[${PYTHON_USEDEP}]
	~dev-python/h11-0.16.0[${PYTHON_USEDEP}]
	~dev-python/outcome-1.3.0_p0[${PYTHON_USEDEP}]
	<dev-python/trio-1.0[${PYTHON_USEDEP}]
	~dev-python/trio-websocket-0.12.2[${PYTHON_USEDEP}]
	~dev-python/wsproto-1.3.2[${PYTHON_USEDEP}]
	~dev-python/websocket-client-1.9.0[${PYTHON_USEDEP}]
	~dev-python/selenium-4.39.0[${PYTHON_USEDEP}]
	~dev-python/cssselect-1.3.0[${PYTHON_USEDEP}]
	~dev-python/nest-asyncio-1.6.0[${PYTHON_USEDEP}]
	~dev-python/sortedcontainers-2.4.0[${PYTHON_USEDEP}]
	~dev-python/execnet-2.1.2[${PYTHON_USEDEP}]
	~dev-python/iniconfig-2.3.0[${PYTHON_USEDEP}]
	~dev-python/pluggy-1.6.0[${PYTHON_USEDEP}]
	~dev-python/pytest-9.0.2[${PYTHON_USEDEP}]
	dev-python/pytest-html[${PYTHON_USEDEP}]
	~dev-python/pytest-metadata-3.1.1[${PYTHON_USEDEP}]
	~dev-python/pytest-ordering-0.6[${PYTHON_USEDEP}]
	~dev-python/pytest-rerunfailures-16.1[${PYTHON_USEDEP}]
	~dev-python/pytest-xdist-3.8.0[${PYTHON_USEDEP}]
	~dev-python/parameterized-0.9.0[${PYTHON_USEDEP}]
	dev-python/behave[${PYTHON_USEDEP}]
	~dev-python/soupsieve-2.8.1[${PYTHON_USEDEP}]
	~dev-python/beautifulsoup4-4.14.3[${PYTHON_USEDEP}]
	~dev-python/pyotp-2.9.0[${PYTHON_USEDEP}]
	pdfminer? ( ~dev-python/pycparser-2.23[${PYTHON_USEDEP}] )
	pillow? ( >=dev-python/pillow-12.1.0[${PYTHON_USEDEP}] )
	proxy? ( dev-python/proxy-py[${PYTHON_USEDEP}] )
	playwright? ( dev-python/playwright[${PYTHON_USEDEP}] )
	psutil? ( >=dev-python/psutil-7.1.3[${PYTHON_USEDEP}] )
	pyautogui? ( dev-python/PyAutoGUI[${PYTHON_USEDEP}] )
	selenium-stealth? ( dev-python/selenium-stealth[${PYTHON_USEDEP}] )
	selenium-wire? ( dev-python/selenium-wire[${PYTHON_USEDEP}] )
	selenium-wire? ( >=dev-python/pyopenssl-24.2.1[${PYTHON_USEDEP}] )
	selenium-wire? ( >=dev-python/pyparsing-3.1.4[${PYTHON_USEDEP}] )
	selenium-wire? ( dev-python/Brotli[${PYTHON_USEDEP}] )
	selenium-wire? ( ~dev-python/blinker-1.7.0[${PYTHON_USEDEP}] )
	selenium-wire? ( ~dev-python/h2-4.1.0[${PYTHON_USEDEP}] )
	selenium-wire? ( ~dev-python/hpack-4.0.0[${PYTHON_USEDEP}] )
	selenium-wire? ( ~dev-python/hyperframe-6.0.1[${PYTHON_USEDEP}] )
	selenium-wire? ( ~dev-python/kaitaistruct-0.10[${PYTHON_USEDEP}] )
	selenium-wire? ( ~dev-python/pyasn1-0.6.1[${PYTHON_USEDEP}] )
	selenium-wire? ( >=dev-python/zstandard-0.23.0[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
