# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Prowler is an Open Source security tool to perform AWS, GCP and Azure security best practices assessments, audits, incident response, continuous monitoring, hardening and forensics readiness. It contains hundreds of controls covering CIS, NIST 800, NIST CSF, CISA, RBI, FedRAMP, PCI-DSS, GDPR, HIPAA, FFIEC, SOC2, GXP, AWS Well-Architected Framework Security Pillar, AWS Foundational Technical Review (FTR), ENS (Spanish National Security Scheme) and your custom security frameworks."

HOMEPAGE="https://github.com/prowler-cloud/prowler"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE=""
DEPENDENCIES="dev-python/alibabacloud-gateway-oss-util[${PYTHON_USEDEP}]
	dev-python/alibabacloud-rds20140815[${PYTHON_USEDEP}]
	dev-python/alibabacloud-sls20201230[${PYTHON_USEDEP}]
	dev-python/alibabacloud-actiontrail20200706[${PYTHON_USEDEP}]
	dev-python/alibabacloud-credentials[${PYTHON_USEDEP}]
	dev-python/alibabacloud-cs20151215[${PYTHON_USEDEP}]
	dev-python/alibabacloud-ecs20140526[${PYTHON_USEDEP}]
	dev-python/alibabacloud-oss20190517[${PYTHON_USEDEP}]
	dev-python/alibabacloud-ram20150501[${PYTHON_USEDEP}]
	dev-python/alibabacloud-sas20181203[${PYTHON_USEDEP}]
	dev-python/alibabacloud-sts20150401[${PYTHON_USEDEP}]
	dev-python/alibabacloud-tea-openapi[${PYTHON_USEDEP}]
	dev-python/alibabacloud-vpc20160428[${PYTHON_USEDEP}]
	dev-python/alive-progress[${PYTHON_USEDEP}]
	dev-python/awsipranges[${PYTHON_USEDEP}]
	dev-python/azure-identity[${PYTHON_USEDEP}]
	dev-python/azure-keyvault-keys[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-apimanagement[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-applicationinsights[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-authorization[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-compute[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-containerregistry[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-containerservice[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-cosmosdb[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-databricks[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-keyvault[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-loganalytics[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-monitor[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-network[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-postgresqlflexibleservers[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-rdbms[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-recoveryservices[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-recoveryservicesbackup[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-resource[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-search[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-security[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-sql[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-storage[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-subscription[${PYTHON_USEDEP}]
	dev-python/azure-mgmt-web[${PYTHON_USEDEP}]
	dev-python/azure-monitor-query[${PYTHON_USEDEP}]
	dev-python/azure-storage-blob[${PYTHON_USEDEP}]
	~dev-python/boto3-1.39.15[${PYTHON_USEDEP}]
	~dev-python/botocore-1.39.15[${PYTHON_USEDEP}]
	~dev-python/colorama-0.4.6[${PYTHON_USEDEP}]
	~dev-python/cryptography-44.0.1[${PYTHON_USEDEP}]
	dev-python/dash[${PYTHON_USEDEP}]
	dev-python/dash-bootstrap-components[${PYTHON_USEDEP}]
	dev-python/detect-secrets[${PYTHON_USEDEP}]
	~dev-python/dulwich-0.23.0[${PYTHON_USEDEP}]
	~dev-python/google-api-python-client-2.163.0[${PYTHON_USEDEP}]
	<dev-python/google-auth-httplib2-0.3[${PYTHON_USEDEP}]
	~dev-python/h2-4.3.0[${PYTHON_USEDEP}]
	~dev-python/jsonschema-4.23.0[${PYTHON_USEDEP}]
	dev-python/kubernetes[${PYTHON_USEDEP}]
	~dev-python/markdown-3.9.0[${PYTHON_USEDEP}]
	dev-python/microsoft-kiota-abstractions[${PYTHON_USEDEP}]
	dev-python/msgraph-sdk[${PYTHON_USEDEP}]
	~dev-python/numpy-2.0.2[${PYTHON_USEDEP}]
	dev-python/oci[${PYTHON_USEDEP}]
	~dev-python/pandas-2.2.3[${PYTHON_USEDEP}]
	dev-python/py-iam-expand[${PYTHON_USEDEP}]
	dev-python/py-ocsf-models[${PYTHON_USEDEP}]
	<dev-python/pydantic-3.0[${PYTHON_USEDEP}]
	~dev-python/pygithub-2.5.0[${PYTHON_USEDEP}]
	<dev-python/python-dateutil-3.0.0[${PYTHON_USEDEP}]
	~dev-python/pytz-2025.1[${PYTHON_USEDEP}]
	~dev-python/schema-0.7.5[${PYTHON_USEDEP}]
	dev-python/shodan[${PYTHON_USEDEP}]
	dev-python/slack-sdk[${PYTHON_USEDEP}]
	~dev-python/tabulate-0.9.0[${PYTHON_USEDEP}]
	~dev-python/tzlocal-5.3.1[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
