# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="robotic telescope software"

HOMEPAGE=""
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="full"
DEPENDENCIES="<dev-python/aiohttp-4.0[${PYTHON_USEDEP}]
	dev-python/astroplan[${PYTHON_USEDEP}]
	dev-python/astropy[${PYTHON_USEDEP}]
	dev-python/astroquery[${PYTHON_USEDEP}]
	<dev-python/colour-0.2[${PYTHON_USEDEP}]
	<dev-python/dbus-next-0.3[${PYTHON_USEDEP}]
	<dev-python/numpy-3.0[${PYTHON_USEDEP}]
	>=dev-python/pandas-2.2.3[${PYTHON_USEDEP}]
	>=dev-python/paramiko-3.5.1[${PYTHON_USEDEP}]
	dev-python/py-expression-eval[${PYTHON_USEDEP}]
	~dev-python/pytz-2025.2[${PYTHON_USEDEP}]
	<dev-python/pyyaml-7.0[${PYTHON_USEDEP}]
	<dev-python/requests-3.0[${PYTHON_USEDEP}]
	<dev-python/scipy-2.0[${PYTHON_USEDEP}]
	dev-python/single-source[${PYTHON_USEDEP}]
	~dev-python/slixmpp-1.8.4[${PYTHON_USEDEP}]
	<dev-python/typing-extensions-5.0[${PYTHON_USEDEP}]
	full? ( dev-python/asyncinotify[${PYTHON_USEDEP}] )
	full? ( dev-python/ccdproc[${PYTHON_USEDEP}] )
	full? ( <dev-python/lmfit-2.0[${PYTHON_USEDEP}] )
	full? ( dev-python/matrix-nio[${PYTHON_USEDEP}] )
	full? ( dev-python/opencv-python-headless[${PYTHON_USEDEP}] )
	full? ( dev-python/photutils[${PYTHON_USEDEP}] )
	full? ( >=dev-python/pillow-11.3.0[${PYTHON_USEDEP}] )
	full? ( <dev-python/python-daemon-4.0[${PYTHON_USEDEP}] )
	full? ( dev-python/python-telegram-bot[${PYTHON_USEDEP}] )
	full? ( dev-python/sep[${PYTHON_USEDEP}] )
	full? ( dev-python/sunpy[${PYTHON_USEDEP}] )
	full? ( <dev-python/tornado-7.0[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
