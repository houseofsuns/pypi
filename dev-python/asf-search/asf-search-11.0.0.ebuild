# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Python wrapper for ASF's SearchAPI"

HOMEPAGE="https://github.com/asfadmin/Discovery-asf_search.git"
LICENSE="BSD"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="asf-enumeration coherence extras test"
DEPENDENCIES="dev-python/requests[${PYTHON_USEDEP}]
	dev-python/shapely[${PYTHON_USEDEP}]
	dev-python/pytz[${PYTHON_USEDEP}]
	dev-python/numpy[${PYTHON_USEDEP}]
	dev-python/dateparser[${PYTHON_USEDEP}]
	dev-python/python-dateutil[${PYTHON_USEDEP}]
	>=dev-python/tenacity-8.2.2[${PYTHON_USEDEP}]
	test? ( ~dev-python/pytest-8.1.1[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-automation[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-xdist[${PYTHON_USEDEP}] )
	test? ( dev-python/coverage[${PYTHON_USEDEP}] )
	test? ( ~dev-python/requests-mock-1.11.0[${PYTHON_USEDEP}] )
	test? ( dev-python/nbformat[${PYTHON_USEDEP}] )
	test? ( dev-python/nbconvert[${PYTHON_USEDEP}] )
	test? ( dev-python/ipykernel[${PYTHON_USEDEP}] )
	extras? ( dev-python/remotezip[${PYTHON_USEDEP}] )
	extras? ( dev-python/ciso8601[${PYTHON_USEDEP}] )
	asf-enumeration? ( dev-python/asf-enumeration[${PYTHON_USEDEP}] )
	coherence? ( dev-python/pandas[${PYTHON_USEDEP}] )
	coherence? ( dev-python/zarr[${PYTHON_USEDEP}] )
	coherence? ( dev-python/s3fs[${PYTHON_USEDEP}] )
	coherence? ( dev-python/rioxarray[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
