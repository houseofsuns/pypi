# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A package to hold various functions to support training of ML models."

HOMEPAGE="https://github.com/ecmwf/anemoi-training/"
LICENSE="Apache License"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="azure dev docs plotting profile tests"
DEPENDENCIES="dev-python/anemoi-datasets[${PYTHON_USEDEP}]
	dev-python/anemoi-graphs[${PYTHON_USEDEP}]
	dev-python/anemoi-models[${PYTHON_USEDEP}]
	dev-python/anemoi-transform[${PYTHON_USEDEP}]
	dev-python/anemoi-utils[${PYTHON_USEDEP}]
	dev-python/datashader[${PYTHON_USEDEP}]
	dev-python/einops[${PYTHON_USEDEP}]
	dev-python/hydra-core[${PYTHON_USEDEP}]
	>=dev-python/matplotlib-3.7.1[${PYTHON_USEDEP}]
	dev-python/mlflow-skinny[${PYTHON_USEDEP}]
	<dev-python/numpy-2.0[${PYTHON_USEDEP}]
	>=dev-python/nvidia-ml-py-13.580.82[${PYTHON_USEDEP}]
	>=dev-python/pydantic-2.9[${PYTHON_USEDEP}]
	dev-python/pyshtools[${PYTHON_USEDEP}]
	dev-python/pytorch-lightning[${PYTHON_USEDEP}]
	dev-python/timm[${PYTHON_USEDEP}]
	dev-python/torch[${PYTHON_USEDEP}]
	dev-python/torch-geometric[${PYTHON_USEDEP}]
	dev-python/torchinfo[${PYTHON_USEDEP}]
	dev-python/torchvision[${PYTHON_USEDEP}]
	>=dev-python/tqdm-4.65[${PYTHON_USEDEP}]
	dev-python/zarr[${PYTHON_USEDEP}]
	azure? ( dev-python/azure-ai-ml[${PYTHON_USEDEP}] )
	azure? ( dev-python/azure-identity[${PYTHON_USEDEP}] )
	azure? ( dev-python/azureml-mlflow[${PYTHON_USEDEP}] )
	azure? ( dev-python/StrEnum[${PYTHON_USEDEP}] )
	dev? ( dev-python/anemoi-training[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	docs? ( dev-python/autodoc_pydantic[${PYTHON_USEDEP}] )
	docs? ( dev-python/nbsphinx[${PYTHON_USEDEP}] )
	docs? ( dev-python/pandoc[${PYTHON_USEDEP}] )
	docs? ( <dev-python/sphinx-8.2[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-argparse[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-rtd-theme[${PYTHON_USEDEP}] )
	plotting? ( dev-python/distinctipy[${PYTHON_USEDEP}] )
	profile? ( dev-python/HolisticTraceAnalysis[${PYTHON_USEDEP}] )
	profile? ( >=dev-python/pandas-1.3.2[${PYTHON_USEDEP}] )
	profile? ( >=dev-python/rich-13.6[${PYTHON_USEDEP}] )
	profile? ( >=dev-python/tabulate-0.9[${PYTHON_USEDEP}] )
	tests? ( dev-python/anemoi-graphs[${PYTHON_USEDEP}] )
	tests? ( dev-python/anemoi-training[${PYTHON_USEDEP}] )
	tests? ( >=dev-python/hypothesis-6.11[${PYTHON_USEDEP}] )
	tests? ( >=dev-python/pytest-8.0[${PYTHON_USEDEP}] )
	tests? ( >=dev-python/pytest-mock-3.0[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-skip-slow[${PYTHON_USEDEP}] )
	tests? ( dev-python/sshfs[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
