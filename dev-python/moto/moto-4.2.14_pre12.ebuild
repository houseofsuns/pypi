# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="4.2.14.dev12"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION=" [topver 5.0.0]"

HOMEPAGE="https://github.com/getmoto/moto"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all apigateway apigatewayv2 appsync awslambda batch cloudformation cognitoidp ds dynamodb dynamodbstreams ebs ec2 efs eks glue iotdata proxy resourcegroupstaggingapi route53resolver s3 s3crc32c server ssm xray"
DEPENDENCIES=">=dev-python/boto3-1.9.201[${PYTHON_USEDEP}]
	>=dev-python/botocore-1.12.201[${PYTHON_USEDEP}]
	>=dev-python/cryptography-3.3.1[${PYTHON_USEDEP}]
	>=dev-python/requests-2.5[${PYTHON_USEDEP}]
	dev-python/xmltodict[${PYTHON_USEDEP}]
	>=dev-python/werkzeug-0.5[${PYTHON_USEDEP}]
	<dev-python/python-dateutil-3.0.0[${PYTHON_USEDEP}]
	>=dev-python/responses-0.13.0[${PYTHON_USEDEP}]
	dev-python/Jinja2[${PYTHON_USEDEP}]
	all? ( <dev-python/python-jose-4.0.0[${PYTHON_USEDEP}] )
	all? ( >dev-python/ecdsa-0.15[${PYTHON_USEDEP}] )
	all? ( >=dev-python/docker-3.0.0[${PYTHON_USEDEP}] )
	all? ( dev-python/graphql-core[${PYTHON_USEDEP}] )
	all? ( dev-python/PyYAML[${PYTHON_USEDEP}] )
	all? ( >=dev-python/cfn-lint-0.40.0[${PYTHON_USEDEP}] )
	all? ( >=dev-python/sshpubkeys-3.1.0[${PYTHON_USEDEP}] )
	all? ( >=dev-python/openapi-spec-validator-0.5.0[${PYTHON_USEDEP}] )
	all? ( >=dev-python/pyparsing-3.0.7[${PYTHON_USEDEP}] )
	all? ( >=dev-python/jsondiff-1.1.2[${PYTHON_USEDEP}] )
	all? ( dev-python/py-partiql-parser[${PYTHON_USEDEP}] )
	all? ( >=dev-python/aws-xray-sdk-0.93[${PYTHON_USEDEP}] )
	all? ( dev-python/setuptools[${PYTHON_USEDEP}] )
	all? ( dev-python/multipart[${PYTHON_USEDEP}] )
	apigateway? ( dev-python/PyYAML[${PYTHON_USEDEP}] )
	apigateway? ( <dev-python/python-jose-4.0.0[${PYTHON_USEDEP}] )
	apigateway? ( >dev-python/ecdsa-0.15[${PYTHON_USEDEP}] )
	apigateway? ( >=dev-python/openapi-spec-validator-0.5.0[${PYTHON_USEDEP}] )
	apigatewayv2? ( dev-python/PyYAML[${PYTHON_USEDEP}] )
	appsync? ( dev-python/graphql-core[${PYTHON_USEDEP}] )
	awslambda? ( >=dev-python/docker-3.0.0[${PYTHON_USEDEP}] )
	batch? ( >=dev-python/docker-3.0.0[${PYTHON_USEDEP}] )
	cloudformation? ( <dev-python/python-jose-4.0.0[${PYTHON_USEDEP}] )
	cloudformation? ( >dev-python/ecdsa-0.15[${PYTHON_USEDEP}] )
	cloudformation? ( >=dev-python/docker-3.0.0[${PYTHON_USEDEP}] )
	cloudformation? ( dev-python/graphql-core[${PYTHON_USEDEP}] )
	cloudformation? ( dev-python/PyYAML[${PYTHON_USEDEP}] )
	cloudformation? ( >=dev-python/cfn-lint-0.40.0[${PYTHON_USEDEP}] )
	cloudformation? ( >=dev-python/sshpubkeys-3.1.0[${PYTHON_USEDEP}] )
	cloudformation? ( >=dev-python/openapi-spec-validator-0.5.0[${PYTHON_USEDEP}] )
	cloudformation? ( >=dev-python/pyparsing-3.0.7[${PYTHON_USEDEP}] )
	cloudformation? ( >=dev-python/jsondiff-1.1.2[${PYTHON_USEDEP}] )
	cloudformation? ( dev-python/py-partiql-parser[${PYTHON_USEDEP}] )
	cloudformation? ( >=dev-python/aws-xray-sdk-0.93[${PYTHON_USEDEP}] )
	cloudformation? ( dev-python/setuptools[${PYTHON_USEDEP}] )
	cognitoidp? ( <dev-python/python-jose-4.0.0[${PYTHON_USEDEP}] )
	cognitoidp? ( >dev-python/ecdsa-0.15[${PYTHON_USEDEP}] )
	ds? ( >=dev-python/sshpubkeys-3.1.0[${PYTHON_USEDEP}] )
	dynamodb? ( >=dev-python/docker-3.0.0[${PYTHON_USEDEP}] )
	dynamodb? ( dev-python/py-partiql-parser[${PYTHON_USEDEP}] )
	dynamodbstreams? ( >=dev-python/docker-3.0.0[${PYTHON_USEDEP}] )
	dynamodbstreams? ( dev-python/py-partiql-parser[${PYTHON_USEDEP}] )
	ebs? ( >=dev-python/sshpubkeys-3.1.0[${PYTHON_USEDEP}] )
	ec2? ( >=dev-python/sshpubkeys-3.1.0[${PYTHON_USEDEP}] )
	efs? ( >=dev-python/sshpubkeys-3.1.0[${PYTHON_USEDEP}] )
	eks? ( >=dev-python/sshpubkeys-3.1.0[${PYTHON_USEDEP}] )
	glue? ( >=dev-python/pyparsing-3.0.7[${PYTHON_USEDEP}] )
	iotdata? ( >=dev-python/jsondiff-1.1.2[${PYTHON_USEDEP}] )
	proxy? ( <dev-python/python-jose-4.0.0[${PYTHON_USEDEP}] )
	proxy? ( >dev-python/ecdsa-0.15[${PYTHON_USEDEP}] )
	proxy? ( >=dev-python/docker-2.5.1[${PYTHON_USEDEP}] )
	proxy? ( dev-python/graphql-core[${PYTHON_USEDEP}] )
	proxy? ( dev-python/PyYAML[${PYTHON_USEDEP}] )
	proxy? ( >=dev-python/cfn-lint-0.40.0[${PYTHON_USEDEP}] )
	proxy? ( >=dev-python/sshpubkeys-3.1.0[${PYTHON_USEDEP}] )
	proxy? ( >=dev-python/openapi-spec-validator-0.5.0[${PYTHON_USEDEP}] )
	proxy? ( >=dev-python/pyparsing-3.0.7[${PYTHON_USEDEP}] )
	proxy? ( >=dev-python/jsondiff-1.1.2[${PYTHON_USEDEP}] )
	proxy? ( dev-python/py-partiql-parser[${PYTHON_USEDEP}] )
	proxy? ( >=dev-python/aws-xray-sdk-0.93[${PYTHON_USEDEP}] )
	proxy? ( dev-python/setuptools[${PYTHON_USEDEP}] )
	proxy? ( dev-python/multipart[${PYTHON_USEDEP}] )
	resourcegroupstaggingapi? ( <dev-python/python-jose-4.0.0[${PYTHON_USEDEP}] )
	resourcegroupstaggingapi? ( >dev-python/ecdsa-0.15[${PYTHON_USEDEP}] )
	resourcegroupstaggingapi? ( >=dev-python/docker-3.0.0[${PYTHON_USEDEP}] )
	resourcegroupstaggingapi? ( dev-python/graphql-core[${PYTHON_USEDEP}] )
	resourcegroupstaggingapi? ( dev-python/PyYAML[${PYTHON_USEDEP}] )
	resourcegroupstaggingapi? ( >=dev-python/cfn-lint-0.40.0[${PYTHON_USEDEP}] )
	resourcegroupstaggingapi? ( >=dev-python/sshpubkeys-3.1.0[${PYTHON_USEDEP}] )
	resourcegroupstaggingapi? ( >=dev-python/openapi-spec-validator-0.5.0[${PYTHON_USEDEP}] )
	resourcegroupstaggingapi? ( >=dev-python/pyparsing-3.0.7[${PYTHON_USEDEP}] )
	resourcegroupstaggingapi? ( >=dev-python/jsondiff-1.1.2[${PYTHON_USEDEP}] )
	resourcegroupstaggingapi? ( dev-python/py-partiql-parser[${PYTHON_USEDEP}] )
	route53resolver? ( >=dev-python/sshpubkeys-3.1.0[${PYTHON_USEDEP}] )
	s3? ( dev-python/PyYAML[${PYTHON_USEDEP}] )
	s3? ( dev-python/py-partiql-parser[${PYTHON_USEDEP}] )
	s3crc32c? ( dev-python/PyYAML[${PYTHON_USEDEP}] )
	s3crc32c? ( dev-python/py-partiql-parser[${PYTHON_USEDEP}] )
	s3crc32c? ( dev-python/crc32c[${PYTHON_USEDEP}] )
	server? ( <dev-python/python-jose-4.0.0[${PYTHON_USEDEP}] )
	server? ( >dev-python/ecdsa-0.15[${PYTHON_USEDEP}] )
	server? ( >=dev-python/docker-3.0.0[${PYTHON_USEDEP}] )
	server? ( dev-python/graphql-core[${PYTHON_USEDEP}] )
	server? ( dev-python/PyYAML[${PYTHON_USEDEP}] )
	server? ( >=dev-python/cfn-lint-0.40.0[${PYTHON_USEDEP}] )
	server? ( >=dev-python/sshpubkeys-3.1.0[${PYTHON_USEDEP}] )
	server? ( >=dev-python/openapi-spec-validator-0.5.0[${PYTHON_USEDEP}] )
	server? ( >=dev-python/pyparsing-3.0.7[${PYTHON_USEDEP}] )
	server? ( >=dev-python/jsondiff-1.1.2[${PYTHON_USEDEP}] )
	server? ( dev-python/py-partiql-parser[${PYTHON_USEDEP}] )
	server? ( >=dev-python/aws-xray-sdk-0.93[${PYTHON_USEDEP}] )
	server? ( dev-python/setuptools[${PYTHON_USEDEP}] )
	server? ( >dev-python/flask-2.2.0[${PYTHON_USEDEP}] )
	server? ( dev-python/flask-cors[${PYTHON_USEDEP}] )
	ssm? ( dev-python/PyYAML[${PYTHON_USEDEP}] )
	xray? ( >=dev-python/aws-xray-sdk-0.93[${PYTHON_USEDEP}] )
	xray? ( dev-python/setuptools[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
