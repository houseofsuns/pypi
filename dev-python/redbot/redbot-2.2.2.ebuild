# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="REDbot is lint for HTTP."

HOMEPAGE="https://redbot.org/project/"
LICENSE="# License"
SRC_URI="https://files.pythonhosted.org/packages/f1/f6/a1ddad7368da542ee3cb4fdfd8111c85f14240f0ced2204597261d496e42/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev systemd"
DEPENDENCIES=">=dev-python/typing-extensions-4.8.0[${PYTHON_USEDEP}]
	>=dev-python/babel-2.14.0[${PYTHON_USEDEP}]
	systemd? ( dev-python/cysystemd[${PYTHON_USEDEP}] )
	dev? ( dev-python/mypy[${PYTHON_USEDEP}] )
	dev? ( dev-python/black[${PYTHON_USEDEP}] )
	dev? ( dev-python/pylint[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-md[${PYTHON_USEDEP}] )
	dev? ( dev-python/validate-pyproject[${PYTHON_USEDEP}] )
	dev? ( dev-python/build[${PYTHON_USEDEP}] )
	dev? ( dev-python/playwright[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-Markdown[${PYTHON_USEDEP}] )
	dev? ( dev-python/llm[${PYTHON_USEDEP}] )
	dev? ( dev-python/llm-mlx[${PYTHON_USEDEP}] )
	dev? ( dev-python/j2lint[${PYTHON_USEDEP}] )
	dev-python/httplint[${PYTHON_USEDEP}]
	dev-python/importlib-resources[${PYTHON_USEDEP}]
	>=dev-python/jinja2-3.1.2[${PYTHON_USEDEP}]
	>=dev-python/markdown-3.4.4[${PYTHON_USEDEP}]
	>=dev-python/markupsafe-2.1.3[${PYTHON_USEDEP}]
	>=dev-python/netaddr-1.2.1[${PYTHON_USEDEP}]
	dev-python/thor[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
