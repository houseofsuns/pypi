# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A Python client library for Ansys MAPDL."

HOMEPAGE=""
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all doc graphics jupyter tests"
DEPENDENCIES="dev-python/ansys-api-mapdl[${PYTHON_USEDEP}]
	dev-python/ansys-mapdl-reader[${PYTHON_USEDEP}]
	dev-python/ansys-math-core[${PYTHON_USEDEP}]
	dev-python/ansys-platform-instancemanagement[${PYTHON_USEDEP}]
	dev-python/ansys-tools-common[${PYTHON_USEDEP}]
	>=dev-python/click-8.1.3[${PYTHON_USEDEP}]
	>=dev-python/grpcio-1.30.0[${PYTHON_USEDEP}]
	<dev-python/numpy-3.0.0[${PYTHON_USEDEP}]
	>=dev-python/pexpect-4.8.0[${PYTHON_USEDEP}]
	>=dev-python/platformdirs-3.6.0[${PYTHON_USEDEP}]
	>=dev-python/protobuf-3.12.2[${PYTHON_USEDEP}]
	>=dev-python/psutil-5.9.4[${PYTHON_USEDEP}]
	dev-python/pyansys-tools-versioning[${PYTHON_USEDEP}]
	dev-python/pyiges[${PYTHON_USEDEP}]
	dev-python/pyvista[${PYTHON_USEDEP}]
	>=dev-python/scipy-1.3.0[${PYTHON_USEDEP}]
	>=dev-python/tabulate-0.8.0[${PYTHON_USEDEP}]
	>=dev-python/tqdm-4.45.0[${PYTHON_USEDEP}]
	dev-python/vtk[${PYTHON_USEDEP}]
	all? ( dev-python/ansys-tools-visualization-interface[${PYTHON_USEDEP}] )
	all? ( dev-python/ipywidgets[${PYTHON_USEDEP}] )
	all? ( >=dev-python/jupyterlab-3.0[${PYTHON_USEDEP}] )
	all? ( >=dev-python/matplotlib-3.0.0[${PYTHON_USEDEP}] )
	doc? ( dev-python/ansys-dpf-core[${PYTHON_USEDEP}] )
	doc? ( dev-python/ansys-mapdl-reader[${PYTHON_USEDEP}] )
	doc? ( dev-python/ansys-sphinx-theme[${PYTHON_USEDEP}] )
	doc? ( dev-python/ansys-tools-visualization-interface[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/grpcio-1.75.1[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/imageio-ffmpeg-0.6.0[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/imageio-2.37.2[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/jupyter-1.1.1[${PYTHON_USEDEP}] )
	doc? ( dev-python/jupyter-sphinx[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/jupyterlab-3.2.8[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/linuxdoc-20240924.0[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/matplotlib-3.10.7[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/nbformat-5.10.4[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/numpydoc-1.9.0[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/pandas-2.3.3[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/plotly-6.4.0[${PYTHON_USEDEP}] )
	doc? ( dev-python/pyiges[${PYTHON_USEDEP}] )
	doc? ( dev-python/pypandoc[${PYTHON_USEDEP}] )
	doc? ( dev-python/pytest-sphinx[${PYTHON_USEDEP}] )
	doc? ( dev-python/pythreejs[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-autobuild[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinx-autodoc-typehints-3.1.0[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinx-copybutton-0.5.2[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx_design[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinx-gallery-0.19.0[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-jinja[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinx-notfound-page-1.1.0[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinx-8.2.3[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinxcontrib-websupport-2.0.0[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinxemoji-0.3.1[${PYTHON_USEDEP}] )
	doc? ( dev-python/vtk[${PYTHON_USEDEP}] )
	graphics? ( dev-python/ansys-tools-visualization-interface[${PYTHON_USEDEP}] )
	graphics? ( >=dev-python/matplotlib-3.0.0[${PYTHON_USEDEP}] )
	jupyter? ( dev-python/ipywidgets[${PYTHON_USEDEP}] )
	jupyter? ( >=dev-python/jupyterlab-3.0[${PYTHON_USEDEP}] )
	tests? ( dev-python/ansys-dpf-core[${PYTHON_USEDEP}] )
	tests? ( dev-python/ansys-tools-visualization-interface[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/autopep8-2.3.2[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/matplotlib-3.10.7[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pandas-2.3.3[${PYTHON_USEDEP}] )
	tests? ( dev-python/pyansys-tools-report[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pyfakefs-5.10.0[${PYTHON_USEDEP}] )
	tests? ( dev-python/pyiges[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pytest-cov-7.0.0[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-profiling[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-pyvista[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-random-order[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-reportlog[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pytest-rerunfailures-16.1[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pytest-timeout-2.4.0[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pytest-8.4.2[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/scipy-1.15.3[${PYTHON_USEDEP}] )
	tests? ( dev-python/vtk[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
