# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="stream-django"
REALVERSION="1.8.0"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_9,3_10,3_11} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A Django app to build activity, news and notification feeds."

HOMEPAGE="https://github.com/GetStream/stream-django"
LICENSE="BSD"
SRC_URI="https://files.pythonhosted.org/packages/source/s/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="ci test"
DEPENDENCIES="dev-python/django[${PYTHON_USEDEP}]
	dev-python/stream-python[${PYTHON_USEDEP}]
	dev-python/pytz[${PYTHON_USEDEP}]
	ci? ( dev-python/flake8[${PYTHON_USEDEP}] )
	ci? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	ci? ( dev-python/pytest-django[${PYTHON_USEDEP}] )
	ci? ( dev-python/black[${PYTHON_USEDEP}] )
	test? ( dev-python/httpretty[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
