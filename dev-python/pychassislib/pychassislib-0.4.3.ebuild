# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A lightweight Python distributed microservice library"

HOMEPAGE="https://github.com/Bryanthelol/pychassislib"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="apiflask dev ha log metric mongodb mysql nameko ob redis schema security"
DEPENDENCIES="nameko? ( dev-python/nameko[${PYTHON_USEDEP}] )
	apiflask? ( dev-python/APIFlask[${PYTHON_USEDEP}] )
	apiflask? ( dev-python/gevent[${PYTHON_USEDEP}] )
	apiflask? ( dev-python/gunicorn[${PYTHON_USEDEP}] )
	ha? ( ~dev-python/cachetools-5.3.2[${PYTHON_USEDEP}] )
	ha? ( dev-python/circuitbreaker[${PYTHON_USEDEP}] )
	ha? ( >=dev-python/tenacity-8.2.3[${PYTHON_USEDEP}] )
	ob? ( dev-python/nameko-sentry[${PYTHON_USEDEP}] )
	ob? ( dev-python/nameko-tracer[${PYTHON_USEDEP}] )
	log? ( ~dev-python/loguru-0.7.2[${PYTHON_USEDEP}] )
	metric? ( ~dev-python/statsd-4.0.1[${PYTHON_USEDEP}] )
	schema? ( ~dev-python/marshmallow-3.20.2[${PYTHON_USEDEP}] )
	mysql? ( >=dev-python/pymysql-1.0.3[${PYTHON_USEDEP}] )
	mysql? ( >=dev-python/sqlalchemy-2.0.15[${PYTHON_USEDEP}] )
	mysql? ( dev-python/sqlacodegen[${PYTHON_USEDEP}] )
	mysql? ( >=dev-python/alembic-1.11.1[${PYTHON_USEDEP}] )
	mongodb? ( dev-python/mongoengine[${PYTHON_USEDEP}] )
	redis? ( dev-python/pottery[${PYTHON_USEDEP}] )
	security? ( >=dev-python/cryptography-42.0.2[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/environs-9.5.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/python-dotenv-1.0.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pdm-2.10.1[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
