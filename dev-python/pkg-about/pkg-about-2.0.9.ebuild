# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Shares Python package metadata at runtime."

HOMEPAGE="https://pypi.org/project/pkg-about/"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="doc test"
DEPENDENCIES=">=dev-python/setuptools-80.9.0[${PYTHON_USEDEP}]
	>=dev-python/typing-extensions-4.15.0[${PYTHON_USEDEP}]
	>=dev-python/packaging-25.0.0[${PYTHON_USEDEP}]
	doc? ( >=dev-python/sphinx-8.1.3[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/sphinx-autodoc-typehints-3.0.1[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-toolbox[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/sphinx-tabs-3.4.5[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/sphinx-copybutton-0.5.2[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/sphinxcontrib-spelling-8.0.2[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-lint[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/restructuredtext-lint-2.0.2[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/nbsphinx-0.9.8[${PYTHON_USEDEP}] )
	test? ( >=dev-python/deepdiff-8.6.1[${PYTHON_USEDEP}] )
	test? ( >=dev-python/rich-14.2.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/appdirs-1.4.4[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
