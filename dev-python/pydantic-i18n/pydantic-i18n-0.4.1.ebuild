# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="pydantic-i18n is an extension to support an i18n for the pydantic error messages."

HOMEPAGE=""
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev doc test"
DEPENDENCIES=">=dev-python/pydantic-1.9.0[${PYTHON_USEDEP}]
	dev? ( dev-python/autoflake[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/flake8-7.0.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/mkdocs-1.5.3[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/mkdocs-material-9.5.11[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/markdown-3.5.2[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/markdown-include-0.8.1[${PYTHON_USEDEP}] )
	doc? ( dev-python/mkdocs-markdownextradata-plugin[${PYTHON_USEDEP}] )
	doc? ( dev-python/typer[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/pyyaml-5.3.1[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pydantic-2.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-7.4.3[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-cov-4.1.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-lazy-fixture-0.6.3[${PYTHON_USEDEP}] )
	test? ( ~dev-python/mypy-1.8.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/flake8-7.0.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/black-24.2.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/isort-5.13.2[${PYTHON_USEDEP}] )
	test? ( ~dev-python/Babel-2.14.0[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
