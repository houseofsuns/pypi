# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Python client for EPO OPS, the European Patent Office's Open Patent Services API."

HOMEPAGE="https://github.com/ip-tools/python-epo-ops-client"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="develop test"
DEPENDENCIES="<dev-python/dogpile-cache-1.2[${PYTHON_USEDEP}]
	<dev-python/python-dateutil-2.9[${PYTHON_USEDEP}]
	<dev-python/requests-3.0[${PYTHON_USEDEP}]
	<dev-python/six-2.0[${PYTHON_USEDEP}]
	develop? ( <dev-python/black-24.0[${PYTHON_USEDEP}] )
	develop? ( <dev-python/twine-5.0[${PYTHON_USEDEP}] )
	develop? ( <dev-python/wheel-1.0[${PYTHON_USEDEP}] )
	develop? ( dev-python/ruff[${PYTHON_USEDEP}] )
	test? ( <dev-python/pytest-8.0[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cache[${PYTHON_USEDEP}] )
	test? ( <dev-python/pytest-cov-4.2[${PYTHON_USEDEP}] )
	test? ( <dev-python/python-dotenv-0.20[${PYTHON_USEDEP}] )
	test? ( <dev-python/responses-0.24[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
