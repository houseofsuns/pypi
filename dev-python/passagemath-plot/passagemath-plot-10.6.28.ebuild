# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="passagemath: Plotting and graphics with Matplotlib, Three.JS, etc. [top-max 10.6.29]"

HOMEPAGE=""
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dot2tex graphs jsmol playwright polyhedra symbolics tachyon test"
DEPENDENCIES="~dev-python/gmpy2-2.1_beta999[${PYTHON_USEDEP}]
	>=dev-python/cysignals-1.11.2[${PYTHON_USEDEP}]
	dev-python/memory-allocator[${PYTHON_USEDEP}]
	dev-python/passagemath-categories[${PYTHON_USEDEP}]
	dev-python/passagemath-modules[${PYTHON_USEDEP}]
	>=dev-python/numpy-1.19[${PYTHON_USEDEP}]
	>=dev-python/numpy-1.22.4[${PYTHON_USEDEP}]
	>=dev-python/scipy-1.5[${PYTHON_USEDEP}]
	>=dev-python/pillow-7.2.0[${PYTHON_USEDEP}]
	>=dev-python/matplotlib-3.5.1[${PYTHON_USEDEP}]
	dev-python/jupyter-threejs-sage[${PYTHON_USEDEP}]
	test? ( dev-python/passagemath-plot[${PYTHON_USEDEP}] )
	test? ( dev-python/passagemath-repl[${PYTHON_USEDEP}] )
	dot2tex? ( dev-python/dot2tex[${PYTHON_USEDEP}] )
	jsmol? ( dev-python/jupyter-jsmol[${PYTHON_USEDEP}] )
	playwright? ( dev-python/playwright[${PYTHON_USEDEP}] )
	tachyon? ( dev-python/passagemath-tachyon[${PYTHON_USEDEP}] )
	graphs? ( dev-python/passagemath-graphs[${PYTHON_USEDEP}] )
	polyhedra? ( dev-python/passagemath-polyhedra[${PYTHON_USEDEP}] )
	symbolics? ( dev-python/passagemath-symbolics[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
