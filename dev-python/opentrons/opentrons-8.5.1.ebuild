# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="The Opentrons API is a simple framework designed to make writing automated biology lab protocols easy. [prod-max (True, <g_sorcery.g_collections.Version object at 0x7f3ab5ac82e0>)]"

HOMEPAGE=""
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="flex-hardware ot2-hardware"
DEPENDENCIES="dev-python/opentrons-shared-data[${PYTHON_USEDEP}]
	dev-python/aionotify[${PYTHON_USEDEP}]
	<dev-python/anyio-4.0.0[${PYTHON_USEDEP}]
	<dev-python/jsonschema-4.18.0[${PYTHON_USEDEP}]
	<dev-python/numpy-2.0[${PYTHON_USEDEP}]
	<dev-python/pydantic-3.0[${PYTHON_USEDEP}]
	dev-python/pydantic-settings[${PYTHON_USEDEP}]
	>=dev-python/pyserial-3.5[${PYTHON_USEDEP}]
	<dev-python/typing-extensions-5.0[${PYTHON_USEDEP}]
	<dev-python/click-9.0[${PYTHON_USEDEP}]
	~dev-python/pyusb-1.2.1[${PYTHON_USEDEP}]
	>=dev-python/packaging-21.0[${PYTHON_USEDEP}]
	flex-hardware? ( dev-python/opentrons-hardware[${PYTHON_USEDEP}] )
	ot2-hardware? ( dev-python/opentrons-hardware[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
