# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="The tiniest of Gaussian Process libraries"

HOMEPAGE="https://github.com/dfm/tinygp"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="coverage docs test"
DEPENDENCIES="dev-python/jax[${PYTHON_USEDEP}]
	dev-python/jaxlib[${PYTHON_USEDEP}]
	coverage? ( dev-python/pytest[${PYTHON_USEDEP}] )
	coverage? ( dev-python/george[${PYTHON_USEDEP}] )
	coverage? ( dev-python/celerite[${PYTHON_USEDEP}] )
	coverage? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-book-theme[${PYTHON_USEDEP}] )
	docs? ( dev-python/myst-nb[${PYTHON_USEDEP}] )
	docs? ( dev-python/matplotlib[${PYTHON_USEDEP}] )
	docs? ( dev-python/statsmodels[${PYTHON_USEDEP}] )
	docs? ( dev-python/numpyro[${PYTHON_USEDEP}] )
	docs? ( dev-python/arviz[${PYTHON_USEDEP}] )
	docs? ( dev-python/flax[${PYTHON_USEDEP}] )
	docs? ( dev-python/optax[${PYTHON_USEDEP}] )
	docs? ( dev-python/jaxopt[${PYTHON_USEDEP}] )
	docs? ( dev-python/ipython[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest[${PYTHON_USEDEP}] )
	test? ( dev-python/george[${PYTHON_USEDEP}] )
	test? ( dev-python/celerite[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
