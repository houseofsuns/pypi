# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Open-source tool for exploring, labeling, and monitoring data for NLP projects."

HOMEPAGE="https://www.argilla.io"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="integrations listeners server server-postgresql tests"
DEPENDENCIES="<=dev-python/httpx-0.26[${PYTHON_USEDEP}]
	~dev-python/deprecated-1.2.0[${PYTHON_USEDEP}]
	>=dev-python/packaging-20.0[${PYTHON_USEDEP}]
	>=dev-python/pandas-1.0.0[${PYTHON_USEDEP}]
	>=dev-python/pydantic-1.10.7[${PYTHON_USEDEP}]
	<dev-python/wrapt-1.15[${PYTHON_USEDEP}]
	<dev-python/numpy-1.24.0[${PYTHON_USEDEP}]
	>=dev-python/tqdm-4.27.0[${PYTHON_USEDEP}]
	dev-python/backoff[${PYTHON_USEDEP}]
	dev-python/monotonic[${PYTHON_USEDEP}]
	>dev-python/rich-13.1.0[${PYTHON_USEDEP}]
	dev-python/typer[${PYTHON_USEDEP}]
	integrations? ( <dev-python/pyyaml-6.1.0[${PYTHON_USEDEP}] )
	integrations? ( dev-python/datasets[${PYTHON_USEDEP}] )
	integrations? ( dev-python/huggingface-hub[${PYTHON_USEDEP}] )
	integrations? ( dev-python/flair[${PYTHON_USEDEP}] )
	integrations? ( dev-python/faiss-cpu[${PYTHON_USEDEP}] )
	integrations? ( dev-python/flyingsquid[${PYTHON_USEDEP}] )
	integrations? ( dev-python/pgmpy[${PYTHON_USEDEP}] )
	integrations? ( >=dev-python/plotly-4.1.0[${PYTHON_USEDEP}] )
	integrations? ( dev-python/snorkel[${PYTHON_USEDEP}] )
	integrations? ( dev-python/spacy[${PYTHON_USEDEP}] )
	integrations? ( dev-python/spacy-transformers[${PYTHON_USEDEP}] )
	integrations? ( dev-python/spacy-huggingface-hub[${PYTHON_USEDEP}] )
	integrations? ( dev-python/transformers[${PYTHON_USEDEP}] )
	integrations? ( dev-python/evaluate[${PYTHON_USEDEP}] )
	integrations? ( dev-python/seqeval[${PYTHON_USEDEP}] )
	integrations? ( dev-python/sentence-transformers[${PYTHON_USEDEP}] )
	integrations? ( dev-python/setfit[${PYTHON_USEDEP}] )
	integrations? ( dev-python/span-marker[${PYTHON_USEDEP}] )
	integrations? ( dev-python/sentence-transformers[${PYTHON_USEDEP}] )
	integrations? ( dev-python/textdescriptives[${PYTHON_USEDEP}] )
	integrations? ( dev-python/openai[${PYTHON_USEDEP}] )
	integrations? ( dev-python/peft[${PYTHON_USEDEP}] )
	integrations? ( dev-python/trl[${PYTHON_USEDEP}] )
	integrations? ( dev-python/ipynbname[${PYTHON_USEDEP}] )
	listeners? ( dev-python/schedule[${PYTHON_USEDEP}] )
	server? ( dev-python/argilla-server[${PYTHON_USEDEP}] )
	server-postgresql? ( dev-python/argilla-server[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-mock[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-asyncio[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-env[${PYTHON_USEDEP}] )
	tests? ( dev-python/factory-boy[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
