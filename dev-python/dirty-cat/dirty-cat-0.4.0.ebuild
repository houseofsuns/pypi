# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="dirty-cat"
REALVERSION="0.4.0"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_9,3_10,3_11} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Machine learning with dirty categories."

HOMEPAGE="https://dirty-cat.github.io/"
LICENSE="BSD"
SRC_URI="https://files.pythonhosted.org/packages/55/2b/5a1a4d829ecacbef7c833484dcebc279778cefc5473681241c4603913bb6/dirty_cat-${REALVERSION}.tar.gz"
SOURCEFILE="dirty_cat-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="benchmarks dev doc min-py310 min-py38 min-py39"
DEPENDENCIES="dev-python/scikit-learn[${PYTHON_USEDEP}]
	dev-python/numpy[${PYTHON_USEDEP}]
	dev-python/scipy[${PYTHON_USEDEP}]
	dev-python/pandas[${PYTHON_USEDEP}]
	dev-python/requests[${PYTHON_USEDEP}]
	dev-python/joblib[${PYTHON_USEDEP}]
	dev-python/pyarrow[${PYTHON_USEDEP}]
	benchmarks? ( dev-python/numpy[${PYTHON_USEDEP}] )
	benchmarks? ( dev-python/pandas[${PYTHON_USEDEP}] )
	benchmarks? ( dev-python/matplotlib[${PYTHON_USEDEP}] )
	benchmarks? ( dev-python/seaborn[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-xdist[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-xdist[${PYTHON_USEDEP}] )
	dev? ( dev-python/coverage[${PYTHON_USEDEP}] )
	dev? ( dev-python/mypy[${PYTHON_USEDEP}] )
	dev? ( dev-python/numpydoc[${PYTHON_USEDEP}] )
	dev? ( dev-python/flake8[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-gallery[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinxext-opengraph[${PYTHON_USEDEP}] )
	doc? ( dev-python/matplotlib[${PYTHON_USEDEP}] )
	doc? ( dev-python/seaborn[${PYTHON_USEDEP}] )
	doc? ( dev-python/statsmodels[${PYTHON_USEDEP}] )
	min-py310? ( dev-python/scikit-learn[${PYTHON_USEDEP}] )
	min-py310? ( dev-python/numpy[${PYTHON_USEDEP}] )
	min-py310? ( dev-python/scipy[${PYTHON_USEDEP}] )
	min-py310? ( dev-python/pandas[${PYTHON_USEDEP}] )
	min-py38? ( dev-python/scikit-learn[${PYTHON_USEDEP}] )
	min-py38? ( dev-python/numpy[${PYTHON_USEDEP}] )
	min-py38? ( dev-python/scipy[${PYTHON_USEDEP}] )
	min-py38? ( dev-python/pandas[${PYTHON_USEDEP}] )
	min-py39? ( dev-python/scikit-learn[${PYTHON_USEDEP}] )
	min-py39? ( dev-python/numpy[${PYTHON_USEDEP}] )
	min-py39? ( dev-python/scipy[${PYTHON_USEDEP}] )
	min-py39? ( dev-python/pandas[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
