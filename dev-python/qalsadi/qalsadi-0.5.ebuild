# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=wheel

inherit python-r1 gs-pypi

DESCRIPTION="Qalsadi Arabic Morphological Analyzer and lemmatizer for Python [wheel]"

HOMEPAGE="http://qalsadi.sourceforge.net/"
LICENSE="GPL-3"
SRC_URI="https://files.pythonhosted.org/packages/py3/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}-py3-none-any.whl"
SOURCEFILE="${REALNAME}-${REALVERSION}-py3-none-any.whl"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="+bin-wheel"
DEPENDENCIES="dev-python/Arabic-Stopwords[${PYTHON_USEDEP}]
	dev-python/alyahmor[${PYTHON_USEDEP}]
	dev-python/arramooz-pysqlite[${PYTHON_USEDEP}]
	dev-python/codernitydb3[${PYTHON_USEDEP}]
	dev-python/libqutrub[${PYTHON_USEDEP}]
	dev-python/mysam-tagmanager[${PYTHON_USEDEP}]
	dev-python/Naftawayh[${PYTHON_USEDEP}]
	dev-python/pickleDB[${PYTHON_USEDEP}]
	dev-python/PyArabic[${PYTHON_USEDEP}]
	dev-python/Tashaphyne[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
