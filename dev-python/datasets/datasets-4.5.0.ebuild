# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="HuggingFace community-driven open-source library of datasets"

HOMEPAGE="https://github.com/huggingface/datasets"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="audio benchmarks dev docs jax nibabel pdfs quality tensorflow tensorflow-gpu tests tests-numpy2 torch vision"
DEPENDENCIES="dev-python/filelock[${PYTHON_USEDEP}]
	>=dev-python/numpy-1.17[${PYTHON_USEDEP}]
	>=dev-python/pyarrow-21.0.0[${PYTHON_USEDEP}]
	<dev-python/dill-0.4.1[${PYTHON_USEDEP}]
	dev-python/pandas[${PYTHON_USEDEP}]
	>=dev-python/requests-2.32.2[${PYTHON_USEDEP}]
	<dev-python/httpx-1.0.0[${PYTHON_USEDEP}]
	>=dev-python/tqdm-4.66.3[${PYTHON_USEDEP}]
	dev-python/xxhash[${PYTHON_USEDEP}]
	<dev-python/multiprocess-0.70.19[${PYTHON_USEDEP}]
	<=dev-python/fsspec-2025.10.0[${PYTHON_USEDEP}]
	dev-python/huggingface-hub[${PYTHON_USEDEP}]
	dev-python/packaging[${PYTHON_USEDEP}]
	>=dev-python/pyyaml-5.1[${PYTHON_USEDEP}]
	audio? ( dev-python/TorchCodec[${PYTHON_USEDEP}] )
	audio? ( dev-python/torch[${PYTHON_USEDEP}] )
	vision? ( >=dev-python/pillow-9.4.0[${PYTHON_USEDEP}] )
	tensorflow? ( dev-python/tensorflow[${PYTHON_USEDEP}] )
	tensorflow-gpu? ( dev-python/tensorflow[${PYTHON_USEDEP}] )
	torch? ( dev-python/torch[${PYTHON_USEDEP}] )
	jax? ( dev-python/jax[${PYTHON_USEDEP}] )
	jax? ( dev-python/jaxlib[${PYTHON_USEDEP}] )
	dev? ( dev-python/numba[${PYTHON_USEDEP}] )
	dev? ( dev-python/absl-py[${PYTHON_USEDEP}] )
	dev? ( dev-python/decorator[${PYTHON_USEDEP}] )
	dev? ( <dev-python/joblib-1.3.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/joblibspark[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-datadir[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-xdist[${PYTHON_USEDEP}] )
	dev? ( dev-python/aiohttp[${PYTHON_USEDEP}] )
	dev? ( <dev-python/elasticsearch-8.0.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/faiss-cpu[${PYTHON_USEDEP}] )
	dev? ( dev-python/h5py[${PYTHON_USEDEP}] )
	dev? ( dev-python/pylance[${PYTHON_USEDEP}] )
	dev? ( dev-python/jax[${PYTHON_USEDEP}] )
	dev? ( dev-python/jaxlib[${PYTHON_USEDEP}] )
	dev? ( dev-python/lz4[${PYTHON_USEDEP}] )
	dev? ( dev-python/moto[${PYTHON_USEDEP}] )
	dev? ( dev-python/pyspark[${PYTHON_USEDEP}] )
	dev? ( dev-python/py7zr[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/rarfile-4.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/sqlalchemy[${PYTHON_USEDEP}] )
	dev? ( <dev-python/protobuf-4.0.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/tensorflow[${PYTHON_USEDEP}] )
	dev? ( dev-python/tiktoken[${PYTHON_USEDEP}] )
	dev? ( dev-python/torch[${PYTHON_USEDEP}] )
	dev? ( dev-python/torchdata[${PYTHON_USEDEP}] )
	dev? ( dev-python/transformers[${PYTHON_USEDEP}] )
	dev? ( dev-python/zstandard[${PYTHON_USEDEP}] )
	dev? ( dev-python/polars[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pillow-9.4.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/TorchCodec[${PYTHON_USEDEP}] )
	dev? ( dev-python/nibabel[${PYTHON_USEDEP}] )
	dev? ( dev-python/ruff[${PYTHON_USEDEP}] )
	dev? ( dev-python/transformers[${PYTHON_USEDEP}] )
	dev? ( dev-python/torch[${PYTHON_USEDEP}] )
	dev? ( dev-python/tensorflow[${PYTHON_USEDEP}] )
	tests? ( dev-python/numba[${PYTHON_USEDEP}] )
	tests? ( dev-python/absl-py[${PYTHON_USEDEP}] )
	tests? ( dev-python/decorator[${PYTHON_USEDEP}] )
	tests? ( <dev-python/joblib-1.3.0[${PYTHON_USEDEP}] )
	tests? ( dev-python/joblibspark[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-datadir[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-xdist[${PYTHON_USEDEP}] )
	tests? ( dev-python/aiohttp[${PYTHON_USEDEP}] )
	tests? ( <dev-python/elasticsearch-8.0.0[${PYTHON_USEDEP}] )
	tests? ( dev-python/faiss-cpu[${PYTHON_USEDEP}] )
	tests? ( dev-python/h5py[${PYTHON_USEDEP}] )
	tests? ( dev-python/pylance[${PYTHON_USEDEP}] )
	tests? ( dev-python/jax[${PYTHON_USEDEP}] )
	tests? ( dev-python/jaxlib[${PYTHON_USEDEP}] )
	tests? ( dev-python/lz4[${PYTHON_USEDEP}] )
	tests? ( dev-python/moto[${PYTHON_USEDEP}] )
	tests? ( dev-python/pyspark[${PYTHON_USEDEP}] )
	tests? ( dev-python/py7zr[${PYTHON_USEDEP}] )
	tests? ( >=dev-python/rarfile-4.0[${PYTHON_USEDEP}] )
	tests? ( dev-python/sqlalchemy[${PYTHON_USEDEP}] )
	tests? ( <dev-python/protobuf-4.0.0[${PYTHON_USEDEP}] )
	tests? ( dev-python/tensorflow[${PYTHON_USEDEP}] )
	tests? ( dev-python/tiktoken[${PYTHON_USEDEP}] )
	tests? ( dev-python/torch[${PYTHON_USEDEP}] )
	tests? ( dev-python/torchdata[${PYTHON_USEDEP}] )
	tests? ( dev-python/transformers[${PYTHON_USEDEP}] )
	tests? ( dev-python/zstandard[${PYTHON_USEDEP}] )
	tests? ( dev-python/polars[${PYTHON_USEDEP}] )
	tests? ( >=dev-python/pillow-9.4.0[${PYTHON_USEDEP}] )
	tests? ( dev-python/TorchCodec[${PYTHON_USEDEP}] )
	tests? ( dev-python/nibabel[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/numba[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/absl-py[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/decorator[${PYTHON_USEDEP}] )
	tests-numpy2? ( <dev-python/joblib-1.3.0[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/joblibspark[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/pytest[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/pytest-datadir[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/pytest-xdist[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/aiohttp[${PYTHON_USEDEP}] )
	tests-numpy2? ( <dev-python/elasticsearch-8.0.0[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/h5py[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/pylance[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/jax[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/jaxlib[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/lz4[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/moto[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/pyspark[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/py7zr[${PYTHON_USEDEP}] )
	tests-numpy2? ( >=dev-python/rarfile-4.0[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/sqlalchemy[${PYTHON_USEDEP}] )
	tests-numpy2? ( <dev-python/protobuf-4.0.0[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/tiktoken[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/torch[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/torchdata[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/transformers[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/zstandard[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/polars[${PYTHON_USEDEP}] )
	tests-numpy2? ( >=dev-python/pillow-9.4.0[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/TorchCodec[${PYTHON_USEDEP}] )
	tests-numpy2? ( dev-python/nibabel[${PYTHON_USEDEP}] )
	quality? ( dev-python/ruff[${PYTHON_USEDEP}] )
	benchmarks? ( dev-python/tensorflow[${PYTHON_USEDEP}] )
	benchmarks? ( dev-python/torch[${PYTHON_USEDEP}] )
	benchmarks? ( dev-python/transformers[${PYTHON_USEDEP}] )
	docs? ( dev-python/transformers[${PYTHON_USEDEP}] )
	docs? ( dev-python/torch[${PYTHON_USEDEP}] )
	docs? ( dev-python/tensorflow[${PYTHON_USEDEP}] )
	pdfs? ( dev-python/pdfplumber[${PYTHON_USEDEP}] )
	nibabel? ( dev-python/nibabel[${PYTHON_USEDEP}] )
	nibabel? ( dev-python/ipyniivue[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
