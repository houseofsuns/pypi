# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="SuperTokens SDK for Python"

HOMEPAGE="https://github.com/supertokens/supertokens-python"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="django django2x drf fastapi flask"
DEPENDENCIES="<dev-python/pyjwt-3.0.0[${PYTHON_USEDEP}]
	<=dev-python/httpx-0.27.2[${PYTHON_USEDEP}]
	<dev-python/pycryptodome-3.21.0[${PYTHON_USEDEP}]
	<dev-python/tldextract-5.1.3[${PYTHON_USEDEP}]
	<dev-python/asgiref-4.0[${PYTHON_USEDEP}]
	<dev-python/typing-extensions-5.0.0[${PYTHON_USEDEP}]
	<dev-python/deprecated-1.3.0[${PYTHON_USEDEP}]
	<dev-python/phonenumbers-9.0[${PYTHON_USEDEP}]
	dev-python/twilio[${PYTHON_USEDEP}]
	dev-python/aiosmtplib[${PYTHON_USEDEP}]
	dev-python/pkce[${PYTHON_USEDEP}]
	<dev-python/pyotp-3.0[${PYTHON_USEDEP}]
	<dev-python/python-dateutil-3.0[${PYTHON_USEDEP}]
	django? ( dev-python/django-cors-headers[${PYTHON_USEDEP}] )
	django? ( >=dev-python/django-3.0[${PYTHON_USEDEP}] )
	django? ( dev-python/django-stubs[${PYTHON_USEDEP}] )
	django? ( dev-python/uvicorn[${PYTHON_USEDEP}] )
	django? ( ~dev-python/python-dotenv-1.0.1[${PYTHON_USEDEP}] )
	django2x? ( ~dev-python/django-cors-headers-3.11.0[${PYTHON_USEDEP}] )
	django2x? ( <dev-python/django-3.0[${PYTHON_USEDEP}] )
	django2x? ( dev-python/django-stubs[${PYTHON_USEDEP}] )
	django2x? ( dev-python/gunicorn[${PYTHON_USEDEP}] )
	django2x? ( ~dev-python/python-dotenv-1.0.1[${PYTHON_USEDEP}] )
	drf? ( dev-python/adrf[${PYTHON_USEDEP}] )
	drf? ( dev-python/django-cors-headers[${PYTHON_USEDEP}] )
	drf? ( >=dev-python/django-4.0[${PYTHON_USEDEP}] )
	drf? ( dev-python/django-stubs[${PYTHON_USEDEP}] )
	drf? ( dev-python/djangorestframework[${PYTHON_USEDEP}] )
	drf? ( dev-python/gunicorn[${PYTHON_USEDEP}] )
	drf? ( dev-python/uvicorn[${PYTHON_USEDEP}] )
	drf? ( ~dev-python/python-dotenv-1.0.1[${PYTHON_USEDEP}] )
	drf? ( dev-python/tzdata[${PYTHON_USEDEP}] )
	fastapi? ( ~dev-python/respx-0.21.1[${PYTHON_USEDEP}] )
	fastapi? ( dev-python/fastapi[${PYTHON_USEDEP}] )
	fastapi? ( dev-python/uvicorn[${PYTHON_USEDEP}] )
	fastapi? ( ~dev-python/python-dotenv-1.0.1[${PYTHON_USEDEP}] )
	flask? ( dev-python/flask-cors[${PYTHON_USEDEP}] )
	flask? ( dev-python/flask[${PYTHON_USEDEP}] )
	flask? ( ~dev-python/python-dotenv-1.0.1[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
