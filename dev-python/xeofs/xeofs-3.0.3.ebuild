# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Comprehensive EOF analysis in Python with xarray: A versatile, multidimensional, and scalable tool for advanced climate data analysis"

HOMEPAGE="https://github.com/xarray-contrib/xeofs"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="complete dev docs etc io"
DEPENDENCIES=">=dev-python/numpy-1.24[${PYTHON_USEDEP}]
	>=dev-python/pandas-2.0[${PYTHON_USEDEP}]
	>=dev-python/xarray-2023.4.0[${PYTHON_USEDEP}]
	>=dev-python/scikit-learn-1.0.2[${PYTHON_USEDEP}]
	>=dev-python/tqdm-4.64.0[${PYTHON_USEDEP}]
	dev-python/dask[${PYTHON_USEDEP}]
	>=dev-python/typing-extensions-4.8.0[${PYTHON_USEDEP}]
	dev-python/xarray-datatree[${PYTHON_USEDEP}]
	complete? ( dev-python/xeofs[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/build-1.0.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/ruff[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-7.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-xdist-3.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/coverage-6.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	dev? ( dev-python/nbstripout[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pooch-1.6.0[${PYTHON_USEDEP}] )
	docs? ( dev-python/rpy2[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-gallery-0.14[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx_design[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-copybutton-0.5[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/nbsphinx-0.9[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/pydata-sphinx-theme-0.15[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-8.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/nbconvert-7.9[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/myst-parser-3.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/matplotlib-3.4[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/seaborn-0.11[${PYTHON_USEDEP}] )
	docs? ( dev-python/Cartopy[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/pooch-1.6.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/ipython-8.14[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/ipykernel-6.23[${PYTHON_USEDEP}] )
	etc? ( dev-python/numba[${PYTHON_USEDEP}] )
	etc? ( >=dev-python/statsmodels-0.14.0[${PYTHON_USEDEP}] )
	io? ( dev-python/h5netcdf[${PYTHON_USEDEP}] )
	io? ( >=dev-python/netcdf4-1.5.8[${PYTHON_USEDEP}] )
	io? ( dev-python/zarr[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
