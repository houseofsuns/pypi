# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="1.5.0a0"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Core bioimage-analysis library and tasks of the Fractal analytics platform"

HOMEPAGE="https://github.com/fractal-analytics-platform/fractal-tasks-core"
LICENSE="BSD"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="fractal-tasks"
DEPENDENCIES="dev-python/dask[${PYTHON_USEDEP}]
	>dev-python/fsspec-2023.9.0[${PYTHON_USEDEP}]
	dev-python/zarr[${PYTHON_USEDEP}]
	<=dev-python/numpy-2.1.0[${PYTHON_USEDEP}]
	>=dev-python/pandas-1.2.0[${PYTHON_USEDEP}]
	<dev-python/lxml-5.0.0[${PYTHON_USEDEP}]
	<=dev-python/pydantic-2.8.2[${PYTHON_USEDEP}]
	dev-python/docstring-parser[${PYTHON_USEDEP}]
	dev-python/anndata[${PYTHON_USEDEP}]
	~dev-python/filelock-3.13[${PYTHON_USEDEP}]
	dev-python/ngio[${PYTHON_USEDEP}]
	dev-python/fractal-task-tools[${PYTHON_USEDEP}]
	fractal-tasks? ( >=dev-python/pillow-9.1.1[${PYTHON_USEDEP}] )
	fractal-tasks? ( <dev-python/imageio-ffmpeg-0.5.0[${PYTHON_USEDEP}] )
	fractal-tasks? ( >=dev-python/scikit-image-0.19[${PYTHON_USEDEP}] )
	fractal-tasks? ( dev-python/napari-segment-blobs-and-things-with-membranes[${PYTHON_USEDEP}] )
	fractal-tasks? ( dev-python/napari-workflows[${PYTHON_USEDEP}] )
	fractal-tasks? ( dev-python/stackview[${PYTHON_USEDEP}] )
	fractal-tasks? ( dev-python/napari-skimage-regionprops[${PYTHON_USEDEP}] )
	fractal-tasks? ( dev-python/napari-tools-menu[${PYTHON_USEDEP}] )
	fractal-tasks? ( dev-python/cellpose[${PYTHON_USEDEP}] )
	fractal-tasks? ( dev-python/torch[${PYTHON_USEDEP}] )
	fractal-tasks? ( dev-python/image-registration[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
