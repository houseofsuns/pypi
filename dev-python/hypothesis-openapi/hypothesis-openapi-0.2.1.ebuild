# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Hypothesis plugin for generating valid Open API documents"

HOMEPAGE=""
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="bench cov dev tests"
DEPENDENCIES="<dev-python/hypothesis-7.0[${PYTHON_USEDEP}]
	bench? ( dev-python/pytest-codspeed[${PYTHON_USEDEP}] )
	cov? ( dev-python/coverage_enable_subprocess[${PYTHON_USEDEP}] )
	cov? ( >=dev-python/coverage-7.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/coverage_enable_subprocess[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/coverage-7.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/coverage-7.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/jsonschema-4.21.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-codspeed[${PYTHON_USEDEP}] )
	dev? ( <dev-python/pytest-xdist-3.0[${PYTHON_USEDEP}] )
	dev? ( <dev-python/pytest-8.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/referencing-0.35[${PYTHON_USEDEP}] )
	tests? ( >=dev-python/coverage-7.0[${PYTHON_USEDEP}] )
	tests? ( >=dev-python/jsonschema-4.21.0[${PYTHON_USEDEP}] )
	tests? ( <dev-python/pytest-xdist-3.0[${PYTHON_USEDEP}] )
	tests? ( <dev-python/pytest-8.0[${PYTHON_USEDEP}] )
	tests? ( >=dev-python/referencing-0.35[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
