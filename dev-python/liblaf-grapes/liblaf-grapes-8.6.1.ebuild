# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="🍇 Supercharge your Python with rich logging, precise timing, and seamless serialization."

HOMEPAGE="https://github.com/liblaf/grapes"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE=""
DEPENDENCIES="dev-python/about-time[${PYTHON_USEDEP}]
	<dev-python/asttokens-4.0[${PYTHON_USEDEP}]
	<dev-python/attrs-26.0[${PYTHON_USEDEP}]
	dev-python/autoregistry[${PYTHON_USEDEP}]
	<dev-python/cachetools-7.0[${PYTHON_USEDEP}]
	dev-python/cytoolz[${PYTHON_USEDEP}]
	<dev-python/environs-15.0[${PYTHON_USEDEP}]
	<dev-python/executing-3.0[${PYTHON_USEDEP}]
	dev-python/fieldz[${PYTHON_USEDEP}]
	<dev-python/joblib-2.0[${PYTHON_USEDEP}]
	<dev-python/lazy-loader-0.5[${PYTHON_USEDEP}]
	<dev-python/msgspec-0.20[${PYTHON_USEDEP}]
	<dev-python/packaging-26.0[${PYTHON_USEDEP}]
	dev-python/pendulum[${PYTHON_USEDEP}]
	<dev-python/platformdirs-5.0[${PYTHON_USEDEP}]
	<dev-python/pydantic-3.0[${PYTHON_USEDEP}]
	<dev-python/rich-15.0[${PYTHON_USEDEP}]
	<dev-python/typing-extensions-5.0[${PYTHON_USEDEP}]
	dev-python/wadler-lindig[${PYTHON_USEDEP}]
	<dev-python/wrapt-3.0[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
