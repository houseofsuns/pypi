# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Skforecast is a Python library for time series forecasting using statistical and machine learning models. It works with any estimator compatible with the scikit-learn API, including popular options like LightGBM, XGBoost, CatBoost, Keras, and many others."

HOMEPAGE="https://www.skforecast.org"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all deeplearning docs full plotting stats test"
DEPENDENCIES=">=dev-python/numpy-1.24[${PYTHON_USEDEP}]
	>=dev-python/pandas-1.5[${PYTHON_USEDEP}]
	>=dev-python/tqdm-4.57[${PYTHON_USEDEP}]
	>=dev-python/scikit-learn-1.2[${PYTHON_USEDEP}]
	>=dev-python/scipy-1.3.2[${PYTHON_USEDEP}]
	dev-python/optuna[${PYTHON_USEDEP}]
	>=dev-python/joblib-1.1[${PYTHON_USEDEP}]
	dev-python/numba[${PYTHON_USEDEP}]
	>=dev-python/rich-13.9[${PYTHON_USEDEP}]
	stats? ( <dev-python/statsmodels-0.15[${PYTHON_USEDEP}] )
	plotting? ( <dev-python/matplotlib-3.11[${PYTHON_USEDEP}] )
	plotting? ( <dev-python/seaborn-0.14[${PYTHON_USEDEP}] )
	plotting? ( <dev-python/statsmodels-0.15[${PYTHON_USEDEP}] )
	deeplearning? ( dev-python/keras[${PYTHON_USEDEP}] )
	deeplearning? ( <dev-python/matplotlib-3.11[${PYTHON_USEDEP}] )
	all? ( <dev-python/matplotlib-3.11[${PYTHON_USEDEP}] )
	all? ( <dev-python/seaborn-0.14[${PYTHON_USEDEP}] )
	all? ( <dev-python/statsmodels-0.15[${PYTHON_USEDEP}] )
	all? ( dev-python/keras[${PYTHON_USEDEP}] )
	all? ( dev-python/tensorflow[${PYTHON_USEDEP}] )
	all? ( dev-python/torch[${PYTHON_USEDEP}] )
	full? ( dev-python/skforecast[${PYTHON_USEDEP}] )
	docs? ( dev-python/mike[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/mkdocs-1.6.1[${PYTHON_USEDEP}] )
	docs? ( dev-python/mkdocs-jupyter[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/mkdocs-material-9.7.0[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/mkdocstrings-0.30.1[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/mkdocstrings-python-1.19.0[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/notebook-6.5.7[${PYTHON_USEDEP}] )
	docs? ( dev-python/jupyter-contrib-nbextensions[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/black-25.11.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-7.3[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-cov-4.1[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-xdist-3.3[${PYTHON_USEDEP}] )
	test? ( <dev-python/statsmodels-0.15[${PYTHON_USEDEP}] )
	test? ( <dev-python/matplotlib-3.11[${PYTHON_USEDEP}] )
	test? ( <dev-python/seaborn-0.14[${PYTHON_USEDEP}] )
	test? ( dev-python/lightgbm[${PYTHON_USEDEP}] )
	test? ( dev-python/xgboost[${PYTHON_USEDEP}] )
	test? ( dev-python/keras[${PYTHON_USEDEP}] )
	test? ( dev-python/torch[${PYTHON_USEDEP}] )
	test? ( >=dev-python/tomli-2.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pandas-1.5[${PYTHON_USEDEP}] )
	test? ( dev-python/aeon[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
