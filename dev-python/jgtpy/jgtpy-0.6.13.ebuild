# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Enhanced JGTPy CDS, IDS, PDSP Services"

HOMEPAGE="https://github.com/jgwill/jgtpy"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev-docs dev-lint dev-test serve"
DEPENDENCIES=">=dev-python/pandas-0.25.1[${PYTHON_USEDEP}]
	>=dev-python/python-dotenv-0.19.2[${PYTHON_USEDEP}]
	dev-python/jgtapy[${PYTHON_USEDEP}]
	dev-python/jgtutils[${PYTHON_USEDEP}]
	dev-python/dropbox[${PYTHON_USEDEP}]
	dev-python/tlid[${PYTHON_USEDEP}]
	>=dev-python/scipy-1.7.3[${PYTHON_USEDEP}]
	>=dev-python/matplotlib-3.3.1[${PYTHON_USEDEP}]
	>=dev-python/plotly-5.18.0[${PYTHON_USEDEP}]
	dev-python/mplfinance[${PYTHON_USEDEP}]
	dev-python/panel[${PYTHON_USEDEP}]
	>=dev-python/tomli-1.2.3[${PYTHON_USEDEP}]
	dev-python/jinja[${PYTHON_USEDEP}]
	dev-python/dash[${PYTHON_USEDEP}]
	dev-python/tdqm[${PYTHON_USEDEP}]
	dev-python/jgtfxcon[${PYTHON_USEDEP}]
	serve? ( dev-python/fastapi[${PYTHON_USEDEP}] )
	serve? ( >=dev-python/uvicorn-0.24.0[${PYTHON_USEDEP}] )
	serve? ( >=dev-python/httpx-0.25.0[${PYTHON_USEDEP}] )
	serve? ( >=dev-python/aiofiles-23.2.1[${PYTHON_USEDEP}] )
	dev-lint? ( <dev-python/flake8-3.7.0[${PYTHON_USEDEP}] )
	dev-lint? ( <dev-python/isort-4.4.0[${PYTHON_USEDEP}] )
	dev-test? ( dev-python/coverage[${PYTHON_USEDEP}] )
	dev-test? ( >=dev-python/pytest-3.10[${PYTHON_USEDEP}] )
	dev-docs? ( dev-python/readme-renderer[${PYTHON_USEDEP}] )
	dev-docs? ( dev-python/sphinx[${PYTHON_USEDEP}] )
	dev-docs? ( >=dev-python/sphinx-rtd-theme-0.4.0[${PYTHON_USEDEP}] )
	dev-docs? ( >=dev-python/html2text-2020.1.16[${PYTHON_USEDEP}] )
	dev-docs? ( dev-python/html2markdown[${PYTHON_USEDEP}] )
	dev-docs? ( dev-python/twine[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
