# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Analysis tools for SL-compatible virtual worlds"

HOMEPAGE="https://github.com/SaladDais/Hippolyzer/"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="gui proxy"
DEPENDENCIES="<dev-python/aiohttp-4.0.0[${PYTHON_USEDEP}]
	dev-python/defusedxml[${PYTHON_USEDEP}]
	dev-python/gltflib[${PYTHON_USEDEP}]
	dev-python/Glymur[${PYTHON_USEDEP}]
	<dev-python/idna-3.0[${PYTHON_USEDEP}]
	dev-python/lazy-object-proxy[${PYTHON_USEDEP}]
	dev-python/llsd[${PYTHON_USEDEP}]
	<dev-python/numpy-2.0[${PYTHON_USEDEP}]
	dev-python/pycollada[${PYTHON_USEDEP}]
	dev-python/recordclass[${PYTHON_USEDEP}]
	dev-python/transformations[${PYTHON_USEDEP}]
	proxy? ( dev-python/arpeggio[${PYTHON_USEDEP}] )
	proxy? ( dev-python/mitmproxy[${PYTHON_USEDEP}] )
	proxy? ( dev-python/outleap[${PYTHON_USEDEP}] )
	proxy? ( dev-python/ptpython[${PYTHON_USEDEP}] )
	proxy? ( <dev-python/werkzeug-4.0[${PYTHON_USEDEP}] )
	gui? ( dev-python/hippolyzer[${PYTHON_USEDEP}] )
	gui? ( dev-python/PySide6-Essentials[${PYTHON_USEDEP}] )
	gui? ( dev-python/qasync[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
