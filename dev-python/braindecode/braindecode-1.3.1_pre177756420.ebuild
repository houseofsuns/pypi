# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="1.3.1.dev177756420"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Deep learning software to decode EEG, ECG or MEG signals"

HOMEPAGE="https://braindecode.org"
LICENSE="BSD"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all docs eegprep hub moabb tests typing"
DEPENDENCIES="dev-python/torch[${PYTHON_USEDEP}]
	dev-python/torchaudio[${PYTHON_USEDEP}]
	dev-python/mne[${PYTHON_USEDEP}]
	dev-python/mne-bids[${PYTHON_USEDEP}]
	dev-python/h5py[${PYTHON_USEDEP}]
	dev-python/skorch[${PYTHON_USEDEP}]
	dev-python/joblib[${PYTHON_USEDEP}]
	dev-python/torchinfo[${PYTHON_USEDEP}]
	dev-python/wfdb[${PYTHON_USEDEP}]
	dev-python/linear-attention-transformer[${PYTHON_USEDEP}]
	dev-python/docstring-inheritance[${PYTHON_USEDEP}]
	dev-python/rotary-embedding-torch[${PYTHON_USEDEP}]
	moabb? ( dev-python/moabb[${PYTHON_USEDEP}] )
	eegprep? ( dev-python/eegprep[${PYTHON_USEDEP}] )
	hub? ( dev-python/huggingface-hub[${PYTHON_USEDEP}] )
	hub? ( dev-python/zarr[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	tests? ( dev-python/codecov[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-cases[${PYTHON_USEDEP}] )
	tests? ( dev-python/mypy[${PYTHON_USEDEP}] )
	tests? ( dev-python/transformers[${PYTHON_USEDEP}] )
	tests? ( dev-python/bids-validator[${PYTHON_USEDEP}] )
	typing? ( dev-python/exca[${PYTHON_USEDEP}] )
	typing? ( dev-python/numpydantic[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-gallery[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-rtd-theme[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-autodoc-typehints[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-autobuild[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinxcontrib-bibtex[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-sitemap[${PYTHON_USEDEP}] )
	docs? ( dev-python/pydata-sphinx-theme[${PYTHON_USEDEP}] )
	docs? ( dev-python/numpydoc[${PYTHON_USEDEP}] )
	docs? ( dev-python/memory-profiler[${PYTHON_USEDEP}] )
	docs? ( dev-python/pillow[${PYTHON_USEDEP}] )
	docs? ( dev-python/ipython[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx_design[${PYTHON_USEDEP}] )
	docs? ( dev-python/lightning[${PYTHON_USEDEP}] )
	docs? ( dev-python/seaborn[${PYTHON_USEDEP}] )
	docs? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	docs? ( dev-python/openneuro-py[${PYTHON_USEDEP}] )
	docs? ( dev-python/plotly[${PYTHON_USEDEP}] )
	docs? ( dev-python/shap[${PYTHON_USEDEP}] )
	docs? ( dev-python/nbformat[${PYTHON_USEDEP}] )
	docs? ( dev-python/transformers[${PYTHON_USEDEP}] )
	all? ( dev-python/braindecode[${PYTHON_USEDEP}] )
	all? ( dev-python/braindecode[${PYTHON_USEDEP}] )
	all? ( dev-python/braindecode[${PYTHON_USEDEP}] )
	all? ( dev-python/braindecode[${PYTHON_USEDEP}] )
	all? ( dev-python/braindecode[${PYTHON_USEDEP}] )
	all? ( dev-python/braindecode[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
