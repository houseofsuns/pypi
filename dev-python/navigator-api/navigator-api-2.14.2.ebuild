# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=wheel

inherit python-r1 gs-pypi

DESCRIPTION="Navigator: a batteries-included Web Framework based on aiohttp [wheel]"

HOMEPAGE="https://github.com/phenobarbital/navigator"
LICENSE="BSD"
SRC_URI="https://files.pythonhosted.org/packages/cp311/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}-cp311-cp311-manylinux2014_x86_64.manylinux_2_17_x86_64.whl"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}-cp311-cp311-manylinux2014_x86_64.manylinux_2_17_x86_64.whl"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all build dev docs gunicorn locale memcache production test uvloop"
DEPENDENCIES=">=dev-python/cython-3.0.11[${PYTHON_USEDEP}]
	>=dev-python/aiohttp-3.10.0[${PYTHON_USEDEP}]
	>=dev-python/pysocks-1.7.1[${PYTHON_USEDEP}]
	>=dev-python/asn1crypto-1.4.0[${PYTHON_USEDEP}]
	>=dev-python/jinja2-3.1.6[${PYTHON_USEDEP}]
	dev-python/psycopg2-binary[${PYTHON_USEDEP}]
	dev-python/aiosocks[${PYTHON_USEDEP}]
	>=dev-python/python-slugify-8.0.1[${PYTHON_USEDEP}]
	dev-python/proxylists[${PYTHON_USEDEP}]
	<=dev-python/httpx-0.28.1[${PYTHON_USEDEP}]
	>=dev-python/beautifulsoup4-4.12.3[${PYTHON_USEDEP}]
	dev-python/polyline[${PYTHON_USEDEP}]
	dev-python/Cartopy[${PYTHON_USEDEP}]
	>=dev-python/matplotlib-3.8.3[${PYTHON_USEDEP}]
	dev-python/sockjs[${PYTHON_USEDEP}]
	dev-python/aiohttp-sse[${PYTHON_USEDEP}]
	dev-python/asyncdb[${PYTHON_USEDEP}]
	<dev-python/pyarrow-21.0.0[${PYTHON_USEDEP}]
	dev-python/navconfig[${PYTHON_USEDEP}]
	dev-python/alt-aiohttp-cors[${PYTHON_USEDEP}]
	dev-python/Brotli[${PYTHON_USEDEP}]
	>=dev-python/brotlicffi-1.1.0.0[${PYTHON_USEDEP}]
	dev-python/aiofile[${PYTHON_USEDEP}]
	>=dev-python/psutil-6.0.0[${PYTHON_USEDEP}]
	dev-python/aiormq[${PYTHON_USEDEP}]
	>=dev-python/faker-22.2.0[${PYTHON_USEDEP}]
	dev-python/google-cloud-core[${PYTHON_USEDEP}]
	dev-python/google-cloud-storage[${PYTHON_USEDEP}]
	>=dev-python/cloudpickle-3.0.0[${PYTHON_USEDEP}]
	>=dev-python/async-timeout-4.0.3[${PYTHON_USEDEP}]
	>=dev-python/msgpack-1.1.0[${PYTHON_USEDEP}]
	<=dev-python/babel-2.17.0[${PYTHON_USEDEP}]
	>=dev-python/watchdog-6.0.0[${PYTHON_USEDEP}]
	dev-python/navigator-session[${PYTHON_USEDEP}]
	dev-python/navigator-auth[${PYTHON_USEDEP}]
	dev-python/python-datamodel[${PYTHON_USEDEP}]
	locale? ( <=dev-python/babel-2.17.0[${PYTHON_USEDEP}] )
	memcache? ( dev-python/aiomcache[${PYTHON_USEDEP}] )
	uvloop? ( >=dev-python/uvloop-0.21.0[${PYTHON_USEDEP}] )
	gunicorn? ( dev-python/gunicorn[${PYTHON_USEDEP}] )
	production? ( >=dev-python/uvloop-0.21.0[${PYTHON_USEDEP}] )
	production? ( dev-python/gunicorn[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-5.4.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-asyncio-0.21.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-xdist-2.5.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-assume[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/coverage-6.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/mypy-1.0.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/black-23.0.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/isort-5.12.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pylint-2.17.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	build? ( >=dev-python/setuptools-67.6.1[${PYTHON_USEDEP}] )
	build? ( ~dev-python/cython-3.0.11[${PYTHON_USEDEP}] )
	build? ( >=dev-python/wheel-0.44.0[${PYTHON_USEDEP}] )
	build? ( >=dev-python/build-0.10.0[${PYTHON_USEDEP}] )
	build? ( dev-python/python-datamodel[${PYTHON_USEDEP}] )
	build? ( dev-python/navconfig[${PYTHON_USEDEP}] )
	build? ( >=dev-python/setuptools-scm-6.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-5.4.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-asyncio-0.21.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-xdist-2.5.0[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-assume[${PYTHON_USEDEP}] )
	test? ( >=dev-python/coverage-6.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/aioresponses-0.7.4[${PYTHON_USEDEP}] )
	test? ( dev-python/asynctest[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-6.0.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-rtd-theme-1.2.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/myst-parser-1.0.0[${PYTHON_USEDEP}] )
	all? ( dev-python/navigator-api[${PYTHON_USEDEP}] )
	virtual/allow-pypi-wheels"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
