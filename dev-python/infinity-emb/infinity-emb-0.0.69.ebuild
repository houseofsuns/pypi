# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Infinity is a high-throughput, low-latency REST API for serving text-embeddings, reranking models and clip."

HOMEPAGE="https://github.com/michaelfeil/infinity"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all audio cache ct2 einops logging onnxruntime-gpu openvino optimum server tensorrt torch vision"
DEPENDENCIES="vision? ( dev-python/colpali-engine[${PYTHON_USEDEP}] )
	all? ( dev-python/colpali-engine[${PYTHON_USEDEP}] )
	ct2? ( dev-python/ctranslate2[${PYTHON_USEDEP}] )
	all? ( dev-python/ctranslate2[${PYTHON_USEDEP}] )
	cache? ( dev-python/diskcache[${PYTHON_USEDEP}] )
	all? ( dev-python/diskcache[${PYTHON_USEDEP}] )
	einops? ( dev-python/einops[${PYTHON_USEDEP}] )
	all? ( dev-python/einops[${PYTHON_USEDEP}] )
	server? ( dev-python/fastapi[${PYTHON_USEDEP}] )
	all? ( dev-python/fastapi[${PYTHON_USEDEP}] )
	dev-python/hf-transfer[${PYTHON_USEDEP}]
	dev-python/huggingface-hub[${PYTHON_USEDEP}]
	<dev-python/numpy-2.0[${PYTHON_USEDEP}]
	onnxruntime-gpu? ( dev-python/onnxruntime-gpu[${PYTHON_USEDEP}] )
	openvino? ( dev-python/onnxruntime-openvino[${PYTHON_USEDEP}] )
	openvino? ( dev-python/openvino[${PYTHON_USEDEP}] )
	openvino? ( dev-python/openvino-tokenizers[${PYTHON_USEDEP}] )
	optimum? ( dev-python/optimum[${PYTHON_USEDEP}] )
	all? ( dev-python/optimum[${PYTHON_USEDEP}] )
	server? ( >=dev-python/orjson-3.9.8[${PYTHON_USEDEP}] )
	all? ( >=dev-python/orjson-3.9.8[${PYTHON_USEDEP}] )
	vision? ( dev-python/pillow[${PYTHON_USEDEP}] )
	all? ( dev-python/pillow[${PYTHON_USEDEP}] )
	server? ( dev-python/posthog[${PYTHON_USEDEP}] )
	all? ( dev-python/posthog[${PYTHON_USEDEP}] )
	server? ( dev-python/prometheus-fastapi-instrumentator[${PYTHON_USEDEP}] )
	all? ( dev-python/prometheus-fastapi-instrumentator[${PYTHON_USEDEP}] )
	server? ( <dev-python/pydantic-3.0[${PYTHON_USEDEP}] )
	all? ( <dev-python/pydantic-3.0[${PYTHON_USEDEP}] )
	logging? ( <dev-python/rich-14.0[${PYTHON_USEDEP}] )
	server? ( <dev-python/rich-14.0[${PYTHON_USEDEP}] )
	all? ( <dev-python/rich-14.0[${PYTHON_USEDEP}] )
	ct2? ( dev-python/sentence-transformers[${PYTHON_USEDEP}] )
	torch? ( dev-python/sentence-transformers[${PYTHON_USEDEP}] )
	all? ( dev-python/sentence-transformers[${PYTHON_USEDEP}] )
	audio? ( dev-python/soundfile[${PYTHON_USEDEP}] )
	all? ( dev-python/soundfile[${PYTHON_USEDEP}] )
	tensorrt? ( dev-python/tensorrt[${PYTHON_USEDEP}] )
	vision? ( dev-python/timm[${PYTHON_USEDEP}] )
	all? ( dev-python/timm[${PYTHON_USEDEP}] )
	ct2? ( dev-python/torch[${PYTHON_USEDEP}] )
	torch? ( dev-python/torch[${PYTHON_USEDEP}] )
	all? ( dev-python/torch[${PYTHON_USEDEP}] )
	vision? ( dev-python/torchvision[${PYTHON_USEDEP}] )
	all? ( dev-python/torchvision[${PYTHON_USEDEP}] )
	ct2? ( dev-python/transformers[${PYTHON_USEDEP}] )
	server? ( dev-python/typer[${PYTHON_USEDEP}] )
	all? ( dev-python/typer[${PYTHON_USEDEP}] )
	server? ( <dev-python/uvicorn-0.33.0[${PYTHON_USEDEP}] )
	all? ( <dev-python/uvicorn-0.33.0[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
