# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="0.18.1rc0"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="RecSys Library"

HOMEPAGE="https://sb-ai-lab.github.io/RePlay/"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all spark torch torch-openvino"
DEPENDENCIES="dev-python/d3rlpy[${PYTHON_USEDEP}]
	dev-python/fixed-install-nmslib[${PYTHON_USEDEP}]
	dev-python/gym[${PYTHON_USEDEP}]
	dev-python/hnswlib[${PYTHON_USEDEP}]
	dev-python/implicit[${PYTHON_USEDEP}]
	dev-python/lightautoml[${PYTHON_USEDEP}]
	dev-python/lightfm[${PYTHON_USEDEP}]
	torch? ( dev-python/lightning[${PYTHON_USEDEP}] )
	torch-openvino? ( dev-python/lightning[${PYTHON_USEDEP}] )
	all? ( dev-python/lightning[${PYTHON_USEDEP}] )
	dev-python/llvmlite[${PYTHON_USEDEP}]
	dev-python/numba[${PYTHON_USEDEP}]
	>=dev-python/numpy-1.20.0[${PYTHON_USEDEP}]
	torch-openvino? ( dev-python/onnx[${PYTHON_USEDEP}] )
	all? ( dev-python/onnx[${PYTHON_USEDEP}] )
	torch-openvino? ( dev-python/openvino[${PYTHON_USEDEP}] )
	all? ( dev-python/openvino[${PYTHON_USEDEP}] )
	dev-python/optuna[${PYTHON_USEDEP}]
	<=dev-python/pandas-2.2.2[${PYTHON_USEDEP}]
	dev-python/polars[${PYTHON_USEDEP}]
	<dev-python/psutil-6.1.0[${PYTHON_USEDEP}]
	>=dev-python/pyarrow-12.0.1[${PYTHON_USEDEP}]
	spark? ( dev-python/pyspark[${PYTHON_USEDEP}] )
	all? ( dev-python/pyspark[${PYTHON_USEDEP}] )
	spark? ( dev-python/pyspark[${PYTHON_USEDEP}] )
	all? ( dev-python/pyspark[${PYTHON_USEDEP}] )
	torch? ( dev-python/pytorch-ranger[${PYTHON_USEDEP}] )
	torch-openvino? ( dev-python/pytorch-ranger[${PYTHON_USEDEP}] )
	all? ( dev-python/pytorch-ranger[${PYTHON_USEDEP}] )
	dev-python/sb-obp[${PYTHON_USEDEP}]
	<dev-python/scikit-learn-2.0.0[${PYTHON_USEDEP}]
	<dev-python/scipy-2.0.0[${PYTHON_USEDEP}]
	torch? ( dev-python/torch[${PYTHON_USEDEP}] )
	torch-openvino? ( dev-python/torch[${PYTHON_USEDEP}] )
	all? ( dev-python/torch[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
