# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A python library for easy connecting between OMERO (jobs) and a Slurm cluster"

HOMEPAGE="https://github.com/NL-BioImaging/biomero"
LICENSE="Apache License"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="full test"
DEPENDENCIES="~dev-python/requests-2.32.4[${PYTHON_USEDEP}]
	~dev-python/requests-cache-1.1.1[${PYTHON_USEDEP}]
	dev-python/fabric[${PYTHON_USEDEP}]
	~dev-python/paramiko-3.4.0[${PYTHON_USEDEP}]
	>=dev-python/importlib-resources-5.4.0[${PYTHON_USEDEP}]
	dev-python/eventsourcing[${PYTHON_USEDEP}]
	~dev-python/sqlalchemy-2.0.32[${PYTHON_USEDEP}]
	dev-python/psycopg2[${PYTHON_USEDEP}]
	dev-python/eventsourcing-sqlalchemy[${PYTHON_USEDEP}]
	full? ( dev-python/ezomero[${PYTHON_USEDEP}] )
	full? ( >=dev-python/tifffile-2020.9.3[${PYTHON_USEDEP}] )
	full? ( dev-python/omero-metadata[${PYTHON_USEDEP}] )
	full? ( dev-python/omero-cli-zarr[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	test? ( dev-python/mock[${PYTHON_USEDEP}] )
	test? ( dev-python/flake8[${PYTHON_USEDEP}] )
	test? ( dev-python/psycopg2-binary[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
