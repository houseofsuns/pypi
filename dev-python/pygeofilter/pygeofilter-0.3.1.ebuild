# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="pygeofilter is a pure Python parser implementation of OGC filtering standards"

HOMEPAGE="https://github.com/geopython/pygeofilter"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="backend-django backend-elasticsearch backend-native backend-opensearch backend-sqlalchemy fes"
DEPENDENCIES="dev-python/dateparser[${PYTHON_USEDEP}]
	dev-python/lark[${PYTHON_USEDEP}]
	dev-python/pygeoif[${PYTHON_USEDEP}]
	backend-django? ( dev-python/django[${PYTHON_USEDEP}] )
	backend-sqlalchemy? ( dev-python/GeoAlchemy2[${PYTHON_USEDEP}] )
	backend-sqlalchemy? ( dev-python/sqlalchemy[${PYTHON_USEDEP}] )
	backend-native? ( dev-python/shapely[${PYTHON_USEDEP}] )
	backend-elasticsearch? ( dev-python/elasticsearch[${PYTHON_USEDEP}] )
	backend-elasticsearch? ( dev-python/elasticsearch-dsl[${PYTHON_USEDEP}] )
	backend-opensearch? ( dev-python/opensearch-py[${PYTHON_USEDEP}] )
	backend-opensearch? ( dev-python/opensearch-dsl[${PYTHON_USEDEP}] )
	fes? ( dev-python/pygml[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
