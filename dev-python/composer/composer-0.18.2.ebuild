...
IUSE="all coco comet_ml databricks deepspeed dev gcs health_checker libcloud mlflow mlperf nlp oci onnx pandas sentencepiece slack streaming system_metrics_monitor tensorboard timm unet vit wandb"
DEPENDENCIES="<dev-python/pyyaml-7.0[${PYTHON_USEDEP}]
	...
	all? ( ~dev-python/coverage-7.3.4[${PYTHON_USEDEP}] )
	all? ( dev-python/sphinxext-opengraph[${PYTHON_USEDEP}] )
	all? ( dev-python/monai[${PYTHON_USEDEP}] )
	all? ( ~dev-python/ipykernel-6.28.0[${PYTHON_USEDEP}] )
	...
	dev? ( ~dev-python/ipykernel-6.28.0[${PYTHON_USEDEP}] )
	...
	system_metrics_monitor? ( dev-python/pynvml[${PYTHON_USEDEP}] )
