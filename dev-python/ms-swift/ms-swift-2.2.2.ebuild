# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Swift: Scalable lightWeight Infrastructure for Fine-Tuning"

HOMEPAGE="https://github.com/modelscope/swift"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="aigc all eval llm seq-parallel"
DEPENDENCIES="dev-python/accelerate[${PYTHON_USEDEP}]
	dev-python/addict[${PYTHON_USEDEP}]
	dev-python/aiohttp[${PYTHON_USEDEP}]
	dev-python/binpacking[${PYTHON_USEDEP}]
	dev-python/dacite[${PYTHON_USEDEP}]
	dev-python/datasets[${PYTHON_USEDEP}]
	dev-python/einops[${PYTHON_USEDEP}]
	dev-python/importlib-metadata[${PYTHON_USEDEP}]
	dev-python/jieba[${PYTHON_USEDEP}]
	dev-python/matplotlib[${PYTHON_USEDEP}]
	dev-python/modelscope[${PYTHON_USEDEP}]
	dev-python/nltk[${PYTHON_USEDEP}]
	<dev-python/numpy-2.0[${PYTHON_USEDEP}]
	dev-python/optimum[${PYTHON_USEDEP}]
	dev-python/oss2[${PYTHON_USEDEP}]
	dev-python/pandas[${PYTHON_USEDEP}]
	dev-python/peft[${PYTHON_USEDEP}]
	dev-python/requests[${PYTHON_USEDEP}]
	dev-python/rouge[${PYTHON_USEDEP}]
	dev-python/safetensors[${PYTHON_USEDEP}]
	dev-python/tensorboard[${PYTHON_USEDEP}]
	dev-python/tqdm[${PYTHON_USEDEP}]
	dev-python/transformers[${PYTHON_USEDEP}]
	dev-python/transformers-stream-generator[${PYTHON_USEDEP}]
	dev-python/trl[${PYTHON_USEDEP}]
	aigc? ( dev-python/decord[${PYTHON_USEDEP}] )
	aigc? ( dev-python/diffusers[${PYTHON_USEDEP}] )
	aigc? ( dev-python/einops[${PYTHON_USEDEP}] )
	aigc? ( dev-python/torchvision[${PYTHON_USEDEP}] )
	all? ( dev-python/accelerate[${PYTHON_USEDEP}] )
	all? ( dev-python/addict[${PYTHON_USEDEP}] )
	all? ( dev-python/aiohttp[${PYTHON_USEDEP}] )
	all? ( dev-python/binpacking[${PYTHON_USEDEP}] )
	all? ( dev-python/dacite[${PYTHON_USEDEP}] )
	all? ( dev-python/datasets[${PYTHON_USEDEP}] )
	all? ( dev-python/einops[${PYTHON_USEDEP}] )
	all? ( dev-python/importlib-metadata[${PYTHON_USEDEP}] )
	all? ( dev-python/jieba[${PYTHON_USEDEP}] )
	all? ( dev-python/matplotlib[${PYTHON_USEDEP}] )
	all? ( dev-python/modelscope[${PYTHON_USEDEP}] )
	all? ( dev-python/nltk[${PYTHON_USEDEP}] )
	all? ( <dev-python/numpy-2.0[${PYTHON_USEDEP}] )
	all? ( dev-python/optimum[${PYTHON_USEDEP}] )
	all? ( dev-python/oss2[${PYTHON_USEDEP}] )
	all? ( dev-python/pandas[${PYTHON_USEDEP}] )
	all? ( dev-python/peft[${PYTHON_USEDEP}] )
	all? ( dev-python/requests[${PYTHON_USEDEP}] )
	all? ( dev-python/rouge[${PYTHON_USEDEP}] )
	all? ( dev-python/safetensors[${PYTHON_USEDEP}] )
	all? ( dev-python/tensorboard[${PYTHON_USEDEP}] )
	all? ( dev-python/tqdm[${PYTHON_USEDEP}] )
	all? ( dev-python/transformers[${PYTHON_USEDEP}] )
	all? ( dev-python/transformers-stream-generator[${PYTHON_USEDEP}] )
	all? ( dev-python/trl[${PYTHON_USEDEP}] )
	all? ( dev-python/attrdict[${PYTHON_USEDEP}] )
	all? ( dev-python/charset-normalizer[${PYTHON_USEDEP}] )
	all? ( dev-python/cpm-kernels[${PYTHON_USEDEP}] )
	all? ( dev-python/fastapi[${PYTHON_USEDEP}] )
	all? ( dev-python/gradio[${PYTHON_USEDEP}] )
	all? ( dev-python/openai[${PYTHON_USEDEP}] )
	all? ( dev-python/sentencepiece[${PYTHON_USEDEP}] )
	all? ( dev-python/tiktoken[${PYTHON_USEDEP}] )
	all? ( dev-python/uvicorn[${PYTHON_USEDEP}] )
	all? ( dev-python/decord[${PYTHON_USEDEP}] )
	all? ( dev-python/diffusers[${PYTHON_USEDEP}] )
	all? ( dev-python/torchvision[${PYTHON_USEDEP}] )
	all? ( dev-python/llmuses[${PYTHON_USEDEP}] )
	all? ( dev-python/ms-opencompass[${PYTHON_USEDEP}] )
	all? ( dev-python/xtuner[${PYTHON_USEDEP}] )
	eval? ( dev-python/llmuses[${PYTHON_USEDEP}] )
	eval? ( dev-python/ms-opencompass[${PYTHON_USEDEP}] )
	llm? ( dev-python/attrdict[${PYTHON_USEDEP}] )
	llm? ( dev-python/charset-normalizer[${PYTHON_USEDEP}] )
	llm? ( dev-python/cpm-kernels[${PYTHON_USEDEP}] )
	llm? ( dev-python/fastapi[${PYTHON_USEDEP}] )
	llm? ( dev-python/gradio[${PYTHON_USEDEP}] )
	llm? ( dev-python/openai[${PYTHON_USEDEP}] )
	llm? ( dev-python/sentencepiece[${PYTHON_USEDEP}] )
	llm? ( dev-python/tiktoken[${PYTHON_USEDEP}] )
	llm? ( dev-python/uvicorn[${PYTHON_USEDEP}] )
	seq-parallel? ( dev-python/xtuner[${PYTHON_USEDEP}] )
	seq-parallel? ( dev-python/attrdict[${PYTHON_USEDEP}] )
	seq-parallel? ( dev-python/charset-normalizer[${PYTHON_USEDEP}] )
	seq-parallel? ( dev-python/cpm-kernels[${PYTHON_USEDEP}] )
	seq-parallel? ( dev-python/fastapi[${PYTHON_USEDEP}] )
	seq-parallel? ( dev-python/gradio[${PYTHON_USEDEP}] )
	seq-parallel? ( dev-python/openai[${PYTHON_USEDEP}] )
	seq-parallel? ( dev-python/sentencepiece[${PYTHON_USEDEP}] )
	seq-parallel? ( dev-python/tiktoken[${PYTHON_USEDEP}] )
	seq-parallel? ( dev-python/uvicorn[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
