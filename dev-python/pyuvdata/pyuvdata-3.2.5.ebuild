# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A pythonic interface for radio astronomy interferometry data"

HOMEPAGE=""
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all astroquery casa dev doc hdf5-compression healpix lunar novas plotting test tutorial"
DEPENDENCIES="dev-python/astropy[${PYTHON_USEDEP}]
	dev-python/docstring_parser[${PYTHON_USEDEP}]
	>=dev-python/h5py-3.7[${PYTHON_USEDEP}]
	dev-python/numba[${PYTHON_USEDEP}]
	>=dev-python/numpy-1.23[${PYTHON_USEDEP}]
	dev-python/pyerfa[${PYTHON_USEDEP}]
	>=dev-python/pyyaml-5.4.1[${PYTHON_USEDEP}]
	>=dev-python/scipy-1.9[${PYTHON_USEDEP}]
	>=dev-python/setuptools-scm-8.1[${PYTHON_USEDEP}]
	astroquery? ( dev-python/astroquery[${PYTHON_USEDEP}] )
	casa? ( dev-python/python-casacore[${PYTHON_USEDEP}] )
	casa? ( <dev-python/numpy-2.0[${PYTHON_USEDEP}] )
	hdf5-compression? ( dev-python/hdf5plugin[${PYTHON_USEDEP}] )
	healpix? ( dev-python/astropy-healpix[${PYTHON_USEDEP}] )
	lunar? ( dev-python/lunarsky[${PYTHON_USEDEP}] )
	novas? ( dev-python/novas[${PYTHON_USEDEP}] )
	novas? ( dev-python/novas_de405[${PYTHON_USEDEP}] )
	plotting? ( >=dev-python/matplotlib-3.7[${PYTHON_USEDEP}] )
	all? ( dev-python/pyuvdata[${PYTHON_USEDEP}] )
	tutorial? ( dev-python/pyuvdata[${PYTHON_USEDEP}] )
	tutorial? ( >=dev-python/pooch-1.8[${PYTHON_USEDEP}] )
	test? ( dev-python/coverage[${PYTHON_USEDEP}] )
	test? ( >=dev-python/cython-3.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pooch-1.8[${PYTHON_USEDEP}] )
	test? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-8.2.0[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-xdist[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cases[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	test? ( >=dev-python/sybil-8.0[${PYTHON_USEDEP}] )
	doc? ( dev-python/pyuvdata[${PYTHON_USEDEP}] )
	doc? ( dev-python/pypandoc[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx[${PYTHON_USEDEP}] )
	dev? ( dev-python/pyuvdata[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
