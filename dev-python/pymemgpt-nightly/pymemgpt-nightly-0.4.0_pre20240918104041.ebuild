# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="0.4.0.dev20240918104041"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Create LLM agents with long-term memory and custom tools"

HOMEPAGE=""
LICENSE="Apache License"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="autogen crewai-tools dev local milvus ollama postgres qdrant server"
DEPENDENCIES="<dev-python/python-box-8.0.0[${PYTHON_USEDEP}]
	milvus? ( dev-python/pymilvus[${PYTHON_USEDEP}] )
	dev-python/typer[${PYTHON_USEDEP}]
	dev-python/questionary[${PYTHON_USEDEP}]
	<dev-python/pytz-2024.0[${PYTHON_USEDEP}]
	<dev-python/tqdm-5.0.0[${PYTHON_USEDEP}]
	dev? ( <dev-python/black-25.0.0[${PYTHON_USEDEP}] )
	<dev-python/setuptools-69.0.0[${PYTHON_USEDEP}]
	dev? ( dev-python/datasets[${PYTHON_USEDEP}] )
	<dev-python/prettytable-4.0.0[${PYTHON_USEDEP}]
	postgres? ( dev-python/pgvector[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	postgres? ( dev-python/pg8000[${PYTHON_USEDEP}] )
	server? ( <dev-python/websockets-13.0[${PYTHON_USEDEP}] )
	dev-python/docstring-parser[${PYTHON_USEDEP}]
	<dev-python/httpx-0.26.0[${PYTHON_USEDEP}]
	<dev-python/numpy-2.0.0[${PYTHON_USEDEP}]
	dev-python/demjson3[${PYTHON_USEDEP}]
	<dev-python/pyyaml-7.0.0[${PYTHON_USEDEP}]
	dev-python/chromadb[${PYTHON_USEDEP}]
	dev-python/sqlalchemy-json[${PYTHON_USEDEP}]
	server? ( dev-python/fastapi[${PYTHON_USEDEP}] )
	server? ( <dev-python/uvicorn-0.25.0[${PYTHON_USEDEP}] )
	<dev-python/pydantic-3.0.0[${PYTHON_USEDEP}]
	autogen? ( dev-python/pyautogen[${PYTHON_USEDEP}] )
	<dev-python/html2text-2021.0.0[${PYTHON_USEDEP}]
	dev-python/docx2txt[${PYTHON_USEDEP}]
	<dev-python/sqlalchemy-3.0.0[${PYTHON_USEDEP}]
	dev? ( <dev-python/pexpect-5.0.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/pyright[${PYTHON_USEDEP}] )
	qdrant? ( dev-python/qdrant-client[${PYTHON_USEDEP}] )
	dev-python/sqlmodel[${PYTHON_USEDEP}]
	dev? ( dev-python/autoflake[${PYTHON_USEDEP}] )
	dev-python/llama-index[${PYTHON_USEDEP}]
	dev-python/llama-index-embeddings-openai[${PYTHON_USEDEP}]
	local? ( dev-python/llama-index-embeddings-huggingface[${PYTHON_USEDEP}] )
	dev-python/llama-index-embeddings-azure-openai[${PYTHON_USEDEP}]
	<dev-python/python-multipart-0.0.10[${PYTHON_USEDEP}]
	dev-python/SQLAlchemy-Utils[${PYTHON_USEDEP}]
	dev? ( <dev-python/pytest-order-2.0.0[${PYTHON_USEDEP}] )
	dev? ( <dev-python/pytest-asyncio-0.24.0[${PYTHON_USEDEP}] )
	dev-python/pydantic-settings[${PYTHON_USEDEP}]
	dev-python/httpx-sse[${PYTHON_USEDEP}]
	dev? ( <dev-python/isort-6.0.0[${PYTHON_USEDEP}] )
	ollama? ( dev-python/llama-index-embeddings-ollama[${PYTHON_USEDEP}] )
	crewai-tools? ( dev-python/crewai[${PYTHON_USEDEP}] )
	crewai-tools? ( dev-python/crewai-tools[${PYTHON_USEDEP}] )
	crewai-tools? ( <dev-python/docker-8.0.0[${PYTHON_USEDEP}] )
	dev-python/tiktoken[${PYTHON_USEDEP}]
	dev-python/nltk[${PYTHON_USEDEP}]
	<dev-python/jinja-4.0.0[${PYTHON_USEDEP}]
	dev-python/locust[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
