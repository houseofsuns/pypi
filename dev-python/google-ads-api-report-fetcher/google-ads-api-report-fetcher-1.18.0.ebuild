# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Library for fetching reports from Google Ads API and saving them locally & remotely."

HOMEPAGE="https://github.com/google/ads-api-reports-fetcher"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="bq full pandas sheets simulator sqlalchemy"
DEPENDENCIES="dev-python/google-ads[${PYTHON_USEDEP}]
	dev-python/smart-open[${PYTHON_USEDEP}]
	dev-python/jinja2[${PYTHON_USEDEP}]
	dev-python/python-dateutil[${PYTHON_USEDEP}]
	dev-python/typing-extensions[${PYTHON_USEDEP}]
	dev-python/rich[${PYTHON_USEDEP}]
	dev-python/tenacity[${PYTHON_USEDEP}]
	dev-python/garf-core[${PYTHON_USEDEP}]
	dev-python/garf-io[${PYTHON_USEDEP}]
	dev-python/garf-executors[${PYTHON_USEDEP}]
	bq? ( dev-python/garf-executors[${PYTHON_USEDEP}] )
	pandas? ( >=dev-python/pandas-1.3.4[${PYTHON_USEDEP}] )
	sqlalchemy? ( dev-python/garf-executors[${PYTHON_USEDEP}] )
	simulator? ( dev-python/faker[${PYTHON_USEDEP}] )
	sheets? ( dev-python/garf-io[${PYTHON_USEDEP}] )
	full? ( dev-python/google-ads-api-report-fetcher[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
