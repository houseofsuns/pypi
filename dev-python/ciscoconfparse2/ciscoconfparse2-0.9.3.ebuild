# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Parse, Audit, Query, Build, and Modify Cisco IOS-style and JunOS-style configs"

HOMEPAGE="http://github.com/mpenning/ciscoconfparse2"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev doc"
DEPENDENCIES="<dev-python/attrs-26.0.0[${PYTHON_USEDEP}]
	~dev-python/dnspython-2.7.0[${PYTHON_USEDEP}]
	dev-python/hier-config[${PYTHON_USEDEP}]
	~dev-python/hypothesis-6.141.1[${PYTHON_USEDEP}]
	dev-python/libpass[${PYTHON_USEDEP}]
	>=dev-python/loguru-0.7.2[${PYTHON_USEDEP}]
	dev-python/macaddress[${PYTHON_USEDEP}]
	~dev-python/pyparsing-3.3.1[${PYTHON_USEDEP}]
	>=dev-python/pyyaml-6.0.2[${PYTHON_USEDEP}]
	>=dev-python/rich-13.6.0[${PYTHON_USEDEP}]
	dev-python/toml[${PYTHON_USEDEP}]
	~dev-python/traitlets-5.14.3[${PYTHON_USEDEP}]
	~dev-python/typeguard-4.4.4[${PYTHON_USEDEP}]
	dev? ( ~dev-python/black-25.11.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/commitizen[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/coverage-7.3.2[${PYTHON_USEDEP}] )
	dev? ( dev-python/fabric[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/hatch-1.15.1[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/hatchling-1.27.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/invoke[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	dev? ( dev-python/pylama[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pylint-3.3.9[${PYTHON_USEDEP}] )
	dev? ( dev-python/pyroma[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-cov-4.1.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-9.0.1[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/python-lsp-server-1.12.2[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pyupgrade-3.21.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/tox-4.30.3[${PYTHON_USEDEP}] )
	dev? ( dev-python/yamlfix[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/numpydoc-1.9.0[${PYTHON_USEDEP}] )
	doc? ( dev-python/pydocstyle[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/sphinx-rtd-theme-2.0.0[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/sphinx-9.0.4[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
