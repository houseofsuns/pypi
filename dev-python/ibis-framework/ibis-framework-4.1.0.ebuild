# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="ibis-framework"
REALVERSION="4.1.0"
PYTHON_COMPAT=( python{3_9,3_10,3_11} )
REPO_URI="http://pypi.python.org/packages/source/${REALNAME:0:1}/${REALNAME}/"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
DISTUTILS_USE_PEP517=standalone

inherit gs-pypi

DESCRIPTION="Productivity-centric Python Big Data Framework"

HOMEPAGE="https://ibis-project.org"
LICENSE="Apache-2.0"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all bigquery clickhouse dask datafusion decompiler duckdb geospatial impala mssql mysql pandas polars postgres pyspark snowflake sqlite trino visualization"
DEPENDENCIES="dev-python/atpublic[${PYTHON_USEDEP}]
	dev-python/multipledispatch[${PYTHON_USEDEP}]
	dev-python/numpy[${PYTHON_USEDEP}]
	dev-python/pandas[${PYTHON_USEDEP}]
	dev-python/parsy[${PYTHON_USEDEP}]
	dev-python/python-dateutil[${PYTHON_USEDEP}]
	dev-python/pytz[${PYTHON_USEDEP}]
	dev-python/rich[${PYTHON_USEDEP}]
	dev-python/sqlglot[${PYTHON_USEDEP}]
	dev-python/toolz[${PYTHON_USEDEP}]
	dev-python/typing-extensions[${PYTHON_USEDEP}]
	all? ( dev-python/black[${PYTHON_USEDEP}] )
	decompiler? ( dev-python/black[${PYTHON_USEDEP}] )
	all? ( dev-python/clickhouse-cityhash[${PYTHON_USEDEP}] )
	clickhouse? ( dev-python/clickhouse-cityhash[${PYTHON_USEDEP}] )
	all? ( dev-python/clickhouse-driver[${PYTHON_USEDEP}] )
	clickhouse? ( dev-python/clickhouse-driver[${PYTHON_USEDEP}] )
	all? ( dev-python/dask[${PYTHON_USEDEP}] )
	dask? ( dev-python/dask[${PYTHON_USEDEP}] )
	all? ( dev-python/datafusion[${PYTHON_USEDEP}] )
	datafusion? ( dev-python/datafusion[${PYTHON_USEDEP}] )
	all? ( dev-python/db-dtypes[${PYTHON_USEDEP}] )
	bigquery? ( dev-python/db-dtypes[${PYTHON_USEDEP}] )
	all? ( dev-python/duckdb[${PYTHON_USEDEP}] )
	duckdb? ( dev-python/duckdb[${PYTHON_USEDEP}] )
	all? ( dev-python/duckdb-engine[${PYTHON_USEDEP}] )
	duckdb? ( dev-python/duckdb-engine[${PYTHON_USEDEP}] )
	all? ( dev-python/fsspec[${PYTHON_USEDEP}] )
	impala? ( dev-python/fsspec[${PYTHON_USEDEP}] )
	all? ( dev-python/GeoAlchemy2[${PYTHON_USEDEP}] )
	geospatial? ( dev-python/GeoAlchemy2[${PYTHON_USEDEP}] )
	all? ( dev-python/geopandas[${PYTHON_USEDEP}] )
	geospatial? ( dev-python/geopandas[${PYTHON_USEDEP}] )
	all? ( dev-python/google-cloud-bigquery[${PYTHON_USEDEP}] )
	bigquery? ( dev-python/google-cloud-bigquery[${PYTHON_USEDEP}] )
	all? ( dev-python/google-cloud-bigquery-storage[${PYTHON_USEDEP}] )
	bigquery? ( dev-python/google-cloud-bigquery-storage[${PYTHON_USEDEP}] )
	all? ( dev-python/graphviz[${PYTHON_USEDEP}] )
	visualization? ( dev-python/graphviz[${PYTHON_USEDEP}] )
	all? ( dev-python/impyla[${PYTHON_USEDEP}] )
	impala? ( dev-python/impyla[${PYTHON_USEDEP}] )
	all? ( dev-python/lz4[${PYTHON_USEDEP}] )
	clickhouse? ( dev-python/lz4[${PYTHON_USEDEP}] )
	all? ( dev-python/polars[${PYTHON_USEDEP}] )
	polars? ( dev-python/polars[${PYTHON_USEDEP}] )
	all? ( dev-python/psycopg2[${PYTHON_USEDEP}] )
	postgres? ( dev-python/psycopg2[${PYTHON_USEDEP}] )
	all? ( dev-python/pyarrow[${PYTHON_USEDEP}] )
	dask? ( dev-python/pyarrow[${PYTHON_USEDEP}] )
	duckdb? ( dev-python/pyarrow[${PYTHON_USEDEP}] )
	polars? ( dev-python/pyarrow[${PYTHON_USEDEP}] )
	pyspark? ( dev-python/pyarrow[${PYTHON_USEDEP}] )
	all? ( dev-python/pymssql[${PYTHON_USEDEP}] )
	mssql? ( dev-python/pymssql[${PYTHON_USEDEP}] )
	all? ( dev-python/pydata-google-auth[${PYTHON_USEDEP}] )
	bigquery? ( dev-python/pydata-google-auth[${PYTHON_USEDEP}] )
	all? ( dev-python/pymysql[${PYTHON_USEDEP}] )
	mysql? ( dev-python/pymysql[${PYTHON_USEDEP}] )
	all? ( dev-python/pyspark[${PYTHON_USEDEP}] )
	pyspark? ( dev-python/pyspark[${PYTHON_USEDEP}] )
	all? ( dev-python/regex[${PYTHON_USEDEP}] )
	dask? ( dev-python/regex[${PYTHON_USEDEP}] )
	pandas? ( dev-python/regex[${PYTHON_USEDEP}] )
	sqlite? ( dev-python/regex[${PYTHON_USEDEP}] )
	all? ( dev-python/requests[${PYTHON_USEDEP}] )
	impala? ( dev-python/requests[${PYTHON_USEDEP}] )
	all? ( dev-python/shapely[${PYTHON_USEDEP}] )
	geospatial? ( dev-python/shapely[${PYTHON_USEDEP}] )
	all? ( dev-python/snowflake-connector-python[${PYTHON_USEDEP}] )
	snowflake? ( dev-python/snowflake-connector-python[${PYTHON_USEDEP}] )
	all? ( dev-python/snowflake-sqlalchemy[${PYTHON_USEDEP}] )
	snowflake? ( dev-python/snowflake-sqlalchemy[${PYTHON_USEDEP}] )
	all? ( dev-python/sqlalchemy[${PYTHON_USEDEP}] )
	duckdb? ( dev-python/sqlalchemy[${PYTHON_USEDEP}] )
	impala? ( dev-python/sqlalchemy[${PYTHON_USEDEP}] )
	mssql? ( dev-python/sqlalchemy[${PYTHON_USEDEP}] )
	mysql? ( dev-python/sqlalchemy[${PYTHON_USEDEP}] )
	postgres? ( dev-python/sqlalchemy[${PYTHON_USEDEP}] )
	sqlite? ( dev-python/sqlalchemy[${PYTHON_USEDEP}] )
	trino? ( dev-python/sqlalchemy[${PYTHON_USEDEP}] )
	duckdb? ( dev-python/sqlalchemy-views[${PYTHON_USEDEP}] )
	all? ( dev-python/trino[${PYTHON_USEDEP}] )
	trino? ( dev-python/trino[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
