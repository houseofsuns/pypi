# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="0.6.0.2b1"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Core data models for Park Lab ENCODE based projects [newver 0.6.0.1b5]"

HOMEPAGE="https://github.com/smaht-dac/encoded-core"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE=""
DEPENDENCIES="dev-python/awscli[${PYTHON_USEDEP}]
	<dev-python/boto3-2.0.0[${PYTHON_USEDEP}]
	<dev-python/botocore-2.0.0[${PYTHON_USEDEP}]
	dev-python/dcicsnovault[${PYTHON_USEDEP}]
	dev-python/dcicutils[${PYTHON_USEDEP}]
	~dev-python/elasticsearch-7.13.4[${PYTHON_USEDEP}]
	dev-python/plaster-pastedeploy[${PYTHON_USEDEP}]
	dev-python/psycopg2-binary[${PYTHON_USEDEP}]
	<dev-python/pyjwt-3.0.0[${PYTHON_USEDEP}]
	<dev-python/pyyaml-7.0.0[${PYTHON_USEDEP}]
	dev-python/pyramid[${PYTHON_USEDEP}]
	dev-python/pyramid_localroles[${PYTHON_USEDEP}]
	dev-python/pyramid-multiauth[${PYTHON_USEDEP}]
	dev-python/pyramid-retry[${PYTHON_USEDEP}]
	dev-python/pyramid-tm[${PYTHON_USEDEP}]
	dev-python/pyramid_translogger[${PYTHON_USEDEP}]
	<dev-python/requests-3.0.0[${PYTHON_USEDEP}]
	dev-python/SPARQLWrapper[${PYTHON_USEDEP}]
	~dev-python/sqlalchemy-1.4.41[${PYTHON_USEDEP}]
	<dev-python/structlog-20.0[${PYTHON_USEDEP}]
	dev-python/subprocess_middleware[${PYTHON_USEDEP}]
	dev-python/supervisor[${PYTHON_USEDEP}]
	dev-python/toml[${PYTHON_USEDEP}]
	dev-python/transaction[${PYTHON_USEDEP}]
	dev-python/translationstring[${PYTHON_USEDEP}]
	dev-python/uptime[${PYTHON_USEDEP}]
	<dev-python/urllib3-2.0.0[${PYTHON_USEDEP}]
	dev-python/venusian[${PYTHON_USEDEP}]
	<dev-python/waitress-3.0.0[${PYTHON_USEDEP}]
	<dev-python/webob-2.0.0[${PYTHON_USEDEP}]
	<dev-python/webtest-3.0.0[${PYTHON_USEDEP}]
	~dev-python/wsgiproxy2-0.4.2[${PYTHON_USEDEP}]
	<dev-python/zope-deprecation-5.0.0[${PYTHON_USEDEP}]
	<dev-python/zope-interface-5.0.0[${PYTHON_USEDEP}]
	dev-python/zope-sqlalchemy[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
