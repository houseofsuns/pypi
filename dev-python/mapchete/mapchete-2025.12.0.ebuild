# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Tile-based geodata processing using rasterio & Fiona"

HOMEPAGE="https://github.com/ungarj/mapchete"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="complete contours http profiling s3 serve spatial-index test vrt"
DEPENDENCIES="dev-python/aiohttp[${PYTHON_USEDEP}]
	dev-python/cachetools[${PYTHON_USEDEP}]
	dev-python/click-plugins[${PYTHON_USEDEP}]
	dev-python/click-spinner[${PYTHON_USEDEP}]
	>=dev-python/click-7.1.1[${PYTHON_USEDEP}]
	dev-python/dask[${PYTHON_USEDEP}]
	dev-python/distributed[${PYTHON_USEDEP}]
	dev-python/fiona[${PYTHON_USEDEP}]
	dev-python/fsspec[${PYTHON_USEDEP}]
	dev-python/geojson-pydantic[${PYTHON_USEDEP}]
	dev-python/importlib-metadata[${PYTHON_USEDEP}]
	dev-python/importlib-resources[${PYTHON_USEDEP}]
	>=dev-python/numpy-1.16[${PYTHON_USEDEP}]
	dev-python/oyaml[${PYTHON_USEDEP}]
	dev-python/pydantic-settings[${PYTHON_USEDEP}]
	>=dev-python/pydantic-2.3.0[${PYTHON_USEDEP}]
	dev-python/pyproj[${PYTHON_USEDEP}]
	dev-python/pystac-client[${PYTHON_USEDEP}]
	dev-python/pystac[${PYTHON_USEDEP}]
	dev-python/python-dateutil[${PYTHON_USEDEP}]
	dev-python/rasterio[${PYTHON_USEDEP}]
	dev-python/retry[${PYTHON_USEDEP}]
	>=dev-python/shapely-2.0.0[${PYTHON_USEDEP}]
	dev-python/tilematrix[${PYTHON_USEDEP}]
	dev-python/tqdm[${PYTHON_USEDEP}]
	complete? ( dev-python/aiobotocore[${PYTHON_USEDEP}] )
	complete? ( dev-python/aiohttp[${PYTHON_USEDEP}] )
	complete? ( >=dev-python/boto3-1.14.44[${PYTHON_USEDEP}] )
	complete? ( dev-python/dask[${PYTHON_USEDEP}] )
	complete? ( dev-python/distributed[${PYTHON_USEDEP}] )
	complete? ( dev-python/flask-rangerequest[${PYTHON_USEDEP}] )
	complete? ( <dev-python/flask-4.0.0[${PYTHON_USEDEP}] )
	complete? ( >=dev-python/fsspec-2023.12.0[${PYTHON_USEDEP}] )
	complete? ( dev-python/lxml[${PYTHON_USEDEP}] )
	complete? ( <dev-python/matplotlib-3.10[${PYTHON_USEDEP}] )
	complete? ( dev-python/memray[${PYTHON_USEDEP}] )
	complete? ( dev-python/pystac[${PYTHON_USEDEP}] )
	complete? ( dev-python/requests[${PYTHON_USEDEP}] )
	complete? ( dev-python/rtree[${PYTHON_USEDEP}] )
	complete? ( dev-python/s3fs[${PYTHON_USEDEP}] )
	complete? ( dev-python/tilebench[${PYTHON_USEDEP}] )
	complete? ( >=dev-python/werkzeug-0.15[${PYTHON_USEDEP}] )
	contours? ( <dev-python/matplotlib-3.10[${PYTHON_USEDEP}] )
	http? ( dev-python/aiohttp[${PYTHON_USEDEP}] )
	http? ( dev-python/fsspec[${PYTHON_USEDEP}] )
	http? ( dev-python/requests[${PYTHON_USEDEP}] )
	profiling? ( dev-python/memray[${PYTHON_USEDEP}] )
	profiling? ( dev-python/tilebench[${PYTHON_USEDEP}] )
	s3? ( dev-python/aiobotocore[${PYTHON_USEDEP}] )
	s3? ( >=dev-python/boto3-1.14.44[${PYTHON_USEDEP}] )
	s3? ( dev-python/fsspec[${PYTHON_USEDEP}] )
	s3? ( dev-python/s3fs[${PYTHON_USEDEP}] )
	serve? ( dev-python/flask-rangerequest[${PYTHON_USEDEP}] )
	serve? ( <dev-python/flask-4.0.0[${PYTHON_USEDEP}] )
	serve? ( >=dev-python/werkzeug-0.15[${PYTHON_USEDEP}] )
	spatial-index? ( dev-python/rtree[${PYTHON_USEDEP}] )
	test? ( >=dev-python/coverage-5.1[${PYTHON_USEDEP}] )
	test? ( dev-python/coveralls[${PYTHON_USEDEP}] )
	test? ( >=dev-python/flake8-3.8.4[${PYTHON_USEDEP}] )
	test? ( dev-python/minio[${PYTHON_USEDEP}] )
	test? ( dev-python/morecantile[${PYTHON_USEDEP}] )
	test? ( dev-python/pystac[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-cov-2.8.1[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-env[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-flask-1.0.0[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-lazy-fixture[${PYTHON_USEDEP}] )
	test? ( <dev-python/pytest-8.0[${PYTHON_USEDEP}] )
	test? ( dev-python/rio-cogeo[${PYTHON_USEDEP}] )
	vrt? ( dev-python/lxml[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
