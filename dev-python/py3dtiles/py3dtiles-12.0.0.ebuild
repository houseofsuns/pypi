# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Python module for 3D tiles format"

HOMEPAGE="https://py3dtiles.org"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev doc ifc las pack ply postgres"
DEPENDENCIES="~dev-python/mapbox-earcut-1.0.3[${PYTHON_USEDEP}]
	dev-python/lz4[${PYTHON_USEDEP}]
	dev-python/numba[${PYTHON_USEDEP}]
	<dev-python/numpy-2.2.0[${PYTHON_USEDEP}]
	dev-python/psutil[${PYTHON_USEDEP}]
	dev-python/pyproj[${PYTHON_USEDEP}]
	<dev-python/pyzmq-26.4.0[${PYTHON_USEDEP}]
	dev-python/pygltflib[${PYTHON_USEDEP}]
	postgres? ( dev-python/psycopg2-binary[${PYTHON_USEDEP}] )
	las? ( dev-python/laspy[${PYTHON_USEDEP}] )
	ply? ( dev-python/plyfile[${PYTHON_USEDEP}] )
	ifc? ( dev-python/ifcopenshell[${PYTHON_USEDEP}] )
	ifc? ( dev-python/lark[${PYTHON_USEDEP}] )
	dev? ( dev-python/line-profiler[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-8.4.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-benchmark[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/mypy-1.19.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/typing-extensions[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-psutil[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-psycopg2[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-multiversion[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinxcontrib-apidoc[${PYTHON_USEDEP}] )
	doc? ( dev-python/pydata-sphinx-theme[${PYTHON_USEDEP}] )
	doc? ( dev-python/myst-parser[${PYTHON_USEDEP}] )
	pack? ( dev-python/commitizen[${PYTHON_USEDEP}] )
	pack? ( dev-python/build[${PYTHON_USEDEP}] )
	pack? ( dev-python/twine[${PYTHON_USEDEP}] )
	pack? ( dev-python/wheel[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
