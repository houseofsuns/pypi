# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Elegant, modern and asynchronous Telegram MTProto API framework in Python for users and bots"

HOMEPAGE="https://kurigram.live"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/23/8a/0bf73959dffc64c47c64cf5e89120ddc52a28f30879888b47194b603a746/kurigram-${REALVERSION}.tar.gz"
SOURCEFILE="kurigram-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev docs fast"
DEPENDENCIES="<=dev-python/pyaes-1.6.1[${PYTHON_USEDEP}]
	<=dev-python/pysocks-1.7.1[${PYTHON_USEDEP}]
	dev? ( <=dev-python/hatch-1.7.0[${PYTHON_USEDEP}] )
	dev? ( <=dev-python/keyring-25.1.0[${PYTHON_USEDEP}] )
	dev? ( <=dev-python/pytest-asyncio-0.21.1[${PYTHON_USEDEP}] )
	dev? ( <=dev-python/pytest-cov-4.1.0[${PYTHON_USEDEP}] )
	dev? ( <=dev-python/pytest-7.4.3[${PYTHON_USEDEP}] )
	dev? ( <=dev-python/twine-4.0.2[${PYTHON_USEDEP}] )
	docs? ( <=dev-python/furo-2024.8.8[${PYTHON_USEDEP}] )
	docs? ( <=dev-python/pygments-2.19.1[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-autobuild[${PYTHON_USEDEP}] )
	docs? ( <=dev-python/sphinx-copybutton-0.5.2[${PYTHON_USEDEP}] )
	docs? ( <=dev-python/sphinx-8.2.3[${PYTHON_USEDEP}] )
	fast? ( dev-python/TgCrypto[${PYTHON_USEDEP}] )
	fast? ( <=dev-python/uvloop-0.21.0[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
