# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Easily download, build, install, upgrade, and uninstall Python packages"

HOMEPAGE=""
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="core doc test"
DEPENDENCIES="core? ( >=dev-python/packaging-24.0[${PYTHON_USEDEP}] )
	core? ( >=dev-python/ordered-set-3.1.1[${PYTHON_USEDEP}] )
	core? ( >=dev-python/more-itertools-8.8[${PYTHON_USEDEP}] )
	core? ( >=dev-python/jaraco-text-3.7[${PYTHON_USEDEP}] )
	core? ( >=dev-python/importlib-resources-5.10.2[${PYTHON_USEDEP}] )
	core? ( >=dev-python/importlib-metadata-6.0[${PYTHON_USEDEP}] )
	core? ( >=dev-python/tomli-2.0.1[${PYTHON_USEDEP}] )
	core? ( >=dev-python/wheel-0.43.0[${PYTHON_USEDEP}] )
	core? ( >=dev-python/platformdirs-2.6.2[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/sphinx-3.5[${PYTHON_USEDEP}] )
	doc? ( dev-python/jaraco-packaging[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/rst-linker-1.9[${PYTHON_USEDEP}] )
	doc? ( dev-python/furo[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-lint[${PYTHON_USEDEP}] )
	doc? ( dev-python/jaraco-tidelift[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/pygments-github-lexers-0.0.5[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-favicon[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-inline-tabs[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinx-reredirects[${PYTHON_USEDEP}] )
	doc? ( dev-python/sphinxcontrib-towncrier[${PYTHON_USEDEP}] )
	doc? ( <dev-python/sphinx-notfound-page-2.0[${PYTHON_USEDEP}] )
	doc? ( >dev-python/pyproject-hooks-1.1[${PYTHON_USEDEP}] )
	doc? ( <dev-python/sphinx-7.4[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-6.0[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-checkdocs[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-mypy[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-enabler[${PYTHON_USEDEP}] )
	test? ( >=dev-python/virtualenv-13.0.0[${PYTHON_USEDEP}] )
	test? ( dev-python/wheel[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pip-19.1[${PYTHON_USEDEP}] )
	test? ( >=dev-python/packaging-23.2[${PYTHON_USEDEP}] )
	test? ( >=dev-python/jaraco-envs-2.2[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-xdist-3.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/jaraco-path-3.2.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/build-1.0.3[${PYTHON_USEDEP}] )
	test? ( >=dev-python/filelock-3.4.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/ini2toml-0.14[${PYTHON_USEDEP}] )
	test? ( >=dev-python/tomli-w-1.0.0[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-timeout[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-home-0.5[${PYTHON_USEDEP}] )
	test? ( ~dev-python/mypy-1.10.0[${PYTHON_USEDEP}] )
	test? ( dev-python/tomli[${PYTHON_USEDEP}] )
	test? ( dev-python/importlib-metadata[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-subprocess[${PYTHON_USEDEP}] )
	test? ( >dev-python/pyproject-hooks-1.1[${PYTHON_USEDEP}] )
	test? ( dev-python/jaraco-test[${PYTHON_USEDEP}] )
	test? ( dev-python/jaraco-develop[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-ruff[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-perf[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-ruff[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
