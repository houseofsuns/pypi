# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="ae-kivy-app"
REALVERSION="0.3.108"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_9,3_10,3_11} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="ae namespace package portion kivy_app: main application classes and widgets for GUIApp-conform Kivy apps"

HOMEPAGE="https://gitlab.com/ae-group/ae_kivy_app"
LICENSE="OSI Approved :: GNU General Public License v3 or later GPLv3+"
SRC_URI="https://files.pythonhosted.org/packages/d9/b7/54c8dfd5d5275ad3f8f1ad3e1d2955eba63f085f9c0e4d6aa45f2ffe38ff/ae_kivy_app-${REALVERSION}.tar.gz"
SOURCEFILE="ae_kivy_app-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev tests"
DEPENDENCIES="dev-python/kivy[${PYTHON_USEDEP}]
	dev-python/plyer[${PYTHON_USEDEP}]
	dev-python/ae-base[${PYTHON_USEDEP}]
	dev-python/ae-files[${PYTHON_USEDEP}]
	dev-python/ae-paths[${PYTHON_USEDEP}]
	dev-python/ae-i18n[${PYTHON_USEDEP}]
	dev-python/ae-core[${PYTHON_USEDEP}]
	dev-python/ae-gui-app[${PYTHON_USEDEP}]
	dev-python/ae-gui-help[${PYTHON_USEDEP}]
	dev-python/ae-kivy-glsl[${PYTHON_USEDEP}]
	dev-python/ae-kivy-auto-width[${PYTHON_USEDEP}]
	dev-python/ae-kivy-dyn-chi[${PYTHON_USEDEP}]
	dev-python/ae-kivy-help[${PYTHON_USEDEP}]
	dev-python/ae-kivy-relief-canvas[${PYTHON_USEDEP}]
	dev? ( dev-python/aedev-tpl-project[${PYTHON_USEDEP}] )
	dev? ( dev-python/ae-ae[${PYTHON_USEDEP}] )
	dev? ( dev-python/anybadge[${PYTHON_USEDEP}] )
	dev? ( dev-python/coverage-badge[${PYTHON_USEDEP}] )
	dev? ( dev-python/aedev-git-repo-manager[${PYTHON_USEDEP}] )
	dev? ( dev-python/flake8[${PYTHON_USEDEP}] )
	dev? ( dev-python/mypy[${PYTHON_USEDEP}] )
	dev? ( dev-python/pylint[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	dev? ( dev-python/typing[${PYTHON_USEDEP}] )
	dev? ( dev-python/types-setuptools[${PYTHON_USEDEP}] )
	dev? ( dev-python/wheel[${PYTHON_USEDEP}] )
	dev? ( dev-python/twine[${PYTHON_USEDEP}] )
	tests? ( dev-python/anybadge[${PYTHON_USEDEP}] )
	tests? ( dev-python/coverage-badge[${PYTHON_USEDEP}] )
	tests? ( dev-python/aedev-git-repo-manager[${PYTHON_USEDEP}] )
	tests? ( dev-python/flake8[${PYTHON_USEDEP}] )
	tests? ( dev-python/mypy[${PYTHON_USEDEP}] )
	tests? ( dev-python/pylint[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest[${PYTHON_USEDEP}] )
	tests? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	tests? ( dev-python/typing[${PYTHON_USEDEP}] )
	tests? ( dev-python/types-setuptools[${PYTHON_USEDEP}] )
	tests? ( dev-python/wheel[${PYTHON_USEDEP}] )
	tests? ( dev-python/twine[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
