# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Package for supernova cosmology based on astropy"

HOMEPAGE="https://sncosmo.readthedocs.org"
LICENSE="BSD 3-Clause License"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all docs oldestdeps test test-coverage"
DEPENDENCIES="dev-python/astropy[${PYTHON_USEDEP}]
	dev-python/extinction[${PYTHON_USEDEP}]
	>=dev-python/h5py-3.11[${PYTHON_USEDEP}]
	>=dev-python/looseversion-1.0.0[${PYTHON_USEDEP}]
	>=dev-python/numpy-1.20.3[${PYTHON_USEDEP}]
	>=dev-python/pyyaml-6.0.1[${PYTHON_USEDEP}]
	>=dev-python/scipy-1.10.0[${PYTHON_USEDEP}]
	oldestdeps? ( dev-python/astropy[${PYTHON_USEDEP}] )
	oldestdeps? ( dev-python/extinction[${PYTHON_USEDEP}] )
	oldestdeps? ( ~dev-python/h5py-3.11[${PYTHON_USEDEP}] )
	oldestdeps? ( ~dev-python/looseversion-1.0.0[${PYTHON_USEDEP}] )
	oldestdeps? ( ~dev-python/numpy-1.20.3[${PYTHON_USEDEP}] )
	oldestdeps? ( ~dev-python/pyyaml-6.0.1[${PYTHON_USEDEP}] )
	oldestdeps? ( ~dev-python/scipy-1.10.0[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-astropy[${PYTHON_USEDEP}] )
	test? ( dev-python/cython[${PYTHON_USEDEP}] )
	test-coverage? ( dev-python/coveralls[${PYTHON_USEDEP}] )
	test-coverage? ( dev-python/coverage[${PYTHON_USEDEP}] )
	test-coverage? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	all? ( dev-python/iminuit[${PYTHON_USEDEP}] )
	all? ( dev-python/matplotlib[${PYTHON_USEDEP}] )
	all? ( >=dev-python/emcee-3.0[${PYTHON_USEDEP}] )
	all? ( dev-python/nestle[${PYTHON_USEDEP}] )
	docs? ( dev-python/cython[${PYTHON_USEDEP}] )
	docs? ( dev-python/numpy[${PYTHON_USEDEP}] )
	docs? ( dev-python/matplotlib[${PYTHON_USEDEP}] )
	docs? ( dev-python/scipy[${PYTHON_USEDEP}] )
	docs? ( dev-python/extinction[${PYTHON_USEDEP}] )
	docs? ( dev-python/astropy[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-gallery[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-rtd-theme[${PYTHON_USEDEP}] )
	docs? ( dev-python/pillow[${PYTHON_USEDEP}] )
	docs? ( dev-python/iminuit[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/emcee-3.0[${PYTHON_USEDEP}] )
	docs? ( dev-python/numpydoc[${PYTHON_USEDEP}] )
	docs? ( dev-python/pyyaml[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
