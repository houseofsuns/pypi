# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="cfr: a Python package for Climate Field Reconstruction"

HOMEPAGE="https://github.com/fzhu2e/cfr"
LICENSE="BSD 3-Clause"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="graphem ml psm"
DEPENDENCIES="dev-python/colorama[${PYTHON_USEDEP}]
	dev-python/seaborn[${PYTHON_USEDEP}]
	dev-python/pandas[${PYTHON_USEDEP}]
	dev-python/tqdm[${PYTHON_USEDEP}]
	dev-python/xarray[${PYTHON_USEDEP}]
	dev-python/netcdf4[${PYTHON_USEDEP}]
	dev-python/nc-time-axis[${PYTHON_USEDEP}]
	dev-python/dask[${PYTHON_USEDEP}]
	dev-python/statsmodels[${PYTHON_USEDEP}]
	dev-python/eofs[${PYTHON_USEDEP}]
	dev-python/plotly[${PYTHON_USEDEP}]
	dev-python/pyresample[${PYTHON_USEDEP}]
	psm? ( dev-python/pathos[${PYTHON_USEDEP}] )
	psm? ( dev-python/fbm[${PYTHON_USEDEP}] )
	psm? ( dev-python/pyvsl[${PYTHON_USEDEP}] )
	psm? ( dev-python/pybaywatch[${PYTHON_USEDEP}] )
	ml? ( dev-python/scikit-learn[${PYTHON_USEDEP}] )
	ml? ( dev-python/torch[${PYTHON_USEDEP}] )
	ml? ( dev-python/torchvision[${PYTHON_USEDEP}] )
	graphem? ( dev-python/cython[${PYTHON_USEDEP}] )
	graphem? ( dev-python/scikit-learn[${PYTHON_USEDEP}] )
	graphem? ( dev-python/cfr-graphem[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
