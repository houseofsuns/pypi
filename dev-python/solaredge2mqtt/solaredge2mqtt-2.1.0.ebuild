# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Read data from SolarEdge Inverter and publish it to MQTT"

HOMEPAGE="https://github.com/DerOetzi/solaredge2mqtt"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev forecast"
DEPENDENCIES="dev-python/aiocsv[${PYTHON_USEDEP}]
	~dev-python/aiohttp-3.12.15[${PYTHON_USEDEP}]
	dev-python/aiomqtt[${PYTHON_USEDEP}]
	dev-python/astral[${PYTHON_USEDEP}]
	dev-python/ephem[${PYTHON_USEDEP}]
	dev-python/influxdb-client[${PYTHON_USEDEP}]
	~dev-python/jsonref-1.1.0[${PYTHON_USEDEP}]
	~dev-python/loguru-0.7.3[${PYTHON_USEDEP}]
	~dev-python/platformdirs-4.4.0[${PYTHON_USEDEP}]
	~dev-python/pydantic-2.11.9[${PYTHON_USEDEP}]
	~dev-python/pyjwt-2.10.1[${PYTHON_USEDEP}]
	dev-python/pymodbus[${PYTHON_USEDEP}]
	~dev-python/tzlocal-5.3.1[${PYTHON_USEDEP}]
	dev? ( >=dev-python/setuptools-scm-8.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/ruff[${PYTHON_USEDEP}] )
	dev? ( dev-python/tomli[${PYTHON_USEDEP}] )
	forecast? ( ~dev-python/numpy-2.3.3[${PYTHON_USEDEP}] )
	forecast? ( ~dev-python/pandas-2.3.2[${PYTHON_USEDEP}] )
	forecast? ( ~dev-python/scikit-learn-1.7.2[${PYTHON_USEDEP}] )
	forecast? ( ~dev-python/scipy-1.16.2[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
