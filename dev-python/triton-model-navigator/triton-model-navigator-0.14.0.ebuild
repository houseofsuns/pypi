# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=wheel

inherit python-r1 gs-pypi

DESCRIPTION="Triton Model Navigator: An inference toolkit for optimizing and deploying machine learning models and pipelines on the Triton Inference Server and PyTriton. [wheel]"

HOMEPAGE=""
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/py3/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}-py3-none-any.whl"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}-py3-none-any.whl"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev doc jax tensorflow test torch"
DEPENDENCIES="dev-python/dacite[${PYTHON_USEDEP}]
	dev-python/fire[${PYTHON_USEDEP}]
	dev-python/jsonlines[${PYTHON_USEDEP}]
	>=dev-python/loguru-0.7.2[${PYTHON_USEDEP}]
	~dev-python/mpmath-1.3[${PYTHON_USEDEP}]
	<dev-python/numpy-2.0[${PYTHON_USEDEP}]
	dev-python/onnx[${PYTHON_USEDEP}]
	dev-python/onnx-graphsurgeon[${PYTHON_USEDEP}]
	dev-python/onnxruntime[${PYTHON_USEDEP}]
	dev-python/onnxruntime-gpu[${PYTHON_USEDEP}]
	dev-python/onnxscript[${PYTHON_USEDEP}]
	~dev-python/packaging-24.1[${PYTHON_USEDEP}]
	dev-python/polygraphy[${PYTHON_USEDEP}]
	>=dev-python/psutil-5.7[${PYTHON_USEDEP}]
	>=dev-python/py-cpuinfo-8.0[${PYTHON_USEDEP}]
	dev-python/pynvml[${PYTHON_USEDEP}]
	~dev-python/python-slugify-8.0[${PYTHON_USEDEP}]
	~dev-python/pyyaml-6.0[${PYTHON_USEDEP}]
	>=dev-python/tabulate-0.8[${PYTHON_USEDEP}]
	dev-python/tensorrt[${PYTHON_USEDEP}]
	dev-python/tritonclient[${PYTHON_USEDEP}]
	>=dev-python/wrapt-1.16[${PYTHON_USEDEP}]
	dev-python/pyee[${PYTHON_USEDEP}]
	>=dev-python/rich-13.7.1[${PYTHON_USEDEP}]
	dev-python/nvidia-modelopt[${PYTHON_USEDEP}]
	torch? ( dev-python/torch[${PYTHON_USEDEP}] )
	torch? ( dev-python/torch-tensorrt[${PYTHON_USEDEP}] )
	torch? ( >=dev-python/protobuf-4.0[${PYTHON_USEDEP}] )
	tensorflow? ( dev-python/tensorflow[${PYTHON_USEDEP}] )
	tensorflow? ( dev-python/tf2onnx[${PYTHON_USEDEP}] )
	tensorflow? ( <dev-python/protobuf-3.21.0[${PYTHON_USEDEP}] )
	jax? ( dev-python/jax[${PYTHON_USEDEP}] )
	jax? ( dev-python/tensorflow[${PYTHON_USEDEP}] )
	jax? ( dev-python/tf2onnx[${PYTHON_USEDEP}] )
	jax? ( <dev-python/protobuf-3.21.0[${PYTHON_USEDEP}] )
	test? ( dev-python/GitPython[${PYTHON_USEDEP}] )
	test? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-5.2[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-mock-3.8.2[${PYTHON_USEDEP}] )
	test? ( dev-python/pytype[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-unordered[${PYTHON_USEDEP}] )
	test? ( ~dev-python/tenacity-8.4.2[${PYTHON_USEDEP}] )
	test? ( >=dev-python/tox-3.23.1[${PYTHON_USEDEP}] )
	test? ( >=dev-python/tqdm-4.64.1[${PYTHON_USEDEP}] )
	doc? ( dev-python/GitPython[${PYTHON_USEDEP}] )
	doc? ( dev-python/mike[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/mkdocs-htmlproofer-plugin-0.8.0[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/mkdocs-material-8.5.6[${PYTHON_USEDEP}] )
	doc? ( <dev-python/mkdocstrings-0.28.3[${PYTHON_USEDEP}] )
	doc? ( >=dev-python/mkdocstrings-python-1.7.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/triton-model-navigator[${PYTHON_USEDEP}] )
	dev? ( dev-python/triton-model-navigator[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/build-0.8[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/ipython-7.16[${PYTHON_USEDEP}] )
	dev? ( dev-python/pdbpp[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pip-21.1[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/psutil-5.1[${PYTHON_USEDEP}] )
	dev? ( dev-python/ruff[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/twine-4.0[${PYTHON_USEDEP}] )
	virtual/allow-pypi-wheels"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
