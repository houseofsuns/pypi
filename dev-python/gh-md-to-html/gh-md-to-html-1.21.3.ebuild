# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Feature-rich Github-flavored Markdown to html python and command line interface."

HOMEPAGE="https://github.com/phseiff/github-flavored-markdown-to-html/"
LICENSE="LICENSE.txt"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="offline-conversion pdf-export"
DEPENDENCIES=">=dev-python/pillow-8.0.1[${PYTHON_USEDEP}]
	dev-python/beautifulsoup4[${PYTHON_USEDEP}]
	dev-python/emoji[${PYTHON_USEDEP}]
	dev-python/requests[${PYTHON_USEDEP}]
	dev-python/shellescape[${PYTHON_USEDEP}]
	dev-python/webcolors[${PYTHON_USEDEP}]
	offline-conversion? ( ~dev-python/mistune-2.0.0_rc1[${PYTHON_USEDEP}] )
	offline-conversion? ( dev-python/pygments[${PYTHON_USEDEP}] )
	pdf-export? ( dev-python/pdfkit[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
