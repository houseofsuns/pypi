# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="0.3.2.post1"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Deprecated in favor of Lightning Bolts - https://pypi.org/project/lightning-bolts"

HOMEPAGE="https://github.com/PyTorchLightning/lightning-bolts"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev extra loggers models test"
DEPENDENCIES="dev-python/pytorch-lightning[${PYTHON_USEDEP}]
	dev-python/torch[${PYTHON_USEDEP}]
	dev-python/torchmetrics[${PYTHON_USEDEP}]
	dev? ( dev-python/pillow[${PYTHON_USEDEP}] )
	dev? ( dev-python/atari-py[${PYTHON_USEDEP}] )
	dev? ( dev-python/check-manifest[${PYTHON_USEDEP}] )
	dev? ( dev-python/codecov[${PYTHON_USEDEP}] )
	dev? ( dev-python/flake8[${PYTHON_USEDEP}] )
	dev? ( dev-python/gym[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/isort-5.6.4[${PYTHON_USEDEP}] )
	dev? ( dev-python/matplotlib[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/mypy-0.790[${PYTHON_USEDEP}] )
	dev? ( dev-python/opencv-python-headless[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	dev? ( >dev-python/pytest-cov-2.10[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-6.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/scikit-learn-0.23[${PYTHON_USEDEP}] )
	dev? ( dev-python/scipy[${PYTHON_USEDEP}] )
	dev? ( dev-python/torchvision[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/twine-3.2[${PYTHON_USEDEP}] )
	dev? ( dev-python/wandb[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/yapf-0.29.0[${PYTHON_USEDEP}] )
	extra? ( dev-python/pillow[${PYTHON_USEDEP}] )
	extra? ( dev-python/gym[${PYTHON_USEDEP}] )
	extra? ( dev-python/matplotlib[${PYTHON_USEDEP}] )
	extra? ( dev-python/opencv-python-headless[${PYTHON_USEDEP}] )
	extra? ( >=dev-python/scikit-learn-0.23[${PYTHON_USEDEP}] )
	extra? ( dev-python/scipy[${PYTHON_USEDEP}] )
	extra? ( dev-python/torchvision[${PYTHON_USEDEP}] )
	extra? ( dev-python/wandb[${PYTHON_USEDEP}] )
	loggers? ( dev-python/matplotlib[${PYTHON_USEDEP}] )
	loggers? ( dev-python/scipy[${PYTHON_USEDEP}] )
	loggers? ( dev-python/wandb[${PYTHON_USEDEP}] )
	models? ( dev-python/pillow[${PYTHON_USEDEP}] )
	models? ( dev-python/gym[${PYTHON_USEDEP}] )
	models? ( dev-python/opencv-python-headless[${PYTHON_USEDEP}] )
	models? ( >=dev-python/scikit-learn-0.23[${PYTHON_USEDEP}] )
	models? ( dev-python/torchvision[${PYTHON_USEDEP}] )
	test? ( dev-python/atari-py[${PYTHON_USEDEP}] )
	test? ( dev-python/check-manifest[${PYTHON_USEDEP}] )
	test? ( dev-python/codecov[${PYTHON_USEDEP}] )
	test? ( dev-python/flake8[${PYTHON_USEDEP}] )
	test? ( >=dev-python/isort-5.6.4[${PYTHON_USEDEP}] )
	test? ( >=dev-python/mypy-0.790[${PYTHON_USEDEP}] )
	test? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	test? ( >dev-python/pytest-cov-2.10[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-6.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/scikit-learn-0.23[${PYTHON_USEDEP}] )
	test? ( >=dev-python/twine-3.2[${PYTHON_USEDEP}] )
	test? ( >=dev-python/yapf-0.29.0[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
