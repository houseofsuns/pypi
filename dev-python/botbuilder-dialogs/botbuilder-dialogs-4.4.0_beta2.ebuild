# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="4.4.0b2"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Microsoft Bot Framework Bot Builder [top-max 4.15.0]"

HOMEPAGE="https://www.github.com/Microsoft/botbuilder-python"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE=""
DEPENDENCIES="dev-python/recognizers-date-time[${PYTHON_USEDEP}]
	dev-python/recognizers-number-with-unit[${PYTHON_USEDEP}]
	dev-python/recognizers-number[${PYTHON_USEDEP}]
	dev-python/recognizers-text[${PYTHON_USEDEP}]
	dev-python/botbuilder-schema[${PYTHON_USEDEP}]
	dev-python/botframework-connector[${PYTHON_USEDEP}]
	dev-python/botbuilder-core[${PYTHON_USEDEP}]
	>=dev-python/aiounittest-1.1.0[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
