# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A minimal client for interacting with D-Wave cloud resources."

HOMEPAGE="https://github.com/dwavesystems/dwave-cloud-client"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="bqm cqm dqm mocks nlm test"
DEPENDENCIES="cqm? ( >=dev-python/numpy-1.17.3[${PYTHON_USEDEP}] )
	<dev-python/requests-3.0[${PYTHON_USEDEP}]
	<dev-python/urllib3-3.0[${PYTHON_USEDEP}]
	<dev-python/pydantic-3.0[${PYTHON_USEDEP}]
	dev-python/homebase[${PYTHON_USEDEP}]
	<dev-python/click-9.0[${PYTHON_USEDEP}]
	<dev-python/python-dateutil-3.0[${PYTHON_USEDEP}]
	dev-python/plucky[${PYTHON_USEDEP}]
	<dev-python/diskcache-6.0[${PYTHON_USEDEP}]
	>=dev-python/packaging-19.0[${PYTHON_USEDEP}]
	<dev-python/werkzeug-4.0[${PYTHON_USEDEP}]
	<dev-python/typing-extensions-5.0[${PYTHON_USEDEP}]
	dev-python/Authlib[${PYTHON_USEDEP}]
	>=dev-python/importlib-metadata-5.0.0[${PYTHON_USEDEP}]
	>=dev-python/orjson-3.11[${PYTHON_USEDEP}]
	dev-python/http-sf[${PYTHON_USEDEP}]
	test? ( dev-python/requests-mock[${PYTHON_USEDEP}] )
	test? ( dev-python/mock[${PYTHON_USEDEP}] )
	test? ( dev-python/numpy[${PYTHON_USEDEP}] )
	test? ( dev-python/coverage[${PYTHON_USEDEP}] )
	bqm? ( dev-python/dimod[${PYTHON_USEDEP}] )
	bqm? ( >=dev-python/numpy-1.17.3[${PYTHON_USEDEP}] )
	dqm? ( dev-python/dimod[${PYTHON_USEDEP}] )
	dqm? ( >=dev-python/numpy-1.17.3[${PYTHON_USEDEP}] )
	cqm? ( dev-python/dimod[${PYTHON_USEDEP}] )
	nlm? ( dev-python/dwave-optimization[${PYTHON_USEDEP}] )
	nlm? ( >=dev-python/numpy-1.20.0[${PYTHON_USEDEP}] )
	mocks? ( dev-python/dwave-networkx[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
