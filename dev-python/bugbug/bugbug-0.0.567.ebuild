# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="ML tools for Mozilla projects"

HOMEPAGE=""
LICENSE="MPL-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="nlp"
DEPENDENCIES="dev-python/langchain-google-genai[${PYTHON_USEDEP}]
	dev-python/langchain-mistralai[${PYTHON_USEDEP}]
	dev-python/langchain-openai[${PYTHON_USEDEP}]
	dev-python/langchain[${PYTHON_USEDEP}]
	dev-python/libmozdata[${PYTHON_USEDEP}]
	dev-python/llama-cpp-python[${PYTHON_USEDEP}]
	~dev-python/lmdb-1.5.1[${PYTHON_USEDEP}]
	~dev-python/lxml-html-clean-0.4.1[${PYTHON_USEDEP}]
	~dev-python/amqp-5.3.1[${PYTHON_USEDEP}]
	~dev-python/beautifulsoup4-4.12.3[${PYTHON_USEDEP}]
	~dev-python/boto3-1.35.94[${PYTHON_USEDEP}]
	dev-python/imbalanced-learn[${PYTHON_USEDEP}]
	dev-python/langchain-anthropic[${PYTHON_USEDEP}]
	dev-python/langchain-community[${PYTHON_USEDEP}]
	~dev-python/markdown2-2.5.2[${PYTHON_USEDEP}]
	~dev-python/matplotlib-3.9.3[${PYTHON_USEDEP}]
	dev-python/mercurial[${PYTHON_USEDEP}]
	dev-python/microannotate[${PYTHON_USEDEP}]
	dev-python/mozci[${PYTHON_USEDEP}]
	~dev-python/numpy-1.26.4[${PYTHON_USEDEP}]
	~dev-python/orjson-3.10.13[${PYTHON_USEDEP}]
	dev-python/ortools[${PYTHON_USEDEP}]
	~dev-python/pandas-2.2.3[${PYTHON_USEDEP}]
	~dev-python/psutil-6.1.1[${PYTHON_USEDEP}]
	>=dev-python/pyopenssl-0.14[${PYTHON_USEDEP}]
	dev-python/PyDriller[${PYTHON_USEDEP}]
	~dev-python/python-dateutil-2.9.0_p0[${PYTHON_USEDEP}]
	dev-python/python-hglib[${PYTHON_USEDEP}]
	dev-python/qdrant-client[${PYTHON_USEDEP}]
	~dev-python/ratelimit-2.2.1[${PYTHON_USEDEP}]
	dev-python/requests-html[${PYTHON_USEDEP}]
	~dev-python/requests-2.32.3[${PYTHON_USEDEP}]
	dev-python/rs-parsepatch[${PYTHON_USEDEP}]
	~dev-python/scikit-learn-1.5.1[${PYTHON_USEDEP}]
	~dev-python/scipy-1.14.1[${PYTHON_USEDEP}]
	dev-python/sendgrid[${PYTHON_USEDEP}]
	dev-python/shap[${PYTHON_USEDEP}]
	~dev-python/tabulate-0.9.0[${PYTHON_USEDEP}]
	dev-python/taskcluster[${PYTHON_USEDEP}]
	~dev-python/tenacity-8.5.0[${PYTHON_USEDEP}]
	~dev-python/tqdm-4.67.1[${PYTHON_USEDEP}]
	~dev-python/unidiff-0.7.5[${PYTHON_USEDEP}]
	dev-python/xgboost[${PYTHON_USEDEP}]
	~dev-python/zstandard-0.23.0[${PYTHON_USEDEP}]
	nlp? ( dev-python/gensim[${PYTHON_USEDEP}] )
	nlp? ( dev-python/spacy[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
