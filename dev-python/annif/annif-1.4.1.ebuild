# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Automated subject indexing and classification tool"

HOMEPAGE="https://annif.org"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="estnltk fasttext nn omikuji spacy stwfsa voikko yake"
DEPENDENCIES="<dev-python/click-8.3.0[${PYTHON_USEDEP}]
	~dev-python/click-log-0.4[${PYTHON_USEDEP}]
	dev-python/connexion[${PYTHON_USEDEP}]
	dev-python/gunicorn[${PYTHON_USEDEP}]
	dev-python/huggingface-hub[${PYTHON_USEDEP}]
	<dev-python/joblib-1.6.0[${PYTHON_USEDEP}]
	<dev-python/jsonschema-4.26.0[${PYTHON_USEDEP}]
	dev-python/nltk[${PYTHON_USEDEP}]
	<dev-python/numpy-2.3.0[${PYTHON_USEDEP}]
	dev-python/optuna[${PYTHON_USEDEP}]
	~dev-python/python-dateutil-2.9[${PYTHON_USEDEP}]
	dev-python/simplemma[${PYTHON_USEDEP}]
	<dev-python/scikit-learn-1.8.0[${PYTHON_USEDEP}]
	<dev-python/scipy-1.16.0[${PYTHON_USEDEP}]
	<dev-python/requests-2.33.0[${PYTHON_USEDEP}]
	<dev-python/rdflib-7.2.0[${PYTHON_USEDEP}]
	fasttext? ( dev-python/fasttext-numpy2[${PYTHON_USEDEP}] )
	estnltk? ( dev-python/estnltk[${PYTHON_USEDEP}] )
	nn? ( dev-python/tensorflow-cpu[${PYTHON_USEDEP}] )
	nn? ( <dev-python/lmdb-1.8.0[${PYTHON_USEDEP}] )
	omikuji? ( dev-python/omikuji[${PYTHON_USEDEP}] )
	spacy? ( dev-python/spacy[${PYTHON_USEDEP}] )
	stwfsa? ( dev-python/stwfsapy[${PYTHON_USEDEP}] )
	voikko? ( dev-python/voikko[${PYTHON_USEDEP}] )
	yake? ( dev-python/yake[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
