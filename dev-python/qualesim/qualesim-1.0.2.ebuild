# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=wheel

inherit python-r1 gs-pypi

DESCRIPTION="Quingo Simulator Backend QuaLeSim [wheel]"

HOMEPAGE="https://gitee.com/quingo/qualesim.git"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/cp311/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}-cp311-cp311-manylinux2014_x86_64.whl"
SOURCEFILE="${REALNAME}-${REALVERSION}-cp311-cp311-manylinux2014_x86_64.whl"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="tequila"
DEPENDENCIES="dev-python/cbor[${PYTHON_USEDEP}]
	dev-python/qualesim-quiets[${PYTHON_USEDEP}]
	dev-python/qualesim-quantumsim[${PYTHON_USEDEP}]
	dev-python/qualesim-qcis[${PYTHON_USEDEP}]
	tequila? ( dev-python/qualesim-tequila[${PYTHON_USEDEP}] )
	virtual/allow-pypi-wheels"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
