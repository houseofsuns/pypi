# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="1.2.0a20"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Interact with the Databricks Generative AI APIs in python"

HOMEPAGE="https://docs.mosaicml.com/projects/mcli/en/latest/finetuning/finetuning.html"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all dev notebook release spark"
DEPENDENCIES=">=dev-python/backoff-2.2.1[${PYTHON_USEDEP}]
	dev-python/databricks-sdk[${PYTHON_USEDEP}]
	dev-python/datasets[${PYTHON_USEDEP}]
	dev-python/gql[${PYTHON_USEDEP}]
	dev-python/mlflow[${PYTHON_USEDEP}]
	dev-python/mosaicml-cli[${PYTHON_USEDEP}]
	<dev-python/requests-3.0[${PYTHON_USEDEP}]
	<dev-python/boto3-2.0[${PYTHON_USEDEP}]
	>=dev-python/typing-extensions-4.7.1[${PYTHON_USEDEP}]
	>=dev-python/pydantic-2.0[${PYTHON_USEDEP}]
	dev-python/databricks-agents[${PYTHON_USEDEP}]
	>=dev-python/tenacity-9.0.0[${PYTHON_USEDEP}]
	>=dev-python/rich-12.6.0[${PYTHON_USEDEP}]
	dev-python/openai[${PYTHON_USEDEP}]
	dev-python/rouge-score[${PYTHON_USEDEP}]
	dev-python/evaluate[${PYTHON_USEDEP}]
	dev-python/catalogue[${PYTHON_USEDEP}]
	<dev-python/ipython-9.0[${PYTHON_USEDEP}]
	<dev-python/freezegun-2.0[${PYTHON_USEDEP}]
	spark? ( dev-python/databricks-connect[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/isort-5.9.3[${PYTHON_USEDEP}] )
	dev? ( <dev-python/packaging-25.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/parameterized-0.9[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pylint-3.0.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/pyright[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-7.4.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/yapf-0.40.0[${PYTHON_USEDEP}] )
	release? ( >=dev-python/build-0.10.0[${PYTHON_USEDEP}] )
	release? ( >=dev-python/twine-4.0.2[${PYTHON_USEDEP}] )
	notebook? ( <dev-python/ipywidgets-9.0[${PYTHON_USEDEP}] )
	all? ( >=dev-python/isort-5.9.3[${PYTHON_USEDEP}] )
	all? ( >=dev-python/pylint-3.0.0[${PYTHON_USEDEP}] )
	all? ( <dev-python/ipywidgets-9.0[${PYTHON_USEDEP}] )
	all? ( >=dev-python/pytest-7.4.0[${PYTHON_USEDEP}] )
	all? ( <dev-python/packaging-25.0[${PYTHON_USEDEP}] )
	all? ( >=dev-python/yapf-0.40.0[${PYTHON_USEDEP}] )
	all? ( >=dev-python/twine-4.0.2[${PYTHON_USEDEP}] )
	all? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	all? ( dev-python/pyright[${PYTHON_USEDEP}] )
	all? ( ~dev-python/parameterized-0.9[${PYTHON_USEDEP}] )
	all? ( >=dev-python/build-0.10.0[${PYTHON_USEDEP}] )
	all? ( dev-python/databricks-connect[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
