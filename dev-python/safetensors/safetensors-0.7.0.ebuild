# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION=""

HOMEPAGE="https://github.com/huggingface/safetensors"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all dev jax mlx numpy paddlepaddle pinned-tf quality tensorflow testing testingfree torch"
DEPENDENCIES="numpy? ( >=dev-python/numpy-1.21.6[${PYTHON_USEDEP}] )
	torch? ( dev-python/packaging[${PYTHON_USEDEP}] )
	torch? ( dev-python/safetensors[${PYTHON_USEDEP}] )
	torch? ( dev-python/torch[${PYTHON_USEDEP}] )
	tensorflow? ( dev-python/safetensors[${PYTHON_USEDEP}] )
	tensorflow? ( dev-python/tensorflow[${PYTHON_USEDEP}] )
	pinned-tf? ( dev-python/safetensors[${PYTHON_USEDEP}] )
	pinned-tf? ( dev-python/tensorflow[${PYTHON_USEDEP}] )
	jax? ( dev-python/safetensors[${PYTHON_USEDEP}] )
	jax? ( dev-python/flax[${PYTHON_USEDEP}] )
	jax? ( dev-python/jax[${PYTHON_USEDEP}] )
	jax? ( dev-python/jaxlib[${PYTHON_USEDEP}] )
	mlx? ( dev-python/mlx[${PYTHON_USEDEP}] )
	paddlepaddle? ( dev-python/safetensors[${PYTHON_USEDEP}] )
	paddlepaddle? ( dev-python/paddlepaddle[${PYTHON_USEDEP}] )
	quality? ( dev-python/ruff[${PYTHON_USEDEP}] )
	testing? ( dev-python/safetensors[${PYTHON_USEDEP}] )
	testing? ( >=dev-python/h5py-3.7.0[${PYTHON_USEDEP}] )
	testing? ( dev-python/huggingface-hub[${PYTHON_USEDEP}] )
	testing? ( >=dev-python/setuptools-rust-1.5.2[${PYTHON_USEDEP}] )
	testing? ( >=dev-python/pytest-7.2.0[${PYTHON_USEDEP}] )
	testing? ( dev-python/pytest-benchmark[${PYTHON_USEDEP}] )
	testing? ( >=dev-python/hypothesis-6.70.2[${PYTHON_USEDEP}] )
	testingfree? ( dev-python/safetensors[${PYTHON_USEDEP}] )
	testingfree? ( dev-python/huggingface-hub[${PYTHON_USEDEP}] )
	testingfree? ( >=dev-python/setuptools-rust-1.5.2[${PYTHON_USEDEP}] )
	testingfree? ( >=dev-python/pytest-7.2.0[${PYTHON_USEDEP}] )
	testingfree? ( dev-python/pytest-benchmark[${PYTHON_USEDEP}] )
	testingfree? ( >=dev-python/hypothesis-6.70.2[${PYTHON_USEDEP}] )
	all? ( dev-python/safetensors[${PYTHON_USEDEP}] )
	all? ( dev-python/safetensors[${PYTHON_USEDEP}] )
	all? ( dev-python/safetensors[${PYTHON_USEDEP}] )
	all? ( dev-python/safetensors[${PYTHON_USEDEP}] )
	all? ( dev-python/safetensors[${PYTHON_USEDEP}] )
	all? ( dev-python/safetensors[${PYTHON_USEDEP}] )
	all? ( dev-python/safetensors[${PYTHON_USEDEP}] )
	dev? ( dev-python/safetensors[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
