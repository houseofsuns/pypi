# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="See everything happening in your running Django app"

HOMEPAGE="https://kolo.app"
LICENSE=" Kolo"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="cli test-generation"
DEPENDENCIES=">=dev-python/asgiref-3.6.0[${PYTHON_USEDEP}]
	>=dev-python/cerberus-1.3.4[${PYTHON_USEDEP}]
	>=dev-python/click-7.1.2[${PYTHON_USEDEP}]
	dev-python/httpx[${PYTHON_USEDEP}]
	>=dev-python/msgpack-1.0.0[${PYTHON_USEDEP}]
	>=dev-python/tomli-2.0.0[${PYTHON_USEDEP}]
	>=dev-python/toolz-0.11.1[${PYTHON_USEDEP}]
	dev-python/ulid-py[${PYTHON_USEDEP}]
	test-generation? ( >=dev-python/jinja-3.0.0[${PYTHON_USEDEP}] )
	test-generation? ( dev-python/more-itertools[${PYTHON_USEDEP}] )
	test-generation? ( >=dev-python/sqlglot-23.6.4[${PYTHON_USEDEP}] )
	test-generation? ( dev-python/tabulate[${PYTHON_USEDEP}] )
	cli? ( dev-python/pygments[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
