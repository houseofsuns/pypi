# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A platform to enable high-throughput, database-driven quantum chemistry and computational materials science"

HOMEPAGE=""
LICENSE="BSD"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="covalent dask defects dev docs jobflow mlp mp newtonnet parsl phonons prefect redun sella tblite"
DEPENDENCIES="mlp? ( dev-python/mace-torch[${PYTHON_USEDEP}] )
	mlp? ( dev-python/torch-dftd[${PYTHON_USEDEP}] )
	mlp? ( dev-python/sevenn[${PYTHON_USEDEP}] )
	mlp? ( dev-python/orb-models[${PYTHON_USEDEP}] )
	mp? ( dev-python/atomate2[${PYTHON_USEDEP}] )
	newtonnet? ( dev-python/newtonnet[${PYTHON_USEDEP}] )
	parsl? ( dev-python/parsl[${PYTHON_USEDEP}] )
	phonons? ( dev-python/phonopy[${PYTHON_USEDEP}] )
	phonons? ( dev-python/seekpath[${PYTHON_USEDEP}] )
	prefect? ( dev-python/prefect[${PYTHON_USEDEP}] )
	prefect? ( dev-python/dask-jobqueue[${PYTHON_USEDEP}] )
	redun? ( dev-python/redun[${PYTHON_USEDEP}] )
	sella? ( dev-python/Sella[${PYTHON_USEDEP}] )
	tblite? ( dev-python/tblite[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-7.4.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-cov-3.0.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/ruff[${PYTHON_USEDEP}] )
	docs? ( dev-python/blacken-docs[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/mkdocs-material-9.5.16[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/mkdocstrings-0.22.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/mkdocs-gen-files-0.5.0[${PYTHON_USEDEP}] )
	docs? ( dev-python/mkdocs-literate-nav[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/pillow-10.0.0[${PYTHON_USEDEP}] )
	docs? ( dev-python/CairoSVG[${PYTHON_USEDEP}] )
	dev-python/ase[${PYTHON_USEDEP}]
	dev-python/custodian[${PYTHON_USEDEP}]
	dev-python/emmet-core[${PYTHON_USEDEP}]
	dev-python/maggma[${PYTHON_USEDEP}]
	dev-python/monty[${PYTHON_USEDEP}]
	>=dev-python/numpy-1.25.0[${PYTHON_USEDEP}]
	dev-python/psutil[${PYTHON_USEDEP}]
	>=dev-python/pydantic-2.0.1[${PYTHON_USEDEP}]
	dev-python/pydantic-settings[${PYTHON_USEDEP}]
	dev-python/pymatgen[${PYTHON_USEDEP}]
	>=dev-python/ruamel-yaml-0.17.40[${PYTHON_USEDEP}]
	dev-python/typer[${PYTHON_USEDEP}]
	<=dev-python/pymongo-4.10.1[${PYTHON_USEDEP}]
	covalent? ( dev-python/covalent-cloud[${PYTHON_USEDEP}] )
	dask? ( dev-python/dask[${PYTHON_USEDEP}] )
	dask? ( dev-python/dask-jobqueue[${PYTHON_USEDEP}] )
	defects? ( dev-python/pymatgen-analysis-defects[${PYTHON_USEDEP}] )
	defects? ( dev-python/shakenbreak[${PYTHON_USEDEP}] )
	jobflow? ( dev-python/jobflow[${PYTHON_USEDEP}] )
	jobflow? ( dev-python/jobflow-remote[${PYTHON_USEDEP}] )
	mlp? ( dev-python/matgl[${PYTHON_USEDEP}] )
	mlp? ( dev-python/chgnet[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
