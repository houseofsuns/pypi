# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="deepchecks"
REALVERSION="0.13.1"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_9,3_10,3_11} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Package for validating your machine learning model and data"

HOMEPAGE="https://github.com/deepchecks/deepchecks"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/d/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="nlp nlp-properties vision"
DEPENDENCIES="dev-python/pandas[${PYTHON_USEDEP}]
	dev-python/numpy[${PYTHON_USEDEP}]
	dev-python/scikit-learn[${PYTHON_USEDEP}]
	dev-python/jsonpickle[${PYTHON_USEDEP}]
	dev-python/PyNomaly[${PYTHON_USEDEP}]
	dev-python/typing-extensions[${PYTHON_USEDEP}]
	dev-python/tqdm[${PYTHON_USEDEP}]
	dev-python/category-encoders[${PYTHON_USEDEP}]
	dev-python/scipy[${PYTHON_USEDEP}]
	dev-python/plotly[${PYTHON_USEDEP}]
	dev-python/matplotlib[${PYTHON_USEDEP}]
	dev-python/pyzmq[${PYTHON_USEDEP}]
	dev-python/beautifulsoup4[${PYTHON_USEDEP}]
	dev-python/statsmodels[${PYTHON_USEDEP}]
	dev-python/ipython[${PYTHON_USEDEP}]
	dev-python/ipykernel[${PYTHON_USEDEP}]
	dev-python/ipywidgets[${PYTHON_USEDEP}]
	nlp? ( dev-python/seqeval[${PYTHON_USEDEP}] )
	nlp? ( dev-python/nltk[${PYTHON_USEDEP}] )
	nlp-properties? ( dev-python/nltk[${PYTHON_USEDEP}] )
	nlp-properties? ( dev-python/datasets[${PYTHON_USEDEP}] )
	nlp-properties? ( dev-python/langdetect[${PYTHON_USEDEP}] )
	nlp-properties? ( dev-python/textblob[${PYTHON_USEDEP}] )
	nlp-properties? ( dev-python/transformers[${PYTHON_USEDEP}] )
	nlp-properties? ( dev-python/optimum[${PYTHON_USEDEP}] )
	nlp-properties? ( dev-python/onnx[${PYTHON_USEDEP}] )
	nlp-properties? ( dev-python/onnxruntime[${PYTHON_USEDEP}] )
	vision? ( dev-python/pytorch-ignite[${PYTHON_USEDEP}] )
	vision? ( dev-python/opencv-python[${PYTHON_USEDEP}] )
	vision? ( dev-python/albumentations[${PYTHON_USEDEP}] )
	vision? ( dev-python/imgaug[${PYTHON_USEDEP}] )
	vision? ( dev-python/requests[${PYTHON_USEDEP}] )
	vision? ( dev-python/seaborn[${PYTHON_USEDEP}] )
	vision? ( dev-python/imagehash[${PYTHON_USEDEP}] )
	vision? ( dev-python/lxml[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
