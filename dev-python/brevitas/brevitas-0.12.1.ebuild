# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Quantization-aware training in PyTorch"

HOMEPAGE="https://github.com/Xilinx/brevitas"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev diffusion docs export finn-integration hadamard llm notebook ort-integration stt test tts vision"
DEPENDENCIES="dev-python/dependencies[${PYTHON_USEDEP}]
	<=dev-python/numpy-1.26.4[${PYTHON_USEDEP}]
	dev-python/packaging[${PYTHON_USEDEP}]
	<dev-python/setuptools-70.0[${PYTHON_USEDEP}]
	dev-python/sympy[${PYTHON_USEDEP}]
	dev-python/torch[${PYTHON_USEDEP}]
	>=dev-python/typing-extensions-3.7.4[${PYTHON_USEDEP}]
	dev-python/unfoldNd[${PYTHON_USEDEP}]
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	diffusion? ( dev-python/accelerate[${PYTHON_USEDEP}] )
	diffusion? ( dev-python/diffusers[${PYTHON_USEDEP}] )
	diffusion? ( dev-python/pandas[${PYTHON_USEDEP}] )
	diffusion? ( dev-python/torch[${PYTHON_USEDEP}] )
	diffusion? ( dev-python/torchmetrics[${PYTHON_USEDEP}] )
	diffusion? ( dev-python/tqdm[${PYTHON_USEDEP}] )
	diffusion? ( dev-python/transformers[${PYTHON_USEDEP}] )
	docs? ( dev-python/m2r2[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/nbsphinx-0.9.3[${PYTHON_USEDEP}] )
	docs? ( dev-python/nbsphinx-link[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/pydata-sphinx-theme-0.15.3[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/sphinx-5.3.0[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/sphinx-autodoc-typehints-1.21.8[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/sphinx-gallery-0.10.1[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinxcontrib-napoleon[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/sphinxemoji-0.2.0[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/lxml-4.9.3[${PYTHON_USEDEP}] )
	export? ( dev-python/onnx[${PYTHON_USEDEP}] )
	export? ( dev-python/onnxoptimizer[${PYTHON_USEDEP}] )
	export? ( dev-python/onnxscript[${PYTHON_USEDEP}] )
	export? ( dev-python/qonnx[${PYTHON_USEDEP}] )
	finn-integration? ( dev-python/bitstring[${PYTHON_USEDEP}] )
	finn-integration? ( dev-python/onnx[${PYTHON_USEDEP}] )
	finn-integration? ( dev-python/onnxoptimizer[${PYTHON_USEDEP}] )
	finn-integration? ( dev-python/onnxruntime[${PYTHON_USEDEP}] )
	finn-integration? ( dev-python/onnxscript[${PYTHON_USEDEP}] )
	finn-integration? ( dev-python/qonnx[${PYTHON_USEDEP}] )
	finn-integration? ( dev-python/toposort[${PYTHON_USEDEP}] )
	hadamard? ( dev-python/scipy[${PYTHON_USEDEP}] )
	llm? ( dev-python/accelerate[${PYTHON_USEDEP}] )
	llm? ( dev-python/datasets[${PYTHON_USEDEP}] )
	llm? ( dev-python/gguf[${PYTHON_USEDEP}] )
	llm? ( dev-python/lm-eval[${PYTHON_USEDEP}] )
	llm? ( dev-python/onnx[${PYTHON_USEDEP}] )
	llm? ( dev-python/onnxruntime[${PYTHON_USEDEP}] )
	llm? ( dev-python/optimum[${PYTHON_USEDEP}] )
	llm? ( dev-python/pandas[${PYTHON_USEDEP}] )
	llm? ( dev-python/torch[${PYTHON_USEDEP}] )
	llm? ( dev-python/tqdm[${PYTHON_USEDEP}] )
	llm? ( dev-python/transformers[${PYTHON_USEDEP}] )
	notebook? ( dev-python/jupyter[${PYTHON_USEDEP}] )
	notebook? ( dev-python/nbmake[${PYTHON_USEDEP}] )
	ort-integration? ( dev-python/onnx[${PYTHON_USEDEP}] )
	ort-integration? ( dev-python/onnxoptimizer[${PYTHON_USEDEP}] )
	ort-integration? ( dev-python/onnxruntime[${PYTHON_USEDEP}] )
	ort-integration? ( dev-python/onnxscript[${PYTHON_USEDEP}] )
	ort-integration? ( dev-python/qonnx[${PYTHON_USEDEP}] )
	stt? ( ~dev-python/inflect-5.6.2[${PYTHON_USEDEP}] )
	stt? ( dev-python/librosa[${PYTHON_USEDEP}] )
	stt? ( dev-python/numba[${PYTHON_USEDEP}] )
	stt? ( >=dev-python/pillow-4.3.0[${PYTHON_USEDEP}] )
	stt? ( dev-python/requests[${PYTHON_USEDEP}] )
	stt? ( dev-python/ruamel-yaml[${PYTHON_USEDEP}] )
	stt? ( dev-python/soundfile[${PYTHON_USEDEP}] )
	stt? ( dev-python/sox[${PYTHON_USEDEP}] )
	stt? ( dev-python/torch-stft[${PYTHON_USEDEP}] )
	stt? ( dev-python/tqdm[${PYTHON_USEDEP}] )
	stt? ( dev-python/unidecode[${PYTHON_USEDEP}] )
	test? ( dev-python/hypothesis[${PYTHON_USEDEP}] )
	test? ( dev-python/mock[${PYTHON_USEDEP}] )
	test? ( dev-python/psutil[${PYTHON_USEDEP}] )
	test? ( <dev-python/pytest-8.4[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-mock[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-xdist[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cases[${PYTHON_USEDEP}] )
	test? ( dev-python/scipy[${PYTHON_USEDEP}] )
	test? ( dev-python/torchvision[${PYTHON_USEDEP}] )
	test? ( dev-python/tqdm[${PYTHON_USEDEP}] )
	tts? ( dev-python/librosa[${PYTHON_USEDEP}] )
	tts? ( dev-python/numpy[${PYTHON_USEDEP}] )
	tts? ( dev-python/pillow[${PYTHON_USEDEP}] )
	tts? ( dev-python/pyyaml[${PYTHON_USEDEP}] )
	tts? ( dev-python/scipy[${PYTHON_USEDEP}] )
	tts? ( dev-python/soundfile[${PYTHON_USEDEP}] )
	tts? ( dev-python/tqdm[${PYTHON_USEDEP}] )
	vision? ( dev-python/accelerate[${PYTHON_USEDEP}] )
	vision? ( dev-python/torch[${PYTHON_USEDEP}] )
	vision? ( dev-python/torchvision[${PYTHON_USEDEP}] )
	vision? ( dev-python/tqdm[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
