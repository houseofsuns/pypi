# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="pyqt-custom-titlebar-window"
REALVERSION="0.0.50"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_9,3_10,3_11} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="PyQt custom titlebar window (resizable, movable, minimize/maximize/close, etc.)"

HOMEPAGE="https://github.com/yjg30737/pyqt-custom-titlebar-window.git"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/p/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE=""
DEPENDENCIES="dev-python/PyQt5[${PYTHON_USEDEP}]
	dev-python/pyqt-frameless-window[${PYTHON_USEDEP}]
	dev-python/pyqt-windows-buttons-widget[${PYTHON_USEDEP}]
	dev-python/pyqt-mac-buttons-widget[${PYTHON_USEDEP}]
	dev-python/pyqt-top-titlebar-widget[${PYTHON_USEDEP}]
	dev-python/pyqt-svg-label[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
