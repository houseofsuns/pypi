...
IUSE="autobuild blendsearch graph ipython lmm local-jupyter-exec mathchat redis retrievechat teachable test websurfer"
DEPENDENCIES="dev-python/diskcache[${PYTHON_USEDEP}]
	...
	ipython? ( >=dev-python/ipykernel-6.29.0[${PYTHON_USEDEP}] )
	ipython? ( >=dev-python/jupyter-client-8.6.0[${PYTHON_USEDEP}] )
	lmm? ( dev-python/pillow[${PYTHON_USEDEP}] )
	lmm? ( dev-python/replicate[${PYTHON_USEDEP}] )
	local-jupyter-exec? ( dev-python/ipykernel[${PYTHON_USEDEP}] )
	local-jupyter-exec? ( dev-python/jupyter-kernel-gateway[${PYTHON_USEDEP}] )
	local-jupyter-exec? ( dev-python/requests[${PYTHON_USEDEP}] )
	local-jupyter-exec? ( dev-python/websocket-client[${PYTHON_USEDEP}] )
