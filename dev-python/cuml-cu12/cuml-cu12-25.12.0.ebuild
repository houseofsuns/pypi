# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=wheel

inherit python-r1 gs-pypi

DESCRIPTION="cuML - RAPIDS ML Algorithms [wheel]"

HOMEPAGE="https://github.com/rapidsai/cuml"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/cp311/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}-cp311-cp311-manylinux_2_24_x86_64.manylinux_2_28_x86_64.whl"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}-cp311-cp311-manylinux_2_24_x86_64.manylinux_2_28_x86_64.whl"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dask test test-dask"
DEPENDENCIES="dev-python/cuda-python[${PYTHON_USEDEP}]
	dev-python/cuda-toolkit[${PYTHON_USEDEP}]
	dev-python/cudf-cu12[${PYTHON_USEDEP}]
	dev-python/cupy-cuda12x[${PYTHON_USEDEP}]
	>=dev-python/joblib-0.11[${PYTHON_USEDEP}]
	dev-python/libcuml-cu12[${PYTHON_USEDEP}]
	dev-python/numba-cuda[${PYTHON_USEDEP}]
	dev-python/numba[${PYTHON_USEDEP}]
	<dev-python/numpy-3.0_alpha0[${PYTHON_USEDEP}]
	dev-python/packaging[${PYTHON_USEDEP}]
	dev-python/pylibraft-cu12[${PYTHON_USEDEP}]
	dev-python/rich[${PYTHON_USEDEP}]
	dev-python/rmm-cu12[${PYTHON_USEDEP}]
	>=dev-python/scikit-learn-1.4[${PYTHON_USEDEP}]
	>=dev-python/scipy-1.11.0[${PYTHON_USEDEP}]
	dev-python/treelite[${PYTHON_USEDEP}]
	test? ( dev-python/certifi[${PYTHON_USEDEP}] )
	test? ( <dev-python/cython-3.2.0_alpha0[${PYTHON_USEDEP}] )
	test? ( dev-python/hdbscan[${PYTHON_USEDEP}] )
	test? ( <dev-python/hypothesis-7.0[${PYTHON_USEDEP}] )
	test? ( dev-python/nltk[${PYTHON_USEDEP}] )
	test? ( <dev-python/numpydoc-1.9[${PYTHON_USEDEP}] )
	test? ( >=dev-python/nvidia-ml-py-12.0[${PYTHON_USEDEP}] )
	test? ( dev-python/pynndescent[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-benchmark[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cases[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-xdist[${PYTHON_USEDEP}] )
	test? ( <dev-python/pytest-9.0[${PYTHON_USEDEP}] )
	test? ( dev-python/pyyaml[${PYTHON_USEDEP}] )
	test? ( <dev-python/scikit-learn-1.8.0[${PYTHON_USEDEP}] )
	test? ( dev-python/seaborn[${PYTHON_USEDEP}] )
	test? ( dev-python/statsmodels[${PYTHON_USEDEP}] )
	test? ( dev-python/tenacity[${PYTHON_USEDEP}] )
	test? ( dev-python/umap-learn[${PYTHON_USEDEP}] )
	test? ( dev-python/xgboost[${PYTHON_USEDEP}] )
	dask? ( dev-python/dask-cudf-cu12[${PYTHON_USEDEP}] )
	dask? ( dev-python/raft-dask-cu12[${PYTHON_USEDEP}] )
	dask? ( dev-python/rapids-dask-dependency[${PYTHON_USEDEP}] )
	test-dask? ( dev-python/dask-cuda[${PYTHON_USEDEP}] )
	test-dask? ( dev-python/dask-ml[${PYTHON_USEDEP}] )
	virtual/allow-pypi-wheels"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
