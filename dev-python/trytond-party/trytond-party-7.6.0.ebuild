# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Tryton module with parties and addresses"

HOMEPAGE="http://www.tryton.org/"
LICENSE="GPL-3"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="phonenumbers phonenumbers-lite test vies"
DEPENDENCIES="dev-python/python-sql[${PYTHON_USEDEP}]
	>=dev-python/python-stdnum-1.19[${PYTHON_USEDEP}]
	dev-python/trytond-country[${PYTHON_USEDEP}]
	dev-python/trytond[${PYTHON_USEDEP}]
	test? ( dev-python/proteus[${PYTHON_USEDEP}] )
	test? ( dev-python/phonenumberslite[${PYTHON_USEDEP}] )
	vies? ( dev-python/python-stdnum[${PYTHON_USEDEP}] )
	phonenumbers-lite? ( dev-python/phonenumberslite[${PYTHON_USEDEP}] )
	phonenumbers? ( dev-python/phonenumbers[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
