# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Adding guardrails to large language models."

HOMEPAGE=""
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="anthropic api databricks dev docs docs-build huggingface llama manifest sql uv vectordb"
DEPENDENCIES="<dev-python/lxml-7.0.0[${PYTHON_USEDEP}]
	dev-python/openai[${PYTHON_USEDEP}]
	<dev-python/rich-15.0.0[${PYTHON_USEDEP}]
	<dev-python/pydantic-3.0[${PYTHON_USEDEP}]
	<dev-python/typer-0.16[${PYTHON_USEDEP}]
	<dev-python/click-8.2.0[${PYTHON_USEDEP}]
	<dev-python/tenacity-10.0.0[${PYTHON_USEDEP}]
	dev-python/rstr[${PYTHON_USEDEP}]
	<dev-python/typing-extensions-5.0.0[${PYTHON_USEDEP}]
	<dev-python/python-dateutil-3.0.0[${PYTHON_USEDEP}]
	dev-python/tiktoken[${PYTHON_USEDEP}]
	dev-python/litellm[${PYTHON_USEDEP}]
	dev-python/pydash[${PYTHON_USEDEP}]
	dev-python/langchain-core[${PYTHON_USEDEP}]
	<dev-python/requests-3.0.0[${PYTHON_USEDEP}]
	<dev-python/faker-38.0.0[${PYTHON_USEDEP}]
	<dev-python/jsonref-2.0.0[${PYTHON_USEDEP}]
	<dev-python/jsonschema-5.0.0[${PYTHON_USEDEP}]
	>=dev-python/pip-22.0[${PYTHON_USEDEP}]
	<dev-python/pyjwt-3.0.0[${PYTHON_USEDEP}]
	<dev-python/opentelemetry-sdk-2.0.0[${PYTHON_USEDEP}]
	dev-python/opentelemetry-exporter-otlp-proto-grpc[${PYTHON_USEDEP}]
	dev-python/opentelemetry-exporter-otlp-proto-http[${PYTHON_USEDEP}]
	dev-python/guardrails-hub-types[${PYTHON_USEDEP}]
	dev-python/guardrails-api-client[${PYTHON_USEDEP}]
	<dev-python/diff-match-patch-20241101.0[${PYTHON_USEDEP}]
	<dev-python/semver-4.0.0[${PYTHON_USEDEP}]
	dev? ( >=dev-python/pytest-7.4.3[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-asyncio-0.21.1[${PYTHON_USEDEP}] )
	dev? ( dev-python/docformatter[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-cov-2.10.1[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/twine-4.0.2[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-mock-3.12.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/pypdfium2[${PYTHON_USEDEP}] )
	dev? ( dev-python/pyright[${PYTHON_USEDEP}] )
	dev? ( dev-python/lxml-stubs[${PYTHON_USEDEP}] )
	dev? ( dev-python/ruff[${PYTHON_USEDEP}] )
	dev? ( dev-python/liccheck[${PYTHON_USEDEP}] )
	sql? ( dev-python/sqlvalidator[${PYTHON_USEDEP}] )
	sql? ( <dev-python/sqlalchemy-3.0.0[${PYTHON_USEDEP}] )
	sql? ( <dev-python/sqlglot-20.0.0[${PYTHON_USEDEP}] )
	vectordb? ( dev-python/faiss-cpu[${PYTHON_USEDEP}] )
	vectordb? ( <dev-python/numpy-3.0[${PYTHON_USEDEP}] )
	manifest? ( dev-python/manifest-ml[${PYTHON_USEDEP}] )
	anthropic? ( dev-python/anthropic[${PYTHON_USEDEP}] )
	docs-build? ( dev-python/nbdoc[${PYTHON_USEDEP}] )
	docs-build? ( dev-python/docspec-python[${PYTHON_USEDEP}] )
	docs-build? ( dev-python/pydoc-markdown[${PYTHON_USEDEP}] )
	huggingface? ( dev-python/transformers[${PYTHON_USEDEP}] )
	huggingface? ( dev-python/torch[${PYTHON_USEDEP}] )
	huggingface? ( dev-python/guardrails-jsonformer[${PYTHON_USEDEP}] )
	api? ( dev-python/guardrails-api[${PYTHON_USEDEP}] )
	api? ( <dev-python/boto3-2.0[${PYTHON_USEDEP}] )
	databricks? ( dev-python/mlflow[${PYTHON_USEDEP}] )
	uv? ( <dev-python/uvloop-1.0.0[${PYTHON_USEDEP}] )
	llama? ( dev-python/llama-index[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/mkdocs-1.5.3[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/mkdocstrings-0.23.0[${PYTHON_USEDEP}] )
	docs? ( dev-python/mkdocs-jupyter[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/mkdocs-material-9.4.8[${PYTHON_USEDEP}] )
	docs? ( dev-python/mknotebooks[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/griffe-1.3.2[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/pillow-10.1.0[${PYTHON_USEDEP}] )
	docs? ( dev-python/CairoSVG[${PYTHON_USEDEP}] )
	docs? ( dev-python/mkdocs-glightbox[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
