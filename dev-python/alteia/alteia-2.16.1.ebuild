# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="High-level Python interface to Alteia API"

HOMEPAGE="https://github.com/alteia-ai/alteia-python-sdk"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="documentation"
DEPENDENCIES="<dev-python/urllib3-3.0.0[${PYTHON_USEDEP}]
	<dev-python/appdirs-2.0.0[${PYTHON_USEDEP}]
	<dev-python/pathvalidate-4.0.0[${PYTHON_USEDEP}]
	<dev-python/semantic-version-3.0.0[${PYTHON_USEDEP}]
	>=dev-python/legacy-cgi-2.6.1[${PYTHON_USEDEP}]
	documentation? ( <dev-python/sphinx-9.0.0[${PYTHON_USEDEP}] )
	documentation? ( <dev-python/sphinx-autodoc-typehints-3.0.0[${PYTHON_USEDEP}] )
	documentation? ( dev-python/sphinx-autobuild[${PYTHON_USEDEP}] )
	documentation? ( <dev-python/recommonmark-0.8.0[${PYTHON_USEDEP}] )
	<dev-python/docutils-0.22.0[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
