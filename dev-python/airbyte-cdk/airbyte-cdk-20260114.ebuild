# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="7.6.3.post6.dev21011212433"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A framework for writing Airbyte Connectors. [badver 7.6.3.post6.dev21011212433]"

HOMEPAGE="https://airbyte.com"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev file-based manifest-server sql vector-db-based"
DEPENDENCIES="<dev-python/jinja2-3.2.0[${PYTHON_USEDEP}]
	<dev-python/pyyaml-7.0.0[${PYTHON_USEDEP}]
	dev-python/airbyte-protocol-models-dataclasses[${PYTHON_USEDEP}]
	<dev-python/anyascii-0.4.0[${PYTHON_USEDEP}]
	file-based? ( dev-python/avro[${PYTHON_USEDEP}] )
	dev-python/backoff[${PYTHON_USEDEP}]
	<dev-python/boltons-26.0.0[${PYTHON_USEDEP}]
	dev-python/cachetools[${PYTHON_USEDEP}]
	<dev-python/click-9.0.0[${PYTHON_USEDEP}]
	vector-db-based? ( dev-python/cohere[${PYTHON_USEDEP}] )
	<dev-python/cryptography-45.0.0[${PYTHON_USEDEP}]
	dev-python/dateparser[${PYTHON_USEDEP}]
	manifest-server? ( dev-python/ddtrace[${PYTHON_USEDEP}] )
	dev-python/dpath[${PYTHON_USEDEP}]
	dev-python/dunamai[${PYTHON_USEDEP}]
	manifest-server? ( dev-python/fastapi[${PYTHON_USEDEP}] )
	file-based? ( dev-python/fastavro[${PYTHON_USEDEP}] )
	~dev-python/genson-1.3.0[${PYTHON_USEDEP}]
	dev-python/google-cloud-secret-manager[${PYTHON_USEDEP}]
	<dev-python/isodate-0.7.0[${PYTHON_USEDEP}]
	<dev-python/jsonref-2.0[${PYTHON_USEDEP}]
	<dev-python/jsonschema-5.0[${PYTHON_USEDEP}]
	vector-db-based? ( dev-python/langchain-community[${PYTHON_USEDEP}] )
	vector-db-based? ( dev-python/langchain-core[${PYTHON_USEDEP}] )
	vector-db-based? ( dev-python/langchain-text-splitters[${PYTHON_USEDEP}] )
	file-based? ( dev-python/markdown[${PYTHON_USEDEP}] )
	dev-python/nltk[${PYTHON_USEDEP}]
	vector-db-based? ( dev-python/openai[${PYTHON_USEDEP}] )
	file-based? ( <dev-python/openpyxl-4.0.0[${PYTHON_USEDEP}] )
	<dev-python/orjson-4.0.0[${PYTHON_USEDEP}]
	dev-python/packaging[${PYTHON_USEDEP}]
	~dev-python/pandas-2.2.3[${PYTHON_USEDEP}]
	file-based? ( dev-python/pdf2image[${PYTHON_USEDEP}] )
	file-based? ( dev-python/pdfminer-six[${PYTHON_USEDEP}] )
	file-based? ( <dev-python/pyarrow-20.0.0[${PYTHON_USEDEP}] )
	<dev-python/pydantic-3.0[${PYTHON_USEDEP}]
	<dev-python/pyjwt-3.0.0[${PYTHON_USEDEP}]
	<dev-python/pyrate-limiter-3.2.0[${PYTHON_USEDEP}]
	file-based? ( ~dev-python/pytesseract-0.3.10[${PYTHON_USEDEP}] )
	dev? ( <dev-python/pytest-8.0[${PYTHON_USEDEP}] )
	file-based? ( dev-python/python-calamine[${PYTHON_USEDEP}] )
	<dev-python/python-dateutil-3.0.0[${PYTHON_USEDEP}]
	file-based? ( ~dev-python/python-snappy-0.7.3[${PYTHON_USEDEP}] )
	dev-python/python-ulid[${PYTHON_USEDEP}]
	~dev-python/pytz-2024.2[${PYTHON_USEDEP}]
	<dev-python/rapidfuzz-4.0.0[${PYTHON_USEDEP}]
	>=dev-python/referencing-0.36.2[${PYTHON_USEDEP}]
	dev-python/requests[${PYTHON_USEDEP}]
	dev-python/requests-cache[${PYTHON_USEDEP}]
	dev-python/rich[${PYTHON_USEDEP}]
	<dev-python/rich-click-2.0.0[${PYTHON_USEDEP}]
	dev-python/serpyco-rs[${PYTHON_USEDEP}]
	<dev-python/setuptools-81.0.0[${PYTHON_USEDEP}]
	sql? ( <dev-python/sqlalchemy-3.0[${PYTHON_USEDEP}] )
	vector-db-based? ( dev-python/tiktoken[${PYTHON_USEDEP}] )
	dev-python/typing-extensions[${PYTHON_USEDEP}]
	<dev-python/unidecode-2.0.0[${PYTHON_USEDEP}]
	file-based? ( dev-python/unstructured-pytesseract[${PYTHON_USEDEP}] )
	file-based? ( dev-python/unstructured[${PYTHON_USEDEP}] )
	manifest-server? ( >=dev-python/uvicorn-0.35.0[${PYTHON_USEDEP}] )
	~dev-python/wcmatch-10.0[${PYTHON_USEDEP}]
	<dev-python/whenever-0.9.0[${PYTHON_USEDEP}]
	<dev-python/xmltodict-0.15[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
