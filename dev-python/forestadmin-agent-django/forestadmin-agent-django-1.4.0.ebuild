# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="django agent for forestadmin python agent"

HOMEPAGE="https://www.forestadmin.com"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE=""
DEPENDENCIES="<dev-python/typing-extensions-5.0[${PYTHON_USEDEP}]
	<dev-python/tzdata-2023.0[${PYTHON_USEDEP}]
	dev-python/forestadmin-agent-toolkit[${PYTHON_USEDEP}]
	dev-python/forestadmin-datasource-django[${PYTHON_USEDEP}]
	<dev-python/django-6.0[${PYTHON_USEDEP}]
	>=dev-python/django-cors-headers-3.8[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
