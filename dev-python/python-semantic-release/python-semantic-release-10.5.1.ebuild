# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Automatic Semantic Versioning for Python projects"

HOMEPAGE="https://python-semantic-release.readthedocs.io"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="build dev docs mypy test"
DEPENDENCIES="~dev-python/click-8.1.0[${PYTHON_USEDEP}]
	~dev-python/click-option-group-0.5[${PYTHON_USEDEP}]
	~dev-python/gitpython-3.0[${PYTHON_USEDEP}]
	~dev-python/requests-2.25[${PYTHON_USEDEP}]
	~dev-python/jinja2-3.1[${PYTHON_USEDEP}]
	dev-python/python-gitlab[${PYTHON_USEDEP}]
	~dev-python/tomlkit-0.13.0[${PYTHON_USEDEP}]
	dev-python/dotty-dict[${PYTHON_USEDEP}]
	~dev-python/importlib-resources-6.0[${PYTHON_USEDEP}]
	~dev-python/pydantic-2.0[${PYTHON_USEDEP}]
	~dev-python/rich-14.0[${PYTHON_USEDEP}]
	~dev-python/shellingham-1.5[${PYTHON_USEDEP}]
	~dev-python/deprecated-1.2[${PYTHON_USEDEP}]
	build? ( ~dev-python/build-1.2[${PYTHON_USEDEP}] )
	build? ( ~dev-python/tomlkit-0.13.0[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/sphinx-7.4[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/sphinxcontrib-apidoc-0.6.0[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-autobuild[${PYTHON_USEDEP}] )
	docs? ( ~dev-python/furo-2025.9[${PYTHON_USEDEP}] )
	test? ( ~dev-python/coverage-7.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/filelock-3.15[${PYTHON_USEDEP}] )
	test? ( ~dev-python/flatdict-4.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/freezegun-1.5[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pyyaml-6.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-8.3[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-clarity[${PYTHON_USEDEP}] )
	test? ( <dev-python/pytest-cov-8.0.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-env-1.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-lazy-fixtures-1.4[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-mock-3.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-order-1.3[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-pretty[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-xdist-3.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/responses-0.25.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/requests-mock-1.10[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/tox-4.11[${PYTHON_USEDEP}] )
	dev? ( dev-python/ruff[${PYTHON_USEDEP}] )
	mypy? ( ~dev-python/mypy-1.16.1[${PYTHON_USEDEP}] )
	mypy? ( dev-python/types-Deprecated[${PYTHON_USEDEP}] )
	mypy? ( dev-python/types-requests[${PYTHON_USEDEP}] )
	mypy? ( dev-python/types-PyYAML[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
