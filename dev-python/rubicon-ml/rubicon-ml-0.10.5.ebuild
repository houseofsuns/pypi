# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="\"an ML library for model development and governance\""

HOMEPAGE="https://github.com/capitalone/rubicon-ml"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all dev docs qa s3 test ui viz"
DEPENDENCIES="<=dev-python/click-8.1.8[${PYTHON_USEDEP}]
	<=dev-python/fsspec-2025.3.2[${PYTHON_USEDEP}]
	dev-python/intake[${PYTHON_USEDEP}]
	<=dev-python/jsonpath-ng-1.7.0[${PYTHON_USEDEP}]
	<=dev-python/numpy-2.2.5[${PYTHON_USEDEP}]
	<=dev-python/pandas-2.2.3[${PYTHON_USEDEP}]
	<=dev-python/pyarrow-20.0.0[${PYTHON_USEDEP}]
	<=dev-python/pyyaml-6.0.2[${PYTHON_USEDEP}]
	<=dev-python/scikit-learn-1.6.1[${PYTHON_USEDEP}]
	all? ( dev-python/rubicon-ml[${PYTHON_USEDEP}] )
	dev? ( dev-python/rubicon-ml[${PYTHON_USEDEP}] )
	docs? ( dev-python/furo[${PYTHON_USEDEP}] )
	docs? ( dev-python/ipython[${PYTHON_USEDEP}] )
	docs? ( dev-python/nbsphinx[${PYTHON_USEDEP}] )
	docs? ( dev-python/numpydoc[${PYTHON_USEDEP}] )
	docs? ( dev-python/pandoc[${PYTHON_USEDEP}] )
	docs? ( dev-python/prefect[${PYTHON_USEDEP}] )
	docs? ( dev-python/rubicon-ml[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-copybutton[${PYTHON_USEDEP}] )
	docs? ( dev-python/backports-strenum[${PYTHON_USEDEP}] )
	qa? ( dev-python/black[${PYTHON_USEDEP}] )
	qa? ( dev-python/edgetest[${PYTHON_USEDEP}] )
	qa? ( dev-python/flake8[${PYTHON_USEDEP}] )
	qa? ( dev-python/isort[${PYTHON_USEDEP}] )
	qa? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	qa? ( dev-python/setuptools[${PYTHON_USEDEP}] )
	qa? ( dev-python/versioneer[${PYTHON_USEDEP}] )
	qa? ( dev-python/wheel[${PYTHON_USEDEP}] )
	s3? ( dev-python/s3fs[${PYTHON_USEDEP}] )
	test? ( dev-python/dask[${PYTHON_USEDEP}] )
	test? ( dev-python/h2o[${PYTHON_USEDEP}] )
	test? ( dev-python/ipykernel[${PYTHON_USEDEP}] )
	test? ( dev-python/jupyterlab[${PYTHON_USEDEP}] )
	test? ( dev-python/lightgbm[${PYTHON_USEDEP}] )
	test? ( dev-python/nbconvert[${PYTHON_USEDEP}] )
	test? ( dev-python/polars[${PYTHON_USEDEP}] )
	test? ( dev-python/prefect[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	test? ( dev-python/xgboost[${PYTHON_USEDEP}] )
	test? ( dev-python/backports-strenum[${PYTHON_USEDEP}] )
	ui? ( dev-python/rubicon-ml[${PYTHON_USEDEP}] )
	viz? ( dev-python/dash[${PYTHON_USEDEP}] )
	viz? ( dev-python/dash-bootstrap-components[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
