# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Orbax Checkpoint"

HOMEPAGE="http://github.com/google/orbax"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="docs testing"
DEPENDENCIES="dev-python/absl-py[${PYTHON_USEDEP}]
	dev-python/etils[${PYTHON_USEDEP}]
	dev-python/typing-extensions[${PYTHON_USEDEP}]
	dev-python/msgpack[${PYTHON_USEDEP}]
	dev-python/jax[${PYTHON_USEDEP}]
	dev-python/numpy[${PYTHON_USEDEP}]
	dev-python/pyyaml[${PYTHON_USEDEP}]
	dev-python/tensorstore[${PYTHON_USEDEP}]
	dev-python/nest-asyncio[${PYTHON_USEDEP}]
	dev-python/aiofiles[${PYTHON_USEDEP}]
	dev-python/protobuf[${PYTHON_USEDEP}]
	dev-python/humanize[${PYTHON_USEDEP}]
	>=dev-python/simplejson-3.16.0[${PYTHON_USEDEP}]
	dev-python/psutil[${PYTHON_USEDEP}]
	docs? ( dev-python/flax[${PYTHON_USEDEP}] )
	docs? ( dev-python/google-cloud-logging[${PYTHON_USEDEP}] )
	docs? ( dev-python/grain[${PYTHON_USEDEP}] )
	docs? ( dev-python/aiofiles[${PYTHON_USEDEP}] )
	docs? ( dev-python/tensorflow-datasets[${PYTHON_USEDEP}] )
	docs? ( dev-python/opencv-python[${PYTHON_USEDEP}] )
	docs? ( dev-python/safetensors[${PYTHON_USEDEP}] )
	docs? ( dev-python/clu[${PYTHON_USEDEP}] )
	testing? ( dev-python/google-cloud-logging[${PYTHON_USEDEP}] )
	testing? ( dev-python/mock[${PYTHON_USEDEP}] )
	testing? ( dev-python/flax[${PYTHON_USEDEP}] )
	testing? ( dev-python/pytest[${PYTHON_USEDEP}] )
	testing? ( dev-python/pytest-xdist[${PYTHON_USEDEP}] )
	testing? ( dev-python/chex[${PYTHON_USEDEP}] )
	testing? ( dev-python/aiofiles[${PYTHON_USEDEP}] )
	testing? ( dev-python/safetensors[${PYTHON_USEDEP}] )
	testing? ( dev-python/torch[${PYTHON_USEDEP}] )
	testing? ( dev-python/clu[${PYTHON_USEDEP}] )
	testing? ( dev-python/tensorflow[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
