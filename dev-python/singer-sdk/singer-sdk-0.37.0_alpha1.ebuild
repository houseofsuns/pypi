# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="0.37.0a1"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A framework for building Singer taps"

HOMEPAGE="https://sdk.meltano.com/en/latest/"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="docs faker jwt parquet s3 testing"
DEPENDENCIES="jwt? ( <dev-python/pyjwt-3.0[${PYTHON_USEDEP}] )
	>=dev-python/pyyaml-6.0[${PYTHON_USEDEP}]
	>=dev-python/backoff-2.0.0[${PYTHON_USEDEP}]
	dev-python/backports-datetime-fromisoformat[${PYTHON_USEDEP}]
	<dev-python/click-9.0[${PYTHON_USEDEP}]
	jwt? ( >=dev-python/cryptography-3.4.6[${PYTHON_USEDEP}] )
	faker? ( <dev-python/Faker-25.0[${PYTHON_USEDEP}] )
	>=dev-python/fs-2.4.16[${PYTHON_USEDEP}]
	s3? ( dev-python/fs-s3fs[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/furo-2022.12.7[${PYTHON_USEDEP}] )
	<dev-python/importlib-metadata-8.0.0[${PYTHON_USEDEP}]
	>=dev-python/inflection-0.5.1[${PYTHON_USEDEP}]
	>=dev-python/joblib-1.3.0[${PYTHON_USEDEP}]
	>=dev-python/jsonpath-ng-1.5.3[${PYTHON_USEDEP}]
	>=dev-python/jsonschema-4.16.0[${PYTHON_USEDEP}]
	docs? ( >=dev-python/myst-parser-1.0[${PYTHON_USEDEP}] )
	parquet? ( >=dev-python/numpy-1.22[${PYTHON_USEDEP}] )
	>=dev-python/packaging-23.1[${PYTHON_USEDEP}]
	dev-python/pendulum[${PYTHON_USEDEP}]
	parquet? ( >=dev-python/pyarrow-13.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/pytest-7.2.1[${PYTHON_USEDEP}] )
	testing? ( >=dev-python/pytest-7.2.1[${PYTHON_USEDEP}] )
	testing? ( dev-python/pytest-durations[${PYTHON_USEDEP}] )
	>=dev-python/python-dateutil-2.8.2[${PYTHON_USEDEP}]
	>=dev-python/python-dotenv-0.20[${PYTHON_USEDEP}]
	>=dev-python/referencing-0.30.0[${PYTHON_USEDEP}]
	>=dev-python/requests-2.25.1[${PYTHON_USEDEP}]
	>=dev-python/simpleeval-0.9.13[${PYTHON_USEDEP}]
	>=dev-python/simplejson-3.17.6[${PYTHON_USEDEP}]
	docs? ( >=dev-python/sphinx-4.5[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-autobuild[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-copybutton-0.3.1[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-inline-tabs-2023.4.21[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-notfound-page-1.0.0[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx-reredirects[${PYTHON_USEDEP}] )
	<dev-python/sqlalchemy-3.0[${PYTHON_USEDEP}]
	>=dev-python/typing-extensions-4.5.0[${PYTHON_USEDEP}]
	<dev-python/urllib3-2.0[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
