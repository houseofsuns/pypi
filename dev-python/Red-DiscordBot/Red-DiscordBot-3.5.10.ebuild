# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="A highly customisable Discord bot"

HOMEPAGE="https://github.com/Cog-Creators/Red-DiscordBot"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/05/a5/802437b811bedd35f1f8382597d8552031d8ae218df831226f192befc5bc/red_discordbot-${REALVERSION}.tar.gz"
SOURCEFILE="red_discordbot-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all dev doc postgres style test"
DEPENDENCIES="~dev-python/aiohttp-3.9.5[${PYTHON_USEDEP}]
	dev-python/aiohttp-json-rpc[${PYTHON_USEDEP}]
	~dev-python/aiosignal-1.3.1[${PYTHON_USEDEP}]
	~dev-python/apsw-3.46.0.1[${PYTHON_USEDEP}]
	~dev-python/attrs-23.2.0[${PYTHON_USEDEP}]
	~dev-python/Babel-2.15.0[${PYTHON_USEDEP}]
	dev-python/Brotli[${PYTHON_USEDEP}]
	~dev-python/click-8.1.7[${PYTHON_USEDEP}]
	dev-python/discord-py[${PYTHON_USEDEP}]
	~dev-python/frozenlist-1.4.1[${PYTHON_USEDEP}]
	~dev-python/idna-3.7[${PYTHON_USEDEP}]
	~dev-python/markdown-3.6[${PYTHON_USEDEP}]
	~dev-python/markdown-it-py-3.0.0[${PYTHON_USEDEP}]
	~dev-python/mdurl-0.1.2[${PYTHON_USEDEP}]
	~dev-python/multidict-6.0.5[${PYTHON_USEDEP}]
	~dev-python/orjson-3.10.6[${PYTHON_USEDEP}]
	~dev-python/packaging-24.1[${PYTHON_USEDEP}]
	~dev-python/platformdirs-4.2.2[${PYTHON_USEDEP}]
	~dev-python/psutil-6.0.0[${PYTHON_USEDEP}]
	~dev-python/pygments-2.18.0[${PYTHON_USEDEP}]
	~dev-python/python-dateutil-2.9.0_p0[${PYTHON_USEDEP}]
	~dev-python/pyyaml-6.0.1[${PYTHON_USEDEP}]
	~dev-python/rapidfuzz-3.9.4[${PYTHON_USEDEP}]
	dev-python/Red-Commons[${PYTHON_USEDEP}]
	dev-python/Red-Lavalink[${PYTHON_USEDEP}]
	~dev-python/rich-13.7.1[${PYTHON_USEDEP}]
	~dev-python/schema-0.7.7[${PYTHON_USEDEP}]
	~dev-python/six-1.16.0[${PYTHON_USEDEP}]
	~dev-python/typing-extensions-4.12.2[${PYTHON_USEDEP}]
	~dev-python/yarl-1.9.4[${PYTHON_USEDEP}]
	~dev-python/uvloop-0.19.0[${PYTHON_USEDEP}]
	~dev-python/importlib-metadata-8.0.0[${PYTHON_USEDEP}]
	~dev-python/zipp-3.19.2[${PYTHON_USEDEP}]
	~dev-python/async-timeout-4.0.3[${PYTHON_USEDEP}]
	~dev-python/distro-1.9.0[${PYTHON_USEDEP}]
	all? ( dev-python/asyncpg[${PYTHON_USEDEP}] )
	all? ( ~dev-python/async-timeout-4.0.3[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/markupsafe-2.1.5[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/requests-2.32.3[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/alabaster-0.7.13[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pylint-3.2.5[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pytz-2024.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/snowballstemmer-2.2.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pathspec-0.12.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/imagesize-1.4.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/mypy_extensions-1.0.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/sphinx-7.1.2[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/sphinxcontrib-jquery-4.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/importlib-metadata-8.0.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/sphinx-prompt-1.7.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/sphinxcontrib-htmlhelp-2.0.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/async-timeout-4.0.3[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/sphinxcontrib-serializinghtml-1.1.5[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/mccabe-0.7.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/certifi-2024.7.4[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/sphinxcontrib-devhelp-1.0.2[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/sphinxcontrib-qthelp-1.0.3[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/tomli-2.0.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pluggy-1.5.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/isort-5.13.2[${PYTHON_USEDEP}] )
	dev? ( dev-python/asyncpg[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pytest-asyncio-0.21.2[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/black-23.12.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pytest-7.4.4[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/charset-normalizer-3.3.2[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/sphinxcontrib-trio-1.1.2[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/pytest-mock-3.14.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/zipp-3.19.2[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/urllib3-2.2.2[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/iniconfig-2.0.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/dill-0.3.8[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/sphinxcontrib-jsmath-1.0.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/astroid-3.2.2[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/tomlkit-0.13.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/jinja-3.1.4[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/docutils-0.20.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/sphinxcontrib-applehelp-1.0.4[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/sphinx-rtd-theme-2.0.0[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/exceptiongroup-1.2.1[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/tomli-2.0.1[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/alabaster-0.7.13[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/certifi-2024.7.4[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/charset-normalizer-3.3.2[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/docutils-0.20.1[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/imagesize-1.4.1[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/importlib-metadata-8.0.0[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/jinja-3.1.4[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/markupsafe-2.1.5[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/pytz-2024.1[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/requests-2.32.3[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/snowballstemmer-2.2.0[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinx-7.1.2[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinx-prompt-1.7.0[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinx-rtd-theme-2.0.0[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinxcontrib-applehelp-1.0.4[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinxcontrib-devhelp-1.0.2[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinxcontrib-htmlhelp-2.0.1[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinxcontrib-jquery-4.1[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinxcontrib-jsmath-1.0.1[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinxcontrib-qthelp-1.0.3[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinxcontrib-serializinghtml-1.1.5[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/sphinxcontrib-trio-1.1.2[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/urllib3-2.2.2[${PYTHON_USEDEP}] )
	doc? ( ~dev-python/zipp-3.19.2[${PYTHON_USEDEP}] )
	postgres? ( ~dev-python/async-timeout-4.0.3[${PYTHON_USEDEP}] )
	postgres? ( dev-python/asyncpg[${PYTHON_USEDEP}] )
	style? ( ~dev-python/black-23.12.1[${PYTHON_USEDEP}] )
	style? ( ~dev-python/mypy_extensions-1.0.0[${PYTHON_USEDEP}] )
	style? ( ~dev-python/pathspec-0.12.1[${PYTHON_USEDEP}] )
	style? ( ~dev-python/tomli-2.0.1[${PYTHON_USEDEP}] )
	test? ( ~dev-python/astroid-3.2.2[${PYTHON_USEDEP}] )
	test? ( ~dev-python/dill-0.3.8[${PYTHON_USEDEP}] )
	test? ( ~dev-python/iniconfig-2.0.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/isort-5.13.2[${PYTHON_USEDEP}] )
	test? ( ~dev-python/mccabe-0.7.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pluggy-1.5.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pylint-3.2.5[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-7.4.4[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-asyncio-0.21.2[${PYTHON_USEDEP}] )
	test? ( ~dev-python/pytest-mock-3.14.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/tomlkit-0.13.0[${PYTHON_USEDEP}] )
	test? ( ~dev-python/exceptiongroup-1.2.1[${PYTHON_USEDEP}] )
	test? ( ~dev-python/tomli-2.0.1[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
