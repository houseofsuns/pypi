# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Coordination library for distributed systems."

HOMEPAGE="https://docs.openstack.org/tooz/latest/"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="consul etcd etcd3gw ipc kubernetes memcached mysql postgresql redis test zake zookeeper"
DEPENDENCIES=">=dev-python/stevedore-1.16.0[${PYTHON_USEDEP}]
	>=dev-python/voluptuous-0.8.9[${PYTHON_USEDEP}]
	>=dev-python/msgpack-0.4.0[${PYTHON_USEDEP}]
	>=dev-python/fasteners-0.7[${PYTHON_USEDEP}]
	>=dev-python/tenacity-5.0.0[${PYTHON_USEDEP}]
	dev-python/futurist[${PYTHON_USEDEP}]
	>=dev-python/oslo-utils-4.7.0[${PYTHON_USEDEP}]
	>=dev-python/oslo-serialization-1.10.0[${PYTHON_USEDEP}]
	>=dev-python/debtcollector-3.0.0[${PYTHON_USEDEP}]
	consul? ( dev-python/python-consul2[${PYTHON_USEDEP}] )
	etcd? ( >=dev-python/requests-2.10.0[${PYTHON_USEDEP}] )
	etcd3gw? ( dev-python/etcd3gw[${PYTHON_USEDEP}] )
	ipc? ( dev-python/sysv-ipc[${PYTHON_USEDEP}] )
	kubernetes? ( dev-python/kubernetes[${PYTHON_USEDEP}] )
	kubernetes? ( dev-python/sherlock[${PYTHON_USEDEP}] )
	memcached? ( dev-python/pymemcache[${PYTHON_USEDEP}] )
	mysql? ( >=dev-python/pymysql-0.6.2[${PYTHON_USEDEP}] )
	postgresql? ( dev-python/psycopg2[${PYTHON_USEDEP}] )
	redis? ( >=dev-python/redis-4.0.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/testtools-1.4.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/coverage-3.6[${PYTHON_USEDEP}] )
	test? ( >=dev-python/fixtures-3.0.0[${PYTHON_USEDEP}] )
	test? ( dev-python/pifpaf[${PYTHON_USEDEP}] )
	test? ( >=dev-python/stestr-2.0.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/ddt-1.2.1[${PYTHON_USEDEP}] )
	zake? ( dev-python/zake[${PYTHON_USEDEP}] )
	zookeeper? ( dev-python/kazoo[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
