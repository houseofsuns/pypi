# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="aiomisc - miscellaneous utils for asyncio"

HOMEPAGE="https://github.com/aiokitchen/aiomisc"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="aiohttp asgi carbon cron dns grpc raven rich uvicorn uvloop"
DEPENDENCIES="<dev-python/colorlog-7.0[${PYTHON_USEDEP}]
	dev-python/logging-journald[${PYTHON_USEDEP}]
	dev-python/typing-extensions[${PYTHON_USEDEP}]
	aiohttp? ( >dev-python/aiohttp-3.0[${PYTHON_USEDEP}] )
	asgi? ( dev-python/aiohttp-asgi[${PYTHON_USEDEP}] )
	carbon? ( dev-python/aiocarbon[${PYTHON_USEDEP}] )
	cron? ( <dev-python/croniter-3.0[${PYTHON_USEDEP}] )
	dns? ( dev-python/dnslib[${PYTHON_USEDEP}] )
	grpc? ( >=dev-python/grpcio-1.56[${PYTHON_USEDEP}] )
	grpc? ( dev-python/grpcio-tools[${PYTHON_USEDEP}] )
	grpc? ( dev-python/grpcio-reflection[${PYTHON_USEDEP}] )
	raven? ( >dev-python/aiohttp-3.0[${PYTHON_USEDEP}] )
	raven? ( dev-python/raven[${PYTHON_USEDEP}] )
	rich? ( dev-python/rich[${PYTHON_USEDEP}] )
	uvicorn? ( >=dev-python/uvicorn-0.27[${PYTHON_USEDEP}] )
	uvicorn? ( >=dev-python/asgiref-3.7[${PYTHON_USEDEP}] )
	uvloop? ( <dev-python/uvloop-1.0[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
