...
IUSE="clarabel cvxopt daqp ecos gurobi highs jaxopt kvxopt mosek open-source-solvers osqp piqp proxqp qpalm qpax quadprog scs sip wheels-only"
DEPENDENCIES=">=dev-python/numpy-1.15.4[${PYTHON_USEDEP}]
	...
	qpax? ( dev-python/qpax[${PYTHON_USEDEP}] )
	quadprog? ( dev-python/quadprog[${PYTHON_USEDEP}] )
	scs? ( dev-python/scs[${PYTHON_USEDEP}] )
	sip? ( dev-python/sip-python[${PYTHON_USEDEP}] )
	wheels-only? ( dev-python/qpsolvers[${PYTHON_USEDEP}] )"
