...
REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="0.13.0.dev20251024223017"
DIGEST_SOURCES="yes"
...
DESCRIPTION="Create LLM agents with long-term memory and custom tools [top-max 0.13.0_pre20251025104015]"
...
IUSE="bedrock cloud-tool-sandbox desktop dev experimental external-tools modal pinecone postgres profiling redis server sqlite"
DEPENDENCIES="dev-python/aiomultiprocess[${PYTHON_USEDEP}]
	...
	desktop? ( ~dev-python/uvicorn-0.29.0[${PYTHON_USEDEP}] )
	...
	profiling? ( dev-python/ddtrace[${PYTHON_USEDEP}] )
	profiling? ( dev-python/google-cloud-profiler[${PYTHON_USEDEP}] )
	redis? ( >=dev-python/redis-6.2.0[${PYTHON_USEDEP}] )
	server? ( dev-python/fastapi[${PYTHON_USEDEP}] )
	server? ( ~dev-python/uvicorn-0.29.0[${PYTHON_USEDEP}] )
