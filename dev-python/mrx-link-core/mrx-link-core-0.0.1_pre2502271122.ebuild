# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="0.0.1.dev2502271122"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=wheel

inherit python-r1 gs-pypi

DESCRIPTION="A simple AI/ML modeling tool, Link™ ensures a smoother flow and a better experience throughout the model development cycle. [wheel]"

HOMEPAGE="https://link.makinarocks.ai"
LICENSE="proprietary"
SRC_URI="https://files.pythonhosted.org/packages/cp311/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}-cp311-cp311-manylinux_2_17_x86_64.manylinux2014_x86_64.whl"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}-cp311-cp311-manylinux_2_17_x86_64.manylinux2014_x86_64.whl"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="dev hpo"
DEPENDENCIES=">=dev-python/astor-0.8[${PYTHON_USEDEP}]
	<=dev-python/black-22.12[${PYTHON_USEDEP}]
	dev-python/celery[${PYTHON_USEDEP}]
	>=dev-python/cloudpickle-1.6[${PYTHON_USEDEP}]
	>=dev-python/colorama-0.4[${PYTHON_USEDEP}]
	>=dev-python/cython-3.0[${PYTHON_USEDEP}]
	>=dev-python/dill-0.3[${PYTHON_USEDEP}]
	dev-python/eventlet[${PYTHON_USEDEP}]
	dev-python/ipykernel[${PYTHON_USEDEP}]
	>=dev-python/ipython-7.30[${PYTHON_USEDEP}]
	>=dev-python/isort-5.7[${PYTHON_USEDEP}]
	dev-python/jinja[${PYTHON_USEDEP}]
	<dev-python/jupyter-server-3.0[${PYTHON_USEDEP}]
	dev-python/kfp[${PYTHON_USEDEP}]
	>=dev-python/msgpack-1.0.3[${PYTHON_USEDEP}]
	>=dev-python/networkx-2.5[${PYTHON_USEDEP}]
	>=dev-python/pydantic-1.8.0[${PYTHON_USEDEP}]
	>=dev-python/python-memcached-1.59[${PYTHON_USEDEP}]
	>=dev-python/redis-4.3[${PYTHON_USEDEP}]
	>=dev-python/requests-2.27[${PYTHON_USEDEP}]
	dev? ( >=dev-python/build-0.7[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/flake8-3.9[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/mypy-0.910[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/packaging-23.1[${PYTHON_USEDEP}] )
	dev? ( dev-python/pre-commit[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pylint-2.9[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-cov-2.12[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-mock-3.6[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-6.2[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/setuptools-68.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/tomli-2.0.1[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/wheel-0.37[${PYTHON_USEDEP}] )
	hpo? ( >=dev-python/matplotlib-2.0[${PYTHON_USEDEP}] )
	hpo? ( >=dev-python/numpy-1.19[${PYTHON_USEDEP}] )
	hpo? ( dev-python/optuna[${PYTHON_USEDEP}] )
	hpo? ( >=dev-python/pandas-1.1[${PYTHON_USEDEP}] )
	hpo? ( >=dev-python/plotly-4.0.0[${PYTHON_USEDEP}] )
	hpo? ( >=dev-python/scikit-learn-0.22.2[${PYTHON_USEDEP}] )
	virtual/allow-pypi-wheels"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
