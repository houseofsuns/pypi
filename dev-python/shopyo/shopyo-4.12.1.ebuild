# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Highly modular web framework built for big apps on top of Flask with Django advantages"

HOMEPAGE="https://github.com/shopyo/shopyo"
LICENSE="MIT"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE=""
DEPENDENCIES=">=dev-python/flask-2.0.2[${PYTHON_USEDEP}]
	dev-python/Flask-Admin[${PYTHON_USEDEP}]
	>=dev-python/flask-login-0.5.0[${PYTHON_USEDEP}]
	dev-python/Flask-Mailman[${PYTHON_USEDEP}]
	>=dev-python/flask-migrate-3.1.0[${PYTHON_USEDEP}]
	>=dev-python/flask-sqlalchemy-2.5.1[${PYTHON_USEDEP}]
	dev-python/Flask-WTF[${PYTHON_USEDEP}]
	>=dev-python/sqlalchemy-1.4.27[${PYTHON_USEDEP}]
	>=dev-python/werkzeug-2.0.2[${PYTHON_USEDEP}]
	>=dev-python/wtforms-3.0.0[${PYTHON_USEDEP}]
	dev-python/email-validator[${PYTHON_USEDEP}]
	dev-python/shopyo-base[${PYTHON_USEDEP}]
	dev-python/shopyo-auth[${PYTHON_USEDEP}]
	dev-python/shopyo-dashboard[${PYTHON_USEDEP}]
	dev-python/shopyo-appadmin[${PYTHON_USEDEP}]"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
