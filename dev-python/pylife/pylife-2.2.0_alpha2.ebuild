# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="2.2.0a2"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="General Fatigue library"

HOMEPAGE="https://github.com/boschresearch/pylife/"
LICENSE="Apache-2.0"
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all analysis docs extras testing tsfresh unit-testing visuals"
DEPENDENCIES=">=dev-python/numpy-1.23.5[${PYTHON_USEDEP}]
	dev-python/scipy[${PYTHON_USEDEP}]
	>=dev-python/pandas-1.4.0[${PYTHON_USEDEP}]
	>dev-python/h5py-3.7.0[${PYTHON_USEDEP}]
	dev-python/cython[${PYTHON_USEDEP}]
	all? ( dev-python/tsfresh[${PYTHON_USEDEP}] )
	analysis? ( dev-python/flake8[${PYTHON_USEDEP}] )
	analysis? ( dev-python/pep8-naming[${PYTHON_USEDEP}] )
	docs? ( dev-python/sphinx[${PYTHON_USEDEP}] )
	docs? ( dev-python/nbsphinx[${PYTHON_USEDEP}] )
	docs? ( dev-python/nbsphinx-link[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-rtd-theme-1.0[${PYTHON_USEDEP}] )
	docs? ( dev-python/plotly[${PYTHON_USEDEP}] )
	docs? ( dev-python/jupyter-sphinx[${PYTHON_USEDEP}] )
	docs? ( dev-python/myst-parser[${PYTHON_USEDEP}] )
	docs? ( dev-python/pyvista[${PYTHON_USEDEP}] )
	docs? ( dev-python/trame[${PYTHON_USEDEP}] )
	docs? ( dev-python/pyvista[${PYTHON_USEDEP}] )
	docs? ( dev-python/xvfbwrapper[${PYTHON_USEDEP}] )
	docs? ( dev-python/ipykernel[${PYTHON_USEDEP}] )
	docs? ( dev-python/ipywidgets[${PYTHON_USEDEP}] )
	docs? ( dev-python/ipython[${PYTHON_USEDEP}] )
	docs? ( dev-python/pickleshare[${PYTHON_USEDEP}] )
	extras? ( dev-python/jupyter[${PYTHON_USEDEP}] )
	extras? ( dev-python/matplotlib[${PYTHON_USEDEP}] )
	extras? ( dev-python/plotly[${PYTHON_USEDEP}] )
	extras? ( dev-python/ansys-mapdl-reader[${PYTHON_USEDEP}] )
	testing? ( dev-python/setuptools[${PYTHON_USEDEP}] )
	testing? ( dev-python/pytest[${PYTHON_USEDEP}] )
	testing? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	testing? ( dev-python/hypothesis[${PYTHON_USEDEP}] )
	testing? ( dev-python/pyvista[${PYTHON_USEDEP}] )
	testing? ( dev-python/xvfbwrapper[${PYTHON_USEDEP}] )
	testing? ( dev-python/ipykernel[${PYTHON_USEDEP}] )
	testing? ( dev-python/ipywidgets[${PYTHON_USEDEP}] )
	testing? ( dev-python/ipython[${PYTHON_USEDEP}] )
	testing? ( dev-python/pickleshare[${PYTHON_USEDEP}] )
	testing? ( dev-python/testbook[${PYTHON_USEDEP}] )
	tsfresh? ( dev-python/tsfresh[${PYTHON_USEDEP}] )
	unit-testing? ( dev-python/setuptools[${PYTHON_USEDEP}] )
	unit-testing? ( dev-python/pytest[${PYTHON_USEDEP}] )
	unit-testing? ( dev-python/pytest-cov[${PYTHON_USEDEP}] )
	unit-testing? ( dev-python/hypothesis[${PYTHON_USEDEP}] )
	visuals? ( dev-python/pyvista[${PYTHON_USEDEP}] )
	visuals? ( dev-python/xvfbwrapper[${PYTHON_USEDEP}] )
	visuals? ( dev-python/ipykernel[${PYTHON_USEDEP}] )
	visuals? ( dev-python/ipywidgets[${PYTHON_USEDEP}] )
	visuals? ( dev-python/ipython[${PYTHON_USEDEP}] )
	visuals? ( dev-python/pickleshare[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
