...
IUSE="all bigquery boto3 cassandra clickhouse couchdb dataframe default elasticsearch hazelcast influxdb jdbc mariadb memcache mongodb msqlserver mysql odbc oracle postgres postgresql pyspark redis redpanda rethinkdb scylla sqlalchemy sqlite uvloop"
DEPENDENCIES=">=dev-python/cryptography-43.0.1[${PYTHON_USEDEP}]
	...
	all? ( ~dev-python/sqlalchemy-2.0.34[${PYTHON_USEDEP}] )
	all? ( ~dev-python/elasticsearch-8.15.1[${PYTHON_USEDEP}] )
	all? ( ~dev-python/pymongo-4.10.1[${PYTHON_USEDEP}] )
	...
	dataframe? ( ~dev-python/pyarrow-17.0.0[${PYTHON_USEDEP}] )
	...
	default? ( ~dev-python/uvloop-0.20.0[${PYTHON_USEDEP}] )
