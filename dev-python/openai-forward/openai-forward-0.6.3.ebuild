# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
REALVERSION="${PV}"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_10,3_11,3_12} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="🚀 OpenAI-Forward 是一个专为大型语言模型设计的高级转发代理，提供包括用户请求速率控制、Token速率限制和自定义API密钥等增强功能。该服务可用于代理本地模型和云端模型。OpenAI API Reverse Proxy"

HOMEPAGE="https://github.com/KenyonY/openai-forward"
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="benchmark database dev leveldb lmdb rocksdb test"
DEPENDENCIES="dev-python/aiohttp[${PYTHON_USEDEP}]
	dev-python/fastapi[${PYTHON_USEDEP}]
	dev-python/fire[${PYTHON_USEDEP}]
	dev-python/loguru[${PYTHON_USEDEP}]
	dev-python/orjson[${PYTHON_USEDEP}]
	dev-python/python-dotenv[${PYTHON_USEDEP}]
	dev-python/pytz[${PYTHON_USEDEP}]
	dev-python/rich[${PYTHON_USEDEP}]
	dev-python/slowapi[${PYTHON_USEDEP}]
	dev-python/uvicorn[${PYTHON_USEDEP}]
	benchmark? ( dev-python/openai[${PYTHON_USEDEP}] )
	benchmark? ( dev-python/tiktoken[${PYTHON_USEDEP}] )
	database? ( dev-python/lmdb[${PYTHON_USEDEP}] )
	database? ( dev-python/plyvel[${PYTHON_USEDEP}] )
	database? ( dev-python/rocksdict[${PYTHON_USEDEP}] )
	dev? ( dev-python/openai[${PYTHON_USEDEP}] )
	dev? ( dev-python/schedule[${PYTHON_USEDEP}] )
	dev? ( dev-python/sparrow-python[${PYTHON_USEDEP}] )
	leveldb? ( dev-python/plyvel[${PYTHON_USEDEP}] )
	lmdb? ( dev-python/lmdb[${PYTHON_USEDEP}] )
	rocksdb? ( dev-python/rocksdict[${PYTHON_USEDEP}] )
	test? ( dev-python/openai[${PYTHON_USEDEP}] )
	test? ( dev-python/psutil[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-aiohttp[${PYTHON_USEDEP}] )
	test? ( dev-python/sparrow-python[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
