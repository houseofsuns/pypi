# automatically generated by g-sorcery
# please do not edit this file

EAPI=8

REALNAME="${PN}"
LITERALNAME="${PN}"
REALVERSION="1.145.0rc4"
DIGEST_SOURCES="yes"
PYTHON_COMPAT=( python{3_11,3_12,3_13,3_14} )
DISTUTILS_USE_PEP517=standalone

inherit python-r1 gs-pypi

DESCRIPTION="Homeserver for the Matrix decentralised comms protocol"

HOMEPAGE=""
LICENSE=""
SRC_URI="https://files.pythonhosted.org/packages/source/${REALNAME::1}/${REALNAME}/${REALNAME//-/_}-${REALVERSION}.tar.gz"
SOURCEFILE="${REALNAME//-/_}-${REALVERSION}.tar.gz"
RESTRICT="test"

SLOT="0"
KEYWORDS="~amd64 ~x86"

IUSE="all cache-memory jwt matrix-synapse-ldap3 oidc opentracing postgres redis saml2 sentry systemd test url-preview"
DEPENDENCIES=">=dev-python/jsonschema-3.0.0[${PYTHON_USEDEP}]
	>=dev-python/rpds-py-0.25.0[${PYTHON_USEDEP}]
	>=dev-python/immutabledict-2.0[${PYTHON_USEDEP}]
	>=dev-python/unpaddedbase64-2.1.0[${PYTHON_USEDEP}]
	<dev-python/canonicaljson-3.0.0[${PYTHON_USEDEP}]
	<dev-python/signedjson-2.0.0[${PYTHON_USEDEP}]
	>=dev-python/service-identity-18.1.0[${PYTHON_USEDEP}]
	>=dev-python/twisted-21.2.0[${PYTHON_USEDEP}]
	>=dev-python/treq-21.5.0[${PYTHON_USEDEP}]
	>=dev-python/pyopenssl-16.2.0[${PYTHON_USEDEP}]
	>=dev-python/pyyaml-5.3[${PYTHON_USEDEP}]
	>=dev-python/pyasn1-0.1.9[${PYTHON_USEDEP}]
	>=dev-python/pyasn1-modules-0.0.7[${PYTHON_USEDEP}]
	>=dev-python/bcrypt-3.1.7[${PYTHON_USEDEP}]
	>=dev-python/pillow-10.0.1[${PYTHON_USEDEP}]
	>=dev-python/sortedcontainers-2.0.5[${PYTHON_USEDEP}]
	>=dev-python/pymacaroons-0.13.0[${PYTHON_USEDEP}]
	>=dev-python/msgpack-0.5.2[${PYTHON_USEDEP}]
	>=dev-python/phonenumbers-8.2.0[${PYTHON_USEDEP}]
	>=dev-python/prometheus-client-0.6.0[${PYTHON_USEDEP}]
	>=dev-python/attrs-19.2.0[${PYTHON_USEDEP}]
	>=dev-python/netaddr-0.7.18[${PYTHON_USEDEP}]
	>=dev-python/jinja2-3.0[${PYTHON_USEDEP}]
	>=dev-python/bleach-3.2.0[${PYTHON_USEDEP}]
	>=dev-python/typing-extensions-4.14.1[${PYTHON_USEDEP}]
	>=dev-python/cryptography-3.4.7[${PYTHON_USEDEP}]
	>=dev-python/ijson-3.1.4[${PYTHON_USEDEP}]
	<dev-python/matrix-common-2.0.0[${PYTHON_USEDEP}]
	>=dev-python/packaging-20.0[${PYTHON_USEDEP}]
	>=dev-python/pydantic-2.8[${PYTHON_USEDEP}]
	>=dev-python/pydantic-2.12[${PYTHON_USEDEP}]
	>=dev-python/setuptools-rust-1.3[${PYTHON_USEDEP}]
	>=dev-python/python-multipart-0.0.9[${PYTHON_USEDEP}]
	>=dev-python/cffi-1.15[${PYTHON_USEDEP}]
	>=dev-python/pynacl-1.3[${PYTHON_USEDEP}]
	>=dev-python/pyparsing-2.4[${PYTHON_USEDEP}]
	>=dev-python/pyrsistent-0.18.0[${PYTHON_USEDEP}]
	>=dev-python/requests-2.16.0[${PYTHON_USEDEP}]
	>=dev-python/urllib3-1.26.5[${PYTHON_USEDEP}]
	>=dev-python/zope-interface-5.2[${PYTHON_USEDEP}]
	all? ( dev-python/matrix-synapse-ldap3[${PYTHON_USEDEP}] )
	all? ( dev-python/psycopg2[${PYTHON_USEDEP}] )
	all? ( dev-python/psycopg2cffi[${PYTHON_USEDEP}] )
	all? ( dev-python/psycopg2cffi-compat[${PYTHON_USEDEP}] )
	all? ( dev-python/pysaml2[${PYTHON_USEDEP}] )
	all? ( dev-python/Authlib[${PYTHON_USEDEP}] )
	all? ( >=dev-python/lxml-4.6.3[${PYTHON_USEDEP}] )
	all? ( dev-python/sentry-sdk[${PYTHON_USEDEP}] )
	all? ( dev-python/jaeger-client[${PYTHON_USEDEP}] )
	all? ( dev-python/opentracing[${PYTHON_USEDEP}] )
	all? ( >=dev-python/txredisapi-1.4.7[${PYTHON_USEDEP}] )
	all? ( >=dev-python/hiredis-0.3[${PYTHON_USEDEP}] )
	all? ( dev-python/Pympler[${PYTHON_USEDEP}] )
	all? ( >=dev-python/defusedxml-0.7.1[${PYTHON_USEDEP}] )
	all? ( >=dev-python/pytz-2018.3[${PYTHON_USEDEP}] )
	all? ( >=dev-python/thrift-0.10[${PYTHON_USEDEP}] )
	all? ( >=dev-python/tornado-6.0[${PYTHON_USEDEP}] )
	cache-memory? ( dev-python/Pympler[${PYTHON_USEDEP}] )
	jwt? ( dev-python/Authlib[${PYTHON_USEDEP}] )
	matrix-synapse-ldap3? ( dev-python/matrix-synapse-ldap3[${PYTHON_USEDEP}] )
	oidc? ( dev-python/Authlib[${PYTHON_USEDEP}] )
	opentracing? ( dev-python/jaeger-client[${PYTHON_USEDEP}] )
	opentracing? ( dev-python/opentracing[${PYTHON_USEDEP}] )
	opentracing? ( >=dev-python/thrift-0.10[${PYTHON_USEDEP}] )
	opentracing? ( >=dev-python/tornado-6.0[${PYTHON_USEDEP}] )
	postgres? ( dev-python/psycopg2[${PYTHON_USEDEP}] )
	postgres? ( dev-python/psycopg2cffi[${PYTHON_USEDEP}] )
	postgres? ( dev-python/psycopg2cffi-compat[${PYTHON_USEDEP}] )
	redis? ( >=dev-python/txredisapi-1.4.7[${PYTHON_USEDEP}] )
	redis? ( >=dev-python/hiredis-0.3[${PYTHON_USEDEP}] )
	saml2? ( dev-python/pysaml2[${PYTHON_USEDEP}] )
	saml2? ( >=dev-python/defusedxml-0.7.1[${PYTHON_USEDEP}] )
	saml2? ( >=dev-python/pytz-2018.3[${PYTHON_USEDEP}] )
	sentry? ( dev-python/sentry-sdk[${PYTHON_USEDEP}] )
	systemd? ( dev-python/systemd-python[${PYTHON_USEDEP}] )
	test? ( >=dev-python/parameterized-0.9.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/idna-3.3[${PYTHON_USEDEP}] )
	url-preview? ( >=dev-python/lxml-4.6.3[${PYTHON_USEDEP}] )"
BDEPEND="${DEPENDENCIES}"
RDEPEND="${DEPENDENCIES}"
